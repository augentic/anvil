...
# one `specify` binary itself.
#
#   bash evals/drivers/guest-execute-loop.sh
#
# Builds the workflow guest and the `specify` binary, seeds the sandbox
# project (init from the release-built omnia component), writes the
# deployment manifest (workflow guest + the eight release-built adapter
# components from the sibling specify-adapters checkout) at the sandbox
# root — the forwarding guest leg honors a project-root omnia.toml over
# the generated manifest in the project cache — then drives `plan author`
# and `plan execute` through `specify` with the live cursor backend,
# stamping Gate 1 natively in between. Post-run probes per the scenario's
# assertion ids run at the end; grading and the run record stay
# operator-owned.
#
...
(
  ...
  cargo build -q -p workflow --target wasm32-wasip2
)
specify="$root/target/debug/specify"
workflow_wasm="$root/target/wasm32-wasip2/debug/workflow.wasm"
...
# Seed the sandbox: init from the release-built omnia component (mirrored
# into the project component cache, so target resolution works both
# natively and inside the guest).
rm -rf "$sandbox"
mkdir -p "$sandbox"
cd "$sandbox"
...
# the "." mount re-pointed at the sandbox. The forwarding guest leg picks
# this up from the project root instead of the generated manifest, which
# lets `plan author` dispatch to the source:intent guest before plan.yaml
# binds it.
{
  printf '[[guest]]\nid = "workflow"\nsource.path = "%s"\n' "$workflow_wasm"
