...
| ------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `[intent-only](scenarios/pure-intent.md)`                                       | N=1 pure intent → one slice (**release blocker** — hard halt on failure) |
...
| `[typescript-multi-slice](scenarios/code-multi-slice.md)`                       | TypeScript code source, multiple slices                                  |
| `[lead-reconciliation](scenarios/cross-source-merge.md)`                        | Cross-source propose-time merge                                          |
| `[single-project-plan](scenarios/plan-single-project.md)`                       | Single-project plan generation                                           |
| `[contract-lifecycle](scenarios/cross-repo-contract-flow.md)`                   | Cross-repo contract flow (full lifecycle, live forge)                    |
| `[target-shape](scenarios/target-shape-injection.md)`                           | Target `shape` injection                                                 |
| `[execute-pause-resume](scenarios/stepthrough-breakout.md)`                             | Step-through breakout mid-execute                                        |
| `[execute-fail-resume](scenarios/execute-build-failure.md)`                     | `/spec:execute` parks on a build failure                                 |
| `[workspace-two-projects](scenarios/workspace-execute-two-projects.md)` | Workspace `/spec:execute` across two projects                            |
| `[workspace-fail-resume](scenarios/workspace-breakout.md)`                         | Workspace breakout after build failure                                   |
| `[workspace-stale-recovery](scenarios/stale-workspace-recovery.md)`             | Stale-workspace recovery                                                 |
