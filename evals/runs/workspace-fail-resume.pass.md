...
Probe transcript highlights: `plan.yaml` present with `lifecycle: approved` before execute; `/spec:execute` parked on `auth-rotate` with `specify plan status` reporting `action: stop`, `stop.reason: build-failed`, one `status: in-progress` entry and `resume: /spec:build auth-rotate`; backend project journal (`backend/.specify/journal.jsonl`) records `slice.build.failed` then breakout `slice.build.succeeded` for `auth-rotate`; breakout resolved `auth-rotate → backend` from workspace CWD without manual `chdir`; resumed loop merged `auth-rotate`, completed `oauth-mobile`, and `specify plan status` reports `action: drained` with two `status: done` entries.
...
- Execute/breakout driven by `evals/.sandbox/workspace_driver.py` following `/spec:execute` and breakout routing from the workspace root; breakout triage committed parked slot dirtiness before retrying build.

## Notes

- Park state captured at `specify plan status`: `next-action: stop build-failed`, `resume: /spec:build auth-rotate`, entry `auth-rotate` `in-progress`.
