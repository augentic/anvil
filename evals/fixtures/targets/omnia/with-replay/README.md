...
Worked example for the **optional** replay hook on the [`omnia` target adapter's `build/replay.md`](https://github.com/augentic/specify-adapters/blob/main/targets/omnia/prose/prompts/build/replay.md) sub-brief (the capture-backed replay workflow, target half). Shared hook contract: [`replay/hook-contract.md`](https://github.com/augentic/specify-adapters/blob/main/codex/references/replay/hook-contract.md). The `metadata.yaml` in this directory carries a top-level `replay:` block alongside the standard slice metadata.
...
The block shape is documented in [`replay/journal-payload.md`](https://github.com/augentic/specify-adapters/blob/main/codex/references/replay/journal-payload.md):
