...
6. On success, `specify slice transition session-cookie-harden built --format json` runs and the body returns. On failure, the structured stop hint is emitted (see [`../failure-replay/expected-stop-hint.md`](../failure-replay/expected-stop-hint.md)).
