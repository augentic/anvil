//! Integration coverage for `Plan::validate` over the public surface.
//!
//! The `duplicate-name`, `cycle-in-depends-on`, `duplicate-source-key`,
//! and `slice-authority-override-orphan-source` emit branches are
//! asserted end-to-end by the repo-root `tests/plan.rs`
//! (`plan_validate_with_errors_json`,
//! `validate_reports_all_health_diagnostics`,
//! `amend_add_source_duplicate_key_rejected`) and the doctor cycle
//! proptests, so their duplicates were deleted. What stays below are
//! the `plan.validate()` emit branches no CLI fixture triggers:
//! unknown-source / unknown-depends-on, the divergence-coherence
//! quartet, multiple-in-progress, slice-dir consistency (orphan +
//! None-skip), no-short-circuit, the registry project-binding matrix,
//! and context-path validation (plus the clean many-slice baseline,
//! which keeps the shared `PLAN_EXAMPLE_YAML` fixture exercised).

use std::collections::HashSet;

use schema::diagnostics::{Severity, blocking};
...
use common::{PLAN_EXAMPLE_YAML, change, plan_with_changes};
...
fn has_code(d: &schema::diagnostics::Diagnostic, code: &str) -> bool {
