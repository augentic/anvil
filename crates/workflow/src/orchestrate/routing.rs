...
//!
//! The `/spec:plan` and `/spec:execute` skills sync workspace slots and
//! chdir into `workspace/<project>/` for project-scoped work; the guest
//! orchestrations have no counterpart, so they refuse rather than write
//! to the wrong tree. Classification is centralized here — before any
