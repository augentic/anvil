//! The shared `plan create` scaffold gates: kebab name, overwrite
//! refusal, and [`Plan::init`]. Shared by the native `plan create`
//! operation and the guest `plan author` orchestration so the two
//! entry points cannot drift; auto-approval, authority-override
//! pre-seeding, Gate 1 prose, and journal events stay with their
//! owning operations.
