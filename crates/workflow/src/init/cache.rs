...
/// found by walking the component's ancestors for the flattened
/// `codex/rules/` tree both framework repos carry at their root.
/// Whichever is found is mirrored into the out-of-tree
/// `<project-cache>/codex/` under the canonical
/// `codex/rules/{universal,core}` layout the codex resolver probes.
///
...
///
/// Probes each ancestor for the flattened repo-root layout
...
fn rules_root_for_component(component: &Path) -> Option<PathBuf> {
    for base in component.ancestors() {
        let candidate = base.join("codex").join("rules");
