...
//! binary (RFC-66 §"Codex ownership becomes real") into the project
//! codex cache at `<project-cache>/codex/`, pinned to the binary
...
pub struct CodexMeta {
    ...
    pub source: String,
    /// Whether the framework `core/` pack was materialized alongside
    /// the shared `universal/` pack (`--include-framework`).
...
/// from `codex/rules/{universal,core}/` at the repo root.
static CODEX_PACK_FILES: &[(&str, &str)] = include!(concat!(env!("OUT_DIR"), "/codex_packs.rs"));

/// Materialize the embedded shared codex packs into the project codex
/// cache and stamp [`CodexMeta`].
///
/// Writes `universal/` always and `core/` under `include_framework`,
/// under the canonical `codex/rules/{universal,core}` layout the codex
/// resolver probes. A cache already stamped by this binary version with
/// the same pack set is left untouched; any mismatch (an older binary's
/// stamp, a different `include_framework`, a missing tree) wipes and
/// re-materializes — the bytes are already in memory, so a refresh is
/// cheap. Returns the stamped [`CodexMeta`].
pub(super) fn cache_codex(
    ...
    if let Some(existing) = read_codex_meta(project_dir)
        ...
        && existing.include_framework == include_framework
    ...
    for (rel, contents) in CODEX_PACK_FILES {
        if !include_framework && rel.starts_with(CORE_RULES_REL) {
            continue;
        }
    ...
    write_codex_meta(project_dir, &version, include_framework, now)
}
