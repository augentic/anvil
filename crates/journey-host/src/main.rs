//! The dev-only journey host (ADR-0009 §5): the shipped runtime shape
//! with one substitution — `WasiModel` answers from a script
//! directory instead of the Cursor backend. Never shipped.
...
omnia::runtime!({
    ...
    guests: [{
        id: "emery",
        // The raw engine component built by this crate's `build.rs`
        // from the same sources the shipped binary embeds.
