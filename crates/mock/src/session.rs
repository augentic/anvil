//! Host-only session helpers over the offline native provider: the
//! recording model handle is held beside the provider.
...
impl Session {
    ...
    pub fn bare(answers: Vec<String>) -> Self {
        ...
        // Production shape: the layout roots (store, cache) live
        ...
        let provider = Provider::new(paths, DynModel::new(model.clone()), crate::catalog());
        Self {
            root,
            provider,
            model,
