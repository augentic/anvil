...
///
/// True when `entry` is on `plan` and `meta` is not
/// [`LifecycleStatus::Dropped`]. A missing metadata document (slice
/// not yet created) is not dropped, so the entry stays in-scope.
/// `plan remove` deletes the row (absent ⇒ not in-scope); `slice drop`
/// abandons the slice and excludes it even when the plan row remains.
#[must_use]
pub fn in_scope(plan: &Plan, entry: &Entry, meta: Option<&SliceMetadata>) -> bool {
    plan.entries.iter().any(|e| e.name == entry.name)
        && !meta.is_some_and(|m| m.status == LifecycleStatus::Dropped)
}
