//! Deterministic baseline identity projection: the `surface[]` /
//! `recent[]` / `decisions[]` axes a project presents to plan-time
//! reconciliation. Purely structural, never an LLM summary.
...
/// projected. Carried on the reconciliation envelope's `ProjectRef`.
...
///
/// The domain slug and a bounded sample of its requirement titles.
/// Carried on the reconciliation envelope's `ProjectRef`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, schemars::JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct Surface {
    ...
    /// stripped) in `REQ-NNN` id order, capped at [`SURFACE_TITLE_CAP`].
...
/// The target-reference grammar.
///
/// `name@version` for a pinned identity, bare `name` for an unpinned
/// cache resolve. The single producer formula; `plan::TargetRef`
/// parses the same grammar back.
#[must_use]
