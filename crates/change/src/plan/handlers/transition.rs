...
/// idempotent no-op (exit 0, no disk write, no journal event) — a
/// repeated explicit transition must not double-stamp the lifecycle
/// nor double-fire `plan.transition.approved`.
///
        ...
        return match target {
            "approved" => {
                ...
                if matches!(previous, Lifecycle::Approved) {
                    // Idempotent Gate-1 contract: the plan is already
                    // approved, so the repeated transition is a
                    // belt-and-braces no-op. No disk or journal write.
                    let body = TransitionBody {
                        plan: plan_ref(plan, plan_path),
