...
use omnia_guest::api::command::{
    App, BuildError, CommandResponse, Completions, Namespace, Outcome, Projector, Router, run,
};
...
use crate::cli::{ABOUT, Globals, InitArgs, UpgradeArgs};
use crate::commands::{
    adapters, archive, journal, plan, plugins, registry, slice, source, target, workspace,
};
use crate::output::{ErrorBody, Exit, Format};
    ...
    fn project(
        ...
        match outcome {
            Outcome::Output(output) => {
                Ok(CommandResponse::success(encode(globals.format, &output, Render::render)?))
            }
...
fn encode<T: Serialize>(
    ...
    text: impl FnOnce(&T, &mut dyn std::io::Write) -> std::io::Result<()>,
) -> Result<Vec<u8>, error::Error> {
    let mut bytes = Vec::new();
    match format {
        Format::Json => {
            serde_json::to_writer_pretty(&mut bytes, value).map_err(|source| {
                error::Error::Diag {
                    code: "json-serialize-failed",
                    detail: format!("failed to serialize JSON response: {source}"),
                }
            })?;
            writeln!(bytes).map_err(error::Error::Io)?;
        }
        Format::Text => text(value, &mut bytes).map_err(error::Error::Io)?,
    }
    Ok(bytes)
}

fn render_body<T: Render>(value: &T, writer: &mut dyn std::io::Write) -> std::io::Result<()> {
    value.render(writer)
}

fn error_response(format: Format, error: &error::Error) -> Result<CommandResponse, error::Error> {
    let body = ErrorBody::from(error);
    let stderr = encode(format, &body, |_, writer| {
...
fn operation_response(
    ...
    match error {
        ...
        workflow::handler::Error::Report { body, source } => {
            let stdout = encode(format, &body, render_body)?;
    ...
    P: Provider + Anchor + Model + SourceSeam + TargetSeam,
{
    ...
    let mut router =
        Router::new(app, invoker).globals::<Globals>().before_dispatch(move |globals| {
            check_plan_dir(globals.plan_dir.as_deref()).and_then(|()| preflight(globals)).err().map(
                |error| {
                    error_response(globals.format, &error).unwrap_or_else(|fallback| {
    ...
    router = router.route(
        ["workspace", "prepare"],
        run::<workspace::cli::PrepareArgs, Unsupported>()
...
impl<P: Provider> Operation<P> for Unsupported {
    ...
    async fn call(
        ...
        Err(error::Error::Argument {
            flag: "<command>",
            detail: format!("`specify {}` has no guest implementation yet", input.command),
        }
        .into())
    }
}
    ...
    ($args:path => $input:path { $($field:ident),* $(,)? }) => {
        impl TryFrom<$args> for $input {
            ...
            fn try_from(args: $args) -> Result<Self, Self::Error> {
                Ok(Self { $($field: args.$field),* })
            }
        }
    };
}

convert!(source::cli::ResolveArgs => workflow::adapter::handlers::ResolveInput { value, project_dir });
convert!(target::cli::ResolveArgs => workflow::adapter::handlers::ResolveInput { value, project_dir });
convert!(source::cli::SurveyArgs => workflow::orchestrate::handlers::SurveyInput { source, plan });
convert!(source::cli::ExtractArgs => workflow::orchestrate::handlers::ExtractInput { source, lead, slice });
convert!(slice::cli::CreateArgs => workflow::slice::handlers::CreateInput { name, target, if_exists });
convert!(slice::cli::ValidateArgs => workflow::slice::handlers::ValidateInput { name });
convert!(slice::cli::ProvenanceArgs => workflow::slice::handlers::ProvenanceInput { name });
convert!(slice::cli::ModelShowArgs => workflow::slice::handlers::ModelShowInput { name });
convert!(slice::cli::RefineArgs => workflow::orchestrate::handlers::RefineInput { name });
convert!(slice::cli::BuildArgs => workflow::orchestrate::handlers::BuildInput { name });
convert!(slice::cli::MergeRunArgs => workflow::orchestrate::handlers::MergeRunInput { name, allow_composition_replace });
convert!(slice::cli::MergePreviewArgs => workflow::slice::handlers::PreviewInput { name });
convert!(slice::cli::ConflictCheckArgs => workflow::slice::handlers::ConflictCheckInput { name });
convert!(slice::cli::TaskProgressArgs => workflow::slice::handlers::TaskProgressInput { name });
convert!(slice::cli::TaskMarkArgs => workflow::slice::handlers::TaskMarkInput { name, task_number });
convert!(slice::cli::TransitionArgs => workflow::slice::handlers::TransitionInput { name, target });
convert!(slice::cli::TouchedSpecsArgs => workflow::slice::handlers::TouchedSpecsInput { name, scan, set });
convert!(slice::cli::OverlapArgs => workflow::slice::handlers::OverlapInput { name });
convert!(slice::cli::DropArgs => workflow::slice::handlers::DropInput { name, reason });
convert!(archive::cli::PruneArgs => workflow::slice::handlers::PruneInput { keep, older_than, dry_run });
convert!(plan::cli::CreateArgs => workflow::change::plan::handlers::CreateInput { name, sources, intent, auto_approve, authority_override });
convert!(plan::cli::ValidateArgs => workflow::change::plan::handlers::ValidateInput {});
convert!(plan::cli::NextArgs => workflow::change::plan::handlers::NextInput {});
convert!(plan::cli::StatusArgs => workflow::change::plan::handlers::StatusInput {});
convert!(plan::cli::ExecuteArgs => workflow::orchestrate::handlers::ExecuteInput {});
convert!(plan::cli::AddArgs => workflow::change::plan::handlers::AddInput { name, depends_on, sources, description, project, context, authority_override });
convert!(plan::cli::AmendArgs => workflow::change::plan::handlers::AmendInput { name, depends_on, sources, add_source, remove_source, divergence, description, project, context, authority_override, clear_authority_override, clear_authority_overrides });
convert!(plan::cli::RemoveArgs => workflow::change::plan::handlers::RemoveInput { name });
convert!(plan::cli::TransitionArgs => workflow::change::plan::handlers::TransitionInput { name, target, undo, actor });
convert!(plan::cli::AuthorArgs => workflow::orchestrate::handlers::AuthorInput { name, sources, intent });
convert!(plan::cli::ArchiveArgs => workflow::change::plan::handlers::ArchiveInput { force });
convert!(journal::cli::EmitArgs => workflow::journal::handlers::EmitInput { event, payload });
convert!(journal::cli::ShowArgs => workflow::journal::handlers::ShowInput { filter, limit });
convert!(registry::cli::ValidateArgs => workflow::registry::handlers::ValidateInput {});
convert!(registry::cli::AddArgs => workflow::registry::handlers::AddInput { name, url, adapter, description });
convert!(registry::cli::RemoveArgs => workflow::registry::handlers::RemoveInput { name });
...
macro_rules! unsupported {
    ($args:path, $command:literal) => {
        impl TryFrom<$args> for UnsupportedInput {
            type Error = error::Error;
...
unsupported!(adapters::cli::SyncArgs, "adapters sync");
unsupported!(workspace::cli::SyncArgs, "workspace sync");
unsupported!(workspace::cli::PrepareArgs, "workspace prepare");
unsupported!(workspace::cli::PushArgs, "workspace push");
unsupported!(UpgradeArgs, "upgrade");
unsupported!(plugins::cli::DoctorArgs, "plugins doctor");
unsupported!(plugins::cli::RefreshArgs, "plugins refresh");
