...
`specify slice validate` gates the shape: every task line MUST match `- [ ] X.Y <description>` (the dotted number is required) and sit under a `## ` heading — a bare `- <item>` or an un-numbered `- [ ] <item>` fails `tasks.use-checkbox-format` / `tasks.grouped-under-headings`. An optional `<!-- skill: plugin:skill-name -->` directive may trail a task when a build step should be executed by a named Cursor skill; emit one only when the target guidance prompt asks for it. `tasks.md` carries no provenance and no narrative prose outside the checkbox list.

**Agent-completable task invariant.** Every task must be executable and verifiable by an agent using code, local tooling, mocks, fixtures, contract validators, build commands, or reviewer skills. Never author a task that depends on manual app testing, real-world API credentials, visual inspection, physical-device-only checks, app-store review, or asking the user to verify behaviour. When a requirement appears to call for human validation, encode the equivalent code-based test or scripted verification task instead. Validation checks checkbox shape only — agent-completability is judged here at write-time, so re-read every checkbox before finalising the response and rewrite any task that fails the check.
