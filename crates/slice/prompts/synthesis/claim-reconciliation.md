...
| `example`       | `behaviour`                 | `spec.md` — folds into the requirement block whose `id` shares the same prefix as the example's `id` (e.g. `users.register.happy-path` corroborates the `users.register` requirement); when no requirement prefix matches, drives its own `spec.md` requirement with `Status: agreed`. `design.md` `## Technical logic` references the mock path for the operator to inspect concrete I/O. | Required `id` (per the per-kind body shape owned by `adapters/sources/captures/prose/prompts/extract.md`). |
...
Plan-time `Lead[]` reconciliation — the reconcile leg inside `specify plan author` that writes `slices[]` rows — is documented in the plan CLI reference (`specify plan author`'s lead-reconciliation leg). Cross-source matching is agent judgment; the kernel validates partition shape only. The operator curates at Gate 1 via `change.md` and `specify plan amend`.
