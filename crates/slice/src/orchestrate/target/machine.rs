...
impl<S: Target + Workspaces> Machine<'_, S> {
    ...
    fn assemble(
        ...
        let (outputs, ui_surface) = rounds
            ...
            .unwrap_or_default();
        // `covered` is projected from the build round's phase report
        // once the RFC-86a coverage carrier moves onto `phase-report`;
        // until then the terminal claim set is empty.
        BuildReport::stamped(
            &self.id,
            self.slice.to_string(),
            status,
    ...
    fn promoted(
        ...
        // Consumed deferral digests (the request's `deferred[]`) are
        // re-homed onto the machine with the RFC-86a coverage carrier;
        // until then the record consumes none.
        BuildRecord::from_capture(patch, self.wave.clone(), report.clone(), Vec::new())
