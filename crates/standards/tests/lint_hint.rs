...
mod ignore_directive_pass {
    ...
    /// synthetic `UNI-022`.
    const URL_WITH_SHORT_DIRECTIVE_FIXTURE_BODY: &str = "// specify-ignore: UNI-014 — too short\nconst BASE_URL: &str = \"https://api.example.com\";\n";
    ...
    fn validation_rule(rule_id: &str) -> ResolvedRule {
        ResolvedRule {
            ...
            path: format!("adapters/codex/rules/universal/{rule_id}.md"),
            body: String::new(),
            deprecated: None,
        }
    }
    ...
    fn run_scenario(scenario: &Scenario) -> (DiagnosticReport, Vec<Diagnostic>) {
        ...
        let url_rule =
            make_rule(scenario.primary_rule_id, vec![hint(HintKind::Regex, "https?://")]);
    ...
    fn scenario_4_missing_rationale_uni_022() {
        ...
        with_no_color(|| {
            assert_pretty_golden(
                &result,
                "ignore_directive_scenario_4_missing_rationale_pretty.txt",
            );
        });
    }
    ...
    fn scenario_6_orphan_mints_uni_023() {
        ...
        let url_finding = findings
            ...
            .expect("URL finding present");
        assert!(
            url_finding.status.is_none(),
            "URL finding stays open — directive id did not match"
        );
