...
//! asserted end-to-end by `tests/lint.rs::framework::duplicate_rule_id_aborts_fatally`
//! and `tests/rules.rs::export::negative_rules_root_required`, so their
...
fn shared_rules_from_explicit_rules_root() {
    ...
    assert_eq!(entry.path, "adapters/codex/rules/universal/uni-001.md");
}

/// Core pack root: rules under
/// `adapters/codex/rules/core/` resolve with `Origin::Core` and
...
fn core_rules_from_explicit_rules_root() {
    ...
    assert_eq!(core.path, "adapters/codex/rules/core/CORE-fixture.md");
...
fn rules_root_probe_falls_back() {
    let project = TempDir::new().expect("project");
    write_rule(
        &project.path().join("adapters/codex/rules/universal/uni-001.md"),
        "UNI-001",
        "Monorepo shared",
    );
...
fn shared_rules_from_codex_cache() {
    ...
    write_rule(
        &codex_cache(project.path()).join("adapters/codex/rules/universal/uni-001.md"),
        "UNI-001",
...
fn monorepo_wins_over_codex_cache() {
    ...
    write_rule(
        &codex_cache(project.path()).join("adapters/codex/rules/universal/uni-002.md"),
        "UNI-002",
        "Cache shared",
    );
...
fn readme_md_is_skipped() {
    ...
    let readme = rules_root.path().join("adapters/codex/rules/universal/README.md");
...
fn discovery_is_non_recursive() {
    ...
    write_rule(
        &rules_root.path().join("adapters/codex/rules/universal/nested/uni-002.md"),
        "UNI-002",
        "Nested",
    );
...
fn parse_error_includes_path() {
    let rules_root = TempDir::new().expect("rules root");
    let project = TempDir::new().expect("project");
    let bad_path = rules_root.path().join("adapters/codex/rules/universal/broken.md");
