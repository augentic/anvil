...
fn shared_rules_from_codex_cache() {
    ...
    write_rule(
        &codex_cache(project.path()).join("codex/rules/universal/uni-001.md"),
        "UNI-001",
...
fn monorepo_wins_over_codex_cache() {
    ...
    write_rule(
        &codex_cache(project.path()).join("codex/rules/universal/uni-002.md"),
        "UNI-002",
        "Cache shared",
    );
...
fn readme_md_is_skipped() {
    ...
    let readme = rules_root.path().join("codex/rules/universal/README.md");
...
fn parse_error_includes_path() {
    let rules_root = TempDir::new().expect("rules root");
    let project = TempDir::new().expect("project");
    let bad_path = rules_root.path().join("codex/rules/universal/broken.md");
