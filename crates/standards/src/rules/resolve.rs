...
pub enum ResolveError {
    ...
    /// golden.
    #[error(
        "rules-root-required: shared UNI-* rules require --rules-root pointing at a tree containing codex/rules/universal/, a monorepo codex/rules/universal/ tree, or a distributed codex cache (run `specify rules sync`)"
    )]
    RulesRootRequired,
    ...
    match err {
        ResolveError::RulesRootRequired => Error::validation_failed(
            ...
            "shared UNI-* rules require --rules-root, a project-local \
             codex/rules/universal/ tree, or a distributed \
...
// CLI e2e coverage (`tests/lint.rs`, `tests/rules.rs`).
