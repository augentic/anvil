...
//! the two indexers share one definition. The adapter extractor takes
//! the `adapters/{sources,targets}/<adapter>/<tail>` prefix split
//! through `parse_adapter_prefix`.
...
/// segment. The adapter extractor applies its own tail check
/// (`adapter.yaml`) on the returned `tail`.
