...
| `report.schema.json`          | target `merge` — report minus `version` / `slice` / `target`, diagnostic inlined                        |
| `phase-report.schema.json`    | target `build` / `verify` / `repair` / `review` (RFC-90) — one typed phase report, `covered` build-only |
