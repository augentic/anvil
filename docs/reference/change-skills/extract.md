...
| `change-dir` | Yes | Target change directory for generated artifacts |
...
|----------|----------|---------|
| `spec.md` (per capability) | `<change-dir>/specs/<capability>/spec.md` | Requirements with BDD scenarios extracted from source |
...
/spec:extract ./src .specify/changes/initial-baseline/

# Extract only the auth module
/spec:extract ./src .specify/changes/migrate-auth/ --include "src/auth/**"

# Extract using a manifest
/spec:extract ./src .specify/changes/migrate-core/ --manifest ./migration-manifest.txt
```
...
- [/spec:analyze](../initiative-skills/analyze.md) -- the plan-time counterpart (cheap capability summaries vs deep extraction)
