...
Discard a change without merging specs into the baseline.
...
|----------|----------|-------------|
| `change-name` | No | Name of the change to drop. If omitted, prompts for selection. |
...
2. Runs `specify change drop` which:
   - Transitions the change to `dropped`.
   - Archives the change directory.
...
| No active changes | No changes exist to drop | Nothing to do |
