...
`refining` is the transient state used while the `emery plan refine` drain runs. If extract fails for any bound source, the slice stays projecting `refining` until the operator amends the plan (e.g. via `emery plan amend <entry> --remove-source <key>`) or fixes the source binding, then re-runs `emery plan refine`. Synthesis tags (`[unknown]`, `[conflict]`, `[divergence]`) never park the slice — refinement still projects `refined`. Open gaps never block build either: the gap gate auto-defers each open row into a durable `gap.deferred` fact at build time; the deferred requirement leaves build scope and is carried as debt instead. The one exception is an open row with no `requirement-digest` (a legacy `spec.md`-fallback inventory): a deferral fact has no match key without a digest, so the gate refuses with `plan-gap-digest-missing` — run `emery plan refine` to rewrite `model.yaml` and mint the digests, then re-run `emery plan execute`.
...
```text
.emery/change/archive/YYYY-MM-DD-<slice-name>/
```
