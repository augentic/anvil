...
Every halt -- registry validation failure, `stuck`, `registry-amendment-required`, `pending-checks`, an unmerged PR -- surfaces verbatim and stops the umbrella. Re-running `/change:plan --orchestrate <name>` resumes at the first incomplete step. See [`/change:plan --orchestrate`](initiative.md) for the full algorithm.

## Skill summary

| Skill | Layer | Purpose | Reads | Writes |
|-------|-------|---------|-------|--------|
| [/change:plan --orchestrate](initiative.md) | 4 | Drive the cross-repo loop end-to-end (brief -> registry -> plan -> execute -> push -> optional merge -> finalize); was `/spec:initiative` | `change.md`, `registry.yaml`, `plan.yaml`, workspace clones | Composition only -- shells out; never writes directly |
