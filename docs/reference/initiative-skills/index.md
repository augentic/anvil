...
`/spec:plan --orchestrate` strings the full platform-first sequence into one operator action. Layer 3 skills (and the default mode of `/spec:plan`) are still callable directly for partial reruns and CI pipelines.

```d2
direction: right

umbrella: "/spec:plan --orchestrate" {shape: rectangle}
...
Every halt -- registry validation failure, `stuck`, `registry-amendment-required`, `pending-checks`, an unmerged PR -- surfaces verbatim and stops the umbrella. Re-running `/spec:plan --orchestrate <name>` resumes at the first incomplete step. See [`/spec:plan --orchestrate`](initiative.md) for the full algorithm.
...
- **Layer 4 (`/spec:plan --orchestrate`)** -- single command for a cross-repo initiative end-to-end (was `/spec:initiative`).
