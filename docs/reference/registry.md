...
The registry is a first-party Emery component — it owns project membership and location and the local materialised view that workspace-mode planning and execution run against.
...
Target adapters own outcome artefacts and their mechanics; the registry coordinates *where* — which project a slice runs against and how that project's working tree is materialised. The plan (`/emery:plan`, `emery plan *`) coordinates *when* — sequencing slices across one or more registered projects.
...
| `.emery/topology.lock`      | derived  | Committed projection of each member project's identity: authored `target` / `description` plus the deterministic baseline projection `surface[]` (owned domains + requirement titles) and `recent[]` (merge-outcome tail). Machine-written by operator-owned topology tooling; never hand-edited. |
...
| `.emery/scratch/`          | derived  | Transient working state: per-operation agent scratch lanes under `<adapter>/{survey,<slice>}/` and the plan handoff lane under `plan/`. Lanes are recreated empty by their owning verb. |

`.emery/scratch/` and top-level `workspace/` are regenerable and must never be checked in; the per-project cache lives out-of-tree. `emery init` appends the matching `.gitignore` lines idempotently.
...
| `projects[].greenfield_seed.domains` | optional            | Greenfield routing seed only — kebab-case domain slugs that project into the project's plan-time `surface[]` as domains with empty requirements *while the project has no baseline*. The greenfield analog of the derived `surface[]` domain list, it lets a fresh project route leads before `.emery/specs/` exists. Ignored once a real baseline exists (the derived surface supersedes it); a still-declared seed then surfaces the advisory `greenfield-seed-shadowed` finding at plan-authoring time. Carries domain slugs only — never adapter or description material. |

A project's `description` is **not** a registry field — it is authored in the project's own `.emery/project.yaml`. Routing identity (`surface[]` / `recent[]`) is derived from that project's baseline and projected into `.emery/topology.lock`; the hand-authored `capabilities` / `keywords` facets are removed.
...
| ------------------------------- | ----------------- |
| `/emery:plan`                    | Validates the registry before propose; refuses to write a plan against a malformed registry. |
...
Emery exposes no workspace materialization or publication verbs. The operator or surrounding repository automation owns:

- Creating or refreshing each slot from `projects[].url`.
- Regenerating `.emery/topology.lock` from member project metadata and baselines.
...
`emery plan execute` expects required slots to exist and routes project-bound work into them. `/emery:finalize` archives only after the operator confirms publication is complete.

## Dependency direction

The dependency edge is one-way; `emery-core` never depends on the registry.

```text
emery (binary) → workflow → emery-tool → error
                                  ↑
...
- Plan or slice status — owned by `emery plan *` and `emery slice *`.
...
- Synthesis output — owned by core (`/emery:refine`).
- PR metadata — owned entirely by the forge and the operator; Emery does not publish branches or track pull-request state.
