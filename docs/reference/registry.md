...
| `.specify/cache/`            | derived  | Adapter-manifest cache (owned by the plugin resolver, split into `manifests/sources/` and `manifests/targets/` subdirectories, with extraction results under `extractions/` and per-operation agent scratch lanes under `scratch/`). |
