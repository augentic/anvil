...
| `.specify/topology.lock`      | derived  | Committed projection of each member project's identity: authored `target` / `description` plus the deterministic baseline projection `surface[]` (owned units + requirement titles) and `recent[]` (merge-outcome tail). Machine-written by `specrun workspace sync`; never hand-edited. |
