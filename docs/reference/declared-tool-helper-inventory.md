...
- Vectis validation and scaffold rendering: [`adapters/targets/vectis/briefs/`](../../adapters/targets/vectis/briefs/) (the consolidated `shape` / `build` / `merge` briefs that absorbed the retired `vectis-{core,test,ios,android}-writer`, `vectis-{core,ios,android}-reviewer`, and `vectis-template-updater` skill bodies), the [`screenshots` source adapter](../../adapters/sources/screenshots/adapter.yaml) (which houses the body of the retired `vectis-image-layout-inferer` skill), and [`plugins/vectis/references/layout-inferer-contract.md`](../../plugins/vectis/references/layout-inferer-contract.md).
