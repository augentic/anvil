...
- **`composition.yaml` (wired lifecycle artifact)** — the same regions enriched with the wiring keys above. Produced during synthesis (the composition brief reads `layout.yaml` when present) and consumed by shell writers. Validated by the vectis adapter's in-guest `validate composition` behaviour, which auto-invokes `tokens` / `assets` modes when sibling manifests exist.
