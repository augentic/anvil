...
| `specify change plan validate`                    | Base shape rules plus the four health diagnostics (cycle, orphan-source, stale-clone, unreachable-entry). First triage step when `/change:execute loop` reports `stuck`.            |
