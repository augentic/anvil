...
Resolution is **local-only** — there is no download path. Until dynamic loading returns, adapter admission is static: the release binary compiles in the first-party components (`documentation`, `intent`, `typescript`; the pins live in `scripts/first-party.txt`), registered under both their bare and pinned ids, and those are the only adapters `emery specify` can dispatch. A local `.wasm` component path still mirrors into the out-of-tree project cache at init; a name or pin outside the static set fails at the dispatch seam. GitHub URLs are refused (`adapter-github-uri-unsupported`).
