...
A pinned package reference (`emery:intent@1.0.0` or the `intent@1.0.0` shorthand) that misses the global adapter store is **installed automatically** from the compiled first-party registry (`ghcr.io/augentic/emery-adapters/<name>:<version>`) into `<store-root>/<name>@<version>.wasm` with a digest `.meta` sidecar, verified after the write. A pull failure is the typed `adapter-install-failed` (exit `1`); a malformed artifact is `adapter-install-invalid`; a verify failure is `adapter-digest-mismatch`.

A **bare name** resolves **local-first**: a seeded project component cache entry always wins; otherwise the newest installed store version; else pull-latest. Local component paths never pull. GitHub URLs are refused (`adapter-github-uri-unsupported`).
