...
| `--hub` | Scaffold a registry-only platform hub instead of a regular project. Refuses to run when `.specify/` already exists. Mutually exclusive with the `<capability>` positional. |
