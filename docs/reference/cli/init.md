...
Resolution is **local-only** — there is no download path. A seeded project component cache entry always wins, for pinned and bare identities alike; a pinned package reference (`emery:intent@1.0.0` or the `intent@1.0.0` shorthand) otherwise resolves its verified global-store entry (`<store-root>/<name>@<version>.wasm`, verify-on-read against its digest `.meta` sidecar — an unverifiable entry is `adapter-sidecar-missing` / `adapter-digest-mismatch`). Nothing local is the typed `adapter-not-found` (exit `1`); the explicit install verb arrives with the distribution surface. The embedded first-party registry is empty until Phase 4 ports the first-party adapters. GitHub URLs are refused (`adapter-github-uri-unsupported`).
