...
Re-running `emery init` in an already-initialized project changes nothing and exits `0` with a message routing to `emery init --upgrade`. `emery init --upgrade` is the re-entry path: it bumps the `project.yaml.emery` pin over an existing project (preserving every operator artifact) and re-resolves the project's declared adapter. When the re-ensured binding drifts from the record — a bare recorded name whose cache entry was since cleared expands to the embedded adapter-train pin (and pulls) instead of failing — the record is rewritten to the effective binding and the rewrite is announced in the output (`adapter binding rewritten to emery:omnia@<train>`). A bare record with a live cache seed expands to itself, so co-dev upgrades stay bare.
...
A **bare name** (`emery init omnia`) is widened at ensure time: when the project component cache has no seeded entry for the name, init expands it to the binary's embedded first-party **adapter train** pin (`emery:omnia@<train>` — the train version is printed by `emery --version`), which installs through the same pull-on-miss path, and persists the pin on `project.yaml.adapter` before first use. When the cache *has* a seeded entry (via [`emery adapter add`](adapter.md) or a local `.wasm` component at init), the bare name stays bare and resolves the seed — the co-dev path is never shadowed by a published component. The persisted binding value is echoed in the init output, so an auto-pin is self-announcing. Bare names at resolve/dispatch time (after init) remain cache-only and never pull; local component paths never pull.
...
- `adapter-binding` -- the binding value recorded on `project.yaml.adapter` (the effective selector after any adapter-train expansion, e.g. `emery:omnia@0.7.0`); absent in workspace mode and the no-op re-entry
- `adapter-binding-rewritten` -- `true` when `--upgrade` rewrote a drifted recorded binding to the effective ensured selector
