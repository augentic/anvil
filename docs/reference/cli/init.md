...
Scaffolds a single-project setup. Creates `.emery/{slices,specs,archive}/`, resolves the adapter identifier into the out-of-tree per-project cache, writes `.emery/project.yaml` with `adapter:` set, records the running binary's version as `emery-version`, and generates root `AGENTS.md` plus `.emery/context.lock` when `AGENTS.md` is absent.
