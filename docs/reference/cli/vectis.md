...
Vectis deterministic helpers are **in-guest library code** compiled into the vectis adapter's published component. There is no host dispatch verb: the vectis build and merge orchestrations invoke these behaviours directly, and no separate Vectis host binary is part of the command surface.
