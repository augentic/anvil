...
Greenfield scaffold trees are copied on the host from a local [`vectis-template`](https://github.com/augentic/vectis-template) checkout (`../vectis-template` or `VECTIS_TEMPLATE_DIR`) — not from embedded adapter templates. See [Vectis target](../targets/vectis.md).
