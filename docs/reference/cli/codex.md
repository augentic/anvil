...
Exports the resolved codex as the consumer contract for future review tooling. The JSON envelope uses the standard CLI `schema-version` and includes `rule-count` plus ordered `rules`.
