...
Relative component paths anchor at `--project-dir`, which also selects the project the cache is keyed by; when the flag is absent, the project root is the nearest ancestor carrying `.specify/project.yaml`, falling back to the current directory pre-init. The seed runs in the engine guest like every other verb: the shipped binary's deployment policy projects the `adapter add` request from argv pre-boot and preopens the component's parent directory read-only under its absolute host path, so the guest opens the argv path unchanged — the component may live anywhere on the host, outside the project mounts.
