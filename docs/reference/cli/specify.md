...
`emery specify` without any source fails typed with `specify-source-required` (exit `1`) — there is no interactive prompt mode; every input arrives as a flag. Binding the same key twice fails `specify-source-duplicate`; a `--value` entry without `<adapter>=` fails as an argument error (exit `1`); combining `--sources` with positional adapters or `--value` fails as an argument error (exit `1`).
...
Every filesystem input is normalized within the project preopen `.`. Absolute paths and relative paths that escape above it fail as an argument error (exit `1`); the engine never tries to infer a host path from the guest's ambient working directory. The `git` and `url` location keys are reserved: they parse but refuse typed (`source-remote-unsupported`) until the remote read-view grant exists.
