...
The synthesis engine turns the slice's `Evidence[]` plus the bound target's `guidance` prompt into the canonical artifacts and the typed `model.yaml`. It runs as the collapsed judgment leg inside the guest-routed `specify slice refine` (and the `plan execute` loop); the standalone two-phase envelope verb retired at the Omnia-migration cutover.

- The **inputs** leg assembles the **inputs** envelope — each bound source's inline `lead` + `claims` (read from `evidence/<source>.yaml`) plus the resolved target guidance body (the wire field is still spelled `shape-brief`). Authority is **not** included (the kernel resolves it after the response). Read-only: writes nothing and emits a `slice.synthesize.agent` journal event.
