...
```bash
specrun tool run contract -- <BASELINE_DIR> [--format text|json]
```
...
`--format json` writes the validator envelope directly to stdout. `specrun tool run` does not wrap successful guest output:
...
Resolver, permission, or runtime failures come from `specrun tool run` and use the standard Specify error envelope.
...
- [specrun tool](tool.md) — the declared WASI tool runner surface.
