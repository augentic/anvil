...
| [`validate`](#specify-change-plan-validate) | Structural and referential integrity check (cycles, unknown deps, multi-repo invariants). |
| [`doctor`](#specify-change-plan-doctor) | First triage step when `/change:execute` reports `stuck`. Strict superset of `validate` with cycle / orphan-source / stale-clone / unreachable-entry diagnostics. |
