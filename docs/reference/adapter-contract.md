...
**`extract(Lead, Source) → Evidence`** runs inside the refine phase of `emery plan execute` (and standalone `emery source extract`). It returns a structured document the CLI persists to `.emery/slices/<slice>/evidence/<source>.yaml`:
