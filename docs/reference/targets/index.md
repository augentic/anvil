...
- `merge` — landing gate: requires lifecycle `built`, re-runs the target's validators per the merge brief, surfaces conflicts, and drives verification commands. v1 adds **no** merge envelope — `specify slice merge` is the writer and `slice.merge.*` events fire on its validator outcome.
...
```yaml
# yaml-language-server: $schema=https://raw.githubusercontent.com/augentic/specify/main/engine/schemas/target.schema.json
name: omnia
version: "1.0.0"
axis: target
execution: agent
