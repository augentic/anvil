...
- **Missing or too-short rationale** — emits [`UNI-022`](../../adapters/shared/prose/rules/universal/ignore-directive-missing-rationale.md). The 16-character floor is the threshold at which a rationale is long enough to be useful to a future reviewer.
- **Orphan directive** — a directive whose rule id matches no finding on its target line emits [`UNI-023`](../../adapters/shared/prose/rules/universal/ignore-directive-orphan.md). Common causes: the rule was retired, the protected code was refactored, an intervening reformat moved the target, or the directive was copy-pasted into a context where the targeted finding never fired.
...
- [Shared `UNI-*` codex inventory](../../adapters/shared/prose/rules/universal/README.md) — every shared rule, including `UNI-022` and `UNI-023`
