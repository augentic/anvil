...
First-party adapter extensions (`contract`, `vectis`) are **not** built or published by this repo. They live with their adapters in `augentic/specify-adapters` and are packaged + published as immutable registry artifacts (`specify:<name>@<version>`) by that repo's own release workflow. The `specify` binary resolves them at read time from the global adapter store; operators only need the runtime binary.
