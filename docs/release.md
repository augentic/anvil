...
2. **Stabilize** — on the release branch only: check the omnia pins (`cargo build --locked` must resolve on a clean runner — re-pin any local-path `[patch.crates-io]` entry to a pushed rev), run the operator rungs when the change warrants them (`cargo make wasm-run` and `cargo make eval` both need command-mode credentials for whichever backend `EMERY_MODEL_BACKEND` selects — see [the developer loop](contributing/dev-loop.md)), and backport fixes from `main` (fixes land on `main` first when applicable).
...
Bump the Homebrew formula `version` and `sha256` values in `augentic/homebrew-tap` when publishing a new host release. Subsequent updates use the same installation channel. Guest-owned verbs additionally need an authenticated agent CLI on `PATH` at run time — `cursor-agent` by default, or `claude` under `EMERY_MODEL_BACKEND=claude`; the model backend spawns whichever is selected. The engine guest ships inside the binary, so replacing the binary replaces the engine with it.
