...
- **Run the outer test rungs.** `cargo make test-wasm` (the composed smoke, if the scheduled workflow has not covered the release candidate) and `cargo make test-live` (the explicit live-model trial — needs command-mode model credentials; see [the developer loop](contributing/dev-loop.md)). Read rising per-leg repair counts as prompt drift even when the run passes.
