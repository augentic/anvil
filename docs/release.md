...
The shipped surface is the `specify` binary **plus the generic `specify-host` host layer it spawns for guest-owned verbs** (the host externalization): `specify_runtime::drive` resolves `specify-host` beside the `specify` executable, so release archives must package both binaries side by side — **`release-binaries.yaml` does not build or package `specify-host` yet; wiring it in is a pending release-pipeline item.** The `specify-runtime-replay` binary target is a dev/test surface and is never packaged.
