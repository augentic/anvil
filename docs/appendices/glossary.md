...
**Layered stack**
Specify is organised in four layers: the CLI primitives (`specify` commands), the single-slice loop (`/spec:define`, `/spec:build`, `/spec:merge`, plus supporting skills), the multi-slice plan-and-drive skills (`/change:plan`, `/change:execute`, `/spec:analyze`), and the cross-repo orchestration mode (`/change:plan <name> orchestrate`). See [The Layered Stack](../explanation/three-layer-stack.md) for the full picture.
