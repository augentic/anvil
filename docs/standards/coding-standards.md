...
| Timestamp written into JSON | `jiff::Timestamp` with the workflow crate's `serde_time::rfc3339` adapter (or `rfc3339_opt` on `Option<Timestamp>`) | serde owns the format |
