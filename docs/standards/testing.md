...
The live prompt-evaluation rung is the lab-only `crates/eval` package (library + `cli` binary). `native` owns the catalog machinery, `DynModel` erasure, and the seam `Provider`; the eval library owns trial execution, prompt scenarios, telemetry, and deterministic grading, receiving its catalog and model factory from its composition root; the `cli` binary owns the Tokio runtime and Cursor backend, and `cargo make eval` passes trial inputs explicitly. Only the model is live. The same library is consumed by the `lab` binary in `augentic/specify-adapters`, which also owns the first-party catalog declaration and its prompt-scenario root.
