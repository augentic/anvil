...
The live prompt-evaluation rung is the lab-only `crates/probe` library composed by the root `eval` example (`examples/eval/`). `native` owns the catalog machinery, `DynModel` erasure, and the seam `Provider`; the probe library owns trial execution, prompt scenarios, telemetry, and deterministic grading, receiving its catalog and model factory from its composition root; the `client` feature adds the shared cursor composition (`probe::client` — the lazily connected `DevModel` and the argv dispatch); the root example owns the Tokio runtime and the mock catalog binding, and `cargo make eval` passes trial inputs explicitly. Only the model is live. The same `probe::client` is consumed by the matching `eval` example in `augentic/specify-adapters`, which also owns the first-party catalog declaration and its prompt-scenario root.
