...
- `tests/specify.rs` — the `specify` → `show` product arc: bindings (argv, `--description`, `--config`, root discovery), extraction and the extras gate, the three typed model turns and their gates (the grouping partition over the id floor, the `spec.md` draft against the rows, the `design.md` draft against the section plan — each refused typed once the backend's rounds are spent, with one corrected-draft scenario asserting the schema hints ride the request and the findings ride the recorded `check` correction), the canonical renderings asserted byte-for-byte against the stored documents, the revision store (commit, pruning, corruption), re-mine diffs, and multi-project isolation.
