...
`module_name_repetitions` does not fire on `#[test]` fns; keep identifiers short anyway. The 25-char cap is mechanically enforced by the `ident_length` Dylint lint (`augentic_style`, part of `cargo make lint`).
