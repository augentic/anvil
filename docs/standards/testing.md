...
The **component rung** rides the same `make test` invocation: `tests/component.rs` runs the shipped engine component (the root build script's `$OUT_DIR/emery.cwasm`) and the built mock adapter component under the real omnia runtime, assembled by the `test-utils` harness over a scripted host-side model (`test_utils::ScriptedModel`, a `WasiModelCtx` double) and the in-memory `wasi:keyvalue`/`wasi:blobstore` defaults. It owns only what the native scenarios cannot reach — the `wasi:cli/run` wrapper, the WIT lowering on both sides of the `emery:adapter/source` seam, the real path loader and its digest pin, the reference-tool closure over real wasi-model tool streams — and never re-asserts a `tests/specify.rs` behavior. The runtime inherits stdout, so component scenarios observe the exit status and the storage handles, not the envelope. This is the one rung that builds a wasm component: `crates/test-utils/build.rs` nested-builds `--example adapter` for `wasm32-wasip2` into its `OUT_DIR` (omnia's `test-programs` shape), incremental after the first build.

Engine state is observed **through the storage provider and the success envelope, not the filesystem**: since the storage capabilities (design/portable-storage.md steps 1–2), engine-owned state (the output home, the generation store) is written through the `omnia_guest::StateStore`/`BlobStore` capabilities — the `wasi:keyvalue`/`wasi:blobstore` imports in deployment, bound host-side to the durable `omnia-filesystem` store — and the native suites script them with `emery_testkit` (`Memory`, `Namespaced`); local-component loads are scripted through the support module's `Plugins` double, recording each load request and its resolved digest. The component rung reads the same state back through the in-memory host backends (`Backends::state`, `Backends::blob`). No suite changes the process working directory — except the config-discovery scenarios, which enter a `tempfile::TempDir` to control what a bindingless run finds (safe because nextest runs each test in its own process) — and no suite asserts an on-disk layout: the backing is host policy, exercised on the live rung, not in tests. Operator-supplied inputs (a local `.wasm` component, an `emery.toml`) stay real files in a `tempfile::TempDir` (the component rung's `test_utils::scratch()`).
...
- `tests/component.rs` — the component rung: bare-name dispatch to a statically declared adapter, path loads with an agreeing and a disagreeing digest pin, a seamless component refused by the loader, and the reference-tool round-trip served in-process.
...
| ---------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Root product integration** | root `tests/` (`specify.rs`, `command.rs`, `plugin.rs`, `component.rs`) | The behavior is reachable through CLI argv and observable at a public boundary — exit code, stdout/stderr, the JSON envelope, scripted storage. This is the default and covers the large majority. `component.rs` takes only what needs the wasm boundary itself | The behavior is a library contract with no product projection, or arranging it through the entry points needs private state no CLI input can produce                                                  |
...
- Put a scenario in `tests/component.rs` only when the wasm boundary is the subject; anything the in-process router can reach belongs in the native suites, where the envelope is observable.
