...
The end-to-end fan-in-twice / fan-out-once path now runs through the guest orchestrators (`orchestrate::{author,execute,refine,build,merge}`) — its coverage lives in `crates/workflow-lib/tests/` (orchestrate, merge_slice) and the guest-routing tests in `tests/guest.rs`; the exhaustive reconcile-code coverage over the same fan-out shape lives in `tests/workflow/`.
