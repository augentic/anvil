...
The prompt-evaluation harness (`crates/eval`) reuses the same `testkit` fixture plumbing — the adapter core, the model-generic `Provider<M>`, the `run` invoker, and the adversarial bindings — but binds a live cursor-agent model behind the judgment legs instead of `Scripted`. Only the model is live; the leads, evidence, and grading assertions are the deterministic fixture set.
