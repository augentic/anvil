...
| ---------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Native integration** | `cargo make test` | The complete workflow — reconciliation, synthesis, build, merge, lifecycle — through the workflow crates' public operations, the `fixture` catalog behind the offline `linked` provider, and scripted model doubles                                                                                                           | Every push (part of `cargo make ci`)                                                                                                                              |
...
The live prompt-evaluation rung is the lab-only `crates/eval` library driven by the unpublished `crates/lab` binary. `linked` owns the catalog machinery, `DynModel` erasure, and the seam `Provider`; `eval` owns trial execution, prompt scenarios, telemetry, and deterministic grading, receiving its catalog and model factory from the composition root; the lab owns the Tokio runtime and Cursor backend, and `cargo make eval` passes trial inputs explicitly. Only the model is live. The same libraries are consumed by the `lab` binary in `augentic/specify-adapters`, which also owns the first-party catalog declaration and its prompt-scenario root.

## Integration-first policy

Integration tests live in each crate's `tests/` directory and assert against public boundaries — stdout JSON, exit codes, filesystem state. Each `tests/<area>.rs` file is its own auto-discovered test binary — `crates/change/tests/handlers.rs`, `crates/change/tests/full_loop.rs`, and so on. Cross-crate helpers come from the `fixture` and `linked` dev-dependencies (`use fixture::…`, `use linked::…`); crate-private helpers live in the dir form `tests/<helper>/mod.rs` (invisible to auto-discovery), declared per binary with `mod <helper>;`. Repo invariants (adapter boundary, plugin authoring shape) live in the lightweight `crates/checks` package — docs/plugin link integrity is lychee's job (`cargo make links`); fixtures are crate-local under `crates/<name>/tests/fixtures/`.
...
- Spin up a real scaffold in a `tempfile::TempDir`. Reach for the shared helpers in `fixture` — `fixture::session::Session` (its constructors mint the tempdir and pin the project cache), the generalized `fixture::invoke::run::<Op, _, _>` invoker helper, and the `fixture::answers` corpus.
