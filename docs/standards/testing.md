...
- `tests/specify.rs` — the `specify` → `show` product arc: bindings (argv, `--description`, `--config`, root discovery), extraction and the extras gate, the three typed model turns and their gates (the grouping partition over the id floor, the `spec.md` draft against the rows, the `design.md` draft against the section plan — each refused typed after every repair, with one repaired-draft scenario asserting the findings ride the second request), the canonical renderings asserted byte-for-byte against the stored documents, the revision store (commit, pruning, corruption), re-mine diffs, and multi-project isolation.
