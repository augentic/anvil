...
The Gate 1 stamp lives in `project::plan::stamp_approved`, invoked by the `plan execute` orchestration on a `pending` plan (idempotent — an already-approved plan is a no-op returning `false`; a performed stamp returns `true` and surfaces as `gate1-stamped` on the execute body) and emitting a `plan.transition.approved` journal event; `plan undo <entry>` is the one-rung reverse walk. In both cases the journal append runs *after* `with_state` returns so the plan write and the journal append cannot interleave on failure.
