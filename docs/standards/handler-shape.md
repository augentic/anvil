...
### The lint handler tail

`specify lint framework` (the one surviving lint surface — `lint project` retired from the operational surface) returns `Result<()>` from the handler in [`crates/dispatch/src/commands/lint/framework.rs`](../../crates/dispatch/src/commands/lint/framework.rs): it assembles its `ResolveInputs` + `PipelineConfig`, runs the pipeline, and renders the envelope on stdout, then collapses the outcome into the terminal `Result<()>` — `deny_blocking_findings` on success, the empty-envelope stdout fallback on a pre-emit abort. The fallback owns only the **stdout** side (an all-zero `DiagnosticReport`, JSON only, so CI consumers keep a stable shape); the stderr `error: …` line is the dispatcher's `output::report`, so the two sinks compose without double-printing.

`specify lint framework` is just another action on the one `specify` binary: it obeys this same `Result<()>` contract and maps its terminal error through the one `Exit::from(&Error)` table in [`crates/dispatch/src/output.rs`](../../crates/dispatch/src/output.rs) exactly as every other verb does. Only bootstrap verbs (`upgrade`) justify a bespoke exit subset; lint does not.
...
`Exit::from(&Error)` in [`crates/dispatch/src/output.rs`](../../crates/dispatch/src/output.rs) is the single source of truth. Every dispatcher in `src/runtime/commands/*` routes its terminal error through `report`, which calls `Exit::from`. Do not invent new exit codes. The long-form decision (including `Exit::Code(u8)`'s WASI passthrough role) lives in [DECISIONS.md §"Exit codes"](../../DECISIONS.md#exit-codes).
...
`axis` to invoke `specify_workflow_lib::adapter::SourceAdapter::resolve(name,
