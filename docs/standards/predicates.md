...
| `checkNoEnvelopeExamples` | Fenced ` ```json ` / ` ```jsonc ` blocks whose body looks like a CLI envelope wrapper (`"envelope-version"` key, or `"ok"` + `"data"` / `"error"` pair). Link to `plugins/references/cli-output-shapes.md` instead. | [scripts/checks/skill_discipline.ts](../../scripts/checks/skill_discipline.ts) | Per-file baseline in `scripts/standards-allowlist.toml`. |
