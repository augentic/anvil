...
Headline verbs: `init`, `source {resolve, survey, extract}` and `target resolve` (the adapter debug/breakout surface), `slice {list, refine, model show, build, validate, provenance, merge, drop}`, `plan {author, execute, add, amend, remove, undo, advance, status, gaps, archive}`, and `journal show`. Workspace slots and topology remain plan inputs, but repository materialization and publication have no Emery command surface. `plan status` is the read-only next-action projection (artifacts + fact union → `refine|build|merge <slice>` / `review-gaps` / `stop <reason>` / `drained`, plus Ready / Authorized); the same body carries `current-step` / `last-completed` (loop position) and `resume` (the literal command that makes progress, `null` when no single command does); it writes nothing. `plan gaps` surfaces the typed gap inventory (shared-lead rollup is presentation only). `journal show [--filter <event-id-prefix>] [--limit N]` is the read verb over the per-writer union — text mode emits the canonical JSONL lines; it emits no event of its own. See [`emery init`](../reference/cli/init.md) and the parent repo's [`AGENTS.md` §"Skill / CLI responsibility split"](https://github.com/augentic/emery/blob/main/AGENTS.md#skill--cli-responsibility-split).
...
Progress is projected, not stored. Topology rows are written by `plan author` / `plan add` / `plan amend` / `plan remove`. Ladder / ownership facts: `plan advance` appends `slice.claimed` + `plan.entry.advanced` (projects `in-progress`); `slice merge` appends wave-commit / archive facts (projects `done`); `plan undo <entry>` retracts rung facts (`fact.retracted`, labelled `plan.transition.undone`) — one rung per call, or several with `--to <status>`. Authorization is a write: starting `emery plan execute` (directly or through `/emery:execute`) appends `plan.execute.started` with typed coverage; there is no `plan approve` verb.
