...
In workspace mode, `specify workspace sync` provisions probe location 1 for each synced slot: it mirrors the workspace's component cache (components plus the provenance sidecar) into the slot's component cache, per-file copy-over.
