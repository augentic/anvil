...
Resolution is project-local plus the global store; there is no sibling-checkout probe and no environment-variable fallback to an out-of-tree framework checkout. An adapter built elsewhere reaches the project as an operator-supplied local `.wasm` at init (mirrored into the project component cache) or a pinned store install. When no probe matches, resolution fails with `adapter-not-found`, naming every probed path.
...
Touching `Slice.target`, `SliceSourceBinding`, `Divergence`, `crates/artifacts/src/spec/provenance.rs`, `crates/project/src/adapter/`, `crates/project/src/journal.rs`, or `crates/project/src/schema_gate.rs` requires a repo-wide `rg` sweep across both the in-tree Rust workspace and the surrounding `augentic/specify` prose in the same PR — the contract spans both trees.
