...
Per-entry: `pending → in-progress → done`. There is no plan-level lifecycle — running `emery plan execute` is the operator's approval, and progress is computed from per-entry status. `done` is absorbing in v1; the operator-reversed flow lives behind `emery plan undo`.
...
Headline verbs: `init`, `source {resolve, survey, extract}` and `target resolve` (the adapter debug/breakout surface), `slice {list, refine, model show, build, validate, provenance, merge, drop}`, `plan {author, execute, add, amend, remove, undo, advance, status, archive}`, and `journal show`. Workspace slots and topology remain plan inputs, but repository materialization and publication have no Emery command surface. `plan status` is the read-only next-action projection (plan entries + slice metadata + journal tail → `refine|build|merge <slice>` / `stop <reason>` / `drained`); the same body carries the RM-15 re-entry fields — `current-step` / `last-completed` (loop position) and `resume` (the literal command that makes progress, `null` when no single command does); it writes nothing. `journal show [--filter <event-id-prefix>] [--limit N]` is the read verb over the journal — text mode emits the canonical JSONL lines; it emits no event of its own. See [`emery init`](../reference/cli/init.md) and the parent repo's [`AGENTS.md` §"Skill / CLI responsibility split"](https://github.com/augentic/emery/blob/main/AGENTS.md#skill--cli-responsibility-split).
...
Per-entry status writes route to exactly one CLI verb each — `plan add` / `plan amend` write `pending`, `plan advance` writes `in-progress`, `slice merge` writes `done` directly (and re-stamps it when healing a torn merge). Approval is not a write: running `emery plan execute` (directly or through `/emery:execute`) is the operator's approval of the plan. The only backward walk is `plan undo <entry>` — one rung per call, or several with `--to <status>`.
...
## D7 — Executing is approving (no approval state)

Invoking `emery plan execute` is the operator's approval of the plan. Nothing is stamped or journaled: there is no `plan approve` verb, no stored plan lifecycle, and no auto-approve flag. `/emery:plan` never runs execute; the pause between authoring and the operator's execute invocation is the review seam.
