...
Touching `Slice.target`, `SliceSourceBinding`, `Divergence`, `crates/artifacts/src/spec/provenance.rs`, `crates/workflow/src/adapter/`, `crates/workflow/src/journal.rs`, `crates/workflow/src/schema.rs`, the `$CAPABILITY_DIR` env var, or the `adapter--<axis>--<slug>` tool cache scope requires a repo-wide `rg` sweep across both the in-tree Rust workspace and the surrounding `augentic/specify` prose in the same PR — the contract spans both trees.
