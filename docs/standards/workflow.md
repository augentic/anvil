...
`emery init <adapter>` accepts a package reference (`emery:<name>@<semver>` — installed into the store on fetch), the first-party **shorthand** (`omnia@1.0.0` is package-reference sugar; bare `omnia` resolves the seeded project component cache), or a local `.wasm` path. GitHub URLs are refused (`adapter-github-uri-unsupported`).
