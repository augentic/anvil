...
The `--auto-approve` surface left with the standalone `plan create` verb. Gate 1 is stamped exclusively by `specify plan transition <plan-name> approved`; a repeated stamp on an already-approved plan is an idempotent no-op (no disk write, no journal event).
