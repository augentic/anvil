...
- Lead with an **imperative verb** drawn from the curated allow-list in [tooling/src/check/skill_frontmatter.rs](../../tooling/src/check/skill_frontmatter.rs) (`IMPERATIVE_VERBS`). Extend the allow-list when a new verb is genuinely imperative; adding a verb is cheaper than rejecting a fine description.
...
`SkillDescriptionGrammarCheck` and the skill schema enforce the three rules respectively. See [tooling/src/check/](../../tooling/src/check/) for the implementation.

## Argument-hint grammar

Each `SKILL.md` `argument-hint:` value is a whitespace-separated sequence of tokens drawn from a fixed grammar (enforced by `SkillArgumentHintGrammarCheck` in [tooling/src/check/skill_frontmatter.rs](../../tooling/src/check/skill_frontmatter.rs)):
...
Both caps are enforced by `SkillBodyLineCount` and `SkillSectionLineCount` (see [tooling/src/check/skill_body.rs](../../tooling/src/check/skill_body.rs)).
...
1. **No frontmatter on briefs.** Briefs are not skills. They do not declare `name`, `description`, `argument-hint`, `id`, or any other YAML frontmatter — the loader resolves briefs by path from `adapter.yaml` and never reads frontmatter, so any leading `---` block is decoration that drifts and duplicates the body H1. Mechanically enforced by `BriefCheck` in [tooling/src/check/brief.rs](../../tooling/src/check/brief.rs).
2. **Parent briefs cap at 150 non-blank lines (hard).** Parent briefs orchestrate; orchestration that needs more than 150 lines means a sub-brief is missing. Enforced by `BriefCheck` in [tooling/src/check/brief.rs](../../tooling/src/check/brief.rs).
