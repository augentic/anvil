...
Binary crate (`name = "specify-cli"`) at the repo root. [`core/runtime.rs`](../../core/runtime.rs) is a single `omnia::runtime!` invocation in command mode over the cursor-bound backends — the binary carries no Specify vocabulary. Every verb runs in the specify guest ([`core/lib.rs`](../../core/lib.rs)) through the shared cli grammar (`crates/cli`). Workspace member crates live under `crates/`; the dependency direction is leaf → root:
...
specify                  # wasm32 wasi:cli/run core guest — co-located under core/lib.rs
...
The root `specify-cli` package carries the Omnia deployment unit under `core/`: the guest lib (`core/lib.rs`), the shipped runtime (`core/runtime.rs`), and the replay sibling (`core/replay.rs`). The whole `specify` clap tree lives in `crates/cli`; clap introspection for shell completions lives in [`crates/cli/src/commands.rs`](../../crates/cli/src/commands.rs) via `Cli::command()`.
...
- **`crates/workflow/src/adapter/`** — axis-split resolver (one component, no manifest). `SourceAdapter::resolve(adapter_ref, project_dir)` and `TargetAdapter::resolve(adapter_ref, project_dir)` are the per-axis entry points; each resolves the identity's single `.wasm` component (pinned identity → global store entry; bare name → project component cache, then the sibling/in-repo development release build) and carries its closed operation set (`SourceOperation` / `TargetOperation`) derived from the WIT contract. Non-identity metadata comes from the component's own deterministic `describe` export, dispatched host-side at resolve time and cached against the component digest; a wrong-axis component is the typed `adapter-axis-mismatch`. See [DECISIONS.md §"Adapter loader axis routing"](../../DECISIONS.md#adapter-loader-axis-routing) for the long form. Operation prompts are compiled into each adapter's guest — the CLI never resolves or reads prompt bodies.
...
- **`crates/workflow/src/schema.rs`** — workflow-aware validation wrappers for the on-disk workflow artifacts (`schemas/plan/plan.schema.json`, `schemas/evidence.schema.json`, `schemas/discovery/lead.schema.json`). The raw embedded schema constants and the generic `jsonschema` plumbing live in `crates/schema/` (`schema`) per [DECISIONS.md §"Standards chain moved to the adapters"](../../DECISIONS.md#standards-chain-moved-to-the-adapters-diagnostics-merged-into-schema); this module imports them and adds the workflow-shaped error aggregation (the `rule_id` strings the CLI surfaces, joined into the payload-free error `detail`). Validators return the payload-free `Error::Validation { code, detail }` so the CLI exits with code 2 (`Exit::ValidationFailed`) with the specific discriminant as the wire `error`; surfaces that render findings (`slice validate`) emit a `DiagnosticReport` on stdout first. `specify plan add` / `plan amend` / `slice validate` are the first-use hooks. See [DECISIONS.md §"Drained `Error::Validation` and the `Diagnostic` substrate"](../../DECISIONS.md#drained-errorvalidation-and-the-diagnostic-substrate).

## Adapter component resolution

`SourceAdapter::resolve` / `TargetAdapter::resolve` probe — in order — the global single-file store (pinned identities: `<store-root>/<name>@<version>.wasm`, verify-on-read), the project component cache (`<project-cache>/components/<name>.wasm`, mirrored at init from an operator-supplied local file), and the sibling/in-repo development release build (`target/wasm32-wasip2/release/<name>.wasm`). A binding names the axis; the describe dispatch checks the component's exports before instantiating (`adapter-axis-mismatch` on the wrong axis). See [DECISIONS.md §"Adapter store"](../../DECISIONS.md#adapter-store).
