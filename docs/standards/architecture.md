...
**New workspace crates** are an exception, not the default. `emery-testkit` is the unpublished storage-double crate for native suites (`Memory`, `Namespaced`); do not grow it into a general helper crate. `test-utils` is the other unpublished exception: the component rung's fixture build (`build.rs` nested-builds `--example adapter` to `wasm32-wasip2`, omnia's `test-programs` shape) and deployment harness — a test-only crate, never a dependency of production code.
