...
- **`crates/project/src/journal.rs`** — newline-delimited JSON per-writer event logs at `<project_dir>/.emery/change/events/<writer>.jsonl`. Closed `Event` / `EventKind` taxonomy; kebab-case dotted wire ids (`plan.entry.advanced`, `plan.amend.divergence`, `slice.transition.refined`, `slice.extract.completed`, `slice.synthesis.{conflict,divergence,unknown}`) bridge to `snake_case` Rust variants via `#[serde(rename = "…")]`. Each journal writer appends only to its file; readers union by `(timestamp, writer, sequence)` and skip blank lines.
