...
examples                 # Omnia-shaped examples package — adapter lib + live / wasm guest / smoke examples
...
`examples` is the Omnia-shaped package that prevents adapter test behavior from being copied across the native workflow suites and the WASM smoke: `native/adapter.rs` is the shared lib, `wasm/adapter.rs` is its thin WIT component example, and the host runners (`wasm-smoke`, `live-model`) sit behind the package `host` feature. It carries no production lifecycle authority and is never linked into the shipped guest. Generic model doubles, temporary manifests, runtime hosting, and HTTP driving remain in upstream `omnia-testkit`.
