...
Deployment crate (`name = "emery"`) at the repo root. [`src/main.rs`](../../src/main.rs) is one `omnia::runtime!` command-mode invocation over the cursor-bound backends: the engine guest is embedded as static component bytes (`include_bytes!` over `$OUT_DIR/emery.cwasm`, produced by the root `build.rs` — a child wasm32 build into an isolated target directory, then in release builds an ahead-of-time wasmtime serialize so startup deserializes instead of JIT-compiling the engine; debug builds embed the raw component and JIT at startup; there is no placeholder fallback), and `program: "emery"` forwards raw argv to the explicit `command_guest`. Deployment policy is static and CWD-rooted, expressed inline in the invocation: the invocation directory mounts read-only as the guest's `.` — nothing writes the working tree, the C3 least-authority posture (no ancestor walk — a verb run below the project root fails typed), the storage hosts bind the generation store to the durable omnia-filesystem store (default `.omnia/storage`), and the `plugins:` block declares the source seam (`emery:adapter/source@0.1.0`) plus the compiled-in acquirer (`acquire: MountAcquire` — preopen-relative reads, fresh on every load, never cached). The shipped `src/main.rs` embeds the engine only: a local `.wasm` adapter loads at run time through the `omnia:plugins/loader` capability (the journey host in [`examples/runtime.rs`](../../examples/runtime.rs) loads its built mock component the same way), while bare and package-pinned adapters still dispatch only statically declared guests. Registry acquisition is deferred; there is no download path or speculative global adapter store. Every invocation — help, version, and grammar rejections included — runs in the emery guest ([`src/lib.rs`](../../src/lib.rs)) through the shared command grammar (`emery_engine::cli`).
...
- **`crates/engine/src/resolve.rs`** — source-adapter resolution over the typed `AdapterSelector`: selector parsing, the `omnia:plugins/loader` load request for local components (over the provider's `Plugins` capability, threading the binding's digest pin and returning the resolved digest), and the adapter `emery`-floor gate, dispatched directly over the provider's `Source` capability (the WIT `metadata` import on wasm32, a scripted mock natively).
...
Resolution dispatches identities by routed id (the WIT `metadata` export). A local component selector loads through the deployment's `omnia:plugins/loader` capability on every `specify` that names it: the host's acquirer reads the file fresh (nothing is mirrored, so a deleted file refuses typed on the next run), verifies the binding's optional sha256 pin before validation, validates the component against the declared source seam, and registers it under its routed id; the resolved digest rides the success envelope so an unpinned first run can be committed as the pin (trust-on-first-use). Bare and package-pinned selectors still dispatch only guests declared in the runtime invocation; anything else fails at dispatch. Registry acquisition is deferred; there is no download path and no sibling-checkout or build-tree probe. Storage locations are fixed key and container-name formulas — the same names resolve against whatever backing the deployment binds. A binding names the axis; a component bound on the wrong axis fails at dispatch — no deployed guest exports the requested `<axis>:<name>` id.
