...
`crates/testkit` is the single test-support crate that prevents adapter test behavior from being copied across the native workflow suites and the WASM smoke: `testkit::adapter` is the shared fixture core, `examples/wasm/adapter.rs` is its thin WIT component example, and the examples host runners (`wasm-smoke`, `live-model`) sit behind that package's `host` feature. Neither carries production lifecycle authority and neither is linked into the shipped guest. Generic model doubles (scripted, replay, recorder), temporary manifests, runtime hosting, and HTTP driving remain in upstream `omnia-testkit`.
