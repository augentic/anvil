...
The `source` example (`examples/source.rs`) is the one mock source adapter: a `emery_adapter::SourceAdapter` implementor whose extract calls the host `Model`. The `runtime` example hosts it with the Cursor backend and a path-only plugin `locations:` list. It carries no production lifecycle authority and never enters the shipped guest. Do not add another mock adapter — extend this example.
