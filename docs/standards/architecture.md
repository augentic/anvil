...
Deployment crate (`name = "specify"`) at the repo root. [`src/omnia.rs`](../../src/omnia.rs) composes the self-assembling deployment launcher (`crates/launcher`, RFC-70 Stage 1) with an `omnia::runtime!` command-mode invocation nested in `mod host` over the cursor-bound backends: the native `main` prints `--version` fast, otherwise runs `launcher::prepare` (anchor → closure → hydrate + verify → assemble) and hands the typed in-memory deployment to the generated `host::run`. The `host` module carries no Specify vocabulary. Every verb runs in the specify guest ([`src/lib.rs`](../../src/lib.rs)) through the shared command grammar (`crates/transport`). Workspace member crates live under `crates/`; the dependency direction is leaf → root:
...
transport                # wasm-clean transport assembly — shared typed command/HTTP routers, Args conversions, projectors, exit contract, and the launcher-facing selector projection (transport::command::selectors); depends on {project,slice,change}
launcher                 # native-only deployment launcher (RFC-70 Stage 1) — project-root anchoring, per-invocation closure derivation, registry hydration (reqwest) with fail-closed store verification, typed deployment assembly; depends on {project,transport}
...
checks                   # dev-only repo invariants — boundaries, authoring (plain cargo tests; book links are mdbook-linkcheck2)
native                   # the native host — validated adapter catalog over the SDK traits, DynModel, seam provider, reference hosting, cli-gated command execution; no concrete adapter deps
eval                     # lab-only live-model evaluation library — trial, scenarios, grading, telemetry, sandbox over native's cli feature; no runtime, no Cursor, no concrete adapters
lab                      # unpublished composition binary — Tokio runtime, Cursor backend, mock catalog binding; dispatches native command mode vs eval::run
guest                    # the engine guest library (wasm32-only) — workflow-world WIT bindings, the WIT-backed Provider, and the guest::export! macro
specify (root crate)     # Omnia deployment unit under src/: wasm32 guest cdylib (one guest::export!()) plus the launcher-fronted binary (native main over launcher::prepare + the nested omnia::runtime! host); carries the examples/wasm adapter components (SDK export macros over mock::Adapter)
```
