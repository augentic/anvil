...
The authoritative leaf → root crate graph (with per-crate roles) lives in [AGENTS.md](../../AGENTS.md). Headline shape: `adapter` is the publishing leaf; `engine` aliases `omnia_guest::Error` as `emery_engine::handler::Error`, owns the domain operations (`specify`, `show`, adapter resolution) and the typed command routing (`emery_engine::cli`); the root package's `src/main.rs` owns native deployment policy inline and its wasm32 lib is the one provider (the component contract is the only production boundary, ADR-0002). The journey's mock source lives in the `source` example, not a workspace crate.
