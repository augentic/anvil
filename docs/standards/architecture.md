...
**New workspace crates** are an exception, not the default. Capability doubles are not a local crate: the native suites take them from omnia's `omnia-test` (`guest::{Scripted, Memory, Namespaced}`, `delegate!`). `test-utils` is the one unpublished exception: the component rung's fixture build (`build.rs` drives `omnia_test::build::Components` over `--example adapter` for `wasm32-wasip2`) and the `run` overlay of the shipped deployment (`omnia_test::host::Deployment::from(emery::manifest())` driven through `emery::Hooks`) — a test-only crate, never a dependency of production code.
