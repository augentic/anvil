...
- **`crates/project/src/journal.rs`** — newline-delimited JSON per-actor event logs at `<project_dir>/.emery/events/<actor>.jsonl`. Closed `Event` / `EventKind` taxonomy; kebab-case dotted wire ids (`plan.entry.advanced`, `plan.amend.divergence`, `slice.transition.refined`, `slice.extract.completed`, `slice.synthesis.{conflict,divergence,unknown}`) bridge to `snake_case` Rust variants via `#[serde(rename = "…")]`. Each actor appends only their file; readers union by `(timestamp, actor, sequence)` and skip blank lines.
