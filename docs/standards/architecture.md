...
One `specify-standards` module tree carries the standards-layer contract; touching it requires a cross-repo `rg` sweep per [AGENTS.md §"When working in this repo"](../../AGENTS.md#when-working-in-this-repo).

- **`crates/standards/src/rules/`** — rules parser and resolver pipeline (`parse.rs`, `resolve.rs`, `resolve/{filter,sort}.rs`). The fingerprint algorithm and finding validators live in the `specify-diagnostics` leaf — import them from there directly. The resolver walks the shared `codex/rules/universal/` pack (`Origin::Shared`) plus per-adapter overlays (`Origin::{SourceOverlay,TargetOverlay}`) and tags every resolved rule with its origin for `specify rules export`. The four formatters live in the neutral `specify-diagnostics` leaf (`crates/diagnostics/src/render/{json,pretty,github,compact}.rs`) and consume the closed `Diagnostic` shape every surface emits.
