...
The phase skills themselves stay unaware of the plan — they operate slice-by-slice. Plan *entries* are written via `emery plan author` (default), `emery plan add`, `emery plan amend`, and `emery plan remove`; per-entry status moves forward through `emery plan next` and `emery slice merge`, and backwards only through `emery plan undo`. A phase that discovers a neighbouring slice mid-run (e.g. a define brief uncovering a bug fix that should be tracked) may shell out to `emery plan add` / `emery plan amend` — the same commands humans run.
...
|---|---|---|
| Plan | `plan.transition.approved`, `plan.transition.undone`, `plan.entry.advanced`, `plan.reconcile.completed`, `plan.amend.authority-override`, `plan.amend.divergence` | the Gate 1 stamp inside `emery plan execute` (with the `actor` field), `emery plan undo`, `emery plan next`, the `plan author` reconcile kernel, `emery plan amend` |
