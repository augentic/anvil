...
Stream roles are part of the contract: the semantic result body (text or JSON) is stdout; the failure body and live `RUST_LOG` tracing are stderr. Skills prefix every `emery` subprocess with an explicit `RUST_LOG` per the plugin rule's tracing contract — the INFO filter for the long-running orchestrations (`plan author`, `plan execute`, `slice refine`, `slice build`, committed `slice merge run`), `RUST_LOG=off` for probes and short deterministic verbs, and the `omnia_cursor=debug,omnia_wasi_http=debug` extension on every subprocess when the operator asks for debug — and relay the semantic result once without repeating tracing lines.
