...
Every CLI verb that skills consume emits a stable envelope: a top-level `envelope-version` plus either a `data` payload (on success) or an `error` block (on failure, with a kebab-case `code` discriminant). Skills invoked with `--format json` parse the envelope and branch on `code` rather than on stdout text.

The canonical envelope shapes — including the success / error variants, the `Diag` shape, and stable anchors for each verb's `data` payload — live in [plugins/references/cli-output-shapes.md](../../plugins/references/cli-output-shapes.md). SKILL.md bodies **link** to that reference rather than embedding envelope JSON inline; the `checkNoEnvelopeExamples` predicate enforces the rule (see [predicates.md](predicates.md)).

The `error.code` discriminants are part of the public contract that skills and tests grep for. Examples skills handle today:
...
| `1` | `EXIT_GENERIC_FAILURE` | Default `Error` mapping; parse `error.code`. |
...
- [docs/standards/predicates.md](predicates.md) — mechanical checks `make checks` runs against the surrounding prose.
