...
1. **Author.** `/emery:plan <change-name> source <key>=<path-or-url> ...` runs each bound source adapter's `survey` operation, reconciles leads across sources into proposed `slices[]` rows, validates the plan, and exits after authoring. The skill stops at the operator topology-review seam — the literal `emery plan refine` command is printed for the operator.
2. **Refine.** Invoking `emery plan refine` (directly or through `/emery:refine`) drains specification refinement over the closed plan: per in-scope leaf in dependency order it extracts, synthesizes, validates, and atomically writes `refinement.yaml`; fresh manifests are skipped, and the drain stops typed on the first failure (`plan-refine-stopped` on stderr, exit 2, with the canonical stop card on stdout). Optional repeated `--slice <name>` targets specific leaves plus their stale-or-missing predecessor closure. No epoch, workspace, wave, or code work. Gaps are review outputs — the drain persists them and points at `emery plan gaps`. The pause afterwards is the specification-review seam; the engine adds no approval file or projected `approved` state.
3. **Execute.** Invoking `emery plan execute` requires a fresh refinement manifest for every in-scope leaf (a missing or stale manifest is the typed `plan-refinement-required`, before any epoch, workspace, or wave — execute never refines), opens the authorization epoch (`plan.execute.started` with typed `closed-plan` coverage — the exact plan digest plus per-leaf refinement digests), and drives the loop. Optional `--waive <slice>/<req> --reason …` nests unknown-waivers on that epoch; `[conflict]` is never waiveable. `/emery:plan` never runs it; `/emery:execute` wraps it. Under the guest lock the loop claims → builds → merges per entry (gap gate before each build). Per-entry `done` projects from merge / archive facts. Exits on the first `stop <reason>` (the `plan-execute-stopped` error envelope on stderr, exit 2, with the canonical plan-status stop card on stdout — no follow-up `emery plan status` call needed), on a hard gap/epoch refusal, or on `drained` (the success body carries `plan` and `phases[]`; text mode prints the phase lines and closes with the canonical `drained — run /emery:finalize <plan>` line). A fresh plan's `plan status` projection exposes `/emery:refine` as its `resume` so the operator path starts with refinement.
