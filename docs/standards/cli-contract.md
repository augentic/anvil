...
|---|---|---|
| Plan | `plan.entry.advanced`, `plan.reconcile.completed`, `plan.amend.authority-override`, `plan.amend.divergence`, `plan.execute.started`, `plan.merge-postflight.acknowledged` | the execute loop's claim step, the `plan author` reconcile kernel, `emery plan amend`, `emery plan execute` (authorization epoch at start — RFC-86 D6; optional `--waive` / `--reason` for unknown-waivers — D17) |
