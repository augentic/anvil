...
1. **Author.** `/spec:plan <change-name> source <key>=<path-or-url> ...` runs each bound source adapter's `survey` operation, reconciles leads across sources into proposed `slices[]` rows, validates the plan, and exits at `plan.lifecycle: pending`. The skill stops at the operator review seam — execution does not start automatically and the literal `specify plan approve` command is printed for the operator.
2. **Gate 1.** Operator runs `specify plan approve` — the only writer of `approved`. `/spec:plan` never stamps `approved` itself; `/spec:execute` runs the same verb only behind an explicit operator confirmation (`--actor` stays at its `operator` default — the stamp relays the operator's decision).
...
|---|---|---|
| Plan | `plan.transition.approved`, `plan.transition.undone`, `plan.entry.advanced`, `plan.reconcile.completed`, `plan.amend.authority-override`, `plan.amend.divergence` | `specify plan approve` (with the `actor` field), `specify plan transition --undo`, `specify plan next`, the `plan author` reconcile kernel, `specify plan amend` |
