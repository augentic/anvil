...
Each `### emery <group> <verb>` section on a CLI reference page carries these elements, in order (verify every fact against `crates/cli/src/lib.rs` and the exit table in [AGENTS.md](../../AGENTS.md#exit-codes)):
