...
Observation-only: classifies every guard, prints the per-project status table, and stops. Never invokes `gh pr merge` (`workspace merge` is a separate verb anyway) and never moves files. Useful for "is this change ready to land?" checks before you commit.
...
- [`specify change finalize` -- output](../reference/cli/change.md#specify-change-finalize) for the finalize status table (`merged`, `unmerged`, `closed`, `no-branch`, `branch-pattern-mismatch`, `dirty`, `failed`) and the JSON v2 envelope.

## See also

- [Cross-Repo Initiatives](../tutorials/cross-repo-change.md) -- end-to-end walkthrough that exercises this landing flow.
...
- [`/change:plan --orchestrate`](../reference/change-skills/change.md) -- the Layer 4 umbrella mode (formerly `/spec:initiative`) that automates this whole sequence with `--auto-merge`.
