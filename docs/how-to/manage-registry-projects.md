...
- A adapter identifier for the new project (`omnia@v1`, `vectis@v1`, or any other resolvable adapter).
...
3. **Adapter non-empty:** the `--schema` adapter value must be non-empty after trim.
...
2. **Recreate the offending entry**: `specify registry remove <existing>`, then `specify registry add <existing> --url <existing-url> --adapter <existing-adapter> --description "..."`. (This second path also triggers the plan-reference warning if the existing entry was already wired into a plan -- preferable to use the hand-edit unless the description rewrite is the intent.)
...
```bash
specify registry add new-project --url ... --adapter <adapter> --description "..."
