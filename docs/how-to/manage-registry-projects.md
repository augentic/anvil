...
For ongoing initiatives where a plan entry was already authored against the now-removed project, the validator will block `/spec:execute` until you run `specify plan amend <entry> --project <other>` (or `specify plan transition <entry> skipped`).
...
`/spec:plan`'s registry-proposal sub-step (RFC-9 Section 2B) enforces the same order automatically. Hand-driven flows must respect it manually.
...
- [Cross-Repo Initiatives](../tutorials/cross-repo-initiative.md) -- how registry descriptions feed into `/spec:plan`'s assignment.
- [Recover from registry-amendment-required](recover-from-registry-amendment.md) -- handling the case where `/spec:execute` halts and proposes a new project.
