...
- An initiative where `/spec:execute --loop` halted with `Completion: stuck` or surfaced a `registry-amendment-required` line in the per-change summary.
...
| **Reject and abort** | The capability does not belong in this initiative at all. | `specify plan transition <change> skipped --reason "..."` and continue with the rest of the plan. |
...
The entry transitions back to `pending`. The next `/spec:execute` cycle picks it up and routes it into the new workspace clone.

## 4. Re-run `/spec:execute`
...
| Change drives to `done` on next run | `/spec:execute --loop` | The change merges; the plan progresses. |
