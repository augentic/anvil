...
Authority order: `intent` > `documentation` > `behaviour` (canonical: [Authority hierarchy](../../crates/workflow/prompts/synthesis/authority.md)). The override surface key must already appear in the slice's `sources[]` — an orphan key is rejected by `specify slice validate` with `slice-authority-override-orphan-source`.
