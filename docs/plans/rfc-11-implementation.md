...
- **Phase 2.1 – 2.5 (specify side):** committed on `rfc-11-phase2` (also pushed to `origin/rfc-11-phase2`) and opened as specify PR [#109](https://github.com/augentic/specify/pull/109); awaiting review / merge as of writing. The cluster is the natural unit to land — every Phase 2.x note below carries `landed locally on rfc-11-phase2` until the PR squash-merges, at which point the leading clause flips to `shipped via specify PR #109 (squashed into <sha> on main)` per the convention above.
- **Phase 3.x / 4.1:** pending. Phases 3.1 – 3.6 are specify-only; Phase 4.1 is the only remaining specify-cli touch (the Vectis schema bumps to `version: 3` and `EMBEDDED_ARTIFACT_PATHS` may need to follow — see Phase 1.10's status note for the in-binary const).
