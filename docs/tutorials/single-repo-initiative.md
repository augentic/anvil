...
1. `/spec:execute` invokes `/spec:drop` to discard the failed change.
...
You can always fall back to manual control. If `/spec:execute` is stuck or you want to handle a specific change yourself:
...
- `/spec:plan` discovers capabilities and proposes changes with an interactive accept/edit/reject loop.
- The plan tracks changes, dependencies, and status in `plan.yaml`.
- `/spec:execute` automates the define-build-merge loop per change.
...
[Cross-Repo Initiatives](cross-repo-initiative.md) -- coordinate changes across multiple repositories.
