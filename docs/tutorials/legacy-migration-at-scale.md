...
Bring existing codebases into Emery through source adapters rather than rewriting intent by hand. This page orients you to the migration path; it is not a full mock walk-through.
...
Multi-slice migrations look like any other multi-slice plan: one operator review step (Gate 1), then `emery plan execute` drives each slice through refine → build → merge.
