...
**New CLI verbs.** `specrun source resolve <name>`, `specrun target resolve <value>`, `specrun plan transition <name> reviewed`, `specrun plan amend --add-source / --remove-source / --divergence`. Retired: `specify adapter *`, `specify change *`, `specify change survey`, `specrun plan doctor`.
