...
|------|-----------------|----------------|
| Legacy-source clone | `/spec:analyze` (via `/rt:git-cloner` for git URLs) | The plan-time discovery brief, invoked by `/spec:plan` step 3(a) |
...
- [The Layered Stack](three-layer-stack.md) -- where `/spec:plan`, `/spec:execute`, and `/spec:initiative` sit in the layered model.
