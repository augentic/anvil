...
Tool declaration structure is enforced at parse time by the `tools[]` serde shape (`workflow_lib::config::tools`); the standalone structural-validation surface retired with it, and no verb assembles a declared-tool inventory today.
