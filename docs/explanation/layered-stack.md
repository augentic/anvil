...
- **JSON Schemas** — the authoritative `source.schema.json`, `target.schema.json`, `evidence.schema.json`, `discovery/lead.schema.json`, `discovery/proposal.schema.json`, and `plan.yaml` schemas are owned by and distributed with the `specrun` binary (sources live in [`augentic/specify-cli`](https://github.com/augentic/specify-cli/tree/main/schemas)); this repo carries only editor-facing mirrors under `.cursor/schemas/`, not a top-level `schemas/` directory.
...
- **`specrun init <target>`** / **`specrun init --workspace`** — one-time scaffold of `.specify/`, writes `project.yaml`.
