...
| **Engineering standards** | Durable engineering policy for generated and hand-written code | Codex markdown under `adapters/shared/codex/`, `adapters/targets/<name>/codex/`, and optional source overlays — resolved by `specrun rules export` and enforced by `specrun lint` |
...
| Consumer standards | `specrun lint` | Downstream projects with `.specify/` | Applicable codex rules with `deterministic_hints`; emits structured findings |
