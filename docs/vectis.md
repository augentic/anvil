...
The design system provides platform-agnostic design tokens with platform-specific implementations: an iOS **Swift Package** and an Android **Jetpack Compose Material 3** library, both regenerated from the same `tokens.yaml`.
...
| `design-system/android/`    | `vectis-design` Gradle module (Compose M3) — generated from `tokens.yaml` |
...
```
spec.md (describes intent) → tokens.yaml (defines values) → iOS Swift + Android Kotlin (generated)
```
...
**4. Regenerate platform code.** Use the `design-system-writer` skill:
...
| `design-system-writer` | Generate VectisDesign from `tokens.yaml` (iOS Swift package + Android Compose library) |
