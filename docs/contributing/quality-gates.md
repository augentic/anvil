...
2. Put an independently useful library contract (the adapter SDK, artifact parsers, diagnostics, error display, the prose walker) in that crate's integration suite; the same holds for a product invariant impractical to arrange through the entry points.
