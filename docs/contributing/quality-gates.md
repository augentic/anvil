...
There is no automated WASM gate. The v1 operator-invoked wasm example was archived at tag `v1`; until a replacement lands with the spec walking skeleton, the model-free signal is the guest compile check: `cargo check --lib -p emery --target wasm32-wasip2`. Run it when a change crosses a WIT, dispatch, hosting, or preopen seam.
...
3. Put cross-crate behavior in the engine suites over the mock adapter and scripted answers.
4. Behavior that only a WebAssembly/WIT/runtime seam can prove has no automated home here.
