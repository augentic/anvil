...
The engine is versioned by the binary — the binary *contains* its engine, so no store entry, first-launch hydration, or version-skew window exists for it. Adapter identities resolve through the same probes the in-guest resolver uses: the global store for pinned routed ids (`<axis>:<name>@<version>`), the seeded project component cache for unpinned ones (`<axis>:<name>`); distinct pins of one adapter coexist because their routed ids differ. `SPECIFY_HOME` is the single relocation override — store and cache derive together beneath it (the Cargo model) — captured once per invocation into a carried `Locations` value; nothing below the composition root reads the environment.
