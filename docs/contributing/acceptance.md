...
3. **Stop and hand back to the operator** at the irreducible human seams — never fabricate a result for these:
   - A missing `specify` binary. `make acceptance` builds one and symlinks it onto your PATH (`~/.local/bin`), or symlink the sibling `specify-cli/target/release/specify` into a PATH dir yourself; the agent hands back when no built binary exists.
