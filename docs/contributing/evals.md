...
3. **Cross-repo finalize orchestration** — `/spec:finalize` sequencing `specify workspace push` (publishing each routed project's `specify/<change>` branch to its `origin`) and `specify plan archive` in one run across multiple project remotes (e.g. `contract-lifecycle`). Pull requests are opened and merged by the operator outside Specify, so no forge client is exercised; the scenario runs against local bare-repo remotes. (Dual-driving refusal used to sit here as a skill-enforced pre-flight; the guest execute marker — the create-exclusive `.specify/guest.lock` — made it deterministic, so it is now a named test in [`crates/workflow/tests/execute.rs`](../../crates/workflow/tests/execute.rs) with no catalog entry.)
