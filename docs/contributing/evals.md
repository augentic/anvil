...
3. **Live-forge interaction** — real PR pushes/merges between `/spec:finalize` invocations (e.g. `cross-repo-contract-flow`, `workspace-execute-two-projects`). No deterministic fixture stands in for a live forge. (Dual-driving refusal used to sit here as a skill-enforced pre-flight; the CLI's plan-lock probe made it deterministic, so it is now the named test `tests/workflow/plan_lock.rs` in `specify-cli` with no catalog entry.)
...
- The gate is **tiered by the catalog's Gate column**. The **release gate is green** when `cargo make test` is green in `specify-cli` (it runs there on every commit) and every `release-blocker` row (`pure-intent`, `execute-build-failure`, `workspace-execute-two-projects`) is `passed` — the `pure-intent` hard halt is unchanged. The **full catalog** drains per minor release or monthly, whichever comes first; a non-blocking `failed` row is triaged via its linked follow-up issue but does not hold a release on its own. A `deferred` entry (capability genuinely missing on the binary under test) must carry a linked follow-up issue and explicit release-owner sign-off.
...
- **Synthesis and reconciliation** (`documentation-multi-slice`, `code-multi-slice`, `cross-source-merge`, `target-shape-injection`): multi-slice decomposition, deterministic cross-source reconciliation, `shape`-brief injection, lifecycle reaching `refined` cleanly.
- **Cross-repo routing** (`cross-repo-contract-flow`, `workspace-execute-two-projects`): contract-first plans, registry-driven routing, workspace slot materialisation, durable end-state (archived plan path, one merged PR per routed project, archived `change.md`).
- **Failure and breakout** (`stepthrough-breakout`, `execute-build-failure`, `workspace-breakout`, `stale-workspace-recovery`): build-failure stop/resume, breakout verbs, and stale-slot recovery. Dual-driving refusal is CLI-enforced (the plan-lock probe) and proven by the named test `tests/workflow/plan_lock.rs`, not a scenario.
