...
3. **Live-forge interaction** — real PR pushes/merges between `/spec:finalize` invocations (e.g. `contract-lifecycle`, `workspace-two-projects`). No deterministic fixture stands in for a live forge. (Dual-driving refusal used to sit here as a skill-enforced pre-flight; the CLI's plan-lock probe made it deterministic, so it is now the named test `tests/workflow/plan_lock.rs` in `specify-cli` with no catalog entry.)
...
- **Cross-repo routing** (`contract-lifecycle`, `workspace-two-projects`): contract-first plans, registry-driven routing, workspace slot materialisation, durable end-state (archived plan path, one merged PR per routed project, archived `change.md`).
