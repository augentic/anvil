...
- [`acceptance/suites/lifecycle/`](../../acceptance/suites/lifecycle/README.md) -- the unified scenario pack covering the `/spec:*` change lifecycle (`/spec:plan`, `/spec:execute`, `/spec:finalize`) from N=1 through multi-repo, happy-path through failure and recovery.
