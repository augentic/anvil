...
| [`change-analyze`](../../plugins/change/skills/analyze/SKILL.md) | Reached only via plan pipeline. | Direct invocation scenario: `/change:analyze <input-path> <output-dir>` against (a) a documentation tree and (b) a code tree, asserting the emitted capability summaries differ on the `kind` positional and that `discovery.md` is structured per the schema. |
