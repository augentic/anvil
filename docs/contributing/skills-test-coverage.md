...
| `code-runtime` source adapter ([`adapters/sources/code-runtime/`](../../adapters/sources/code-runtime/)) | `enumerate` + `extract` briefs, invoked by `/spec:plan` and `/spec:refine` | -- | -- | gap | No scenario binds a captured fixture tree, runs enumerate/extract, and asserts `kind: example` claims with `fixture-digest` anchors in `evidence/*.yaml`. |
...
| `code-runtime` source adapter ([`adapters/sources/code-runtime/`](../../adapters/sources/code-runtime/)) | No fixture-tree scenario. | Scenario that supplies a wiretap-format fixture tree under `tests/data/replay/`, binds `code-runtime` at plan time, runs `/spec:refine`, and asserts `evidence/*.yaml` carries `kind: example` claims with stable `fixture-digest` values. |
| `omnia` target — `build/replay.md` ([`adapters/targets/omnia/briefs/build/replay.md`](../../adapters/targets/omnia/briefs/build/replay.md)) | No end-to-end replay scenario. | Extends the Omnia build brief fixture: slice with `code-runtime` binding, generated replay tests under `tests/data/replay/`, `build/replay.md` phase runs, and journal emits `slice.fixture-replay.completed`. |
