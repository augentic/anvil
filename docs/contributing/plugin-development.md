...
| Contracts | `plugins/contracts/` | `/contracts:` | API contract generation and validation |
| RT | `plugins/rt/` | `/rt:` | Repository cloning, fixture capture, regression testing |
| Plan | `plugins/plan/` | `/plan:` | Statement of Work generation |
...
Schemas are read from the filesystem at `/spec:init` time, not from the plugin cache. To iterate on schemas in a target project, symlink them:
...
| `review-checks.md` | Standard review checklist used by reviewer skills |
