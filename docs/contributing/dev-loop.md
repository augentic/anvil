...
Runs `cargo nextest --workspace` over the workspace crates. The suites prove the pure engine kernels (the output home, the extras gate, reconciliation and synthesis over scripted model doubles), the CLI wire contract (grammar, exit codes, the C3 HTTP refusal), and the in-process `specify` → `show` journey (`tests/source.rs`) — without a component, a model call, or filesystem engine state.
