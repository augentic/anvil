...
Runs `cargo nextest --workspace` over the workspace. The root scenario suites carry the product: `tests/specify.rs` (the in-process `specify` → `show` arc — bindings, extraction, reconciliation, synthesis, the generation home), `tests/command.rs` (the CLI wire contract: grammar, exit codes, channels), `tests/shelf.rs` (the MCP spec shelf and the C3 HTTP refusal), and `tests/plugin.rs` (plugin-rule mentions vs the shipped grammar), all over scripted `Model` + `Source` + storage — without a component, a model call, or filesystem engine state. The surviving crate suites prove independent library contracts and the CLI-impractical engine invariants.
