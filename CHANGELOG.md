...
- New CLI verbs: `specify registry add/remove`, the four health diagnostics on `specify plan validate` (`cycle-in-depends-on`, `orphan-source-key`, `stale-workspace-clone`, `unreachable-entry`), and `specify change finalize` (`--clean`, `dry-run`) for atomic plan closure.
