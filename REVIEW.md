...
- **S1 (single pass)** — ΔLOC −25 net including this post-mortem line (S1 cleanup −26) vs predicted −33; done-when flipped cleanly (assertion exit 0: reference file absent and README `Adapter Resolution` rg clean); no S1 regression; `make checks` pass, sibling `cargo make check` fails on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **S2 (single pass)** — ΔLOC −123 net including this post-mortem line (S2 cleanup −124) vs predicted −124; done-when flipped cleanly (assertion exit 0: sidecars absent and init skill rg clean); no S2 regression; `make checks` pass, sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **S3 (single pass)** — ΔLOC +0 net including this post-mortem line (CLI S3 cleanup −1) vs predicted −5; done-when flipped cleanly (assertion exit 0: JSON detour rg clean); no S3 regression; `make checks` pass, sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **S4 (single pass)** — ΔLOC +1 net including this post-mortem line (CLI S4 cleanup +0) vs predicted ±0; done-when flipped cleanly (assertion exit 0: clamped-expect rg clean); no S4 regression; `make checks` pass, sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **S5 (single pass)** — ΔLOC −2 net including this post-mortem line (CLI S5 cleanup −3) vs predicted +8; done-when flipped cleanly (assertion exit 0: journal/cache serialisation rg clean); no S5 regression; `make checks` pass, sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs` and targeted `specify-domain` clippy passes.
- **T1 (single pass)** — ΔLOC −6 net including this post-mortem line (CLI T1 cleanup −7) vs predicted +2; done-when flipped cleanly (assertion exit 0: `candidate located above` rg clean); no T1 regression; `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`, while touched `specify-domain` clippy and rollback unit test pass.
- **T2 (single pass)** — ΔLOC −2 net including this post-mortem line (CLI T2 cleanup −3) vs predicted +3; done-when flipped cleanly (assertion exit 0: `just transitioned` rg clean); no T2 regression; `make checks` pass, sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **T3 (single pass)** — ΔLOC +0 net including this post-mortem line (CLI T3 cleanup −1) vs predicted +1; done-when flipped cleanly (assertion exit 0: context bytes rg clean); no T3 regression; sibling `cargo make check` still fails only on unrelated untouched CLI `clippy::large_enum_variant` in `src/cli.rs`.
- **Final validation (single pass)** — cleared the unrelated `clippy::large_enum_variant` validation blocker with a narrow top-level clap enum `#[expect]`; final `cargo make ci` and direct `cargo make check` pass; no regressions.
