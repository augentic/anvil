...
| **2** | **Consumer `specify lint` integration depth**     | Framework lint is CI-native and fast (~1.4s release on full `specify` tree locally). Consumer lint (`specify lint`) is thinly covered (`tests/lint_product.rs` + a few `crates/standards/tests/lint_hint_*`); engineering-standards regressions are easier to miss. |
