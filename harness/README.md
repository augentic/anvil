...
**Parked** means: the packages stay workspace members (so `cargo make lint` keeps them compiling and they cannot rot), but their tests are excluded from the default `cargo make test` / `cargo make ci` run. The deterministic dispatch contract they used to gate — grammar, routing, envelopes, exit codes — is covered natively by `crates/dispatch/tests/`; wasmtime and omnia own the argv/exit plumbing itself, and re-proving that on every commit is not worth the wasm build cost.

Run the rig on demand when touching the WIT contract, the guest shim (`crates/specify`), or the omnia pin:
...
cargo nextest run -p runtime            # the whole rig
