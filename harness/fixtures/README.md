...
Example WASI guest components for the Specify Omnia runtime, following the same layout as [omnia's `examples/`](https://github.com/augentic/omnia/tree/main/examples): one `examples` package, each guest a `crate-type = ["cdylib"]` example compiled for `wasm32-wasip2`.

## echo

The echo adapter guests — skeleton `specify:adapter` components used as fixtures by the composed runtime tests in [`crates/runtime/tests/`](../crates/runtime/tests/):
...
Artifacts land at `target/wasm32-wasip2/debug/examples/echo_{source,target}.wasm`.
