...
impl Project {
    fn new() -> Self {
        ...
        workflow::adapter::describe::register_describe_runner(|_request| {
            Ok(workflow::adapter::describe::DescribeAnswer::default())
        });
    ...
    fn layout(&self) -> workflow::config::Layout<'_> {
        workflow::config::Layout::new(&self.root)
    }
