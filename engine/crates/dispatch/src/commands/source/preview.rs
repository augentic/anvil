...
//! `survey` / `extract` runners layer on the same seam.
...
/// Run the workflow-free `specify source preview` flow: resolve the
/// adapter, validate the bound source path, scaffold the output tree,
/// and emit the preview handoff envelope.
///
/// # Errors
///
/// Propagates argument-validation, adapter-resolution, and scaffold
/// failures.
pub fn preview(
