...
//!
//! `run` (private) drives the whole flow. Source extraction is
//! agent-only — every operation splits into `prepare` / `finalize`,
//! and the CLI never blocks on agent work. Each operation supplies its
//! handoff envelope, commit step (lead-set merge / Evidence persist),
//! and completion event through the private `Flow` trait, while
//! `Common` carries the shared inputs.
...
use crate::commands::source::prep;
