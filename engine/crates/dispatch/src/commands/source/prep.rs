...
pub enum SourceOp {
    ...
    /// runner ([`crate::commands::source::extract`]).
    Extract {
...
/// reads, its access mode, and the host path.
///
/// The path is absent when the root is not mounted — `$PROJECT_DIR`
/// always, `$SOURCE_DIR` for value-bound sources.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Preopen {
...
/// `plan_root`.
///
/// Absolute paths pass through, relative paths join onto the directory
/// `plan.yaml` lives in (the project root, or the initiating workspace
/// under a `--plan-dir` override) — relative bindings are authored
/// against the plan's home. Shared by the `survey` and `extract`
/// runners so the `$SOURCE_DIR` host path is computed in one place.
#[must_use]
