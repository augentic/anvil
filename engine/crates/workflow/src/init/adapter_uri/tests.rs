...
// The 12 pure parse/identity tests collapse into three matrices (GitHub-URI +
// value identity, package references, first-party shorthand) — none has a CLI
// fixture pinning the per-shape branches. The two store-resolve tests are kept
// (they drive a real content-addressed store), as is the ignored networked
// smoke test. Every former input is preserved.

#[test]
fn github_uri_and_value_identity() {
    // `GithubAdapterUri::parse`: default `main` (no ref), a `@suffix` ref, and
    // a `tree/<ref>` path.
    assert_eq!(
        GithubAdapterUri::parse("https://github.com/owner/repo/schemas/demo-target")
    ...
    let suffix = GithubAdapterUri::parse("https://github.com/owner/repo/schemas/demo-target@v1")
        .expect("parse GitHub URI");
    assert_eq!(suffix.checkout_ref.as_deref(), Some("v1"));
    assert_eq!(suffix.adapter_path, "schemas/demo-target");
    assert_eq!(suffix.adapter_name, "demo-target");
    let tree =
        GithubAdapterUri::parse("https://github.com/owner/repo/tree/main/schemas/demo-target")
            .expect("parse GitHub URI");
    assert_eq!(tree.checkout_ref.as_deref(), Some("main"));
    assert_eq!(tree.adapter_path, "schemas/demo-target");
    assert_eq!(tree.adapter_name, "demo-target");
    ...
    // `adapter_ref_from_value` recovers a semver pin; a bare name, a `file://`
    // path, and a non-semver git ref yield a bare ref; a package reference
    // recovers the bare `(name, version)` identity, stripping `<namespace>:`.
...
fn first_party_shorthand() {
    // A bare name carries no pin (resolves the single installed identity); a
    ...
    // Paths and URLs flow through from_local / from_github; a non-kebab name or
    // a `@suffix` that is not exact semver is not shorthand.
    ...
    // Bundled adapters (a WASI extension) resolve from specify-adapters;
    // prose-only adapters still resolve from the platform repo (RFC-48 / RFC-49).
...
fn package_ref_resolves_from_store_entry() {
    ...
    // canonical wire value as `adapter_value`. Kept: store-resolve has no
    // in-loop CLI fixture.
    let store = tempfile::tempdir().expect("store root");
    let _guard = crate::test_cache::scoped_store(store.path());
