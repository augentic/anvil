...
//! tests live with their crates in `augentic/specify-adapters`. Shared
...
mod run {
    ...
    impl ToolFixtures {
        ...
        fn new() -> Self {
            ...
            for path in [
                ...
                fs::write(
                    &path,
                    text.replace("/__SPECIFY_FIXTURE_ROOT__", &root.to_string_lossy()),
                )
    ...
    fn https_network_failure_is_typed() {
        ...
        assert!(
            matches!(
                code,
                "tool-network-other" | "tool-network-timeout" | "tool-network-malformed"
            ),
            "{value}"
        );
    ...
    fn adapter_non_zero_exit_caches_by_scope() {
        ...
        assert!(
            cache.join("adapter--target--tools-test-adp/exit-seven/1.0.0/module.wasm").is_file()
        );
    }
...
mod schema {
    ...
    use crate::common::{
        dispatch_fixture_adapter_dir, parse_json, scaffold_tool_project, specify_cmd,
    };

    #[test]
    fn schema_dispatches_fixture_tokens() {
        let wasm = dispatch_fixture_adapter_dir().join("adapter.wasm");
        assert!(wasm.is_file(), "dispatch-fixture adapter.wasm must be checked in");

        let tmp = tempdir().expect("tempdir");
        let (project, cache) = scaffold_tool_project(&tmp, "dispatch-fixture", &wasm);

        let assert = specify_cmd()
            .current_dir(&project)
            .env("SPECIFY_EXTENSIONS_CACHE", &cache)
            .args(["extension", "schema", "dispatch-fixture", "tokens"])
            .assert()
            .success();

        let value = parse_json(&assert.get_output().stdout);
        assert_eq!(value["$id"].as_str(), Some("https://example.com/dispatch-fixture/tokens"));
    }
    ...
    fn schema_undeclared_tool_exits_two() {
        ...
        let cache = std::env::temp_dir()
            .join(format!("specify-tool-schema-undeclared-{}", std::process::id()));
