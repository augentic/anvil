...
Per-adapter `adapter.yaml` carries `name`, `version`, `axis`, `description`, an optional closed `execution` mode, an optional `briefs.<operation>` map, and an optional singular `extension` declaration. The closed operation set is determined by the manifest's `axis`: a manifest that carries `briefs` must declare the full per-axis key set, and a shrunk post-cutover manifest that omits it derives the set from the closed WIT contract (RFC-61 S2 two-shape window — see [DECISIONS.md §"Manifest-schema relax"](../../DECISIONS.md#manifest-schema-relax-two-shape-coexistence-window-rfc-61-step-5-milestone-s2)). Source adapters are agent-only (`source.schema.json` enumerates `execution: ["agent"]`); target manifests may declare `agent` or `tool`. Native consumers that need the machinery refuse a shrunk manifest at the point of use (`adapter-briefs-missing` on the source prep seam, `adapter-execution-mode-required` on the native `slice build` dispatch). Implementation: [`crates/workflow/src/adapter/`](../../crates/workflow/src/adapter); per-axis schemas at [`schemas/adapter.schema.json`](../../../schemas/adapter.schema.json), [`source.schema.json`](../../../schemas/source.schema.json), [`target.schema.json`](../../../schemas/target.schema.json).
...
WASI extension runner pre-opens `$PROJECT_DIR` always, `$CAPABILITY_DIR` only for adapter-scope extensions. No host environment leaks. See [`DECISIONS.md` §"`$CAPABILITY_DIR` replaces `$ADAPTER_DIR`"](../../DECISIONS.md#capability_dir-replaces-adapter_dir).
...
The loop-phase plan-state writers (`plan next`, per-entry `plan transition` including `--undo`, and a plan-backed `slice merge run`) additionally probe the OS advisory lock on `<plan-root>/.specify/plan.lock` and refuse an unlocked driver with `plan-lock-not-held` (exit 2) — dual-driving refusal as a runtime property ([`crates/workflow/src/plan_lock.rs`](../../crates/workflow/src/plan_lock.rs), proven by [`tests/workflow/plan_lock.rs`](../../tests/workflow/plan_lock.rs)). Lock *acquisition* is CLI-owned: `specify plan lock -- <cmd>` takes the lock and runs the loop driver under it (the framework's `/spec:execute` and breakout skills wrap their loop in this verb), so the same binary owns acquisition cross-platform with no `flock(1)` / `zsystem` / Python snippet. The plan-level Gate 1 stamp and standalone plan-less merges are exempt. Both `acquire` and the probe resolve through `Layout::plan_dir()`, so slot-side merge work under `--plan-dir` locks the *workspace*.
