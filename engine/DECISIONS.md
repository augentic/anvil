...
- **Async everywhere.** wRPC is async (Tokio), so guest-to-guest dispatch is async on every path; this aligns with the component-model-async / WASI 0.3 direction (whose stream/future stabilization is still in flight). Instance-per-call is unchanged: each invocation is served by a fresh instance from the registry, so the multi-guest single-binary registry holds.
