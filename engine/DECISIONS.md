...
## Remote transport: wRPC behind the `selection` seam

**Decision (2026-06).** When a host-mediated `selection` call (the runtime move, RFC-56) crosses a node boundary, the host carries it over [wRPC](https://github.com/bytecodealliance/wrpc) — the Bytecode Alliance's WIT-native, transport-agnostic component RPC — rather than a hand-rolled `wasi:messaging` request/reply shim. wRPC encodes the typed `augentic:specify` records (and their async `stream` / `future` values) from the same WIT the in-process bindgen path uses, over a concrete transport (NATS first; QUIC / UDS available). Why: the remote leg needs typed invocation framing, multiplexing, and async stream/future transport — exactly wRPC's surface — and wasmCloud proves the brokered peer-to-peer model at scale; building it over raw `wasi:messaging` would re-implement a subset of wRPC with none of the maturity.

Consequences a change must reckon with:

- **Behind the backend boundary, never in the contract.** wRPC is a swappable transport backend; the `augentic:specify` WIT package names no transport. Local dispatch stays native in-process Wasmtime `Linker` calls — wRPC is for the cross-node case only, because an in-process call must not pay serialization.
- **Resources do not cross.** wRPC carries WIT resources only as opaque, application-defined handles, so a live `wasi:filesystem` `descriptor` (inside `working-tree`) cannot be transported. `build` / `merge` therefore ship the content-addressed `revision` / `changeset` values and the callee re-materializes its own tree (the working-tree backend, RFC-55); `survey` / `extract` / `guidance` return plain records and cross directly.
- **Pre-1.0 pin.** wRPC and its transport crates are pre-1.0 (Apache-2.0 WITH LLVM-exception); pin them and track wRPC's Wasmtime pin against the runtime's. Prefer `wrpc-transport` + `wit-bindgen-wrpc` generated handlers wired to the existing instance-per-call registry over adopting `wrpc-runtime-wasmtime`, to avoid a second Wasmtime version pin.
- **Async.** wRPC is async (Tokio); it aligns with the component-model-async / WASI 0.3 direction, whose stream/future stabilization is still in flight.
