...
This is a narrower surface than CORE-002 (which checks every relative markdown link under the adapter trees, `plugins/`, `docs/`, and `.cursor/`): CORE-019 is scoped to skill bodies and to their `references/`/`examples/` payload. The deterministic-hint interpreter consumes the `markdown_link` facts the indexer already produced, restricted to the `references/`/`examples/` target prefixes carried in `config`.
