...
Working on Specify itself means a local CLI on `PATH` and the working-tree plugin (not the marketplace copy):

```bash
cargo install --path . --locked          # real specify binary (reinstall after CLI changes)
cursor-agent --plugin-dir plugins/spec   # load plugins/spec/ for /spec:* in chat
```

Then use the [Quick start](#quick-start) flow as usual. `cargo make specify` is the lab/mock shim — skills need the installed binary above. Adapters live in [`augentic/specify-adapters`](https://github.com/augentic/specify-adapters). Before committing: `cargo make ci`.
