...
[`specify.wit`](specify.wit) is the `specify:adapter` WebAssembly Component Model package: the typed adapter contract — the shared `types` interface, the per-axis `target` / `source` interfaces, and their worlds. The file reflects the [RFC-61](../rfcs/rfc-61-omnia-migration.md) contract revision: no resource types cross guest-to-guest calls, each adapter world exports `wasi:http/incoming-handler` as its MCP reference surface (superseding the typed `references` interface of earlier drafts), and every judgment answer is schema-gated. The original contract RFC (RFC-51) was removed from the tree and is recoverable from git history.
