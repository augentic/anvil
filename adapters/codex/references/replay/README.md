...
Cross-target, build-time `replay` rules under `adapters/codex/` — read by any target adapter that opts into the the capture-backed replay workflow hook during `/spec:build`. This directory is shared support material, not a target adapter, and does not add a fourth operation.
