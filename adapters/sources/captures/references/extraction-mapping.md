...
Return one Evidence document matching `schemas/evidence.schema.json`. The CLI atomically writes it to `evidence/<source-key>.yaml`; the brief produces the body. Top-level field order is fixed (`source`, `adapter`, `authority`, `candidate`, `claims`):
