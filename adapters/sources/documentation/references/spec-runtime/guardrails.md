...
- **Never hand-edit `plan.yaml`.** Append entries through `specrun plan add`; transition entries through `specrun plan transition`; close out the plan through `specrun plan archive`. The single-writer contract lives in the [`plan` skill](https://specify.augentic.io/reference/change-skills/plan.html).
