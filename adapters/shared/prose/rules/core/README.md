...
First-party rules that enforce framework-repository invariants through the shared deterministic-hint interpreter. The pack root activates the second shared resolution root (`adapters/shared/prose/rules/<pack>/`) with pack name `core`; resolved rules carry `origin: core`. `CORE-*` rules participate in `specify lint framework` runs by default and are excluded from consumer-side `specify rules export` / `specify lint` unless the operator passes `--include-core`.

This directory is the peer of [`adapters/shared/prose/rules/universal/`](../universal/README.md): same file shape, same JSON Schema, different namespace ownership. `CORE-*` is the only namespace allowed under `adapters/shared/prose/rules/core/`; the `rules` WASI tool (CORE-009) rejects any non-`CORE-*` rule placed here and any `CORE-*` rule placed elsewhere.
...
| `adapter`   | Adapter guest sources (`adapters/**/src/**`, `wit/`) |
...
**Chassis quirk — prefer `path-pattern` over `applicability.artifacts` until further notice.** The framework-profile resolver passes `include_unmatched: false` into `artifact_dimension_matches`, which drops any rule that declares a populated `applicability.artifacts` set from the resolved output before hints run. Until the chassis flips that behaviour for the framework profile (or wires artifact-kind facts off `WorkspaceModel`), leave `applicability.artifacts` unset and narrow the candidate file set with a `kind: path-pattern` deterministic hint instead (see [`CORE-002-links-unresolved.md`](CORE-002-links-unresolved.md) for the worked example). Revisit once a chassis follow-up enabling artifact-token filtering for the framework profile lands.
...
- [ ] **Do** add a `kind: path-pattern` hint whose `value` glob matches the target files (e.g. `plugins/**/SKILL.md`, `docs/**/*.md`).
...
- [ ] Cross-check against [`CORE-002-links-unresolved.md`](CORE-002-links-unresolved.md), which scopes with `path-pattern` rather than `applicability.artifacts`.
...
Every v1 hint kind is executable: `path-pattern`, `schema`, `regex`, `tool`, `unique`, `reference-resolves`, `constant-eq`, `fenced-block`, `presence`, and `field-grammar`. Prefer native declarative kinds for new rules; reach for `kind: tool` (an in-process framework checker) only when a check is branchy, whole-tree, cross-fact, or registry-backed. No kind carries `"x-hint-status": "reserved"` in the canonical `rule.schema.json`.

Two kinds dispatch on a `value:` mechanism selector with policy in `config:`: `presence` (`frontmatter`, `file`, `directory-index`) for a missing required artifact and `field-grammar` (`field-tokens`, `field-first-word`) for a frontmatter field grammar. The `schema` and `unique` kinds additionally accept a whole-tree `value: scenario` selector over the scenario fact family. These serve `presence` → CORE-042 / CORE-011 / CORE-059, `field-grammar` → CORE-035 / CORE-036, `schema` scenario → CORE-032, and `unique` scenario → CORE-030.

**Lint posture.** `specify lint framework` is a generic dispatcher running entirely through declarative hints (Road A) and name-resolved framework checkers (Road B) — there is no imperative `Check` rule producer and no imperative-predicate bridge. Whole-tree and branchy checks (CORE-009 namespace ownership, CORE-026 duplicate id, and the `scenarios` / `links-registry` / `marketplace` / `prose` families) run through their `kind: tool` family checker; all policy rides the rule's `config:`. Benchmark locally with `/usr/bin/time make lint`.

### Hint config cookbook (native rules)

`config:`-driven evaluators carry rule policy out of the engine. Examples: `regex` accepts optional `config` (capture-group threshold, negative-match, suffix guard), `path-pattern` `value`s accept `!` exclusion globs, and the fact-consuming kinds (`constant-eq`, `unique`, `fenced-block`) read their map / field / substring policy from `config:`. The canonical `config:` shape for each kind is pinned by the `$def`s in `schemas/rules/rule.schema.json` (embedded in the `specify` binary); see existing `CORE-*` rule files for worked examples.
...
2. Mirror an existing rule (start from [`CORE-002-links-unresolved.md`](CORE-002-links-unresolved.md)) for the frontmatter shape; the schema is the source of truth.
...
4. Keep all policy in the rule's `config:`. The engine's `lint_no_embedded_policy` Layer-3 guard rejects any rule-specific literal that creeps into the dispatcher. For a Road B (`kind: tool`) rule, add the check to the in-process family checker under `crates/standards/src/lint/framework_tools/<name>.rs` (see [docs/contributing/checks.md](../../../../../docs/contributing/checks.md)).
