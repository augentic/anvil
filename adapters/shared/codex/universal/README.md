...
4. Run `make check`. The relevant predicate is `CodexCheck` in [`tooling/src/check/codex.rs`](../../../../tooling/src/check/codex.rs), which enforces frontmatter validity, the `## Rule` body heading, namespace ownership, and id uniqueness across the shared tree and every per-adapter overlay.
