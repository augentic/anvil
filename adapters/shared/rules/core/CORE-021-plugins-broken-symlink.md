...
The deterministic-hint interpreter consumes the `symlink` facts the indexer records and flags those whose `broken` flag is set, scoped to the `plugins/` path prefix carried in `config` (mirroring the retired imperative `plugins.broken-symlink` predicate).
