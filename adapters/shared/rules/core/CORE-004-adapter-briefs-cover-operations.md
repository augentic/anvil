...
severity: important
trigger: An `adapters/{sources,targets}/<name>/adapter.yaml` declares a `briefs:` map whose key set does not cover every operation the axis requires (`enumerate` + `extract` for source adapters; `shape` + `build` + `merge` for target adapters), leaving the loader without a brief to dispatch for at least one declared operation.
deterministic_hints:
  - kind: path-pattern
    value: "adapters/sources/*/adapter.yaml"
    description: Source adapter manifests; `briefs.keys()` must cover `enumerate` and `extract`.
  - kind: path-pattern
  ...
  - kind: set-coverage
    ...
    description: For each `AdapterManifest` fact in the candidate set, assert that `briefs.keys()` covers the closed axis-appropriate operation enum (`SourceOperation::{Enumerate, Extract}` xor `TargetOperation::{Shape, Build, Merge}`). One finding per missing `(adapter, operation)` pair; extras are silent until the `set-eq` reserved kind lands.
---

## Rule

Every `adapters/{sources,targets}/<name>/adapter.yaml` declares its operation dispatch through the `briefs:` map. The set of keys in that map must cover the closed axis-appropriate operation enum: `SourceOperation::{Enumerate, Extract}` for source adapters, `TargetOperation::{Shape, Build, Merge}` for target adapters. A missing key leaves the workflow loader without a brief to dispatch when the per-axis verb fires (`specrun source resolve enumerate <name>`, `specrun target resolve build <name>`, …).
...
- A newly added source adapter whose `briefs:` map declares `enumerate:` but forgets `extract:` (or vice versa).
