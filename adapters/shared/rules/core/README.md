...
4. If retiring an imperative `Check` row alongside the rule, land the parity test at `crates/lints/tests/core_parity_<rule>.rs` in `augentic/specify-cli` and delete the predicate row in the same PR; the existing fingerprint algorithm collapses duplicate findings during the overlap.
