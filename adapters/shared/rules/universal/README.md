...
4. Run `make check` (which forwards to `specdev lint --framework-root .`). The relevant predicate is `check::rules` in the `specify-authoring` crate, which enforces frontmatter validity, the `## Rule` body heading, namespace ownership, and id uniqueness across the shared tree and every per-adapter overlay.
