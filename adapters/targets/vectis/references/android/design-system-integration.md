...
The `specrun tool run vectis -- scaffold android <AppName>` render step produces the package
...
`specrun tool run vectis -- validate composition` before the Android writer runs, so
