...
- Occurrence prefixes (`CRX-1`, `LOG-1`, `IOS-1`, `AND-1`, `SWF-1`, `KTL-1`, `INT-1`, `GEN-1`, `UNI-1`, `NEW-1`) are **report-local** counters — the `id` field on a structured `LintFinding` (RFC-28 illustrates the equivalent shape as `FIND-0001`; this report uses prefixed counters for human triage). They restart in each report and must not be confused with codex `rule-id`s.
- Codex citations carry a separate `rule_id` field (markdown prose) that maps to the kebab-case `rule-id` field on the RFC-28 `LintFinding` wire shape. Vectis codex ids match `^VECTIS-[0-9]{3}$` (e.g. `VECTIS-001`, `VECTIS-101`, `VECTIS-201`); shared ids match `^UNI-[0-9]{3}$`. Leave the field out for genuinely unmapped findings rather than inventing a rule id.
