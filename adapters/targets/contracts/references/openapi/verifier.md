...
| `cross-project`    | contracts adapter merge brief                  | Producer-side merge of an OpenAPI contract change | Walk the merged `contracts/` baseline; enforce contract identity/version validation | JSON envelope produced by `specrun tool run contract` |
