...
```bash
specrun tool run contract -- "$PROJECT_ROOT/contracts" --format json
```
...
These classifiers are contract-domain evidence fields — **not** the closed RFC-28 severity enum (`critical` / `important` / `suggestion` / `optional`). When a future consumer-impact report surfaces findings as RFC-28 `ReviewFinding` records (see [RFC-28 §Structured review finding schema](../../../../rfcs/done/rfc-28-standards-contract.md#structured-review-finding-schema) and §"Relationship to contracts and compatibility"), the closed severity enum sits on the envelope while the classifier and the rest of the consumer-impact payload travel inside `evidence.kind: structured` under `evidence.data`. The two dimensions are independent: a `breaking` change may carry any RFC-28 severity, and an `additive` change may still warrant `suggestion` or `optional`.
