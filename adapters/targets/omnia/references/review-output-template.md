...
| `UNI-` | Lead universal-checks pass | Gaps not covered by SEC/COR/QUA                   | Per [`adapters/shared/rules/universal/`](../../../shared/rules/universal/) |
...
`SEC-1`, `COR-1`, `UNI-3` and similar are **report-local occurrence ids** — the `id` field on a structured `LintFinding` (the RFC-28 schema illustrates the equivalent shape as `FIND-0001`; this report uses prefixed counters for human triage). They are distinct from the codex `rule-id`. When a finding maps to a stable rule, add a separate `rule_id` line with the canonical three-digit value (e.g. `OMNIA-001`, `OMNIA-002`, `RUST-001`, `SEC-001`, or a `UNI-NNN` rule). Markdown reports render this as `rule_id:` or `Rule:` prose; the structured `LintFinding` wire shape uses the kebab-case `rule-id` field per RFC-28. Leave the field out for genuinely unmapped findings; do not invent a rule ID.
