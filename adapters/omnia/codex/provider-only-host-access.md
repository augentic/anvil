...
Omnia business logic must treat host resources as provider capabilities. External I/O, configuration, identity tokens, durable state, document/table/blob storage, publishing, and broadcast access must flow through `omnia_sdk` provider traits on explicit generic `provider: &P` bounds. Domain crates must not construct host clients, call raw WASI modules, open sockets or files, or hide I/O behind custom abstractions.
...
- Calls to raw WASI modules or guest boundary helpers from domain crates instead of `omnia_sdk` traits.
...
- Custom wrapper clients around provider traits that make tests mock the wrapper instead of the Omnia provider capability.

## Spec Guidance

When artifacts describe an external dependency but not its Omnia capability, ask for the provider mapping instead of recreating the source runtime client. Prefer the closest first-party provider: `HttpRequest`, `Config`, `Identity`, `StateStore`, `TableStore`, `DocumentStore`, `Blobstore`, `Publish`, or `Broadcast`.
