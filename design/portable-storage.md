...
After `git clone`, how does `specify` know the adapters? From argv — the Makefile, the skill, CI — which may point at a committed `sources.toml` ([§5.2](#52-the-replacement-authority)). That is how a clone remembers the bindings without an engine-owned record, and with adapter-floor-only versioning it leaves no `project.yaml`-shaped residue for step 6 and `atomic.rs` to worry about. The residual case — a clone that must `specify` with no argv at all — is recorded as risk 3, not designed for: any answer to it (bindings re-homed into engine storage, implicit discovery) reintroduces the record this section deletes.
...
3. **Clone story for bindings** ([§5.3](#53-the-gating-question)): the residual case is a clone that must `specify` with no argv at all — a decision to re-home the bindings into engine storage or adopt implicit `sources.toml` discovery, each of which reintroduces a `project.yaml`-shaped residue. Do not leave that implicit.
