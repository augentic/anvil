...
| Global adapter store + `.meta` sidecars       | host-side `<store>/<name>@<version>.wasm` + sidecar          | **deleted** — static package pins dispatch by routed id; a future dynamic resolver must bind content-addressed identity to the component the host executes       |
...
**The file carrier — `sources.toml` via `--sources [<path>]`.** The operator may write the binding list in a file they own and select it explicitly: `emery specify --sources sources.toml`, or equivalently `emery specify --sources` for that conventional project-relative name. The precedent is v1's hand-authored definition home (archived at the `v1` tag): an operator-authored input file, loaded fail-closed with typed errors, never engine-written. Rules:

- **Explicit `--sources` only.** No implicit `./sources.toml` discovery — discovery is what turns a file into a config noun; the explicit flag keeps it an argv macro. Omitting the option's value is only shorthand for the project-relative `sources.toml`, not permission to read it when the flag is absent.
...
- `ComponentMeta`, the cache's `<name>.meta.yaml` sidecar, and the speculative global adapter store with its digest sidecar. The cache metadata was write-only provenance whose one reader died with `init`; the global store branch had no production caller or writer, and its verified blob was not bound to routed execution. Dynamic loading must reintroduce storage around a content-addressed artifact identity supplied to the host loader, not a mutable companion record.
...
**Step 4 — component cache move (lands before step 3).** Cache entries become blobstore objects. The cache's `ComponentMeta` sidecar is deleted rather than ported ([§6](#6-deletions)) — its one reader is `init`'s `bind`, which falls back to `persist_value` for the single step until `init` dies in step 5. Local-`.wasm` mirroring (today `init`'s `ensure`; after step 5, `specify`'s) writes through the capability — incidentally fixing that `ensure::seed` copies with plain non-atomic `fs::copy` / `fs::write` today. The unreachable global-store and verify-on-read sidecar branch is deleted with dynamic loading deferred. Delete the cache mount, `cache_dir()` and its mkdir in `main.rs`, `CACHE_MOUNT`, and the path-math methods of `Locations` this obsoletes.
