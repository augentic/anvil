...
//
// Discovers opted-in scenario files under the four roots documented
// in `acceptance/README.md` §Scenario Discovery, validates frontmatter
...
async function discoverScenarioCandidates(): Promise<string[]> {
  ...
  // Discovery root 1: acceptance/suites/<suite>/scenario.md
  const suitesDir = join(REPO_ROOT, "acceptance", "suites");
  try {
    const stat = await Deno.stat(suitesDir);
    if (stat.isDirectory) {
      for await (const entry of walk(suitesDir, {
        maxDepth: 2,
        includeDirs: false,
        match: [/scenario\.md$/],
      })) {
...
async function validateScenarioFrontmatter(): Promise<void> {
  ...
  // C08 stub-backend cross-field invariants:
  //   - `stubbed-stages` is only meaningful when `backend: stub`.
  //   - every entry must appear in `stages`.
  //   - `stub-fixtures` keys must also appear in `stubbed-stages`.
  //   - declared `stub-fixtures` paths must resolve under the repo root.
  // These cannot be expressed cleanly in JSON Schema (cross-field
  // dependencies), so the runtime gate lives here.
  for (const sc of opted) {
    const stubbedStages = sc.frontmatter["stubbed-stages"];
    const stubFixtures = sc.frontmatter["stub-fixtures"];
    const stages = sc.frontmatter.stages;
    const backend = sc.frontmatter.backend;

    if (Array.isArray(stubbedStages)) {
      if (backend !== "stub") {
        fail(
          `Scenario frontmatter: ${sc.rel} — 'stubbed-stages' is only valid when 'backend: stub' (got backend='${backend}'). Drop the field or change the backend.`,
        );
      }
      if (Array.isArray(stages)) {
        for (const ss of stubbedStages) {
          if (typeof ss !== "string") continue;
          if (!stages.includes(ss)) {
            fail(
              `Scenario frontmatter: ${sc.rel} — 'stubbed-stages' entry '${ss}' is not in 'stages' ${
                JSON.stringify(stages)
              }; the stub backend can only drive declared stages.`,
            );
          }
        }
      }
      ...
      fail(
        `Scenario frontmatter: ${sc.rel} — scenarios with 'backend: stub' must declare 'stubbed-stages: [...]'; without it the stub has nothing to drive.`,
      );
    }

    if (
      stubFixtures !== undefined &&
      stubFixtures !== null &&
      typeof stubFixtures === "object" &&
      !Array.isArray(stubFixtures)
    ) {
      const declaredStubbed = new Set(
        Array.isArray(stubbedStages)
          ? stubbedStages.filter((s): s is string => typeof s === "string")
          : [],
      );
      for (
        const [stage, value] of Object.entries(
          stubFixtures as Record<string, unknown>,
        )
      ) {
        if (!declaredStubbed.has(stage)) {
          fail(
            `Scenario frontmatter: ${sc.rel} — 'stub-fixtures.${stage}' has no matching entry in 'stubbed-stages'; remove the fixture or add the stage.`,
          );
        }
        if (typeof value !== "string") continue;
        if (value.startsWith("/")) {
          fail(
            `Scenario frontmatter: ${sc.rel} — 'stub-fixtures.${stage}' must be a repo-relative path, not absolute: ${value}`,
          );
          continue;
        }
        ...
        try {
          ...
          fail(
            `Scenario frontmatter: ${sc.rel} — 'stub-fixtures.${stage}' points at '${value}' but that path does not exist under the repo root.`,
          );
        }
      }
    }
...
async function checkRecordedTraceFreshness(): Promise<void> {
  const recordedRoot = join(REPO_ROOT, "acceptance", "recorded");
  let rootExists = true;
  try {
  ...
  for await (const entry of walk(recordedRoot, {
    exts: [".jsonl"],
    includeDirs: false,
  })) {
    if (await isUnderSymlink(entry.path)) continue;
  ...
  try {
    ...
    const diff = new TextDecoder()
      ...
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
    const tracesByRel = new Map(tracePaths.map((p) => [relative(REPO_ROOT, p), p]));
