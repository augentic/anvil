...
// -----------------------------------------------------------------------------
// Transform 8: warn on composition.yaml + rewrite stale Vectis schema URLs.

const VECTIS_SCHEMA_REWRITES: Array<{ before: RegExp; after: string }> = [
  {
    before:
      /https:\/\/github\.com\/augentic\/specify\/adapters\/vectis\/composition\.schema\.json/g,
    after:
      "https://github.com/augentic/specify/targets/vectis/schemas/composition.schema.json",
  },
  {
    before:
      /https:\/\/github\.com\/augentic\/specify\/(?:adapters|schemas)\/vectis\/tokens\.schema\.json/g,
    after:
      "https://github.com/augentic/specify/targets/vectis/schemas/tokens.schema.json",
  },
  {
    before:
      /https:\/\/github\.com\/augentic\/specify\/(?:adapters|schemas)\/vectis\/assets\.schema\.json/g,
    after:
      "https://github.com/augentic/specify/targets/vectis/schemas/assets.schema.json",
  },
];

async function rewriteVectisSchemaRefs(
  path: string,
  opts: Options,
  log: Logger,
): Promise<boolean> {
  const before = await Deno.readTextFile(path);
  let after = before;
  for (const { before: pat, after: repl } of VECTIS_SCHEMA_REWRITES) {
    after = after.replace(pat, repl);
  }
  if (after === before) return false;
  log.change(path, "rewrite Vectis schema URL(s) to targets/vectis/schemas/");
  if (!opts.dryRun) await Deno.writeTextFile(path, after);
  return true;
}
...
async function warnCompositionYaml(opts: Options, log: Logger): Promise<void> {
  ...
  for await (
    const entry of walk(root, {
      includeDirs: false,
      match: [/(^|\/)(composition|tokens|assets)\.yaml$/],
    })
  ) {
    await rewriteVectisSchemaRefs(entry.path, opts, log);
    if (/composition\.yaml$/.test(entry.path)) {
      log.warn(
