//! Harness for the component rung (`tests/component.rs`).
//!
...
use omnia::{ExitStatus, SourceSpec};
...
/// The engine component the shipped runtime embeds — a raw component under
/// `dev`, the profile the rung runs under (release builds embed an
/// ahead-of-time compiled artifact instead).
///
/// # Errors
///
/// Returns an error if the compiled-in manifest declares no embedded
/// `emery` guest.
pub fn engine() -> Result<Vec<u8>> {
    let manifest = emery::manifest().into_manifest().context("the compiled-in manifest")?;
    manifest
        .guests
        .into_iter()
        .find(|guest| guest.id == "emery")
        .and_then(|guest| match guest.source {
            SourceSpec::Bytes(bytes) => Some(bytes.into_owned()),
            SourceSpec::Path(_) | SourceSpec::Oci(_) => None,
        })
        .context("the runtime embeds the engine guest as bytes")
}
