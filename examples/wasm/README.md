...
Authenticate the agent CLI for whichever backend `EMERY_MODEL_BACKEND` selects. For the default cursor backend:

```bash
agent login
```

or set `CURSOR_API_KEY` in `.env`. For `EMERY_MODEL_BACKEND=claude`, run `claude login` or set `ANTHROPIC_API_KEY`.
...
Set `RUST_LOG` yourself when debugging the seam (the run keeps the env-driven filter because it wants `omnia_wasi_http=debug` without the model backend's own debug lines); for ad-hoc `emery` invocations the reserved host flags `--debug` / `--quiet` are the ergonomic route — they win over any ambient `RUST_LOG`. Note that `--debug` names `omnia_cursor` and not the Claude backend, so Claude spawn detail needs `RUST_LOG=info,model=debug`.
