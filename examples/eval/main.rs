...
//! the catalog and cases declarations. It is a development tool,
//! never an install or release artifact. Driven by `cargo make
//! specify` and `cargo make eval`.
...
async fn main() -> std::process::ExitCode {
    ...
    match probe::client::run(std::env::args().collect(), mock::catalog(), Some(cases)).await {
        Ok(code) => code,
        Err(error) => {
