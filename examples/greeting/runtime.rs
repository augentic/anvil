...
//! bound on both axes. This runtime supplies the same hosts as the shipped
//! binary, replacing the live model with deterministic example answers.
...
use omnia_wasi_model::{Answer, Format, FutureResult, Request, ToolHost, WasiModel, WasiModelCtx};
        ...
        let answer = match request.format {
            ...
            format => {
                return Box::pin(async move {
                    Err(anyhow!("the WASM example has no answer for format {format:?}"))
                });
            }
        };
        Box::pin(async move {
