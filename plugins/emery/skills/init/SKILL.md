...
or `emery init --workspace --quiet` for a registry-only workspace. Init is a short deterministic verb — it runs with `--quiet` per the plugin rule's *Tracing and output* contract (`--debug` replaces it when the operator asks for debug).
