...
- On `plan-execute-stopped` (exit 2), the loop already prints the canonical stop card (`stop: <reason>` / `hint:` / `resume:`) on stdout beside the error envelope — relay both verbatim; the resume line is `emery plan execute` (fix the reported problem, then re-run — the loop resumes at the parked phase) or a waive invocation. No follow-up `emery plan status` call is needed.
