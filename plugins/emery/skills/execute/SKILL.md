...
1. **Status probe** — run `emery plan status` (read-only — never substitute `emery plan next`, a plan-state writer). Branch on the header's `plan: <name> (pending|approved)` line and the `next-action` projection:
