...
- Surface the CLI output verbatim. The `resume:` line is the answer to "what do I do next" — a skill invocation (`/emery:refine`, `/emery:execute`, `/emery:finalize <name>`) or a literal command (`emery plan refine` when refinement is missing or stale, `emery plan execute`, or `emery plan defer …` when open gaps block).
