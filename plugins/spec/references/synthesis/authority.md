...
- The override applies to **every** claim of the named kind in that Evidence document; per-claim overrides remain out of scope (see [RFC-27](../../../../rfcs/rfc-27-synthesis.md) §Non-goals).
...
- Per-claim overrides remain out of scope (see [RFC-27](../../../../rfcs/rfc-27-synthesis.md) §Non-goals). The override seam below per-kind granularity stays as today: hand-edit `spec.md` after `/spec:refine` transitions the slice to `refined`.
