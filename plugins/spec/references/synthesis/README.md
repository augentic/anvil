...
The synthesis playbook is the agent-facing reconciliation contract `/spec:refine` follows when it authors the **synthesis response** that the `specify slice refine` synthesis leg projects into the canonical slice artifacts (`proposal.md`, `specs/<domain>/spec.md`, `design.md`, `tasks.md`) and the typed `model.yaml`. The skill body owns the CLI choreography (slice create, serial extract, the `specify slice refine` handoff, validate, transition); this playbook owns **what to write into the response**.

## Agent / kernel split

The synthesis leg inside `specify slice refine` assembles the **inputs** envelope (each bound source's inline `lead` + `claims`, plus the resolved target `shape` brief); the agent authors the **response**; the persist tail runs the CLI-owned projection kernel and persists everything. The line between the two is sharp:
