...
Plan-time `Lead[]` reconciliation — the `propose` sub-step inside `/spec:plan` that writes `slices[]` rows — is specified by [RFC-29b §"Plan-Time Lead Reconciliation"](../../rfcs/rfc-29b-reconciliation.md). The kernel locks exact id / alias matches; the agent authors semantic merges; `match-basis` is a Gate 1 review signal in `change.md`, not persisted on `plan.yaml`.
