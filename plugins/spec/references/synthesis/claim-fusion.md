...
| Kind            | Carrying authority class    | Where it lands                                                                                  | Fusion key                                |
| --------------- | --------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------- |
...
| `decision`      | `documentation`             | `design.md` — under the H2 the decision informs (transport → APIs; error strategy → Technical logic; provider choice → Configuration). Quote verbatim with `(from <source-key>)`. | None (free-form; not fused).              |
| `section`       | `documentation`             | `design.md` — folded as context under the most relevant H2; or `proposal.md` `## Motivation` when the section names the slice's *why*. | None (free-form; not fused).              |
...
### Deterministic fusion on `claim-id`

`requirement` and `criterion` claims MUST carry `claim-id` (enforced by `schemas/evidence.schema.json`). Synthesis groups every contributing claim by exact `claim-id` match across all Evidence documents — that is the cross-source fusion key.
...
### `example` claims from `runtime-fixtures`

`example` claims are emitted by the `runtime-fixtures` source adapter from captured request/response fixtures. They share the `behaviour` authority class with `excerpt` and `call` claims, and they tie-break the same way:
...
- **Per-Evidence override.** A `runtime-fixtures` Evidence document MAY emit `authority-overrides: { example: documentation }` to lift its `example` claims above the document-level `behaviour` default — rare, but useful when the captured fixtures encode an explicit contract the operator wants treated as documentation-class.
- **Per-kind body.** `example` claims carry `claim-id`, `path` (the on-disk fixture anchor), `fixture-digest` (a `sha256:` fingerprint the cache keys against), `input` (the captured request shape), `output` (the captured response and side-effect shape), and an optional `statement:` line that paraphrases the example for prose use. The per-kind body shape is owned by `adapters/sources/runtime-fixtures/briefs/extract.md` (created in RFC-27 Change 3.1); refer to that brief rather than mirroring the fields here.
...
- Group fusion is deterministic: sort `Sources:` by authority class (`intent` < `documentation` < `behaviour`), then alphabetically by source key within a class. The requirement block lists the highest-authority key first.
