...
| `criterion`     | `documentation`             | `spec.md` — folds into the requirement block whose `claim-id` shares the same `<requirement>.*` prefix as a `#### Scenario:` H4 inline within that block; when no requirement prefix matches, attaches to the nearest requirement by source order. | `claim-id` (required by the schema).      |
...
| `section`       | `documentation`             | `design.md` — folded as context under the most relevant H2; or `proposal.md` `## Why` when the section names the slice's *why*. | None (free-form; not fused).              |
