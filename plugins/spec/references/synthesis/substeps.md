...
- **Single-source code (port)** — `## Why` is "preserve observed legacy behaviour for `<candidate-id>`"; `## Units` lists the handler/endpoint families surfaced by `excerpt` / `call` claims (one kebab-case slug per family); non-goals call out behaviours the legacy code does *not* exhibit when a `documentation` source contradicts.
