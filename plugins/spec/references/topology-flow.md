...
- **Regular project** — a single repository that contains both code and `.specify/`. Phase pipelines (define / build / merge) run against this repo's working tree, driven by the active **capability**.
...
## Capability vs `--hub` is mutually exclusive
...
- **`$CAPABILITY`** *(regular only — skip in hub mode)* — if supplied as an argument, use it directly. Otherwise prefer the canonical Omnia capability identifier (`https://github.com/augentic/specify/capabilities/omnia`) unless project context indicates another capability. A local capability directory (`./capabilities/omnia`) is also valid. If multiple candidates are plausible, use the **AskQuestion tool**. Do not pre-populate `.specify/.cache/`; the CLI owns capability fetch/copy.
...
Never combine the two — see [Capability vs `--hub` is mutually exclusive](#capability-vs---hub-is-mutually-exclusive).
...
After step 6 of the Critical Path, render exactly one template from [init-output-templates.md](init-output-templates.md), substituting the resolved `$CAPABILITY` (regular and brownfield only; hub omits it):
