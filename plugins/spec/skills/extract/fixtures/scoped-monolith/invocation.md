...
- `src/b/handler.ts` and `src/common/util.ts` are **not** analyzed in Step 2
  even though they live under `source/`.
- `package.json` and top-level `README.md` are always read (see
  §*Sentinels always read* in `../../SKILL.md`); language detection and
  dependency pinning run unchanged.
