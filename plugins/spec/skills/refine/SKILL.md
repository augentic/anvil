...
5. **Write `reconciliation.yaml`** — atomically (sibling temp file, then rename; partial writes must never land on disk) at `.specify/slices/$SLICE_NAME/reconciliation.yaml` per the block grammar in [`../../references/synthesis/reconciliation.md`](../../references/synthesis/reconciliation.md); one entry per `REQ-*` id in `spec.md`, every contributing `(source, claim-id)` pair, inline truncated `value`, `winner` markers, closed `resolution` enum, and `resolution-trace` on override paths. No `specrun slice reconcile` verb exists — the skill body is the writer and step 6 catches structural drift. After the rename succeeds, emit `slice.reconciliation.written` with payload `{ slice-name, generator: specify@<version>, requirement-count: <N> }` on `.specify/journal.jsonl`.
