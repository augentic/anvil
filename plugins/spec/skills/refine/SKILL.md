...
```text
Refine halted: synthesis output failed validation (<error-code>). Slice <slice-name> stays refining. Fix the synthesis response and re-run specrun slice synthesize <slice-name> --from <response.json> (the kernel owns provenance — never hand-edit ID:/Sources:/Status: lines), then specrun slice transition <slice-name> refined.
```
