...
```text
Refine halted: source <source-key> extract failed (<error-code>). Slice <slice-name> stays refining. Amend the plan (specrun plan amend <slice-name> --remove-source <source-key>, or --add-source <source-key>=<candidate-id>) and re-run /spec:refine <slice-name>.
```
