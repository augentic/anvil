...
description: "Wrap the post-execute tail of a change: push branches via `specify workspace push`, observe PR state until `MERGED`, then run `specify plan archive` to archive the plan. Use when every per-entry plan status is `done` and the operator is ready to close the change; not for plan authoring (`/spec:plan`) or per-slice execution (`/spec:execute`)."
argument-hint: <name>
---
