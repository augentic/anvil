...
1. **Acquire the plan lock when invoked standalone.** When env var `SPECIFY_PLAN_LOCK_HELD=1` the parent loop holds it — do not re-acquire. Otherwise acquire `.specify/plan.lock` (workspace in workspace mode) *before* any plan verb; see [plan-lock.md](../../references/plan-lock.md) — `specify plan next` and `specify slice merge run` are CLI-gated and refuse an unlocked driver with `plan-lock-not-held`.
...
3. **Workspace routing.** When `.specify/project.yaml` carries `workspace: true`, run `specify workspace sync $PROJECT`, `chdir` into `.specify/workspace/$PROJECT/`, and export `SPECIFY_PLAN_DIR=<workspace-root>` — the slot carries no `plan.yaml`, and the export is how step 6's `done` stamp reaches the workspace plan (restore both on exit). Single-repo mode is a no-op.
