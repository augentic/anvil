...
7. **Run the post-merge target hook.** Some target merge briefs (notably `contracts`) re-run a validator against the promoted baseline (the contracts adapter's in-guest contract validator). A failure here is a post-merge signal, not a rollback — the slice is already `merged` and the plan entry is already `done`. Surface the failure so the operator can queue a repair slice; do not attempt to revert.
...
- `adapters/targets/<target>/prose/briefs/merge.md` — pre-merge gate and post-merge hook this skill drives (omnia, vectis, contracts).
