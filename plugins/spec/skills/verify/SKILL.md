...
license: MIT
allowed-tools: Read, Glob, Grep, Shell, SemanticSearch
...
4. **Extract current behavior from source**
   ...
   **Do not invoke the full analyze skill** -- this step performs a lightweight read-and-compare, not a full artifact reconstruction. Focus on identifying whether baseline requirements are implemented and whether new unspecified behaviors exist.

5. **Compare against baseline requirements**

   Read each baseline spec from `.specify/specs/<capability>/spec.md`. Parse requirement blocks using the heading conventions from `references/spec-format.md`.
...
- Do not run the full analyze skill -- perform lightweight comparison only
