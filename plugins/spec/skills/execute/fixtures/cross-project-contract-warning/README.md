...
3. Walks the registry to find consumers — `mobile.contracts.consumes`
   matches.
4. Invokes `/contracts:validator --mode cross-project` with the
   producer's just-merged contract path and the consumer's workspace
   clone path.
...
├── validator-output.yaml                       # /contracts:validator --mode cross-project output
...
- Before changing the `/contracts:validator` cross-project output
  shape, re-run the validator's own
  [output format section](../../../../../contracts/skills/validator/SKILL.md#output-format--cross-project)
