...
There is no automated harness that runs these fixtures. They are prose artefacts: a human reviewing a change to `/spec:execute`'s loop behaviour should be able to trace each on-disk transition by reading the files in order, and any drift between the `.metadata.yaml` outcome and the `plan.yaml.after` `status-reason` is a regression.
...
1. **Lock held once, across all iterations.** `specify initiative lock acquire` runs once at step 2 of the `--loop` algorithm; `specify plan lock release` runs once at step 6. Per-iteration lock churn is visible nowhere in any transcript.
...
5. **`driver-interrupted` leaves the active entry as `in-progress`.** The interrupt handler does NOT run `specify initiative transition` on the active entry; self-heal on the next run is responsible for reclaiming it based on `.metadata.yaml:outcome`.
