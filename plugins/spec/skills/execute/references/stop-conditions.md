...
Re-entry contract: the next `/spec:execute` resumes at `/spec:merge` (slice lifecycle is still `built`). If the operator chose to drop the slice instead, they run `/spec:drop <slice> reason "<rationale>"` followed by `specify plan transition <slice> blocked --reason "<rationale>"`; `/spec:execute` then re-picks the next pending entry.
