...
1. **Save CWD.** The initiating directory is the workspace; `specify plan status`, `specify plan next`, `specify plan transition`, and the guest lock marker all live there. Restore to it at the end of the iteration.
...
6. **Export the plan root.** Set `SPECIFY_PLAN_DIR=<workspace-root>` for the duration of slot-side phase work (or pass `--plan-dir <workspace-root>` on each `specify` invocation). No slot grows its own `plan.yaml`; the override is how slot-side plan readers (`source extract`, the `slice refine` synthesis leg, `slice validate`, `slice provenance`) and `slice merge`'s `done` stamp resolve the workspace's plan, and how relative `sources.<key>.path` bindings keep resolving against the workspace. Unset it on CWD restore. Source adapters still resolve slot-locally (vendored `adapters/` tree or manifest cache) — the step-3 `specify workspace sync` mirrors the workspace's adapter set into the slot's manifest cache, so a workspace-vendored adapter resolves in the slot with no manual staging.
...
9. **Restore CWD.** `chdir` back to the saved workspace (and unset `SPECIFY_PLAN_DIR`) before the next `specify plan status` / `specify plan next` so plan reads and writes resolve against the correct `plan.yaml`.
...
1. Mutual exclusion is guest-owned via the workspace's `.specify/guest.lock` marker per [`../../../references/plan-lock.md`](../../../references/plan-lock.md); no explicit acquisition step remains.
...
The guest lock at the workspace is what guarantees that an operator running `/spec:build` from the workspace cannot race a background `/spec:execute` (or a sibling operator running `/spec:merge`) on the same plan: the guest-routed drivers hold the workspace's `.specify/guest.lock` marker for the run's lifetime, and a concurrent driver fails loudly host-side. Scenario #11 in [`../../../../../evals/fixtures/skills/execute/`](../../../../../evals/fixtures/skills/execute/) pins the breakout-after-build-failure path: `/spec:execute` parks on `auth-rotate` in `backend` and exits, and the operator runs `/spec:build` from the workspace — which resolves `auth-rotate → backend`, `chdir`s into the slot, and resumes the failing task.

## CWD restore in the loop

Every iteration brackets the `chdir` with a save/restore. The closure is intentionally conservative: even an iteration that doesn't change CWD (single-repo entry, or workspace entry whose `project` resolution failed before step 5) still runs the restore. This keeps `specify plan status`, `specify plan next`, `specify plan transition`, and the trailing-edge lock release on the trailing iteration anchored at the workspace.
