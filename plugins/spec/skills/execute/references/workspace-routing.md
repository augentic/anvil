...
1. **Save CWD.** The initiating directory is the workspace; `specrun plan next`, `specrun plan transition`, and the plan lock all live there. Restore to it at the end of the iteration.
...
8. **Restore CWD.** `chdir` back to the saved workspace before the next `specrun plan next` so plan writes resolve against the correct `plan.yaml`.
