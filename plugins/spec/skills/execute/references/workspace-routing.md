...
8. **Restore CWD.** `chdir` back to the saved workspace root before the next `specify plan next` so plan writes resolve against the correct `plan.yaml`.
