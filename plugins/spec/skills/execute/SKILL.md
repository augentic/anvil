...
description: Drive an approved plan through refine → build → merge per entry under the guest-owned driver lock. Use when Gate 1 has stamped plan approved; not before Gate 1, nor after all entries are done.
---
...
2. Driver mutual exclusion is guest-owned: the guest-routed loop holds the `.specify/guest.lock` marker for the run's lifetime per [`../../references/plan-lock.md`](../../references/plan-lock.md); a concurrent driver fails loudly host-side. No explicit lock acquisition step remains.
...
Mutual exclusion for skills that touch plan state from outside the loop is guest-owned per [`../../references/plan-lock.md`](../../references/plan-lock.md): the guest-routed verbs hold the `.specify/guest.lock` marker for the run's lifetime, and a concurrent driver fails loudly host-side.
...
- **Never race the driver.** Mutual exclusion is guest-owned via the `.specify/guest.lock` marker — including breakouts of `/spec:refine`, `/spec:build`, and `/spec:merge` when an operator runs them standalone. See [`../../references/plan-lock.md`](../../references/plan-lock.md); a concurrent guest-routed driver fails loudly host-side.
