...
Two slices, both fused by `propose` from a documentation source and a TypeScript legacy source. The propose sub-step matched `user-registration` cleanly across both sources; the `password-reset` row pairs the documentation candidate with the legacy candidate `account-pwd-reset` after a tentative match on intent.
