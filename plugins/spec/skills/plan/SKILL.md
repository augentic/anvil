...
- Surface the CLI output verbatim, including the closing Gate 1 hint (the literal `specify plan approve` command). Never run that stamp yourself — Gate 1 is operator-only; the operator stamps it directly or through `/spec:execute`'s explicit approval gate.
