...
3. **Submit** — write the response envelope to `.specify/scratch/plan/propose-response.json` (the canonical location, reset by the dry-run — never the project root), then `specify plan propose --from .specify/scratch/plan/propose-response.json --reconcile-platforms` schema-gates the response against `proposal.schema.json`, validates it against a fresh catalog recomputed from `discovery.md`, replaces all `plan.yaml.slices[]` rows, each binding a `project` (the target adapter is resolved on demand from the project, not stored per slice), and runs the platform reconciliation pass. The `--reconcile-platforms` flag triggers a post-write pass that reads `project.yaml.platforms`, runs the vectis tool in detect mode to find declared-but-absent shells, and deterministically inserts bootstrap slices (e.g. `app-foundation` for greenfield, `bootstrap-<platform>` for incremental) with feature slices depending on them. The command emits a single `plan.reconcile.completed` journal event itself — the skill never runs `specify journal emit` for D2.
4. **Gate 1 review prose** — render cross-source merges into `change.md`. Read any non-blocking `lead-decision-topic-overlap` advisories the `propose --from` output surfaces and confirm each flagged slice aligns with the named accepted decision (or plan a superseding decision); these never abort the write. When reconciliation is uncertain, add `## Tentative merges` (never edit `discovery.md`). When a lead is multi-homed across slices, add `## Cross-cutting leads` listing each such lead and its member slices; when it captures durable policy, note it as a candidate for promotion to an engineering rule (`adapters/**/rules/`) at Gate 1. When merged summaries materially disagree, add the operator-facing side-by-side under `## Likely divergences`; the machine-readable values belong in the slice's `disagreements[]` (authored in step 2 on the response). When a slice was not flagged at propose, stamp it via `specify plan amend <entry> --divergence likely` (after `propose --from`, the only slice writer).
