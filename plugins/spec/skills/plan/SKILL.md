...
3. **Run the plan brief pipeline** from `schema.yaml`:
   - **(a) Discovery** — invoke the discovery brief via `/spec:analyze`; writes `discovery.md`. May surface a `## Proposed registry topology` block that triggers the **greenfield registry bootstrap** (RFC-9 §2B) before step 3(b) when no `.specify/registry.yaml` exists yet. See [discovery.md](discovery.md).
