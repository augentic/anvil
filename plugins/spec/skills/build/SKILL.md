...
7. **Transition on success.** Run `specrun slice transition $SLICE built --format json`. The CLI stamps `.metadata.yaml`. Return control to the caller; `/spec:execute` (when present) advances to merge, otherwise the operator runs `/spec:merge $SLICE`.
...
- **Never write `plan.yaml` from this body.** Per-entry transitions are owned by `specrun plan next` (writes `in-progress`) and `specrun slice merge` (writes `done`). `/spec:build` only writes the slice's `.metadata.yaml` via `specrun slice transition`.
