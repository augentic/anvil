...
Implement tasks from a Specify slice. Deterministic bookkeeping — slice selection, lifecycle transitions, adapter resolution, brief completion checks, task progress counting, checkbox flips — is delegated to the `specify` CLI. This skill drives the agent-side work: reading the build brief body, dispatching skill directives, and making code changes.
...
1. The action is executable by an agent in this repo using code, tooling, mocks, fixtures, contract validators, build commands, or one of the reviewer / writer skills available to the adapter (consult the adapter's `briefs/tasks.md` Available Skills table if unsure).
