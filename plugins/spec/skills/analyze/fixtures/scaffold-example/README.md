...
Illustrative fixture pinning the on-disk shapes emitted by `/spec:analyze`. This fixture is **structural**, not a test target — it exists so reviewers and downstream briefs can eyeball the exact shapes without squinting at prose.
...
- [`expected/discovery.md`](expected/discovery.md) — the expected `$DISCOVERY` after a single `/spec:analyze --kind legacy-code --source-key monolith` invocation against `inputs/`.
- [`expected/plans/scaffold-example/analyze/monolith/metadata.json`](expected/plans/scaffold-example/analyze/monolith/metadata.json) — the structural-metadata sidecar written alongside `$DISCOVERY` by the same invocation. Populated per [`../../SKILL.md` §*Structural metadata*](../../SKILL.md). `scaffold-example` is the stand-in initiative name for this fixture; in a real run the segment is the actual `<initiative-name>` under `.specify/plans/<initiative-name>/`.
