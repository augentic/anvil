...
5. **Invoke `specify init`** — run either `specify init "$ADAPTER" ...` or `specify init --workspace ...`; the CLI scaffolds files, chains `workspace sync` on workspace init, and generates starter context. Do not call `specify workspace sync` separately after workspace init. Surface non-zero CLI errors without hand-rolling scaffold files.
