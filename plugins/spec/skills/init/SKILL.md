...
- **Adapter vs `--workspace` is mutually exclusive.** Pass `$ADAPTER` as the first positional for regular projects, or `/spec:init workspace` / `specrun init --workspace` for workspaces. Workspace init refuses to run over an existing `.specify/`; converting a regular project to a workspace requires the operator to remove `.specify/` first.
