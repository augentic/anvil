...
> **Renamed.** This skill was previously `/spec:plan` (skill name `specify-plan`); RFC-13 §3.9 moved it to the change surface as `/change:plan`. The old slash-command remains available as a thin deprecation shim that delegates here — see [RFC-13 §Migration](../../../../rfcs/rfc-13-extensibility.md#migration).
