---
name: change-plan
description: "Authors `plan.yaml` for a change on the change surface via the planning brief pipeline; with `--orchestrate`, drives the cross-repo umbrella end to end. Use when scoping a new change or coordinating multi-repo execution from a single command on the `/change` surface."
...
> **Renamed.** This skill was previously `/spec:plan` (skill name `specify-plan`); RFC-13 §3.9 moved it to the change surface as `/change:plan`. The old slash-command remains available as a thin deprecation shim that delegates here — see [RFC-13 §Migration](../../../../rfcs/archive/rfc-13-extensibility.md#migration).
...
1. Parse inputs; resolve source paths; assert plan.yaml absent
   ...
   - Require at least one of --from, --against, --source, or a
     ...
     inputs). A bare /change:plan <name> with neither CLI inputs nor
