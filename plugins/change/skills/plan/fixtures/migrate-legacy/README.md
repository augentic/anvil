...
This fixture pins the **happy path** of `/change:plan <name> orchestrate` (formerly `/spec:initiative`) driving the `migrate-legacy` shape against an empty platform hub. Sources arrive via `source <key>=<git-url>`; the registry is empty, so step 3 enters the 2B greenfield path. The umbrella opens PRs, stops for operator merge, then finalizes on re-entry after `specify change finalize` verifies the remote PR state.
