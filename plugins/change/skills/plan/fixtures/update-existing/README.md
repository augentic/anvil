...
7. **Finalize.** `specify change finalize` confirms both PRs `MERGED` on remote and archives `plan.yaml` + `change.md` + `.specify/plans/polish-pass/` into `.specify/archive/plans/<YYYYMMDD>-polish-pass/`.
...
| [`expected/change.md.after`](expected/change.md.after) | Brief as scaffolded by step 1 with empty `inputs:` and operator-supplied prose. |
