...
| `metadata-product-catalog-mid-build.yaml` | `.specify/slices/product-catalog/.metadata.yaml` at the SIGKILL moment: `status: building`, no `outcome` field yet. |
...
- `../self-heal/` pins the four classification paths in isolation (no in-progress, done reclaim, failed reclaim, ambiguity halt, mid-change resume) against minimal one-entry plans. This fixture exercises the mid-slice-resume path against a realistic nine-entry plan.
