# `/change:execute` — single-slice behavioural fixtures

These fixtures pin the three terminal outcomes of a supervised single-slice run (no `--loop`): `done`, `failed`, and `blocked`. They correspond to RFC-2 Change L2.F; the algorithm they illustrate lives in [`../../SKILL.md` → §Supervised single-slice run](../../SKILL.md).

There is no automated harness that runs these fixtures. They are prose artefacts: a human reviewing a change to `/change:execute` should be able to trace each on-disk transition by reading the files in order, and any drift between the `.metadata.yaml` outcome and the `plan.yaml.after` `status-reason` is a regression.
...
3. **No driver writes to `.metadata.yaml` or `journal.yaml`.** The metadata snapshots and `journal.yaml` contents are authored by the phase skills (via `specify slice outcome set` and `specify slice journal append`). `/change:execute` only writes plan status transitions.
...
- Future Changes (L2.G self-heal, L2.H `--loop`, L2.I sources wiring) will ship their own fixtures alongside these; none of them should require editing the single-slice fixtures to stay coherent.
