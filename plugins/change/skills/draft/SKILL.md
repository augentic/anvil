...
4. **Plan brief pipeline** from `capability.yaml`:
   - **(a) Discovery** — invoke the discovery brief via `/change:analyze`; writes `discovery.md`. May surface a `## Proposed registry topology` block that triggers the **greenfield registry bootstrap** before step 4(b) when no `registry.yaml` exists yet. See [discovery.md](discovery.md).
...
| [`discovery.md`](discovery.md) | Discovery brief (step 4a) — `/change:analyze` integration and greenfield registry bootstrap |
...
| [`propose.md`](propose.md) | Propose brief (step 4c) — accept/edit/reject loop, `specify plan add` |
| [`assignment.md`](assignment.md) | Assignment brief (step 4d, multi-repo only) — `--project` inference and registry-proposal sub-step |
