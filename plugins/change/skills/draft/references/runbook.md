...
2. Brief scaffold — scaffold the brief and plan together.
   ...
   `sources` map). `changes: []` until step 4(c) populates
...
└── plans/
    └── <change-name>/
        ...
        ├── proposal.md             # from the propose brief (step 4c) + assignment table (step 4d)
...
| 4 Plan brief pipeline | `specify capability resolve --format json`, `/change:analyze`, `specify plan add`, `specify plan amend`, `specify registry add`, `specify workspace sync` |
...
5. `.specify/plans/<change-name>/proposal.md` written by the propose brief (step 4c).
6. `.specify/plans/<change-name>/workspace.md` written by step 4(b) when the registry declares more than one project.
