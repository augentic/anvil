...
Pins the 1:1 adapter → slice mapping the propose brief applies to a unified-YAML `discovery.md`. Consumes the C22 fixture's discovery output verbatim and emits a three-entry `plan.yaml` with rich `description` fields carrying file-path hints from each adapter's `sources:` list and delta-targeting intent.

This fixture is the acceptance target for [RFC-3a C24](../../../../../../../rfcs/archive/rfc-3a-monoliths.md) (the decomposition-heuristic rewrite in [`plugins/change/skills/draft/briefs/omnia/propose.md`](../../../briefs/omnia/propose.md)).
...
| [`inputs/discovery-manifest.md`](inputs/discovery-manifest.md) | Same three adapters as `discovery.md`, but `user-registration` is `confidence: low`. |
