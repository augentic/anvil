...
```yaml
summary: Re-publish a previously captured batch from Redis to the primary Kafka topic during incident recovery.
sources:
  - src/ingest.rs
depends-on: [ingest-submit]
hints:
  entry_points: [POST /ingest/replay]
  external_deps: [kafka, redis]
confidence: high
```
...
```yaml
summary: Persist a single upstream ingest payload to the primary Kafka topic.
sources:
  - src/ingest.rs
depends-on: []
hints:
  entry_points: [POST /ingest]
  external_deps: [kafka]
confidence: high
```
