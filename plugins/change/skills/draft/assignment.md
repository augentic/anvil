# Assignment and registry proposal (step 4d, multi-repo only)

After the propose brief completes step 4(c) and all accepted entries have been written to `plan.yaml` (without `project`), the draft skill runs the assignment pass when `workspace.md` is present and contains more than one project section. Single-project registries skip this step entirely.
...
When the registry is absent or single-project, step 4(d) is skipped entirely. No `--project` is written to plan entries.

## Step 4(d).1 — Registry proposal sub-step (RFC-9 §2B)

Step 4(d) routes entries to **existing** registry projects. When the operator's response in step 4(d).3 names a project that does **not** exist in `registry.yaml`, the registry-proposal sub-step is invoked **before** continuing assignment. This sub-step is the only place `/change:draft` ever calls `specify registry add`.
...
When `/change:draft` inserts a contract slice for an API boundary between projects, it populates the `contracts` block on the relevant registry project entries:
