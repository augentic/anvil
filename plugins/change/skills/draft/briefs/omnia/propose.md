...
Given [`plugins/change/skills/draft/fixtures/discovery/monolith/expected/discovery.md`](../../fixtures/discovery/monolith/expected/discovery.md):
...
Skip the `specify change draft` scaffold step (the caller —
typically the `/change:draft` skill — has already ensured both
