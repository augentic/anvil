...
2. Present the full topology table to the operator in a single batch review:
   ...
   The operator approves each row, edits any field, or rejects the entry. Rejection of a candidate drops it from the bootstrap; capabilities that would have routed to that project surface as `unresolved` during step 4(d) Assignment and re-enter the registry-proposal sub-step (4(d).1) one at a time.
