...
Step 4(c) invokes the propose brief bundled with this skill at `briefs/<adapter>/propose.md` (for Omnia, [`briefs/omnia/propose.md`](briefs/omnia/propose.md); for Vectis, [`briefs/vectis/propose.md`](briefs/vectis/propose.md); other adapters ship their own variant alongside). Planning is orchestration, not adapter-owned slice work, so planning briefs live with this skill rather than in `adapter.yaml`. Propose reads `discovery.md`, applies the adapter's slice heuristics to decompose the inventory into draft slices with `depends-on` edges, and iterates with the human on each slice (accept / edit / reject / abort). For every accepted slice, the skill shells out to:
...
- **Spec slices**: When a slice targets existing adapters via `affects`, `context` entries are populated with the corresponding baseline spec paths (e.g. `specs/user-registration/spec.md`).
