...
Propose reads both `## Capability inventory` blocks and `## Candidate inventory` blocks from `discovery.md`. Candidate blocks with `unresolved: true` are refused — the propose brief surfaces a diagnostic and the operator must edit the candidate in `discovery.md` before re-running propose.
