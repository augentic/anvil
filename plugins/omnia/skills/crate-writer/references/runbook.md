# Crate Writer Runbook

Operational detail for `omnia-crate-writer`. The SKILL.md keeps only the orientation surface (Critical Path + Reference table + Guardrails); everything procedural lives here.
...
7. [guardrails.md](guardrails.md) -- WASM constraints and forbidden patterns
8. [cargo-toml.md](cargo-toml.md) -- Cargo.toml template and dependency rules
9. [guest-wiring.md](guest-wiring.md) -- how crates wire into the WASM guest
...
13. [update-patterns.md](update-patterns.md) -- update strategy patterns by category
14. [change-classification.md](change-classification.md) -- how to classify artifact-vs-code differences
...
- **design.md External Services + Source Capabilities Summary + Business Logic cues** -> handler trait bounds (via [capability-mapping.md](capability-mapping.md))
- **design.md Implementation Requirements `[runtime]` constraints** -> Omnia patterns (via [wasm-constraints.md](wasm-constraints.md))
...
See [sdk-api.md](sdk-api.md) for the Handler trait definition, Input Type Decision Tree, and Response Types (IntoBody).
...
See [error-handling.md](error-handling.md) for domain error patterns, macro usage, context chaining, validation placement rules, timestamp semantics, and serde conventions.
...
After generating or updating the crate, inject or update wiring in the guest project. See [guest-wiring.md](guest-wiring.md) for templates.
...
7. **Cross-Cutting Analysis** — before generating any handler code, build the three matrices (Side-Effect, Outbound Message, Transaction Boundary) defined in [cross-cutting-matrices.md](cross-cutting-matrices.md). The matrices are working artifacts (not persisted) but every cell must be satisfied in the generated code. If a cell cannot be implemented, mark it with a TODO per the todo-markers rules.
8. Run pre-generation checklist above (verify artifact completeness)
9. Generate `Cargo.toml` (see [cargo-toml.md](cargo-toml.md))
...
16. **Traceability Verification** — verify every spec requirement, design.md Business Logic step, and matrix cell has a corresponding code path per [cross-cutting-matrices.md → Traceability Verification](cross-cutting-matrices.md#traceability-verification). Implement any missing path before handoff and re-run `cargo check`.
...
See [change-classification.md](change-classification.md) for detailed classification rules and edge cases.

#### Step 2a: Cross-Cutting Analysis (changed handlers only)

For every handler classified as **Additive** or **Modifying** — plus any unchanged handler whose cross-cutting behavior depends on a modified entity or handler — build the three matrices defined in [cross-cutting-matrices.md](cross-cutting-matrices.md). Every cell must be satisfied in the updated code.

#### Step 3: Generate Update Plan

For each slice, determine the specific edit operations and produce a structured plan grouped by category (structural → subtractive → modifying → additive). See [cross-cutting-matrices.md → Update mode example plan](cross-cutting-matrices.md#update-mode--example-plan) for the canonical shape. Log the plan for traceability; do not modify any files in this step.
...
- Patterns: See [update-patterns.md](update-patterns.md#structural-patterns)
...
- Patterns: See [update-patterns.md](update-patterns.md#subtractive-patterns)
...
- Patterns: See [update-patterns.md](update-patterns.md#modifying-patterns)
...
- Patterns: See [update-patterns.md](update-patterns.md#additive-patterns)
...
For every handler classified as **Additive** or **Modifying**, run the verification rules in [cross-cutting-matrices.md → Traceability Verification](cross-cutting-matrices.md#traceability-verification) against the updated code. Implement any missing path and re-run `cargo check`.
