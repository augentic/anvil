...
- `plugins/omnia/references/codex/` — Omnia-specific rules: `OMNIA-001` Provider-Only Host Access, `OMNIA-002` WASM Guest Runtime Constraints, `RUST-001` Classified SDK Errors, No Panic Paths, and `SEC-001` Host-Managed Secrets and Identity.
- `plugins/omnia/references/codex/` — universal `UNI-001` through `UNI-021` rules. Read these codex files directly.
