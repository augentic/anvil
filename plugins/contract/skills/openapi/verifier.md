...
| `cross-project` | contracts capability merge brief; `/change:execute` post-merge step | Producer-side merge of an OpenAPI contract change | Walk the merged `contracts/` baseline; enforce RFC-12 §Validation | JSON envelope produced by `specify-contract-validate` |
