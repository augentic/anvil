...
The author runs four sequential steps end-to-end whenever the contracts capability build brief asks for shared payload coverage. Each step is a focused, independently-checkable phase; downstream steps assume the upstream output is well-formed.
...
- A platform-wide vocabulary type that appears across briefs (`ErrorResponse`, `ResourceLink`).
