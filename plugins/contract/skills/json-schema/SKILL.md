...
|---|---|---|
| Author or extend reusable schemas from a spec | contracts adapter build brief during `/spec:build`; operator extending the baseline for new payload types | `author.md` |
...
| Verify `$ref` consistency, metadata, cross-format consumer compatibility, or merge-time baseline validation | contracts adapter build verification; post-merge contract baseline gate | `verifier.md` |
...
When a slice touches more than one contract format (HTTP + events + shared schemas), the contracts adapter build brief invokes the format skills in this fixed order:
