...
`UIKit` directly without the `#if canImport` guard the previous design-system
package required (the package had to compile for both `iOS(.v17)` and
`macOS(.v14)`; the shell target deploys to iOS only).
