...
When no Android shell exists yet, the skill runs `vectis add-shell android
...
- **Create Mode** -- `{project-dir}/` does **not** exist. The skill invokes
  `vectis add-shell android` to scaffold the baseline, then proceeds directly
...
```bash
vectis add-shell android --dir {app-dir} [--android-package com.example.app]
```
...
```bash
vectis add-shell android --dir {app-dir} [--android-package <package>]
```
...
from the CLI's embedded `versions.toml`; use `vectis update-versions` to
...
Pre-flight checks before build (all generated by `vectis add-shell android` --
...
starter Kotlin layout are owned by the CLI's embedded templates
(`crates/vectis-cli/src/init/android.rs` and
`crates/vectis-cli/embedded/android/`). Do not hand-edit those files in
Create Mode; let `vectis add-shell android` write them and then modify in
Update Mode.
...
| `./gradlew: No such file or directory` | Scaffold was missing the wrapper -- re-run `vectis add-shell android` (the CLI bootstraps `gradlew` from a scratch Gradle invocation) |
| `Minimum supported Gradle version is X.Y` | Gradle/AGP drift -- run `vectis update-versions` and re-run `vectis add-shell android` (the CLI pins `gradle-wrapper.properties` to match AGP from `versions.toml`) |
...
| `resource style/Theme.{AppName} not found` | Scaffold was missing `res/values/themes.xml` -- re-run `vectis add-shell android` |
...
- **Gradle wrapper is required**: The `gradlew` script must exist before any
  `./gradlew` command works. `vectis add-shell android` bootstraps it by
...
- **Java 21 LTS required**: Java 25+ has a version string that Gradle's
  Kotlin compiler cannot parse. `vectis add-shell android` detects Java 21
