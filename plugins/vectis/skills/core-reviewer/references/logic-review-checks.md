...
`REQ-XXX` ID. If traceability comments are absent (legacy tests), fall back
to matching by test function name against scenario title (approximate).
...
3. For each spec scenario, check whether a matching test exists.
...
compare against spec scenarios by `REQ-XXX` ID. For edge-case coverage,
