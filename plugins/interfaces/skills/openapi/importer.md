...
> **When to read this.** Read this when an operator drops an external OpenAPI (or Swagger 2.0) document into a change's `contracts/http/` directory and the contracts brief needs the file normalised onto Specify conventions. Skip this file when authoring from a spec (use [`author.md`](./author.md)) or when verifying an existing artefact (use [`verifier.md`](./verifier.md)).
...
Run [`verifier.md`](./verifier.md) in `single` mode against `$CHANGE_DIR` to confirm `$ref` resolution, schema metadata completeness, and binding coverage. If the verifier reports issues, re-enter Steps 4–5 for targeted repair before producing the report.
