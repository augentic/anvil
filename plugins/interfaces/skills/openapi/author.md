...
> **When to read this.** Read this when authoring or extending the OpenAPI document for a Specify change — i.e. when the contracts brief during `/spec:define` selects the author intent, or an operator wants to add new HTTP interactions to the platform's HTTP baseline. Skip this file when importing an external document (use [`importer.md`](./importer.md)) or when verifying an existing artefact (use [`verifier.md`](./verifier.md)).
...
The author runs four steps end-to-end whenever the contracts brief asks for HTTP coverage. Each step is a focused, independently-checkable phase; downstream steps assume the upstream output is well-formed.
...
- **Never inline** a domain type. If the spec mentions a new payload type, route the schema work to `/interfaces:json-schema` (or, in the same `/spec:define` invocation, the contracts brief calls the json-schema skill first per the cross-format ordering rule).
