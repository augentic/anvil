...
> **When to read this.** Read this when an operator drops an external AsyncAPI document into a change's `contracts/messages/` directory and the contracts brief needs the file normalised onto Specify conventions. Skip this file when authoring from a spec (use [`author.md`](./author.md)) or when verifying an existing artefact (use [`verifier.md`](./verifier.md)).
