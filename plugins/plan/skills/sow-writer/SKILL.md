...
This skill reads the Specify artifacts (specs and design.md) produced by `analyze` or `epic-analyzer` and translates technical content into client-facing deliverables. The output is a Markdown document structured to match the standard $COMPANY_NAME SoW format.
