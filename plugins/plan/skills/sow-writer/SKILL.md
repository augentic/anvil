...
This skill reads the Specify artifacts (specs and design.md) produced by `analyze` and translates technical content into client-facing deliverables. The output is a Markdown document structured to match the standard $COMPANY_NAME SoW format.
...
- Business Logic Blocks OR Spec Requirements (at least one)
...
- `requirements` — Greenfield development (from Specify artifacts or design document)
...
- **Source Reference**: From design.md header → Source field (repo URL, Specify change name, or design document path).
...
- **Greenfield** (`requirements` origin): Frame as new capability delivery. Reference the business need from spec requirements or Component purpose. Close with "This Statement of Work defines the scope of the $PROJECT_NAME delivery in accordance with the requirements specified in $SOURCE_REFERENCE."
...
For `code-analysis` artifacts, reference the source TypeScript files. For `requirements` artifacts, reference the Specify artifacts (specs and design). Also include any external API documentation.
...
| Spec Requirements / BDD Scenarios (specs) | Feature capabilities |
...
If the artifacts contain test-related information (e.g., from crate-writer's integration test output or spec acceptance criteria with BDD scenarios), generate a Testing Guidelines appendix covering:
...
2. [greenfield-sow.md](examples/greenfield-sow.md) — SoW generated from `requirements` artifacts (Specify)
...
| No Business Logic or Spec Requirements | Artifacts lack actionable content | Re-run the analyzer skill to enrich artifacts before generating SoW |
