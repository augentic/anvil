...
| [`authoring/skill.schema.json`](authoring/skill.schema.json) | Validates `SKILL.md` YAML frontmatter (name, description, argument-hint, allowed-tools). Cross-file invariants (plugin prefix, verb allow-list, known tools) are enforced by the `framework_quality` cargo tests. |
| [`authoring/scenario.schema.json`](authoring/scenario.schema.json) | Validates optional YAML frontmatter on scenario pack files under accepted scenario roots. Ordering and path-safety rules beyond JSON Schema are enforced by the `framework_quality` cargo tests. |
| [`authoring/marketplace.schema.json`](authoring/marketplace.schema.json) | Validates `.cursor-plugin/marketplace.json`. Cross-file consistency with `plugins/` is enforced by the `framework_quality` cargo tests. |
| [`rules/rule.schema.json`](rules/rule.schema.json) | Validates rule markdown YAML frontmatter at every `specify rules export`. |
| [`diagnostics/diagnostic.schema.json`](diagnostics/diagnostic.schema.json) | Validates a single structured finding emitted by validate surfaces. |
