...
4. **One plan entry per Crux unit.** The Crux units are:
   ...
   Never merge two independently-shippable shell views into a single entry; never split a single `App` trait across entries; never bundle two distinct UI-input artifacts (e.g. `tokens.yaml` AND `assets.yaml`) into a single slice — they are validated and reviewed independently by `specify vectis validate tokens` / `validate assets`.
