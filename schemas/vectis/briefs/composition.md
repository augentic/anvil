...
- **Do not rewrite token names or asset IDs** unless the existing reference is invalid AND a single confirmed replacement exists in `tokens.yaml` / `assets.yaml`. Otherwise emit a `# GAP` comment naming the unresolved reference and let the post-write `validate composition` run surface the warning.
