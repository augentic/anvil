...
This is the acceptance gate for propose-time slicing stability: propose (RFC-3a C24) caches its `scope.<k>.include` decisions off each capability's `sources` list, and `specify initiative validate` (RFC-3a C25) diffs `metadata.json` across runs to surface drift. If either artifact drifts under unchanged inputs, both downstream consumers see spurious change signals.
