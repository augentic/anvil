...
4. **Warn on inferred targets with no extract match.** For each name in the inferred affects set with no corresponding `<change-dir>/specs/<name>/spec.md` after the merge, emit a brief-level warning naming the orphan target. Suggest that the description may be inaccurate — the operator can amend it via `specify initiative amend <change> --description "..."`.
