#!/bin/bash
# Runtime executor for the canonical `guest-execute-loop` scenario.
#
#   bash quality/profiles/guest-execute-loop.sh            # composed wasm deployment
#   SPECIFY_SHIM=native bash quality/profiles/guest-execute-loop.sh
#
...
# Post-run evidence for the canonical hard-assertion grader.
echo "--- probes ---"
...
echo "guest-execute-loop ($shim): done"
