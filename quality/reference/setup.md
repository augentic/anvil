...
- A `specify` binary on your PATH that is the build under test. Run `cargo install --path . --locked` from this checkout to install the working-tree CLI into `~/.cargo/bin`; use Cargo's `--root <directory>` when a different prefix is required. Confirm the resolved executable with `command -v specify` and run `specify --help` before starting. To test a different binary, put it earlier on your PATH.
