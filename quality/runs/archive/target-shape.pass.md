...
| --- | --- | --- |
| `plan-exists` | pass | |
| `spec-reflects-shape-idioms` | pass | Intent: `.specify/slices/greeting/specs/greeting/spec.md` — per-requirement `ID:` / `Sources:` / `Status:` blocks; two operation-observable requirements (personalised response + empty-name rejection); `model.requirements[].scenarios[]` populated for validation; scenarios name inputs and observable 200/`BadRequest` outcomes. Docs fixture matches shape-derived requirement prose; `Sources: brief` only. |
| `design-reflects-shape-idioms` | pass | Both fixtures: `design.md` carries all eight Omnia `shape` sections in order — Domain model (newtypes), Provider trait dependencies (`Config` on the greeting operation), Operation delegation (`Operation<P>`, typed request `Input`, `CallContext<'_, P>`, no `Utc::now()` during transport decoding), External surfaces (`GET /greeting`, Axum brace syntax), Configuration (`GREETING_DEFAULT_NAME`), Error mapping (`thiserror` + `From<GreetingError> for omnia_guest::Error` with `code`/`description`), Validation placement across decoding / `Operation::call` / delegated domain logic, Observability (`monotonic_counter.*` metrics). |
