...
- **Sandbox:** `quality/.sandbox/workspace-stale-recovery/` (`platform/`, `backend/`, `mobile/`, `contracts/`)
...
- Plan authored headlessly via `quality/profiles/workflow/workspace.sh workspace-stale-recovery` (`specify plan create`, survey finalize, `propose --from`); Gate 1 stamped with `--actor agent`.
- Phase work driven by `quality/profiles/workflow/workspace.sh workspace-stale-recovery` following `/spec:execute` routing; interrupt simulated by stopping after `build --phase prepare` with an extra dirty root file, then releasing the plan lock.
...
- **Retained at:** `quality/.sandbox/workspace-stale-recovery/`
