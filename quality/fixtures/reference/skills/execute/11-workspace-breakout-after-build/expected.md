...
Pins the cross-cutting workspace + breakout contract: `/spec:execute` parks on a build failure in `backend`; the operator stays at the workspace and runs `/spec:build` standalone; the breakout body resolves the active slice's project and `chdir`s into the slot without operator intervention.
...
1. **Operator runs `/spec:build` from the workspace.**
   - The breakout observes the execution marker described by [`specify plan execute`](../../../../../../docs/reference/cli/plan.md#specify-plan-execute) at the workspace root before resolving the active entry.
...
2. **Operator runs `/spec:execute`.**
   - Lock acquired.
   - `specify plan status` returns `next-action: merge auth-rotate` (slice 1 still `in-progress`, lifecycle `built`, and the slot journal's newest terminal is the build success); loop dispatches `/spec:merge`.
   ...
   - Next iteration: `plan status` names slice 2; it routes into `mobile` (slot materialised); phase sequence runs end-to-end; `done`.
   - Next iteration: `plan status` reports drained → closing hint `drained — run /spec:finalize identity-rotation`.
...
- The breakout `/spec:build` resolved `auth-rotate → backend` without the operator passing `--project` or `chdir`-ing manually. The skill body's routing is the same code path the loop uses.
