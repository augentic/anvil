...
Pins the happy-path output of `/spec:merge` against an `omnia` slice and the per-entry `done` write that only `specify slice merge` produces.
...
3. Read and execute `adapters/targets/omnia/prose/briefs/merge.md`'s pre-merge gate.
