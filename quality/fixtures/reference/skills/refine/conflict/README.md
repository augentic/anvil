...
Two `documentation` Evidence contribute claims with the same `id` but disagreeing bodies. Authority is tied at `documentation` — synthesis cannot pick a winner, so the requirement carries `[conflict]` and preserves both values for operator reconciliation. Matches workflow §Eval scenario #5c.
