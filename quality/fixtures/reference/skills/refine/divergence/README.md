...
Two sources contribute claims with the same `id` but contradictory bodies; authority hierarchy (`documentation > behaviour`) picks the winner. Mirrors workflow §Per-requirement provenance variants — `[divergence]` from authority resolution.
...
Slice: `identity-password-reset`, target `omnia`. `documentation` says expiry is 30 minutes; `typescript` observed 24 hours.
