...
The Evidence YAMLs under `inputs/evidence/` validate against [`schemas/evidence.schema.json`](https://github.com/augentic/specify/blob/main/schemas/evidence.schema.json). The `expected/specs/<domain>/spec.md` requirement blocks validate against the provenance parser at [`crates/artifacts/src/spec/provenance.rs`](https://github.com/augentic/specify/blob/main/crates/artifacts/src/spec/provenance.rs).
...
The five fixtures together cover workflow §Eval scenarios `#5`, `#5a`, `#5b`, `#5c`, `#5h` (target `shape` injection happens in every fixture since all five target `omnia`).
