...
- `RegisterUser` operation depends on `TableStore` (write) for the identity-store table.

## APIs and integrations

- HTTP `POST /users` — registration operation; request body `{ email, password }`, response `201` with the persisted user or `400` with `{ "error": "invalid-email" }`.
- Internal call: the operation delegates persistence to a thin repository wrapper around `TableStore::put`, mirroring the `insertUser` shape from `legacy-monolith` `src/users/register.ts#L31`.
...
- `RegisterUser` implements `Operation<P>` with `RegisterUserRequest` as its typed input; `Operation::call` constructs an `EmailAddress` (which performs the RFC-5322 check), then delegates the `TableStore` write and returns the persisted record.
...
- Structural validation in typed decoding and `Operation::call`: required `email` and `password` fields; RFC-5322 check on `email` via `EmailAddress::try_new`.
