...
4. `specify slice build password-hash-rotate --phase prepare --format json` assembles + schema-validates the request and prints the handoff envelope; the body reads `adapters/targets/omnia/prose/briefs/build.md` from its `build-brief` field.
5. The brief's verify-repair loop converges: `cargo fmt --check`, `cargo check`, `cargo clippy -- -D warnings`, `cargo test` all pass; `adapters/targets/omnia/prose/briefs/build.md` § Code reviewer emits `REVIEW.md` with no critical / high findings outstanding. The brief writes `build/report.yaml` with `status: success`.
