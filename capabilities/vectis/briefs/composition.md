...
This brief discovers `layout.yaml` first from the active slice and then from the project design-system directory, falling back to existing `composition.yaml` inputs only when no layout is present (RFC-11 §H). It calls `specify-vectis validate layout` (or `validate composition` when the resolved input is already wired) on the input before consuming it, and `specify-vectis validate composition` on its output for cross-artifact token / asset checks.
