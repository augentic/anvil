...
// plan, C07).
//
// Every Specify lifecycle mutation goes through `specify` per the
// CLI-Authoritative Invariant in `acceptance/README.md`. This file is
// the single place subprocess flags / env propagation lives so the
// helpers in `hub.ts`, `projects.ts`, `workspace-sync.ts`, and the
// smoke driver all behave identically.
...
export class SpecifyCommandError extends Error {
  constructor(public readonly run: SpecifyRun, bin: string) {
    super(
      `${bin} ${run.args.join(" ")} failed in ${run.cwd} (exit ${run.exitCode})\n` +
...
async function captureSpecify(
  ...
  const header = `\n$ ${bin} ${run.args.join(" ")}  (cwd=${run.cwd}, exit=${run.exitCode})\n`;
