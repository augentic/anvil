...
//
// Step 8 (the four `setup-*` invariant assertions) is layered on top of
// the result by `acceptance/assertions/setup.ts`. `setupHub` itself is
// intentionally side-effect-only on disk + CLI; it does NOT decide
// pass/fail.
...
export async function setupHub(opts: SetupHubOptions): Promise<SetupHubResult> {
  ...
  for (const p of opts.projects) {
    ...
    if (p.orgSlug) {
      (result as { githubUrl: string }).githubUrl = defaultGithubUrl(p.name, p.orgSlug);
    }
