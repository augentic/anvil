...
export const DEFAULT_PR_NUMBERS: Readonly<Record<string, number>> = Object.freeze({
  "shop-backend": 41,
  "shop-mobile": 18,
});
...
export async function markPrMerged(opts: {
  ...
  const before = parsePrFile(path, (await Deno.readTextFile(path)).replace(/\n+$/, ""));
  const next: PrState = {
    ...before,
    state: "MERGED",
    merged: true,
...
export function formatPrFile(state: Omit<PrState, "repoKey" | "sourcePath">): string {
...
function renderFakeGh(prNumbers: Record<string, number>): string {
  ...
  return `#!/bin/sh
# Fake gh installed by acceptance/runner/fake-gh.ts.
...
# Fake SSH installed by acceptance/runner/fake-gh.ts.
