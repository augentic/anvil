// Cross-repo VECTIS-BUILD smoke driver (RM-01 plan, C14b).
//
// Wraps `acceptance/runner/main.ts --suite rm01-cross-repo
// --backend vectis-build --allow-backend-mismatch` with the same
// environment-skip policy the C09–C14a smokes use, plus the
// contracts WASI tool probe (the vectis-build backend keeps the
...
//
// The C14b backend is execute-only by design (boundary documented
// in `backends/vectis-build.ts`): it drives the contract slice with
// `ContractsBuildPhaseDriver` (real OpenAPI / JSON Schema emission),
// the mobile slice with `VectisBuildPhaseDriver` (real Vectis
// composition + SwiftUI shell emission), and the backend slice with
// `StubPhaseDriver` (C14a's `OmniaBuildPhaseDriver` is independent;
// a future "real builds for everything" backend composes both by
// chaining capability checks). Push / finalize coverage stays on
// `scripted-finalize` / `agent`.
...
async function main(): Promise<number> {
  ...
  if (!bin) {
    console.log(
      "[skip] acceptance-cross-repo-vectis-build-smoke: no `specify` binary found on PATH (and SPECIFY_BIN unset).",
    );
    console.log(
  ...
  console.log(
    `[c14b] specify: ${bin.path} (${bin.version ?? "unknown version"})`,
  );

  if (!(await supportsHubSurface(bin.path))) {
    console.log(
      `[c14b skip] acceptance-cross-repo-vectis-build-smoke: ${bin.path} does not expose ` +
  ...
  if (!(await supportsContractTool(bin.path))) {
    console.log(
      `[c14b skip] acceptance-cross-repo-vectis-build-smoke: ${bin.path} does not expose ` +
        `\`specify tool run contract\` (pre-RFC-15 release; the vectis-build ` +
  ...
  const cmd = new Deno.Command(Deno.execPath(), {
    args: [
      ...
      // vectis-build backend is a strict superset (plan creation +
      // execute loop + per-slice contracts/vectis phase drivers);
