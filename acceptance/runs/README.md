...
Filled run-summaries for manual `lifecycle` scenario runs. Run records are the audit trail; they are kept **separate from the scenario catalog** so the scenarios stay pristine fixtures and the catalog (`acceptance/lifecycle/README.md`) tracks only status.
...
3. Capture the **Artefact snapshot** with `scripts/acceptance-snapshot.sh "$SANDBOX"` so the record stands alone; deeper read-only inspection verbs are in [`acceptance/shared/inspect.md`](../shared/inspect.md).
4. Set the **Verdict** to `pass` / `fail` / `deferred`.
5. Update the scenario's status in the [catalog](../lifecycle/README.md).
