...
> **Post-run reconciliation (scenario rescoped).** After this run, `01-pure-intent` was rescoped to `stages: [plan, refine]` and `execute-loop-all-done` was replaced by `refine-reaches-refined`, moving generated-output correctness out of the Wave-0 hard halt (per [RFC-40 Phase 2](../../rfcs/rfc-40-acceptance-capability.md#phase-2--the-generated-output-gate-capability-2) / [RFC-39](../../rfcs/future/rfc-39-acceptance-shape-traces.md)). Every now-in-scope assertion in this run is `pass`, so the catalog status for `01-pure-intent` is `passed`. This run record is preserved verbatim above as the 2026-06-05 audit trail; the `needs-human` row reflects the old, since-removed assertion.
