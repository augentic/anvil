...
Operator: copy the field-set from [`operator/run-summary-template.md`](../operator/run-summary-template.md) into this file, fill every section against the live run, and update the **Status:** line above to `passed` / `failed` / `deferred`. Halt all subsequent scenarios on `failed` per the queue rule in [`README.md`](README.md).
