...
These documents are manual scenarios for `/spec:plan` from an operator's point of view. They exercise plan authoring only:
...
These are shared plan-generation scenario documents. They live under `acceptance/suites/plan-authoring/` because `/spec:plan` is orchestration: it authors a change-level plan that coordinates slices rather than owning one adapter's slice loop.

End-to-end cross-repo acceptance remains in [`acceptance/suites/cross-repo-workflow/`](../cross-repo-workflow/). Adapter-local tests, such as the contracts scenarios, stay under `adapters/<adapter>/tests/`.
...
4. Run the `/spec:plan` prompt from **Invocation** exactly as written unless the scenario documents an allowed local substitution.
5. Run the listed validation and inspection commands.
6. Check each item in **Assertions** and **Negative Expectations**.
7. Preserve failure evidence: `plan.yaml`, `.specify/plans/<change-name>/`, command output, registry state when relevant, and `inspect plan.yaml` output.
...
```text
Run all plan-generation test scenarios in acceptance/suites/plan-authoring/ in this order:
1. single-project/scenario.md
2. contract-routing/scenario.md
...
- Run only the listed /spec:plan prompt and validation commands.
- Do not run /spec:execute, workspace push, finalize, a scenario runner, or a
