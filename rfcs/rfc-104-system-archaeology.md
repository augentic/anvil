...
>
> Owns: the durable definition home; the `system *` command-root mount (`--dir` or CWD); the declared system boundary and coverage record; source-location binding and observed-tree provenance; mixed-file surgical writes and definition decision records; the source WIT change that passes source key and prepared input into `survey` / `extract` (not parent-lead `focus`); adapter-native surface grain for survey leads; system-wide Evidence extraction and correlation; the reviewed as-is architecture model; architecture-document and diagram projections; modernization dispositions; target and transition architecture; the migration plan; the immutable wave handoff; and its explicit review fact before RFC-88 import. RFC-104 does not pin delivery CIDs into the handoff.
>
> Builds on [RFC-86](rfc-86-change-facts.md)'s per-writer event and lossless-union shape. It changes the source WIT so `survey` / `extract` take an explicit source key and prepared input rather than recovering location from `plan.yaml`; [RFC-88](rfc-88-detached-changes.md) later extends `survey` with parent-lead focus over the delivery pin. It adds `system.wave.reviewed` to the closed event taxonomy for definition-home writers without making that fact a delivery authorization epoch. RFC-88 consumes one reviewed migration wave and owns delivery decomposition, execution, source and target CID pinning, and accepted target CIDs. [RFC-94](rfc-94-target-readiness.md) assesses whether selected delivery targets can execute that wave. [RFC-98](rfc-98-behavioural-conservation.md) verifies preserved behaviour against protected captures.
>
> Patch ownership: this RFC does not revise in-flight RFC-88. Successor consumption patches — surface grain, WIT `focus` over the delivery pin, the observed-vs-delivery CID fence, and declared adapter identity on the handoff — live in [rfc-104-rfc-88-patches.md](rfc-104-rfc-88-patches.md) for the next RFC-88 cut. Field-patch `plan amend` retirement is recommended there, not decided here. Implemented RFC-86 / RFC-86a / RFC-91 texts remain unchanged, including RFC-88's `plan amend --proposal`.
>
