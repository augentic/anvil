...
> Status: Draft · Serves: the eval catalog's [`workspace-execute-two-projects`](../evals/scenarios/workspace-execute-two-projects.md) release blocker · Complements: [RFC-44](rfc-44-architecture-seams.md) R2 (CLI-owned control flow; the `--plan-dir` seam as worked precedent)
> Provenance: 2026-06-11, revised the same day. The first draft of this RFC ratified a resolve-time plan-root fallback written mid-run; review flipped the decision to sync-time provisioning. The interim fallback was implemented and then removed in specify-cli `acceptance` (added `0e55a633`, removed `204e3867`) — the loader contract was never amended.
...
**R2 — prove it at both seams.** A sync-level test asserting the mirror lands (workspace adapter visible in the slot cache after sync, refreshed on re-sync), plus one end-to-end slot-extract integration test beside `prepare_resolves_via_plan_dir` in `tests/source/extract.rs`: adapter vendored only at the workspace, slot extract succeeds through ordinary cache resolution after sync. No new resolution semantics to pin — the string-discriminant fragility from F2 no longer exists.
...
**R4 — close the live loop.** Resume the parked `workspace-execute-two-projects` sandbox; until R1 lands, the documented interim is the manual cache-stage (the vectis trick, recorded as a run deviation). The run record cites the slot-resolved adapter root as live evidence; on a green run, flip the catalog row and the RM-05 rollup.
...
| 5 | R1 sync mirror + R2 tests + R3 doc lines | specify-cli (+ specify docs) | next |
...
1. **Mirror GC.** Should sync prune mirrored adapters that have disappeared from the workspace (delete-then-copy per axis), or leave orphans to age out? Delete-then-copy is the cleaner default and matches "re-sync refreshes".
2. **Conflict posture.** If a slot project vendors its own adapter under the same name, the vendored tree should win over the mirrored cache copy (or the cache copy should be skipped) — confirm the loader's probe order already yields that and pin it in the R2 sync test.
