...
RFC-55 lists "a host-service capability in `augentic/omnia`" as a hard dependency and names the extension point an `eval` backend plugs into without specifying it. This RFC is that specification, and it earns its own document for three reasons:
...
- **It is the seam two Specify RFCs lean on.** RFC-55 needs *a* model host to bind `eval` to; RFC-56's fleet backends need a *contract* to register into. Specifying it once, here, keeps both honest and lets the two repos version against a named boundary.
...
- **The concrete backends and the fleet router.** The frontier-API and spawned-agent backends and the difficulty/cost router are [RFC-56](rfc-56-eval-fleet.md); the replay backend is RFC-52; the SLM backend is [RFC-18](future/rfc-18-slm.md). Omnia provides the slot, never the fleet.
...
| The **model-service backend**: fleet router + frontier-API + spawned-agent      | **Specify** — [RFC-56](rfc-56-eval-fleet.md)  |
| The **SLM backend**                                                             | **Specify** — [RFC-18](future/rfc-18-slm.md)   |
| Binding `eval` during the runtime move (Specify-side adoption)                 | **Specify** — [RFC-55](rfc-55-runtime-move.md) |
...
5. The fleet (RFC-56) plugs in as **exactly one** backend; Omnia binds one backend per host — no second `eval` binding.
6. [RFC-55](rfc-55-runtime-move.md) can bind `eval` to this host, closing its cross-repo dependency; the `augentic/omnia` workspace gate stays green, and this RFC's prose passes `make lint` here.
...
- **Reentrancy via the resolver.** The inbound `resolve` leg must only ever re-enter on a fresh instance (RFC-55), never the operation instance suspended in `eval`.
