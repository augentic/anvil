...
That separation is also the answer to **self-evaluation bias**: an agent that implemented a change is worse at judging that change than a fresh pass with a different incentive. `build` and `repair` produce candidates; `verify` and `review` evaluate them; a returned `repair` report never selects the next operation; and the engine—not the implementer—assembles terminal success. Comparable products reach the same incentive split by spawning separate validator agents; Emery reaches it with fresh phase dispatches under a deterministic machine ([platform.md § Absorbed lessons](platform.md#lessons-absorbed-from-comparable-systems)).
...
There is no `build-scope` variant: every operation names a slice except `verify`, which receives only the candidate workspace. Domain identity for RFC-96 convergence stays engine-side; the adapter runs the same check pass against whatever workspace the engine lends.
...
Separating `verify` and `review` keeps mechanical checks and standards judgment as different product surfaces: different prompts, budgets, and (later) execution trust. RFC-96 reuses the same `verify` against a composed candidate workspace; domain id never crosses this seam.
...
A model-assisted verification pass may execute tests stored in the candidate workspace, including tests authored or edited by the same build. A green result therefore means **the candidate passes its own reported checks**, not that it passed an independent or protected oracle. The report source and operator output must preserve that assurance boundary. RFC-96 may protect independently governed verification inputs through its write-ownership envelope; RFC-97 defines host-owned profile and protected-oracle evidence.
...
Root-cause suppression is producer-specific and does not belong in the neutral engine projection. A verifier may suppress cascades before returning its complete report only when its deterministic normalization contract defines that relationship. RFC-97 owns that contract for host-tool profiles.
...
The phase event is also the join anchor for model-backend telemetry when the deployment exposes it: effective route and model identity, input and output tokens, and reported cost. Missing usage data remains missing rather than estimated by the engine. These observations stay outside report digests and lifecycle projection; RFC-96's harness comparison derives cost per accepted result and worker spend induced by decomposition instead of optimizing per-call price.
...
12. A blind live-evaluation acceptance set is unavailable to decomposition, build, repair, verify, and review, and its post-attempt grade remains outside build reports, records, and lifecycle projection. Documentation describes the workflow gate as model-assisted, calls candidate-owned checks self-consistency evidence rather than an independent oracle, and identifies RFC-97 as the deterministic native-verification follow-on.



## Rejected alternatives

- **Wait for RFC-97 / native host verification before exposing the loop** — leaves retry authority hidden in prose and blocks RFC-96's observable worker orchestration on the native-verification follow-on. (RFC-97 itself does not wait on `wasi:exec`; rejecting the wait is about shipping the engine-owned phase machine first.)
