...
6. **RFC-20 detector registry generalisation.** Refactor `DetectorRegistry` from a single global registry to a `BTreeMap<ProfileName, Vec<Box<dyn Detector>>>`. `specify change survey` consults the resolved profile's `detectors:` list at dispatch time. Empty list = run every detector (preserves RFC-20 v1 behaviour for projects without a profile binding). New error discriminant: `profile-unknown`.
