...
Invocation form: `specify <verb …>`. A provisioning verb runs natively; any other argv forwards unparsed to the host layer **mounted in-process** — the macro-generated command-mode runtime driven over the generated manifest with the forwarded argv, the binary resolving the manifest path itself (it generated it — operators never type `--config`). *(Amended by operator decision: the RFC as proposed spawned the host layer as a sibling `specify-host run --config … -- <verb …>` binary; YAGNI — no subprocess, no second binary. `omnia::runtime!` generates a public sync `drive` beside `main` for exactly this mount; see DECISIONS.md §"In-process host mount". There is no bare host form to ship — debugging and Omnia-native deployments use the replay sibling or omnia's own CLI over the same manifest.)*
...
- **Dev/prod divergence on the path override.** Release binaries always run their release core (the published pin or the embed); the path override exists for the development loop only and never ships enabled — graded evidence (suites, evals) comes from release-mode builds, mirroring the prose-overlay posture of the adapter dev loop (RFC-62 — landed; removed from the tree).
