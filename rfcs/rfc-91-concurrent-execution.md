...
### Composition waves and shared paths

One convergence wave accepts only results derived from the same base snapshot. Their touched-path sets must be pairwise disjoint; the composer copies each touched path's exact value from its result tree into a fresh integration workspace, then captures the next immutable snapshot. Fixed dependency order makes the output byte-stable even though disjoint results commute.

A path needed by several otherwise independent tasks is a fan-in boundary, not a text-merge opportunity. The partitioner keeps that path out of the parallel manifests and creates a later integration task with exclusive ownership. That worker starts from the composed snapshot and updates the shared module once. If no safe fan-in exists, the tasks serialize. Undeclared overlap, base mismatch, and implicit text merge are composition failures.

Manifests are predictions; RFC-87 captured touched paths are authoritative. If an undeclared overlap appears only after workers finish, the gate rejects the whole wave without partial composition and routes an ownership finding to every worker that changed the path. Those workers retain their immutable results, recapture replacements that omit the shared path, and the gate composes the repaired disjoint wave. A later fan-in task owns the shared path and integrates once from that composed snapshot.
...
| D3 | **Write-ownership partitioning.** Every worker carries an explicit manifest; predicted overlaps become an explicit dependency or fan-in integration task before dispatch. Captured touched paths are authoritative: an undeclared overlap rejects the wave and enters the same repair shape; out-of-manifest writes are blocking findings. | The invariant that makes Stage B — and [RFC-92](rfc-92-node-sync.md) D10's plan-level manifests — safe without relying on textual merge luck. |
...
| D6 | **Code-patch composition is a reusable deterministic kernel.** Given one base snapshot id and an ordered list of RFC-87 code patches that all name that base, the kernel requires disjoint touched paths, copies their exact result-tree values into a fresh integration workspace, captures the next snapshot, and refuses base mismatch or overlap before verify. Worker dependencies and fan-in tasks form later waves; RFC-92 projects plan dependencies into per-project wave lists for the same kernel. | RFC-87 remains a prepare/capture step, RFC-91 completely owns composition mechanics, and later scheduling cannot redefine merge semantics. |
...
| D9 | **Parallel survey / extract fan-outs** (over Stage B): `survey_all` dispatches all bound sources concurrently and merges the lead inventory in binding order; refine's extract fan-out likewise, with per-source evidence files as the natural disjoint write set. The inventory is `discovery.md` until RFC-88 renames it `leads.md`. This is Author-slices survey after [RFC-88](rfc-88-detached-changes.md) D3 pins topology — not Discover-topology host reads, which stay under RFC-88 D9's concurrency budget. | Concurrency is a dispatch property, never an output property: merged outputs stay byte-identical to the serial order. Plan-time surveys are the first consumer. |
...
8. With Stage B available, `plan author`'s Author-slices phase over N sources (after RFC-88 discovery has pinned topology) dispatches surveys concurrently and the lead inventory (`discovery.md`, then RFC-88's `leads.md`) is byte-identical to the serial run's.
9. Three disjoint workers that require one shared module converge as one parallel wave plus a later exclusive integration task; no worker in the parallel wave owns the shared path, and no textual auto-merge occurs.
10. When three workers unexpectedly modify the same shared module, their first wave is retained but not composed; each recaptures a result omitting that path, the repaired wave composes, and one fan-in task integrates the shared module.
11. `cargo make ci` green in touched repos; goldens regenerated with D8; live eval (`omnia-r9k` / `orders-contracts`) shows quality parity after each of D7 and D8, evaluated separately.
...
- A shared path is removed from parallel manifests and assigned to one generated fan-in integration task in the next wave; ambiguous ownership fails planning.
- Before applying any result, the gate compares the complete captured touched-path sets. Runtime overlap rejects the wave atomically and issues recapture findings; no conflict-free subset becomes visible early.
