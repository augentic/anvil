...
Only then does the host run the selected verify profiles. If a test failure points to `crates/payments/src/client.rs`, the finding returns to the crate writer. The worker resumes its existing session, edits its private workspace, and returns a replacement patch. No cargo command appears in any worker prompt.
...
### D1 — The Omnia adapter core is the deterministic orchestrator

Compiled guest code performs partition → dispatch → converge → fold and produces the existing `BuildReport`. It decides sequencing and arbitration; workers supply judgment. There is no lead agent deciding which other agents to run.

Reusable brief, manifest, and outcome helpers live in `augentic/emery`'s adapter SDK. The Omnia implementation lives in `augentic/emery-adapters/targets/omnia`. The WIT seam does not change, and this RFC does not require Vectis or Contracts to replace their serial build behavior.
...
Verify remains RFC-90's security boundary. More verify calls must not widen accepted commands or execution authority.
...
- **Run cargo commands inside worker prompts** — restores unobservable, unbounded verify behavior. Closed host profiles are the only verification path.
...
- **Change the WIT target interface or `BuildReport`** — the adapter core can orchestrate focused requests behind the existing one-build-per-slice seam.
