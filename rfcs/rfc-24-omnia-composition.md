...
3. **Capability-discriminated `specify plan validate` findings for Omnia-typed projects** — handler-identifier uniqueness within a service, publisher uniqueness per topic, and a closed-enum check that every declared surface kind is one Omnia can host.
...
1. **Service identity is the project, full stop.** An Omnia service is a `project` in `registry.yaml` whose `capability` resolves to `omnia` (any version). This RFC adds no new field for "service".
...
- **Capability-gated.** Validation fires only when the slice's resolved project capability is Omnia. For non-Omnia projects the field is allowed (forward compatibility) but produces no findings.
...
### Capability-gated `specify plan validate` findings

Five new findings, all gated on the slice's resolved project capability being Omnia:
...
1. If `registry.yaml` exists and has a `projects[]` entry matching the slice's `project`, the project's `capability` field decides. Match is by prefix: `omnia`, `omnia@1`, `omnia@2`, etc. all gate the Omnia validator on.
2. If the slice has no `project` (capability-only slice — see `planSlice.capability` in the existing schema), the slice's `capability` field decides.
...
3. **Capability resolution.** Add `Plan::resolve_capability(&PlanSlice, &Registry) -> Option<CapabilityRef>` in `specify-domain`. Mirror the resolution rule in §Capability resolution; cover registry-present, registry-absent, and capability-on-slice paths.
4. **Validator findings.** Extend `Plan::validate` with the five capability-gated findings. New discriminants in `specify-error`: `plan-omnia-crate-name-collision`, `plan-omnia-crate-name-illegal`, `plan-omnia-surface-kind-unsupported`, `plan-omnia-handler-conflict`, `plan-omnia-publisher-conflict`.
...
7. **`specify plan compose`.** New read verb under `src/commands/plan/compose.rs`. Walks `plan.yaml` slices, resolves capability per slice, fills `surfaces[]` from the plan field or falls back to `<plan-dir>/analyze/<source-key>/surfaces.json`. JSON envelope sibling of `plan-validate-output`.
8. **Propose brief integration.** Update `plugins/change/skills/draft/briefs/propose.md` to pre-fill `crate` (defaulting to kebab→snake of the leaf's accepted slice name) and `surfaces` (intersected with the Omnia enum) when the leaf's `target_project` resolves to an Omnia project. Capability authors who own non-Omnia briefs do nothing.
...
**For capability authors.** Only the `omnia` capability's propose brief changes — to populate `crate` and `surfaces`. Vectis, contracts, and any third-party capability are unaffected. The new validator findings are guarded by the `omnia` capability check, so non-Omnia projects see no behavioural change.
...
There is **no breaking change** to: existing `plan.yaml` files (both new fields are optional), existing `registry.yaml` files (untouched), existing surveys (`surfaces.json` schema unchanged), existing exit codes (new discriminants within `EXIT_VALIDATION_FAILED=2`), or any non-Omnia capability brief.

## Alternatives Considered

**Add an Omnia-shaped sub-document under each slice (`omnia: { crate: …, surfaces: […] }`).** Rejected. A capability-scoped namespace inside `planSlice` would invite Vectis, contracts, and every future capability to claim their own sub-document, fragmenting the schema and forcing skill authors to learn a per-capability vocabulary inside the same file. Top-level optional fields with capability-gated validation cost less and compose better; the validator already needs to know the project's capability for routing-hint precedence anyway.
...
**Generalise the validator findings beyond Omnia (e.g. cross-capability handler-conflict).** Rejected for v1. Vectis's "service" is a shared-core Crux app with platform shells, not a hosted-handler workspace; contracts targets have no hosted handlers; neither would benefit from the Omnia uniqueness checks. The capability-gated validator keeps this RFC tightly scoped; analogous companion RFCs for Vectis or contracts can mirror the structure if and when those capabilities grow comparable composition concerns.
...
**Make `crate` required when the project capability is Omnia.** Rejected for v1. The default derivation handles the vast majority of slices; requiring it would force every Omnia plan written before this RFC to amend before it re-validates, which violates the additivity principle.
...
- Vectis or contracts composition. Those capabilities have different deployable shapes; if either grows comparable plan-time concerns, a companion RFC mirroring this one is the right move.
...
1. Should `crate` and `surfaces` move under a single optional `omnia` sub-document anyway, on the theory that the per-capability namespace becomes self-explanatory once two or three capabilities populate it? Current preference: no — keep them top-level optional, let the validator's capability gate carry the discrimination, and revisit if a third capability materially benefits from a similar lift.
...
7. Should `omnia-crate-writer` reject when the slice's resolved capability is *not* Omnia (e.g. the operator pointed the writer at a Vectis slice by mistake)? Current preference: yes — fail-closed with a clear discriminant, since the writer's output is wasm32-target-specific and cannot meaningfully apply to a non-Omnia project.
...
- [`schemas/capability.schema.json`](https://github.com/augentic/specify-cli/blob/main/schemas/capability.schema.json) — capability manifest format the validator consults for the Omnia gate.
- [`crates/domain/src/registry/catalog.rs`](https://github.com/augentic/specify-cli/blob/main/crates/domain/src/registry/catalog.rs) — `Registry::projects[].capability` source the capability resolution reads.
