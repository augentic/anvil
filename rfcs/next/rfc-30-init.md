...
- `plan.yaml` `slices[].target` → `outputs[]` once RFC-29 lands (cross-RFC coordination via the same `MigrationPlan` shape).
...
- Defining `specify check` or `specify review`. RFC-5 and RFC-28 own those surfaces.
...
- [RFC-29: Fan-In/Fan-Out](rfc-29-fan-in-fan-out.md) — multi-output plan entries the `V1ToV2` migrator must produce.
