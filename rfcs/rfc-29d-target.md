...
documentation + code-typescript
        ...
        -> plan propose --dry-run           (kernel returns a flat lead catalog + locked groups)
        -> plan propose --from              (envelope: agent-led cross-source matching — locked
                                             + semantic — and per-scope target binding;
                                             kernel: validate, partition/locked-group invariants, scope-id
...
- `specrun source survey` produces schema-valid leads for both sources, including a deliberate semantic-only pair (`docs` lead `password-reset` and `legacy` lead `reset-password`) that shares no id or alias, exercising the agent's semantic grouping of open leads.
- `specrun plan propose --dry-run --format json` returns a `kind: request` envelope whose `leads[]` carries a `group: { group-id: identity-api, match-basis: exact-id }` block on each of the two shared `identity-api` rows (and no `group` on the open `password-reset` / `reset-password` rows), and whose `projects[]` surfaces the two registry projects (`identity-contracts` → `contracts@v1`, `identity-service` → `omnia@v1`); it validates against `proposal.schema.json` and writes nothing.
- The fixture's `/spec:plan` agent step (or the test harness simulating it) returns a `kind: response` whose `scopes[]` keeps the `identity-api` locked group intact and **semantically merges** `password-reset` with `reset-password` (`match-basis: semantic`) into the same scope, whose `slices[]` binds that scope to projects with explicit `name` values (`identity-contracts`, `identity-service`) and **bound registry projects** (`identity-contracts`, `identity-service`); `specrun plan propose --from` writes two single-target slices each carrying its kernel-derived `target` and bound `project`, with `identity-service.depends-on: [identity-contracts]`, emits `plan.reconcile.agent` + `plan.reconcile.completed`, and the semantic merge is rendered into `change.md` for Gate-1 review.
- `specrun plan propose --from` rejects a response that splits the `identity-api` locked group (`plan-reconcile-locked-group-split`), one that leaves a surveyed lead unaccounted for or double-counts one (`plan-reconcile-partition`), one that cites a `(source-key, lead-id)` pair absent from the **recomputed** catalog (`plan-reconcile-lead-orphan`), one whose slice cites no declared scope (`plan-reconcile-scope-orphan`), one whose scope has no slice row (`plan-reconcile-scope-unbound`), one with duplicate `(scope-id, project)` rows (`plan-reconcile-slice-duplicate`), one whose derived slice names collide (`plan-reconcile-slice-name-collision`), one whose `depends-on` graph cycles (`plan-reconcile-depends-on-cycle`), one that omits `project` on a slice when more than one project is offered (`plan-reconcile-project-binding-required`), and one that binds a slice to a `project` absent from the request's `projects[]` (`plan-reconcile-project-orphan`); `specrun plan propose` with neither `--dry-run` nor `--from` exits non-zero with `plan-propose-missing-grouping`.
