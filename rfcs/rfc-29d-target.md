...
documentation + code-typescript
        ...
        -> plan propose --dry-run           (kernel returns locked + advisory clusters)
        -> plan propose --from              (envelope: agent-led cross-source matching — locked
                                             + semantic — and per-concept target binding;
                                             kernel: validate, partition/locked-cluster invariants, concept-id
...
- `specrun source survey` produces schema-valid leads for both sources, including a deliberate semantic-only pair (`docs` lead `password-reset` and `legacy` lead `reset-password`) that shares no id or alias but does share identity facets for advisory cluster generation.
- `specrun plan propose --dry-run --format json` returns a `kind: request` envelope whose `clusters[]` contains a locked cluster for the shared `identity-api` lead (`cluster-binding.basis: exact-id` on each lead) and an advisory cluster for the `password-reset` / `reset-password` pair with shared identity facets under `clusters[].shared-facets[]`, and whose `projects[]` surfaces the two registry projects (`identity-contracts` → `contracts@v1`, `identity-service` → `omnia@v1`); it validates against `proposal.schema.json` and writes nothing.
- The fixture's `/spec:plan` agent step (or the test harness simulating it) returns a `kind: response` whose `concepts[]` preserves the `identity-api` locked cluster and **semantically merges** `password-reset` with `reset-password` (`match-basis: semantic`) into the same concept, whose `slices[]` binds that concept to targets with explicit `name` values (`identity-contracts`, `identity-service`), and whose slices are **bound to registry projects** (`identity-contracts` → `identity-contracts`, `identity-service` → `identity-service`); `specrun plan propose --from` writes two single-target slices each carrying its bound `project`, with `identity-service.depends-on: [identity-contracts]`, emits `plan.reconcile.agent` + `plan.reconcile.completed`, and the semantic merge is rendered into `change.md` for Gate-1 review.
- `specrun plan propose --from` rejects a response that splits the `identity-api` locked cluster (`plan-reconcile-required-group-split`), one that leaves a surveyed surfacing unaccounted for or double-counts one (`plan-reconcile-partition`), one that cites a `(source, lead)` pair absent from the request catalog (`plan-reconcile-lead-orphan`), one whose slice cites no declared concept (`plan-reconcile-concept-orphan`), one whose concept has no slice row (`plan-reconcile-concept-unbound`), one that omits `project` on a workspace slice (`plan-reconcile-project-binding-required`), one that binds a slice to a `project` absent from `registry.yaml` (`plan-reconcile-project-orphan`), and one that binds the `contracts@v1` slice to the `omnia@v1` project (`plan-reconcile-project-target-mismatch`); `specrun plan propose` with neither `--dry-run` nor `--from` exits non-zero with `plan-propose-missing-grouping`.
