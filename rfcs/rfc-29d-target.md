...
A slice builds against the **merged working tree**, not against a cross-slice data channel. When a slice depends on another slice's output — e.g. `identity-service` (omnia) consuming `identity-contracts`' generated schema — the dependency is declared at the plan layer via `plan.yaml.slices[].depends-on`, and `specrun plan next` orders execution so the depended-on slice reaches `done` (merged into the baseline) before the dependent slice starts. The dependent target then reads the upstream output from the working tree as ordinary files, the way its build tooling already resolves dependencies — this is the **same-tree** path, where merging the upstream slice into the baseline lands its output in the tree the dependent slice builds against. No per-request cross-slice channel is introduced. When the two slices instead bind *different working trees* (separate-repo workspace mode), the upstream output is consumed as a published dependency, not from a shared tree; a Specify-native channel for that case is the deferred open question — see [RFC-29 §"Open questions"](rfc-29-fan-in-fan-out.md#open-questions) Q1.
...
The fixture exercises the **same-tree** topology: both registry projects resolve into one working tree (their `registry.yaml` URLs are `.` / repo-relative paths, materialised as symlinks per [Registry §"Files and state"](../docs/reference/registry.md#files-and-state)), so they share a single `.specify/slices/` tree and one baseline. Cross-slice handoff therefore lands in that shared tree when the upstream slice merges. The separate-remote-repo case — where each project is its own clone on its own branch — is deliberately **not** on the D7 path; it consumes the upstream as an ordinary published dependency ([RFC-29 §"Open questions"](rfc-29-fan-in-fan-out.md#open-questions) Q1), and a Specify-native channel for it is deferred.
...
- `specrun plan propose --dry-run --format json` returns a `kind: request` envelope whose `leads[]` carries one row per `(source, lead)` (including two rows that share slug `identity-api` and two rows with different slugs for password reset), and whose `projects[]` surfaces the two registry projects (`identity-contracts` → `contracts@v1`, `identity-service` → `omnia@v1`, both resolving into the one working tree per the topology note above); it validates against `proposal.schema.json` and writes nothing.
- The fixture's `/spec:plan` agent step (or the test harness simulating it) returns a `kind: response` whose `slices[]` matches `docs:identity-api` with `legacy:identity-api` by shared slug and fans them out into two slices with explicit `name` values `identity-contracts` and `identity-service` and **bound registry projects** `identity-contracts`, `identity-service` (both carrying identical `sources[]`, joined by `depends-on`), and matches `password-reset` with `reset-password` by synopsis judgment into a third slice named `password-reset`; `specrun plan propose --from` writes the single-target slices each carrying its bound `project` (the target is resolved on demand from that project, not written to `plan.yaml`), with `identity-service.depends-on: [identity-contracts]`, emits `plan.reconcile.completed`, and the cross-source matches are rendered into `change.md` for Gate-1 review.
- `specrun plan propose --from` rejects a response that leaves a surveyed lead unaccounted for, failing total lead coverage (`plan-reconcile-partition`) — a lead referenced by more than one slice is legal fan-out, not a failure; one that cites a `(source, lead)` pair absent from the **recomputed** catalog (`plan-reconcile-lead-orphan`); one whose `sources[]` names two leads from the same source (`plan-reconcile-slice-source-collision`); one whose agent-supplied explicit `name` values collide (`plan-reconcile-slice-name-collision`, the sole duplicate gate now that `scope` is removed); one whose `depends-on` graph cycles (`plan-reconcile-depends-on-cycle`); one that omits `project` on a slice when more than one project is offered (`plan-reconcile-project-binding-required`); one that binds a slice to a `project` absent from the request's `projects[]` (`plan-reconcile-project-orphan`); and one that would replace an approved or partially executed plan (`plan-reconcile-plan-not-replaceable`); `specrun plan propose` with neither `--dry-run` nor `--from` exits non-zero with `plan-propose-mode-required`.
...
- Each slice builds independently against its single bound target. Because both projects share one working tree, `identity-service`'s `cargo check` resolves `identity-contracts`' output as an in-tree path dependency: `depends-on` + `plan next` order `identity-contracts` to merge into the baseline first, the merge lands its generated files in the shared tree, and `identity-service` then reads them as ordinary files — no cross-slice build-request channel and no out-of-band published dependency. (For the separate-repo topology the dependency would instead be a published artifact; see the topology note above and Q1.)
