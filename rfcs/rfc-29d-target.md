...
documentation + code-typescript
        ...
        -> plan propose --from              (envelope: agent-led cross-source matching — locked
                                             ...
                                             kernel: validate, partition/locked-group invariants, concept-id
...
- The fixture's `/spec:plan` agent step (or the test harness simulating it) returns a `kind: response` whose `concepts[]` keeps the `identity-api` locked group intact and **semantically merges** `password-reset` with `reset-password` (`match-basis: semantic`) into the same concept, whose `slices[]` binds that concept to targets with explicit `name` values (`identity-contracts`, `identity-service`), and whose slices are **bound to registry projects** (`identity-contracts` → `identity-contracts`, `identity-service` → `identity-service`); `specrun plan propose --from` writes two single-target slices each carrying its bound `project`, with `identity-service.depends-on: [identity-contracts]`, emits `plan.reconcile.agent` + `plan.reconcile.completed`, and the semantic merge is rendered into `change.md` for Gate-1 review.
- `specrun plan propose --from` rejects a response that splits the `identity-api` locked group (`plan-reconcile-locked-group-split`), one that leaves a surveyed lead unaccounted for or double-counts one (`plan-reconcile-partition`), one that cites a `(source-key, lead-id)` pair absent from the request catalog (`plan-reconcile-lead-orphan`), one whose slice cites no declared concept (`plan-reconcile-concept-orphan`), one whose concept has no slice row (`plan-reconcile-concept-unbound`), one that omits `project` on a slice when more than one project is offered (`plan-reconcile-project-binding-required`), one that binds a slice to a `project` absent from `registry.yaml` (`plan-reconcile-project-orphan`), and one that binds the `contracts@v1` slice to the `omnia@v1` project (`plan-reconcile-project-target-mismatch`); `specrun plan propose` with neither `--dry-run` nor `--from` exits non-zero with `plan-propose-missing-grouping`.
