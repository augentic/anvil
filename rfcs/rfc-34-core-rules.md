...
1. **`BUILTIN_NAMESPACES` extension.** Add `adapters/shared/rules/core/` → `{"CORE"}` to the built-in namespace map at `crates/authoring/src/check/rules.rs`. Owner discovery uses the same first-segment-under-`adapters/` rule already in place; the core pack appears as a peer of `universal/` rather than as a per-adapter overlay.
...
1. **Schema + predicate updates.** Add `core` to the closed `Origin` enum in `schemas/rules/resolved.schema.json`. Extend `BUILTIN_NAMESPACES` in `crates/authoring/src/check/rules.rs` to map `adapters/shared/rules/core/` → `{"CORE"}`. Update `check::rules` placement predicate to require `CORE-*` at the new path and reject non-`CORE-*` rules there. Add the `--include-core` flag to `specrun rules export` (default off; no behaviour change without the flag).
