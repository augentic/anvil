...
The fact-based change substrate is the foundation of the [Services Delivery Programme](platform.md): [RFC-86](rfc-86-change-facts.md) supplies projected status, per-writer event logs, pinned judgment inputs, and digest-bound `plan.execute.started`; [RFC-87](rfc-87-working-trees.md) supplies private snapshot-backed workspaces; [RFC-88](rfc-88-detached-changes.md) completes single-node detached discovery and accepted-CID execution. RFC-90 moves model-assisted build verification and repair into an engine-owned loop over separate `build` / `repair` / `verify` / `review` operations; [RFC-97](rfc-97-native-verification.md) adds deterministic native-tool verification through the native host-verification capability. Parked [RFC-100](rfc-100-distributed-execution.md) retains fact and snapshot transport as a future multi-node option.
