...
The fact-based substrate is the foundation of the [Services Delivery Programme](platform.md): [RFC-86](rfc-86-change-facts.md) supplies projected change status, per-writer event logs, pinned judgment inputs, and digest-bound `plan.execute.started`; [RFC-87](rfc-87-working-trees.md) supplies content-addressed source values and private snapshot-backed workspaces; [RFC-104](rfc-104-system-archaeology.md) adds the durable definition home, coverage-accounted system model, architecture projections, an immutable migration-wave handoff, and its exact `system.wave.reviewed` fact; [RFC-88](rfc-88-detached-changes.md) imports that handoff and review envelope before turning the wave into accepted CIDs across detached targets. RFC-90 moves model-assisted build verification and repair into an engine-owned loop over separate `build` / `repair` / `verify` / `review` operations; [RFC-97](rfc-97-native-verification.md) adds deterministic native-tool verification through the native host-verification capability. Parked [RFC-100](rfc-100-distributed-execution.md) retains fact and snapshot transport as a future multi-node option.
