...
- **Adapter resolution at init**: `emery init` ensures the adapter needed for project scaffolding (pull-latest provisioning on a bare-name total local miss; `emery adapter update <name>` is the explicit refresh). There is no separate adapter hydration command.
