...
Locator fetch, publication worktrees, and forge reads share one domain seam: the `emery:vcs` package (`trees` / `worktree` / `forge`), where credential and transport policy travel as typed data per call and publication export is a single host call — [RFC-95 host surface](archive/rfc-95-host-surface.md). It retires `emery:origins` / `emery:ingest` as its first cut. `emery:adapter` remains the product contract; `emery:exec-mode` remains the WASI mode-bit gap.
...
Because "Emery is Omnia compiled with Emery-specific backends," there is no separate runtime to download — the binary *is* the runtime, linked with its backends. The shipped `emery` binary is one `omnia::runtime!` command-mode invocation (`src/main.rs` — no handwritten `main`): the engine guest rides the macro's `guests:` key as embedded component bytes, `program:` forwards raw argv (minus the reserved host log flags `--debug` / `--quiet`, peeled into the host log preset), and the mounts and the fail-closed `GuestResolver` are expressions the root package's `launcher` module evaluates once per process (ADR-0011). Every invocation runs in the engine guest — help, version, grammar rejections, and `adapter add` (over its read-only seed preopen) included — with envelopes and exit codes passing through verbatim. Today the Omnia runtime core and macro carry no Emery vocabulary; Emery-specific deployment policy lives in the launcher/provider expressions. RFC-97 later adds its named profile registry to that deployment layer without teaching Omnia core those names. See [CLI architecture](../docs/contributing/cli-architecture.md) and [architecture standards](../docs/standards/architecture.md).

The runtime admits adapter guests by **resolver-backed admission on first dispatch** ([CLI architecture](../docs/contributing/cli-architecture.md); pull-on-miss in [`emery init`](../docs/reference/cli/init.md)): pinned identities resolve the **global single-file store** at `$HOME/.emery/store` (relocatable via `$EMERY_HOME`) with launcher pull-on-miss from the fixed first-party GHCR mapping (`src/launcher.rs` / `launcher::install`); bare names resolve local-first — project component cache seed, else newest installed store version, else pull-latest provisioning; component selectors resolve the project component cache only.
...
- **Deployment policy**: the root package's `launcher` module expressions anchor the project root from argv and the working directory, capture the layout once, create the writable project / cache mount directories, derive the optional `adapter add` seed preopen, construct the fail-closed adapters-only `GuestResolver` with pull-on-miss, and bind the MCP HTTP listener + route hook ([CLI architecture](../docs/contributing/cli-architecture.md)). The global store is host-owned (no guest mount). Remaining: a persisted resolution record and a read-only `deployment show|doctor` surface.

## Deferred relatives

The fact-based substrate is the foundation of the [Services Delivery Programme](platform.md): [RFC-86](archive/rfc-86-change-facts.md) supplies projected change status, per-writer event logs, pinned judgment inputs, and digest-bound `plan.execute.started`; [RFC-87](archive/rfc-87-working-trees.md) supplies content-addressed source values and private snapshot-backed workspaces; [RFC-104](archive/rfc-104-system-archaeology.md) adds the durable definition home, coverage-accounted system model, architecture projections, an immutable migration-wave handoff, and its exact `system.wave.reviewed` fact; [RFC-88](archive/rfc-88-detached-changes.md) imports that handoff and review envelope before turning the wave into accepted CIDs across detached targets. RFC-90 moves model-assisted build verification and repair into an engine-owned loop over separate `build` / `repair` / `verify` / `review` operations; [RFC-97](rfc-97-native-verification.md) adds deterministic native-tool verification through the native host-verification capability. Parked [RFC-100](future/rfc-100-distributed-execution.md) retains fact and snapshot transport as a future multi-node option.
