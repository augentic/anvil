...
- `part-unmatched` — a defined part whose fingerprint matched **no** group in the baseline as it stands at this read. `specify catalog infer` emits this as a **non-blocking, informational** report (in its normal output and in the `--dry-run` diff) listing every such part. It never aborts and is never promoted to a merge precondition.
