...
| Build finalize warnings + `ui_surface` signal (A4) | specify-cli | `src/runtime/commands/slice/build.rs` (coherence checks), `crates/workflow/src/slice/build/wire.rs` (`BuildReport` `ui_surface` field), `schemas/target/build-report.schema.json` (additive field) |
...
**Resolution (settled).** Fold the doc-inversion sweep into **Phase 2 step 5** (documentation — the original "step 9" reference was stale; Phase 2 was renumbered to five steps) and broaden the **B6 cross-repo-touchpoints row** to name every location, with the exact edits:

- `plugins/spec/references/components.md` — **delete** the two "What the catalog does not do" bullets ("No auto-population — operator-curated only"; "No retroactive baseline rewrite without a refactor slice"), both inverted by B1/B7. In the same pass invert the residual operator-curated framing: the lede ("Vectis-only, opt-in" → agent-inferred / operator-reviewable, auto-created by `specify catalog infer` when shared structures exist), the "cross-slice component drift" close ("the catalog lets the operator declare shared components once" → agent infers from the accumulated baseline + candidate cache), and the "Operator workflow" section (inference-first: build runs `specify catalog infer`; operator reviews / rejects per B5).
...
The merge brief carries no operator-curated catalog language (`merge.md` line 38 is tokens / assets only), so it is **not** in the sweep; the earlier "merge-brief" reference is dropped. Folded into Phase 2 step 5 and the B6 cross-repo-touchpoints row.
