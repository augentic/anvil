...
**F2 — control flow lives in skill prose.** The `/spec:execute` loop's park/resume/all-done decisions and the dual-driving pre-flight refusal are skill-markdown behavior, testable only by live trials (the "skill-loop orchestration" category in [docs/contributing/acceptance.md](../docs/contributing/acceptance.md)). The two-phase `slice build --phase prepare|finalize` migration is the worked precedent: the skill stopped hand-transitioning and the gate became deterministic.
