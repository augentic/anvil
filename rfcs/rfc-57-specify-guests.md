...
`/spec:plan` becomes an orchestration guest that issues effects — `eval(survey-brief)` per bound source, `eval(reconcile-leads)` for reconciliation — with the runtime's deterministic operations (`plan add`, validation, Gate 1) as **non-yielding** steps surfaced through the lifecycle effect. `/spec:execute` becomes the drained-loop reducer it already morally is. Skills thin from orchestrators to launchers.
...
- **Skill fate.** Whether skills dissolve into typed orchestration + `eval` bodies, or remain the operator entry surface with orchestration underneath.
