...
The conflicting result snapshots remain immutable inputs to recovery. Each producer recaptures a result without the shared path. When the shared edit is a coherent cross-slice unit of intent, Emery proposes a new integration slice that depends on those producers and exclusively owns the path; otherwise the entries serialize. Adding the slice changes the plan digest, so RFC-86's existing approval becomes stale and the affected project domain remains paused until the amended plan is approved. The integration slice then starts from the repaired composed snapshot and emits the next project snapshot through the ordinary RFC-91 gate.
