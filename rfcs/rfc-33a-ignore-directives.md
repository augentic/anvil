...
| **D4 Directive-without-rationale is a finding** | An unrationaled directive emits `UNI-022` (`ignore-directive-missing-rationale`); a directive whose `<RULE-ID>` does not match any finding on its target line emits `UNI-023` (`ignore-directive-orphan`).                                                                  | Add two `UNI-`* rules under `adapters/shared/rules/universal/` with `regex` deterministic hints; first hints whose evidence comes from the indexer's own `ignore_directive` facts.                                                                                                        |
...
6. **First-party rules.** Author the two `UNI-`* rules pinned by D13: `adapters/shared/rules/universal/ignore-directive-missing-rationale.md` (id `UNI-022`) and `adapters/shared/rules/universal/ignore-directive-orphan.md` (id `UNI-023`), each with a `regex` deterministic hint sourced from the indexer's own `ignore_directive` facts. These are the first hints that exist *because* of the indexer's own facts; resolver failures degrade per the §"Graceful degradation" rule below.
...
The only operator-visible regression in this configuration is that unrationaled directives and orphan directives slip through silently. The fix is to either pass `--rules-root` to `specrun lint` until the distribution follow-up lands, or to vendor the shared tree into the consumer project's `.specify/cache/rules/` location read by `resolve_rules_root` (see `[src/runtime/commands/review/run.rs](https://github.com/augentic/specify-cli/blob/main/src/runtime/commands/review/run.rs)`).
