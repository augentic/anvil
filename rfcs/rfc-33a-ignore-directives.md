...
> Status: Accepted · Depends: [RFC-28](done/rfc-28-standards-contract.md), [RFC-32](done/rfc-32-standards-enforcement.md) · Companion: [RFC-33b](rfc-33b-standards-finding-baseline.md) (deferred) · Enables: [roadmap RM-10](roadmap.md#rm-10-ci-native-standards-enforcement), [roadmap RM-14](roadmap.md#rm-14-local-structured-workflow-events)
...
1. **In-source ignore directives** — a single `specify-ignore: <RULE-ID> — <rationale>` grammar the `WorkspaceModel` indexer recognises across known comment styles, surfaced as an `ignore_directive` fact and consumed by the hint interpreter at finding-emission time.
2. **Finding status taxonomy and `disposition` field** — the existing `status` field on `LintFinding` widens to add `ignored` to RFC-28's `open` / `fixed` / `accepted` / `false-positive` set, with the new optional `disposition` field carrying the source of the decision. RFC-33a producers emit `open`, `ignored`, and `false-positive`. [RFC-33b](rfc-33b-standards-finding-baseline.md) widens the enum a second time (adding `new` and `baselined`) and adds the optional `disposition.baseline` sub-field when it lands; both widenings are additive.
...
The cross-run baseline + diff layer is split into [RFC-33b: Standards Finding Baseline](rfc-33b-standards-finding-baseline.md), which is deferred behind the trigger conditions named in its own §"Trigger conditions". RFC-33b extends RFC-33a's D5 / D6 wire-shape decisions additively: it widens the `status` enum to add `new` and `baselined`, and adds the optional `disposition.baseline` sub-field when it lands.
...
The two pressure points RFC-33a deliberately does not address, and why they live in [RFC-33b](rfc-33b-standards-finding-baseline.md):
...
The principles below apply to both RFC-33a (this document) and [RFC-33b](rfc-33b-standards-finding-baseline.md). RFC-33a implements (1), (3), (4), (5), and (7); RFC-33b picks up (2) and (6) when its surfaces land.
...
2. **Baseline is opt-in but on-by-default once present.** ([RFC-33b](rfc-33b-standards-finding-baseline.md).)
...
4. **Directives do not aggregate.** One directive covers one rule on one location; no file-level or directory-level ignore syntax. Mass adoption is the baseline file's job ([RFC-33b](rfc-33b-standards-finding-baseline.md)), not the directive's.
5. **Status is post-hoc, never blocking by itself.** A scanner emits findings with `status: open`. The directive post-pass may demote them to `ignored` (or `false-positive` for prefixed rationale). [RFC-33b](rfc-33b-standards-finding-baseline.md) adds a baseline pass that demotes to `baselined`. CI policy decides whether `status != open` means "ignore" or "report-only."
6. **One writer per file.** ([RFC-33b](rfc-33b-standards-finding-baseline.md), once the baseline file and `last.json` exist.)
...
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **D3 Ignore directive grammar**                 | Single grammar `specify-ignore: <RULE-ID> — <rationale>` recognised inside known comment delimiters (`//`, `#`, `<!-- -->`, `--`, `/* */`). The directive applies to the next non-blank, non-comment line.                                                                  | New `ignore_directive` fact in `WorkspaceModel`; new extractor in `review/index/`; new `disposition.directive` payload in emitted findings.                                                                                                                                               |
| **D4 Directive-without-rationale is a finding** | An unrationaled directive emits `UNI-022` (`ignore-directive-missing-rationale`); a directive whose `<RULE-ID>` does not match any finding on its target line emits `UNI-023` (`ignore-directive-orphan`).                                                                  | Add two `UNI-`* rules under `adapters/shared/codex/universal/` with `regex` deterministic hints; first hints whose evidence comes from the indexer's own `ignore_directive` facts.                                                                                                        |
| **D5 Status enum widened**                      | `LintFinding.status` adds `ignored` to RFC-28's existing `open` / `fixed` / `accepted` / `false-positive` set. [RFC-33b](rfc-33b-standards-finding-baseline.md) widens the enum a second time (adding `new` and `baselined`) when it lands. | One additive bump to `schemas/review/finding.schema.json`; RFC-33b's widening is a second additive bump under the same fingerprint contract. |
| **D6 Disposition object**                       | New optional `disposition: { source, directive?, since? }` on `LintFinding`. RFC-33a producers populate `disposition.directive`. [RFC-33b](rfc-33b-standards-finding-baseline.md) adds the optional `disposition.baseline` sub-field when it lands. | One new field in the schema; one new struct in `specify-domain`; populated only when `status != open`. |
| **D8 Journal event**                            | New `lint-completed` variant on the closed `EventKind` taxonomy.                                                                                                                                                                                                          | One new variant in `crates/domain/src/journal.rs`; payload counts per-status; no per-finding detail to keep journal lines bounded. While [RFC-33b](rfc-33b-standards-finding-baseline.md) is deferred, `baseline_present` is always `false` and the payload carries `counts.{open, ignored, false_positive}` only; RFC-33b adds `counts.{new, baselined}` and makes `baseline_present` scan-derived when it lands. |
...
| **D13 First-party rule files**                  | The two new universal codex rules are filed at fixed paths so the implementation has concrete targets.                                                                                                                                                                      | `adapters/shared/codex/universal/ignore-directive-missing-rationale.md` (id `UNI-022`) and `adapters/shared/codex/universal/ignore-directive-orphan.md` (id `UNI-023`).                                                                                                                   |


The decision IDs (D3, D4, D5, D6, D8, D12, D13) are preserved from the original combined RFC-33 so prior references in commit messages, PRs, and historical discussion continue to resolve. [RFC-33b](rfc-33b-standards-finding-baseline.md) owns D1, D2, D7, D9, D10, D11.
...
- File-scoped and block-scoped variants are explicitly **not in v1**. Mass adoption is the baseline file's job (principle 4) and lands with [RFC-33b](rfc-33b-standards-finding-baseline.md).
- A directive whose `<RULE-ID>` does not match any finding on its target line emits `UNI-023` (`ignore-directive-orphan`, new shared codex rule). Operators see dead directives in the same envelope as everything else.

WorkspaceModel addition (one new fact family):

```text
ignore_directive  { path, line, rule_id, rationale, target_line }
```

The indexer extracts directives once per scan; the hint interpreter consumes them at finding-emission time and stamps `status: ignored` plus `disposition.directive` on matches.
...
While [RFC-33b](rfc-33b-standards-finding-baseline.md) is deferred, the scanner always emits `baseline_present: false` and the payload carries only `counts.{open, ignored, false_positive}`. RFC-33b adds `counts.{new, baselined}` to the payload and makes `baseline_present` scan-derived when it lands; both additions are backwards-compatible — consumers tolerant of unknown keys see no regression on the RFC-33a payload.
...
RFC-33a adds no new CLI verbs or flags. The surface remains exactly what RM-10 already promises:
...
- The enum widening (`ignored`) is additive; consumers that only handle RFC-28's `open` / `fixed` / `accepted` / `false-positive` set see `null` if they reject unknown values, but the schema enum widens cleanly. [RFC-33b](rfc-33b-standards-finding-baseline.md) widens the enum a second time (adding `new` and `baselined`) when it lands.
...
1. **Schema.** Extend `schemas/review/finding.schema.json` (status enum adds `ignored`; optional `disposition` field added with `source`, `directive?`, `since?` sub-fields). Golden test: fingerprint stability across the enum widening, asserted against the RFC-28 Phase 2 fixtures.
2. **Domain types.** Add `Disposition` DTO to `specify-domain`. Reuse RFC-28's canonical-JSON helper for stable serialisation. ([RFC-33b](rfc-33b-standards-finding-baseline.md) adds `Baseline`, `BaselineEntry`, `ReviewRun` when it lands.)
3. **Indexer fact.** Add `ignore_directive` extractor under `crates/domain/src/review/index/`. Honour the closed comment-style list; ignore everything else without falling back to heuristics.
4. **Scanner pipeline.** Insert the directive pass between hint evaluation and envelope emission. Order: directive → status assignment → ordering. ([RFC-33b](rfc-33b-standards-finding-baseline.md) inserts a baseline pass between directive and status when it lands.)
...
6. **First-party rules.** Author the two `UNI-`* codex rules pinned by D13: `adapters/shared/codex/universal/ignore-directive-missing-rationale.md` (id `UNI-022`) and `adapters/shared/codex/universal/ignore-directive-orphan.md` (id `UNI-023`), each with a `regex` deterministic hint sourced from the indexer's own `ignore_directive` facts. These are the first hints that exist *because* of the indexer's own facts; resolver failures degrade per the §"Graceful degradation" rule below.
...
**For adapter authors:** No required action. Adapter codex rules may add directives in their own examples; the new shared `UNI-022` / `UNI-023` rules apply to consumer-project code, not to adapter source.
...
**For CLI maintainers:** RFC-33a introduces no filesystem state under `.specify/lint/`. The directory shape is reserved for [RFC-33b](rfc-33b-standards-finding-baseline.md).
...
The only operator-visible regression in this configuration is that unrationaled directives and orphan directives slip through silently. The fix is to either pass `--codex-root` to `specrun lint` until the distribution follow-up lands, or to vendor the shared tree into the consumer project's `.specify/cache/codex/` location read by `resolve_codex_root` (see `[src/runtime/commands/review/run.rs](https://github.com/augentic/specify-cli/blob/main/src/runtime/commands/review/run.rs)`).
...
RFC-33a has no operational open questions; the directive grammar, indexer fact, and journal payload are fully specified above. Open questions surface as implementation discovers them. The three operational questions previously held against RFC-33b are tracked in [RFC-33b](rfc-33b-standards-finding-baseline.md) §"Open Questions".
...
- [RFC-33b: Standards Finding Baseline](rfc-33b-standards-finding-baseline.md) — companion RFC, deferred behind trigger conditions
