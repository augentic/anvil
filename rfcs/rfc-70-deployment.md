...
Make the installed `specify` executable the only command an operator needs to run. The executable remains an Omnia command-mode runtime hosting the Specify workflow guest and independently versioned adapter components, but it assembles that deployment itself.

The target operator surface is:

```bash
specify init omnia@1.0.0 --name payments
specify plan author migrate-payments --source legacy=typescript@1.0.0:./legacy
specify plan transition migrate-payments approved
specify plan execute
```

Operators do not write `omnia.toml`, locate the workflow component, enumerate adapter guests, configure adapter MCP routes, or provide cache and store mounts. Those are derived deployment details.
...
- the Specify workflow component;
...
The wrapper has the desired command shape, but it does not own deployment closure. Adding a new adapter still requires changing the Omnia configuration before the workflow can dispatch to it.

The repository already has most of the required substrate:

- the shipped binary is an Omnia runtime;
- workflow-to-adapter calls carry an axis-qualified adapter id;
...
4. Keep the Omnia runtime generic and free of Specify workflow vocabulary.
...
- Adding workflow orchestration to the launcher.
...
2. **Omnia runtime** — hosts the workflow component, satisfies its effect imports, dispatches axis-qualified adapter calls, and forwards the command to the workflow guest.
...
The workflow guest remains the only `wasi:cli/run` exporter. Command envelopes and exit codes pass through unchanged.
...
- Specify binary and workflow component versions;
...
For the `payments` project from the abstract — target `specify:omnia@1.0.0`, one plan-bound source `legacy=typescript@1.0.0:./legacy`, binary version `0.30.0` — the derived `omnia.toml` is the closure projected onto the shape the composed example authors by hand today:

```toml
# GENERATED by specify 0.30.0 — never committed, never edited.
# Regenerated whenever the resolution fingerprint changes; safe to delete.

[[guest]]
id = "specify"
source.path = "/home/op/.specify/adapters/core@0.30.0.wasm"
link = ["specify:adapter/source@0.1.0", "specify:adapter/target@0.1.0"]

[[guest]]
id = "source:typescript"
source.path = "/home/op/.specify/adapters/typescript@1.0.0.wasm"

[[guest]]
id = "target:omnia"
source.path = "/home/op/.specify/adapters/omnia@1.0.0.wasm"

[[route.http]]
prefix = "/mcp/source/typescript"
guest = "source:typescript"

[[route.http]]
prefix = "/mcp/target/omnia"
guest = "target:omnia"
...
```json
{
  "specify": "0.30.0",
  "core": {
    "package": "specify:core@0.30.0",
    "component": "/home/op/.specify/adapters/core@0.30.0.wasm",
    "origin": "registry",
    "digest": "sha256:9c41…"
  },
  "adapters": [
    {
      "axis": "source",
      "selector": "specify:typescript@1.0.0",
      "resolved": "typescript@1.0.0",
      "component": "/home/op/.specify/adapters/typescript@1.0.0.wasm",
      "origin": "store",
      "digest": "sha256:5b0e…"
    },
    {
      "axis": "target",
      "selector": "specify:omnia@1.0.0",
      "resolved": "omnia@1.0.0",
      "component": "/home/op/.specify/adapters/omnia@1.0.0.wasm",
      "origin": "store",
      "digest": "sha256:1d9c…"
    }
  ],
...
```yaml
# .specify/project.yaml (written by `specify init omnia@1.0.0 --name payments`)
name: payments
...
```yaml
# plan.yaml (written by `specify plan author migrate-payments --source legacy=typescript@1.0.0:./legacy`)
name: migrate-payments
sources:
  legacy:
    adapter: typescript@1.0.0
    path: ./legacy
slices: [...]
```
...
| ------------------------- | ------------------- | -------------------------------------- |
| `specify:core@0.30.0` | `specify` | always — the workflow guest |
| `specify:omnia@1.0.0` | `target:omnia` | `project.yaml.adapter` |
| `specify:typescript@1.0.0`| `source:typescript` | `plan.yaml.sources.legacy` |

`specify slice refine` and `specify slice build` on the same project derive the same three entries. `specify journal show` reaches no adapter, so the core guest alone would satisfy it; in practice the launcher reuses the persisted deployment while its fingerprint still matches and regenerates only when the project or plan bindings change. A `specify init vectis@1.2.0 --name other` in a different project contributes nothing here: closures never leak across project caches.
...
1. the workflow guest parses the command and calls `ensure_*`;
...
- release packaging installs the Specify workflow component and the supported first-party adapter set into the global store;
...
```json
{
  "schema-version": 1,
  "specify": "0.30.0",
  "core": { "package": "specify:core@0.30.0", "digest": "sha256:9c41…" },
  "adapters": [
    ...
    { "axis": "source", "package": "specify:typescript@1.0.0",    "digest": "sha256:5b0e…" },
    ...
    { "axis": "target", "package": "specify:omnia@1.0.0",         "digest": "sha256:1d9c…" },
...
```text
~/.specify/adapters/
├── core@0.30.0.wasm          # hydrated on first launch (origin: registry)
├── core@0.30.0.meta          # tree-digest: sha256:9c41…
├── omnia@1.0.0.wasm          # hydrated by `specify init omnia@1.0.0` (origin: registry)
├── omnia@1.0.0.meta
├── typescript@1.0.0.wasm     # hydrated by `plan author --source legacy=typescript@1.0.0:…`
└── typescript@1.0.0.meta
```
...
Illustrative shape — the preflight is a thin projection over the grammar the workflow guest already owns (`transport::command`'s `InitArgs` / plan `AuthorArgs`) and the existing `AdapterSelector::parse`:
...
fn main() -> std::process::ExitCode {
    ...
    // 4. Drive: in-process Omnia command mode; argv and the workflow
...
- core and adapter components exist;
...
core    specify:core@0.30.0   ok
target  omnia@1.0.0           ok
source  typescript@1.0.0      adapter-digest-mismatch: recorded sha256:5b0e… but recomputed sha256:99d1…
...
struct ResolvedGuest {
    ...
    identity: String,          // "specify:typescript@1.0.0"
    digest: String,            // verified before compilation
    bytes: Vec<u8>,
}
```

Specify's implementation of that trait is the existing store probe plus verify-on-read — the same code path `ensure_*` leaves its results in — behind an allow-list carried by the generated manifest. After Stage 3 the derived `omnia.toml` shrinks to the workflow guest, effects, mounts, and that policy block; no per-adapter `[[guest]]` entries remain:
...
This is an Omnia runtime capability, not Specify workflow code. Specify supplies the resolver policy and store implementation through deployment composition.

After lazy resolution lands:

- the generated manifest names the workflow guest, effects, mounts, and resolver policy;
- it no longer names every adapter guest;
...
2. Generate a release inventory for the workflow component and first-party adapters.
...
6. The workflow guest remains the sole owner of command semantics and lifecycle transitions.
...
4. Which third-party installation command breaks the bootstrap cycle before lazy resolution lands without reintroducing a general package-manager surface into the workflow CLI? (Moot if the Omnia hook lands first.)
