...
- **Deployment modes are not yet "just backend swaps."** They become swaps only once the host-service surface — including `infer` — is the single seam every backend plugs into.

This RFC is the engineering behind the architecture's "Omnia as the runtime — committed" bet. It is deliberately *not* the model fleet (that is [RFC-56](rfc-56-infer-fleet.md)) and *not* the workflow-as-guest move (that is [RFC-55](rfc-55-workflow-and-development-guests.md)); it is the floor both of those stand on.
...
- **The model fleet is out of scope.** Real `infer` backends, the router, and the deployment topologies are [RFC-56](rfc-56-infer-fleet.md); this RFC only requires that `infer` is *a* host like the others (satisfied at minimum by the RFC-52 replay backend).
...
- **`infer`** — bound to whatever backend the deployment selects; the replay stub suffices until [RFC-56](rfc-56-infer-fleet.md).
