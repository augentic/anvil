...
| `rt/wiretapper`                         | "Add wiretap code to a cloned legacy TypeScript repo to capture request/response and side-effect data as replay-ready JSON; …"                                                    | (append trigger) "… Use when capturing runtime data from a legacy TypeScript service before migration, or when the user mentions `wiretapper`."                                                                                                                                                                                                                                                     |
| `rt/replay-writer`                      | "Add tests from real-life JSON captures in tests/data/replays/, run tests, and review code so tests pass. For crates already generated by the Specify workflow."                   | (append trigger) "… Use when turning captured legacy scenarios into regression tests for a generated crate, or when the user mentions `replay-writer`."                                                                                                                                                                                                                                             |
...
The check follows the existing `scripts/checks.ts` pattern; once [RFC-5](../rfc-5-tooling.md) lands, it migrates into `framework-check` alongside the other framework invariants.
