# RM-01 Outside-In Acceptance Harness Handoff
...
The missing layer is an outside-in harness that starts where users start: change brief and documentation, then runs the actual workflow skills and capability pipelines.
...
## What The Outside-In Layer Must Prove

The outside-in harness should prove this full path:
...
Recommended approach: hybrid outside-in replay.
...
### Phase 2: Plan-Level Outside-In Test
...
Full outside-in execution may be slow. Keep a cheap CLI test (`specify-cli/tests/cross_repo.rs`) for every PR and a heavier outside-in harness for release confidence.
...
This handoff targets the missing outside-in layer for RM-01.
...
```text
Implement the outside-in RM-01 acceptance harness described in
rfcs/rm-01-outside-in-harness.md.
