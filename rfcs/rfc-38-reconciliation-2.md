...
- **`topics[]` (survey time).** A source adapter may tag each lead with kebab-case domain tokens (`session-token`, `replay-protection`, …) it observed while sizing. This is the metadata [RFC-29b §"Appendix: Deferred Work" item 2](rfc-29b-reconciliation.md) said advisory clustering needed and [RFC-29a §"survey"](rfc-29a-source.md) deferred. Optional: a source that emits none degrades to today's `synopsis`-only behaviour.
- **Advisory `clusters[]` (propose time).** The kernel computes connected components over catalog rows whose `{lead} ∪ aliases[] ∪ topics[]` token sets intersect *across different source keys* (never within one source — same-source fusion stays forbidden, [RFC-29b §"Cross-Source Matching"](rfc-29b-reconciliation.md)). The result is emitted as a hint:
...
This is the token-intersection idea from [RFC-29b §"Appendix" item 1](rfc-29b-reconciliation.md), demoted from a kernel *lock* to a kernel *hint*: the agent still emits `slices[]`, the kernel still enforces shape-only invariants, and Gate 1 still curates. A wrong cluster costs nothing because nothing keys off it.
...
RFC-36 created the durable decision catalogue but pointed it only at topology routing. The same catalogue is the natural answer to [RFC-29 Q2](rfc-29-fan-in-fan-out.md) (the "feed the existing baseline `spec.md` into synthesis as read-only context" litmus). This RFC populates RFC-29c's deferred `advisory-context` input deterministically from the bound project's baseline:
...
- **Kernel-side auto-merge.** Clustering stays advisory; the kernel never fuses leads or overrides agent grouping ([RFC-29b §"Cross-Source Matching"](rfc-29b-reconciliation.md)), and same-source fusion stays a Gate 1 operator action.
...
- **An agent-side defer bucket.** Total coverage stays the propose invariant ([RFC-29b §"Cross-Source Matching"](rfc-29b-reconciliation.md)); a defer bucket remains deferred to its own RFC.
