...
- **S2 · Name the effects** ([RFC-52](rfc-52-effect.md)) — the typed effects become WIT imports: `wasi:filesystem`, `wasi:keyvalue`, the lifecycle hooks (`journal` / `transition`), and the **`wasi-model` `eval` host**, with the `references` shelf as a guest export. The payoff is record/replay, served by the replay model backend.
- **S3 · Guests orchestrate** ([RFC-54](rfc-54-orchestration.md)) — the deterministic `tool` operations become callable through the RFC-51 bindings (retiring `wasi:cli/run` on that path), and judgment operations run through **`wasi-model.eval`** and its tool loop ([RFC-53](rfc-53-wasi-model.md)) over the adapter's brief and `references` shelf rather than a single handoff.
- **The judgment spine** ([RFC-53](rfc-53-wasi-model.md)) — the `wasi-model` host (`eval`), its tool surface (`resolve` / `read` / `list` / `write` / `verify`), the `verify` seam, and the model-backend boundary that carries record/replay and the vendor model id. It threads through S2 (the host) and S3 (the loop driving operations).
- **S4 · The runtime move** ([RFC-56](rfc-56-runtime-move.md)) — the generic Omnia binary plus Specify backends retires the bespoke `specify` host: the effects get real host-service backends, execution becomes instance-per-call, the multi-guest registry and the CLI trigger land, the `wasi-model` backend is bound, and shipping a WASM component becomes mandatory on both axes (the prose-only adapter ends). The working-tree backend ([RFC-55](rfc-55-working-tree.md)) — a custom git-aware `wasi:filesystem` backend that materializes a content-addressed tree per operation, so `build` and `merge` can run on different nodes and out of order — lands with it.
...
- **Parallel · Model backends** ([RFC-58](rfc-58-model-backends.md)) — the frontier (`genai`), spawned-agent, SLM, and replay backends behind `wasi-model`, plus the routing backend that picks one per call by difficulty and cost, delivering the interactive and headless deployment modes. Independent of the runtime move — it needs only the `wasi-model` host — and it brings [RFC-18](future/rfc-18-slm.md) (the SLM) in as a backend.
