...
- **S4 · The runtime move** ([RFC-56](rfc-56-runtime-move.md)) — the keystone: the generic Omnia binary plus Specify backends retires the bespoke `specify` host, the deterministic effects get real host-service backends, execution becomes instance-per-call, and shipping a WASM component becomes mandatory on both axes (the prose-only adapter ends). The runtime move is **committed**, and it binds two companions carved out of it: the Omnia model host for `eval` — a sanctioned *new* generic host ([RFC-54](rfc-54-model-host.md), cross-repo with `augentic/omnia`) — and the working-tree backend ([RFC-55](rfc-55-working-tree.md)), a custom git-aware `wasi:filesystem` backend (not a new host) that replaces the persistent single checkout with a content-addressed tree the host materializes per operation, so `build` and `merge` can run on different nodes and out of order.
- **S4 · Workflow (and development) as guests** ([RFC-58](rfc-58-specify-guests.md)) — the workflow runs on the new runtime like every adapter; *how much* of each phase compiles into the guest versus stays agent-driven behind `eval` is the per-phase, evidence-gated call RFC-58 owns. The framework's own development tooling follows as the mechanical tail.
- **Parallel · The model fleet** ([RFC-57](rfc-57-eval-fleet.md)) — turns the S2 `eval` seam into real backends (frontier API, spawned agent, difficulty/cost router) and delivers the interactive and headless deployment modes. Independent of the runtime move — it needs only the seam — and it brings [RFC-18](future/rfc-18-slm.md) (the SLM) in as a fleet member.
...
- **`augentic/omnia`** — owns the generic runtime floor (the Wasmtime interpreter and the pluggable host-service framework), the general-purpose host traits Specify's deterministic-effect backends plug into (`wasi:filesystem`, `wasi:blobstore`, `kv`, …), plus the one sanctioned new-host slot for the model host and its `ModelBackend` trait ([RFC-54](rfc-54-model-host.md)); it carries zero Specify domain knowledge (law 2).
