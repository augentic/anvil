...
- **S4 · The runtime move** ([RFC-56](rfc-56-runtime-move.md)) — the keystone: the generic Omnia binary plus Specify backends retires the bespoke `specify` host, the deterministic effects get real host-service backends, execution becomes instance-per-call, and shipping a WASM component becomes mandatory on both axes (the prose-only adapter ends). The runtime move is **committed**, and it binds two backend companions carved out of it: the Omnia model host for `eval` ([RFC-54](rfc-54-model-host.md), cross-repo with `augentic/omnia`) and its deterministic-effects mirror, the working-tree data backend ([RFC-55](rfc-55-working-tree.md)) — which replaces the persistent single checkout with a content-addressed tree the host materializes per operation, so `build` and `merge` can run on different nodes and out of order.
