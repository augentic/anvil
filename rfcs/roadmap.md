...
The runtime architecture — Specify as a family of Wasm guests on the Omnia runtime, with **judgment as the `wasi-model` host effect** behind a swappable model backend — is fixed in [architecture.md](architecture.md). The original S1–S4 staging (RFC-51–60) predates the Omnia refactoring and has been removed from the tree (recoverable from git history); RFC-61, the in-place migration of the framework and its adapters onto the implemented runtime, is complete (all five steps and the Step 4 parity audit) and likewise removed, as are RFC-62 (the adapter author's dev loop — the prose overlay, operation selector, model override, and watch task, implemented in `specify-adapters`) and RFC-66 (publishing and distribution — the one-transport wasm-pkg/OCI registry backing behind `augentic.io`, the idempotent tag-driven publish legs for the adapters, the core guest, and the WIT contract, and the codex ownership flip; tap automation and the overlay manifest-cache extraction ride later work). The follow-on [RFC-65](rfc-65-standalone-deployment.md) (the standalone deployment — hydration, the central store, and the provisioning / runtime surface split) is implemented; see §"Recently implemented". Two relatives are deferred, not archived: [RFC-55](future/rfc-55-working-tree.md) (distributed working trees — not needed while every guest shares the deployment's `[[mount]]` preopens) and [RFC-60](future/rfc-60-verify-profiles.md) (verify profiles — the `verify` grant is accepted but stubbed).
...
- [RFC-65: Standalone deployment](rfc-65-standalone-deployment.md) — the two-layer binary (native provisioning front: `init`, `adapters sync`, `upgrade`, `plugins`; every other argv forwards unparsed to the core guest through the in-process macro-mounted command-mode runtime), the hydration kernel over the global single-file store at `$HOME/.specify/adapters` (`$SPECIFY_ADAPTER_STORE` override), the committed `.specify/adapters.lock` digest pin, the generated deployment manifest in the per-project derived cache, the core guest versioned by the binary (`specify:core@<binary version>`, `SPECIFY_CORE_PATH` dev override), and the `specify:` naming cut. Known trailing item: the `workspace sync/push/prepare` verbs stay native until the pinned host git capability lands (DECISIONS.md §"Workspace git transport from the guest").
