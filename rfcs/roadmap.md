...
- **S4 · The runtime move** ([RFC-55](rfc-55-omnia-runtime-move.md)) — the keystone: the generic Omnia binary plus Specify backends retires the bespoke `specify` host, the deterministic effects get real host-service backends, and execution becomes instance-per-call. The runtime move is **committed**, and it is a cross-repo pairing with `augentic/omnia` (the Omnia model host, [RFC-54](rfc-54-omnia-model-host.md)).
- **S4 · Workflow (and development) as guests** ([RFC-57](rfc-57-workflow-and-development-guests.md)) — the workflow runs on the new runtime like every adapter; *how much* of each phase compiles into the guest versus stays agent-driven behind `judge` is the per-phase, evidence-gated call RFC-57 owns. The framework's own development tooling follows as the mechanical tail.
