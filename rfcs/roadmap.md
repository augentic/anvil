...
## Effect-oriented architecture stages (S1–S4)

The runtime architecture — Specify as a family of Wasm guests on the generic Omnia runtime, with **judgment as the `wasi-model` host effect** (`eval`, [RFC-53](rfc-53-wasi-model.md)) behind a swappable model backend — is fixed in [architecture.md](architecture.md); this section sequences it. Each stage is independently mergeable, independently valuable, and forward-compatible on the same typed contract.

- **S1 · Typed contract** ([RFC-51](rfc-51-adapter-wit.md)) — the versioned `augentic:specify` WIT package (records, per-axis `source` / `target` interfaces, the `references` shelf, the worlds) authored and published with host bindings as the single source of truth.
- **S2 · Effect map** ([RFC-52](rfc-52-effect.md)) — the typed effects are named and assigned owners: `wasi:filesystem`, `wasi:keyvalue`, lifecycle (`journal` / `transition`), `references`, and `wasi-model`.
- **S2 · Judgment boundary** ([RFC-53](rfc-53-wasi-model.md)) — the `wasi-model` host (`eval`), prompt / answer records, backend trait, answer validation, and the minimal replay seam.
- **S3 · Model tool loop** ([RFC-59](rfc-59-model-tool-loop.md)) — the standard tool surface (`resolve` / `read` / `list` / `write` / `verify`), session state, repair-loop semantics, and lazy reference loading over the adapter's shelf.
- **S3 · Vertical operation proof** ([RFC-54](rfc-54-orchestration.md)) — one deterministic `tool` operation through the RFC-51 bindings and one judgment operation through `wasi-model.eval`, proving the path before the workflow moves.
- **S4 · Working-tree backend** ([RFC-55](rfc-55-working-tree.md)) — the custom git-aware `wasi:filesystem` backend materializes a content-addressed tree per operation, so `build` and `merge` can run on different nodes and out of order.
- **S4 · Verify profiles** ([RFC-60](rfc-60-verify-profiles.md)) — closed check profiles, host-owned argv, sandbox policy, report normalization, and capability signaling.
- **S4 · The runtime move** ([RFC-56](rfc-56-runtime-move.md)) — the generic Omnia binary plus Specify backends retires the bespoke `specify` host: the effects get real host-service backends, execution becomes instance-per-call, the multi-guest registry and the CLI trigger land, the working-tree backend is bound behind `wasi:filesystem`, the `wasi-model` backend is bound, and shipping a WASM component becomes mandatory on both axes (the prose-only adapter ends).
- **S4 · Workflow (and development) as guests** ([RFC-57](rfc-57-specify-guests.md)) — the workflow runs on the runtime like every adapter, reaching adapters by host-mediated dynamic linking and judgment by `eval`. The framework's own development tooling follows.
- **Parallel · Model backends** ([RFC-58](rfc-58-model-backends.md)) — the frontier (`genai`), spawned-agent, SLM, replay expansion, and routing backends behind `wasi-model`, delivering the interactive and headless deployment modes. It builds on the core `wasi-model` boundary and brings [RFC-18](future/rfc-18-slm.md) (the SLM) in as a backend.

[architecture.md](architecture.md) fixes the direction; the per-stage detail and open questions live in the linked RFCs.
...
- **`augentic/specify`** (this repo) — owns the typed contract (the `augentic:specify` package, including the `references` shelf), the Specify-side backends behind Omnia's host interfaces (the working-tree backend [RFC-55](rfc-55-working-tree.md) and the **model backend** behind `wasi-model` — frontier / spawned-agent / SLM / replay, [RFC-58](rfc-58-model-backends.md)), the guests, and the operator CLI.
- **`augentic/omnia`** — owns the generic runtime core (the Wasmtime interpreter and the pluggable host-service framework) and the general-purpose host interfaces Specify's backends plug into: `wasi:filesystem`, `wasi:keyvalue`, `wasi:blobstore`, and `wasi-model` (`eval`). It carries zero Specify domain knowledge and zero model knowledge — which model backs `wasi-model` is Specify's backend, never Omnia's (Law 2 in the runtime core).
...
One Specify-owned seam is versioned across the boundary: `augentic:specify` (this repo → adapters). Land a published `augentic:specify` pin before the adapter components that consume it, and treat the seam as a contract so neither repo blocks the other. The Omnia runtime core — including the `wasi-model` host interface — is consumed as an ordinary upstream dependency. Cross-repo version skew is a headline risk the Stage-4 RFCs ([RFC-55](rfc-55-working-tree.md) / [RFC-56](rfc-56-runtime-move.md)) call out.
