...
- **Orchestration trace replay for eval scenarios.** The *shape-assertion* half of this idea has shipped: deterministic structural grading lives in the [assertion taxonomy](../evals/shared/assertions.md) (per-assertion `Probe` vs `Judgment flag`), so structure is self-graded and only prose is human-judged — no new tier required. What remains deferred is recorded-transcript **orchestration replay** (capture a `cursor-agent` run via `@cursor/sdk`, replay it against the real CLI), parked in [`docs/contributing/evals.md` §"Synthesis byte-replay (deferred)"](../docs/contributing/evals.md). Activation now needs *both* a stable `@cursor/sdk` capture surface *and* a reversal of the `transcript-replay-added` / `automated-runner-added` negative-expectations that every scenario currently encodes — a deliberate posture, not an oversight. (RFC-39, which proposed a `backend: shape` tier, was removed: its carrier is gone, its shape half shipped, and its replay half is now in tension with the encoded operator-driven posture.)
...
- Which reconciliation-polish surfaces land first — lead-side `topics[]` / `clusters[]`, binding `affinity`, or synthesis `advisory-context`? ([RFC-46](future/rfc-46-reconciliation-polish.md) proposes `topics[]` first, then `clusters[]` + coverage checks, then `affinity` + decision-conflict warnings, with `advisory-context` / greenfield seed last.)
