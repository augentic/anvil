...
**Status:** Partial — the unified [`evals/scenarios/`](../evals/scenarios/README.md) pack defines 14 operator-driven scenarios including execute build failure (`execute-build-failure`), step-through breakout (`stepthrough-breakout`), workspace breakout (`workspace-breakout`), dual-driving refusal (`dual-driving-refused`), and stale-workspace recovery (`stale-workspace-recovery`); fully deterministic behaviors (extract failure, invalid evidence, source sandbox denial, …) are named tests in `specify-cli`, not catalog entries. 8 of 14 scenarios are `passed` per the catalog — `pure-intent` (the N=1 release blocker), `execute-build-failure` (the build-failure park/resume release blocker), the core synthesis, planning, and routing set, and `target-shape-injection` — each backed by a committed record under [`evals/runs/`](../evals/runs/README.md); the remaining 6 are `pending`, including the one outstanding `release-blocker` row (`workspace-execute-two-projects`).
**Immediate task:** Drain the outstanding `workspace-execute-two-projects` release blocker against the live `specify` binary, then the remaining `full`-tier catalog. `pure-intent` hard halt unchanged on any re-run.
