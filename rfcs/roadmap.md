...
**Status:** Superseded — folded into the `specify` CLI as the `specify vectis ...` subcommand tree in [`augentic/specify-cli`](https://github.com/augentic/specify-cli) (`crates/vectis/` library + `templates/vectis/`). The standalone `vectis` binary the RFC originally proposed no longer exists; the RFC and its task list are kept under a "superseded" banner for historical context.
