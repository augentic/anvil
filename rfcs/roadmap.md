...
One Specify-owned seam is versioned across the boundary: `augentic:specify` (this repo → adapters). Land a published `augentic:specify` pin before the adapter components that consume it, and treat the seam as a contract so neither repo blocks the other. The Omnia floor — including the `wasi-model` host interface — is consumed as an ordinary upstream dependency. Cross-repo version skew is a headline risk the Stage-4 RFCs ([RFC-55](rfc-55-working-tree.md) / [RFC-56](rfc-56-runtime-move.md)) call out.
