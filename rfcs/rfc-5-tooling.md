...
Replace this repo's Deno tooling (`scripts/check.ts`, `scripts/gen-envelope-doc.ts`, `tests/cross_repo.ts`) with a small Rust workspace at **`augentic/specify/tooling/`**, introduced in phases. The workspace lives in a dedicated subdirectory so skill and adapter authors see a markdown-first repo root (`plugins/`, `adapters/`, `docs/`) without `Cargo.toml`, `crates/`, or `rust-toolchain.toml` beside the content they edit. The first phase is deliberately narrow: JSON Schemas are the canonical contract for every artifact whose shape can be expressed declaratively; cross-file rules that schemas cannot express live in a single `rules` library crate; that library backs a thin `check` CLI for local and CI use. Once that rule engine is proven, sibling Rust tools port doc generation and fixture acceptance. The operator `specify` binary in `augentic/specify-cli` is deliberately **not** extended; framework tooling stays with the framework it validates.
...
└────────────────────────────────────────────────────────────────┘
   ...
   │ check binary  │ │ lsp (future)  │ │ pre-commit    │
   │ CI + local    │ │ Cursor LSP    │ │ hook          │
   │               │ │               │ │ Best effort   │
   └───────────────┘ └───────────────┘ └───────────────┘
              │
              ▼
...
- `check` binary that scans the **framework repo root** (`plugins/`, `adapters/`, `docs/`, …) for CI and local use — not the `tooling/` subtree alone.
...
- Minimal JSON output for `check` once rule ids and finding locations are stable enough to fixture.
- `scripts/run-tool` shell entry point that auto-builds tooling binaries so `make check` and hooks never require a manual `cargo build`.

Follow-on scope in the same workspace, after `check` proves the dependency and output model:
...
- PR annotation helpers or a composite GitHub Action wrapping `check --format json`.
...
- **Tooling root** — `augentic/specify/tooling/`; what Rust contributors build. Contains the Cargo workspace, framework-only schemas, binaries, and hook shims. Nothing author-facing lives here except contributor docs that point back at the framework root.
...
│   └── run-tool                            # shell: resolve/build/run tooling binaries (survives Deno retirement)
...
│   ├── tools/
│   │   ├── check/                # binary: CI + local
│   │   │   ├── Cargo.toml
│   │   │   └── src/main.rs                 # ~100 LOC dispatcher
│   │   └── docgen/                         # follow-on: envelope doc regeneration
│   │       ├── Cargo.toml
│   │       └── src/main.rs
│   └── hooks/
│       └── pre-commit                      # shell shim → scripts/run-tool check -- --repo .. --changed
└── .github/
    ├── actions/check/            # follow-on: PR annotations
...
The library crate is **`rules`** (`tooling/crates/rules/`) and the CI/local binary is **`check`** (`tooling/tools/check/`). The `framework-` prefix from earlier drafts is dropped because the `tooling/` workspace already scopes these crates away from the operator product; short names match sibling tools (`accept`, `docgen`). This is **not** `specify check` or `specify review` — those surfaces validate consumer projects on the operator binary (rejected in §Alternatives and reserved separately in RFC-28 / roadmap RM-10). Contributors disambiguate `check` from `make check`, `scripts/check.ts` (during Deno overlap), and `cargo check` by context: day-to-day invocation is always `./scripts/run-tool check` or `make check`, never a global install.

**Broader renaming.** This RFC is canonical for the shortened names. Landing the workspace must update every cross-reference that still says `framework-rules`, `framework-check`, or `framework-lsp`: [RFC-1](done/rfc-1-cli.md), [RFC-4](future/rfc-4-dsl.md), [RFC-10](done/rfc-10-skills.md), [RFC-13](done/rfc-13-extensibility.md), [RFC-28](next/rfc-28-codex-rules.md), [RFC-30](next/rfc-30-init.md), [roadmap RM-16 / RM-07](roadmap.md), [docs/contributing/checks.md](../docs/contributing/checks.md), and the `.github/actions/check/` composite (not `.github/actions/framework-check/`). Module paths in prose become `rules::codex` rather than `framework-rules::codex`. No rename is required in `specify-cli` — the operator binary is unchanged.
...
A ~100 LOC clap dispatcher with four modes. **`--repo`** names the framework root to scan; it defaults to the parent of the workspace directory when the binary is invoked from `tooling/`. Day-to-day callers at the framework root use `scripts/run-tool` or `make check` instead of invoking Cargo or the binary path directly — see §*Invisible entry points and auto-build*.
...
cargo build --manifest-path tooling/Cargo.toml -p check
tooling/target/debug/check --repo ..
cargo run --manifest-path tooling/Cargo.toml -p check -- --repo ..
```
...
This is a sibling workspace tool, not part of the linter core. It ports `tests/cross_repo.ts` and its `tests/lib/` helpers into a Rust integration crate that:
...
This is a sibling workspace tool, not part of the linter core. It ports `scripts/gen-envelope-doc.ts`. Local and CI callers use `scripts/run-tool docgen -- …`; tooling contributors may invoke the binary in `tooling/target/debug/` directly.
...
The pre-commit hook is part of the first `check` rollout; PR annotations are deferred until JSON findings have fixtures:
...
- **`.github/actions/check/`** — a follow-on composite action that runs a release-built `check --repo . --format json`, parses the envelope, and posts PR annotations via `actions/github-script`. CI builds the binary once per job, then invokes it directly (see §*Invisible entry points and auto-build*).
...
| Tooling contributors | `cd tooling && cargo test`, `cargo build -p check`, … | Yes |
...
A small shell script at the framework root (alongside the retiring Deno scripts) is the **single auto-build entry point** for every tooling binary:

```bash
# scripts/run-tool <package> [-- forwarded-args...]
# 1. Resolve tooling/target/debug/<package> (or release when TOOLING_RELEASE=1).
# 2. Run cargo build --manifest-path tooling/Cargo.toml -p <package>
...
- run: cargo build --manifest-path tooling/Cargo.toml -p check --release
- run: tooling/target/release/check --repo .
  env:
    SPECIFY_CLI_ROOT: specify-cli
```

The same pattern applies to `docgen --check` and `cargo test` once the Deno harness is gone. `tooling/target/` is never committed.
...
- **Local gate** — `make check` auto-builds and runs `check`; first run after clone compiles once, subsequent runs reuse the cached binary until tooling sources change.
...
- **`cargo install check`** — adds global install/update ceremony; repo-local binaries via `scripts/run-tool` are simpler.
...
2. **Rule-engine scaffold.** Land `tooling/Cargo.toml`, `tooling/rust-toolchain.toml`, `scripts/run-tool`, `rules`, and `check` crates that compile and run a no-op full scan against `--repo ..` via `make check`. CI still runs Deno; Rust is allowed to be empty. Mechanical, self-contained PR.
...
4. **Stabilize `check` output.** Add `--format json`, rule-id filters, and the best-effort `--changed` mode only after several modules have stable ids and locations. Keep CI on full scans; use `--changed` only for pre-commit speed.
...
6. **Port `docgen`.** Once the workspace dependency story is proven by the rule engine, port the small envelope-doc generator and delete `scripts/gen-envelope-doc.ts`.
...
- **RFC-4 (typed skill expression).** Option 1 (framework-tooling skill validation) is satisfied by the schema-first pass plus the `skill_frontmatter` / `skill_body` modules in `rules`. The "CLI" in that RFC is reinterpreted as `check`, not `specify check`. Options 2 and 3 are unchanged.
...
- **Roadmap RM-16.** The roadmap entry tracks this RFC's framework dev-tooling workspace: schema-first authoring feedback, `check`, `accept`, `docgen`, and Deno retirement. The unblocks line (RFC-4 Option 1, declared-WASI-tool helper migration) is unchanged.
...
**WASI rules as the day-one shape.** Considered: ship `check.wasm` declared via an adapter manifest and invoked through `specify tool run check`. Reuses the Vectis pattern and opens third-party rule packs immediately. Rejected as overkill when there is exactly one rule pack and one consumer (CI); the library shape adopted here makes future WASI exposure a small refactor, not a re-architecture.
...
- [Specify CLI handler-shape contract](https://github.com/augentic/specify-cli/blob/main/docs/standards/handler-shape.md) — JSON envelope shape `check --format json` mirrors.
