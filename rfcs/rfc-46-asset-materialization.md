...
5. **Phase 0 (prerequisite):** close a pre-existing workflow footgun by making platform-bootstrap slice insertion automatic on `specify plan propose --from` and centralising shell-presence detection in a **vectis-owned shared library** (same semantics as `vectis verify --mode detect`; host callers link in-process — they do not spawn the vectis WASM component). Non-Vectis targets (e.g. Omnia microservices) are unaffected.
...
- **CLI owns determinism.** Materialization, catalog emission, shell-presence detection, and bootstrap validation are deterministic host operations backed by vectis-owned logic. Shell writers copy pre-validated outputs and emit view code; they do not convert formats or invent icons. Crux shell layout heuristics (`shared/`, `iOS/`, `Android/`) are **implemented once** in a vectis-owned library (`specify-vectis-shell-detect`); `specify-workflow` must not duplicate them and must not spawn vectis WASM for plan-time detect.
- **Automatic bootstrap inference (Phase 0).** When `propose --from` runs for a project with declared `platforms`, the CLI always runs vectis shell-detect semantics (in-process for Vectis-bound projects) and inserts bootstrap slices (`app-foundation`, `bootstrap-<platform>`) for absent shells. Operators do not pass a separate reconcile flag. `specify tool run vectis -- verify --mode detect` remains the operator-facing JSON probe; it wraps the same library behind the WASM CLI.
...
| **2** | `plugins/spec/skills/plan/SKILL.md` still mandates `propose --from … --reconcile-platforms` and claims vectis detect runs on propose — but `propose.rs` today gates reconciliation on the flag and calls workflow `detect_missing_platforms`, not vectis-owned detect. `plan-propose.md` omits bootstrap reconciliation entirely. An interim `rfc-46` branch incorrectly wired propose through vectis WASM dispatch (CI breakage). | **Phase 0 code:** remove the flag; always reconcile when `project.yaml.platforms` is non-empty; route presence detection through the **vectis-owned shell-detect library** (in-process) for Vectis-bound projects — not `specify tool run vectis` from propose/plan validate. **Phase 0 docs:** update the plan skill submit step and `plan-propose.md` to describe default-on reconciliation and vectis shell-detect — no flag, no workflow heuristics, no plan-time WASM. Add a `CORE-*` or `cli-contract` framework check if prose reintroduces `--reconcile-platforms` after removal. | **0** |
...
| **Vectis owns shell detection (library + thin WASM wrapper)** | Extract Crux presence heuristics into `specify-vectis-shell-detect` (host workspace crate). `wasi-tools/vectis` `verify --mode detect` wraps the library behind JSON; **host** `propose` / `plan validate` link the library in-process for Vectis-bound projects. Delete duplicated `detect_missing_platforms` from `specify-workflow`. Heuristics: `shared/src/app.rs`, `iOS/**/*.swift`, `Android/**/*.kt`. |
| **Workflow keeps DAG insertion only** | `Plan::reconcile_platforms` remains in `specify-workflow`: prepend `app-foundation` / `bootstrap-<platform>`, wire `depends-on`. No target-specific presence rules and **no WASI dispatch** in the workflow crate for detect. |
...
**Deliverables:** `specify-vectis-shell-detect` shared crate; vectis `verify` refactored to call it; propose handler + clap flag removal; revert any host WASM dispatch added for plan-time detect; workflow `platforms.rs` heuristic deletion; propose/plan-validate wired to the library (Vectis-bound gate only); propose integration tests use tempdir fixtures only (no `cargo make vectis-wasm`, no `tools.yaml` for bootstrap); §10 remediations **#2–#4** (plan skill, `plan-propose.md`, `DECISIONS.md` / `AGENTS.md`, eval run records, `tests/workflow/propose.rs`); optional `cli-contract` guard against reintroduced `--reconcile-platforms` prose.

#### Detect architecture (normative)

Shell-presence detection is **pure filesystem logic** today. The correct layering:

| Layer | Responsibility | Tests |
|-------|----------------|-------|
| `specify-vectis-shell-detect` | Single source of truth for Crux layout probes | Unit tests with tempdirs in the crate; no WASM |
| `wasi-tools/vectis` `verify --mode detect` | JSON envelope + CLI args; calls the library | `wasi-tools/vectis` crate tests + `tests/cli.rs` in the carve-out CI job |
| `specify-workflow` `vectis_missing_platforms` | Vectis-bound gate + map `Platform` ↔ library output | Unit tests with tempdirs; no WASM |
| `specify plan propose --from` | Calls `vectis_missing_platforms`, then `reconcile_platforms` | `tests/workflow/propose.rs` bootstrap cases — filesystem fixtures only |
| `specify-tool` / Wasmtime | Operator `tool run`, catalog infer, lint `kind: tool`, framework lint | Separate from plan-time detect; optional smoke may skip when vectis WASM absent |

**Anti-patterns (do not ship):** spawning vectis WASM from `propose` or `specify-workflow`; hard-`assert!`ing `target/vectis-wasi-tools/release/vectis.wasm` in propose/bootstrap CI tests; requiring a unique CI build step for plan reconciliation.
