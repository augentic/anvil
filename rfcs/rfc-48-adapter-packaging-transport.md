...
`specify lint framework --framework-root .` already parameterises the framework root, so pointing the framework checkers at the adapters repo is configuration, not new machinery — the repo-split discipline extends to the second repo (the platform repo is the consolidated core + CLI per [RFC-49](rfc-49-repository-topology.md)).

**Migration is sequenced and designed.** The move is in scope (Phasing step 4), executed as a clean cut once [D12](#shared-content-dependencies-d12) lands — consistent with *"no migration framework — pre-1.0 this is a re-init major cut"*. The mechanics:
...
- **The move carries prose + tool crates + the `vendor` / `build` / `pack` machinery; the publish job is stood up in the new repo (D6).** The `release.yaml` adapter and shared-content publish jobs and the `lint framework` job relocate to — or split across — `augentic/specify-adapters`; the `--framework-root` seam makes the lint side configuration, the publish side a job move.
- **Namespace continuity holds.** The publish ref (`specify:<name>@<ver>`, D6/D8) names a registry namespace, not a source repo, so a published artifact's identity is unchanged by the source move — proven by a pull-back (D6) from the new origin. The cross-repo `rg` discipline and the AGENTS.md / DECISIONS.md pointers stay two-way — platform ([RFC-49](rfc-49-repository-topology.md)) ↔ adapters — in the same change.
- **Timing of the physical cut.** The default is the **earliest safe point** — immediately after D12 (Phasing step 4). A team may instead defer the cut until the full pipeline (D4–D5) is proven in `augentic/specify` and relocate once, or scaffold `augentic/specify-adapters` early and migrate incrementally under transitional coupling (e.g. when separate owners or an external contribution cadence force the repo to exist sooner). Whichever variant is chosen should be recorded here when the work starts, so the transitional coupling is a justified decision rather than latent churn.
...
1. **Transport spike — publish + pull + verify.** Probes `wkg publish`, `wasm-pkg-client` pull, and re-verify-on-read for an opaque blob; confirms (B) as the working default or clears (A). Smallest first step; everything downstream keys on it.
2. **D2 — package-ref form + immutable locator.** Teach `adapter_uri.rs` the `specify:<name>@<semver>` form and require an immutable fetch.
3. **Self-contained artifact + authoring structure (D3 / D9 / D10 / D11 / D12), decomposed into three independently-testable steps so the riskiest work is not one cut:**
   - **3a — manifest + tool unification (D11).** Rewrite the `toolDeclaration` `$def` (drop `version`, structure `permissions`), unify `AdapterToolDeclaration` with `ToolPermissions`, retire the `tools.yaml` reader, and replace the `adapter-tool` cross-reference rule. Pure schema/type work; lands with no transport change.
   - **3b — `requires` + `vendor` for prose deps (D9 / D12).** Publish shared content (`spec-runtime`, `core-rules`, `review-team-protocol`) as versioned artifacts, teach `adapter.yaml.requires` + `adapter.lock`, and write the gitignored reserved trees with `specify adapter vendor` (no toolchain). This is the step that severs the `adapters/shared/` coupling — validate it before D10.
   - **3c — co-located tool crates + `build` (D10) + bundling (D3).** Move the contract/vectis crates beside their prose, add `specify adapter build` (the only toolchain step), commit the digest-pinned `module.wasm`, and bundle it at publish. Retires the CLI `wasi-tools` job (D7).

   Required before any digest is stable or any store entry is shareable. The dedicated adapters repo is **not** part of this phase — it is the next one, unblocked by 3b severing the `adapters/shared/` coupling (see [Co-located tool source (D10)](#co-located-tool-source-d10)).
4. **Extract adapters to `augentic/specify-adapters` (D7 / D10).** Earliest safe point: 3b has severed the `adapters/shared/` coupling and 3c has co-located the tool crates. Relocate the adapter trees (prose + tool crates + the `vendor` / `build` / `pack` machinery), stand up the two-repo contract (platform ↔ adapters; the platform is the consolidated core + CLI per [RFC-49](rfc-49-repository-topology.md)), and move the framework-lint job (`--framework-root`) and publish credentials to the new repo; remaining phases land in the new home. Mechanics under [Co-located tool source (D10)](#co-located-tool-source-d10).
5. **D4 — verify on read.** Record the registry digest at install; re-verify on read. Tamper-evident now that the artifact is self-contained.
6. **D1 + D6 — packaging + publish tooling.** Pack the tree and stand up the publish job (in `augentic/specify-adapters`), in the shape the spike chose.
7. **D5 — shared store + projection.** The dedup/offline win, once identity is immutable and the install tree is byte-stable.

D5 must follow D2–D4 (sharing is only correct once identity is immutable and the install tree is self-contained), and the repo extraction (step 4) must follow D12 (step 3b). All of this keys on [RFC-47](rfc-47-adapter-identity.md)'s semver identity, which can land first and independently.
...
- **Extract adapters into a dedicated repo *before* D12.** Rejected — relocating `adapters/` while `spec-runtime` is still a symlink hub into `plugins/spec/references/` + `docs/reference/` ([D12](#shared-content-dependencies-d12)) either dangles those symlinks or drags spec-plugin prose into the wrong repo. The extraction is committed (D7/D10, Phasing step 4) but sequenced *after* D12 turns shared content into versioned `requires` artifacts the new repo resolves by identity.
