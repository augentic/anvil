...
| Adapter components | `specify:<name>@<ver>` | `specify-adapters` release workflow (RFC-64, as landed) | `install_tofu` hydration into the RFC-65 store |
| Core guest | `specify:core@<binary version>` | `specify` release workflow, same tag as the binary | hydrated at init, or embedded (RFC-65 move 4) |
...
- **Consume auth is nothing.** First-party packages are public GHCR packages; TOFU pull works on a fresh laptop or CI runner with zero configuration, which is what makes the hydration kernel's "non-interactive, credentials from the environment" property (RFC-65) trivially true.
...
1. **Publish `specify:core@<tag version>`** — build the core guest from the tagged tree to `wasm32-wasip2` and `wkg publish` it, idempotent like the adapters loop. The job runs whether or not the binary embeds its core (RFC-65 move 4 owns the two resolution modes and the deletion of the committed guest artifact), keeping the pulled path universal; the "refresh the embedded guest" release-checklist item dies with the committed blob.
...
The operator never learns the registry exists. The whole journey is the two commands of RFC-65 §"Operator onboarding" — `brew install augentic/tap/specify`, then `specify init` — with everything wasm (the core guest and the adapters) arriving transparently through RFC-65's hydration kernel and store. This RFC changes nothing in that design; it supplies the registry backing that makes it work on a fresh machine, and the tap that makes the first command exist.
...
- **Version-range resolution.** Exact pins everywhere (RFC-65's determinism boundary). "Latest" exists only at human decision points: `specify upgrade`'s release probe and `init` choosing a pin. RM-21 keeps ownership of ranges and floors.
...
- **Hydration mechanics, the naming cut, the embed mechanism, and the core guest's existence** — RFC-65's (the kernel, store root, lock, and `--frozen` included); this RFC lands the surrounding publish/acquire plumbing and sequences after it.
...
- **Store garbage collection** — unchanged from RFC-65's deferral.
