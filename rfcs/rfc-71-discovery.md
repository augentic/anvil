...
Installation is already solved: a pinned identity pulls from the compiled first-party GHCR mapping on miss ([RFC-70](rfc-70-deployment.md) Stage 3), and the mapping accepts any kebab-case name. What is missing is the fact that decides *which* pin to install. Today a source adapter's whole metadata record is `specify-floor`, so nothing on the wire says a component reads TypeScript or builds mobile shells.
...
Illustrative shape, authored beside each adapter in `augentic/specify-adapters`:

```yaml
# sources/typescript/descriptor.yaml
axis: source
name: typescript
specify-floor: 0.28.0
inspects:
...
The closed `workload-kind` vocabulary is the join key between a repository profile ([RFC-72](rfc-72-migration.md)) and a target's `produces` ([RFC-74](rfc-74-program.md)). Every value below is grounded in existing adapter grammar rather than speculative:
...
| D3 | Restated fields (`specify-floor`, and any future field appearing in both) are advisory in the index and authoritative in `metadata`; the adapter publish gate asserts the two agree. | Drift is caught at publish in [RFC-77](rfc-77-release-process.md), not at an operator's resolve. |
...
| D6 | `specify-floor` participates in filtering. | A candidate the running host cannot dispatch is excluded with that reason up front, instead of being recommended and then failing at the metadata gate. |
| D7 | Discovery emits an immutable recommendation report and stops. It never installs, binds, or writes config. | Approval is the consumer's gate — plan-time Gate 1 for sources ([RFC-72](rfc-72-migration.md)), Gate M1 for targets ([RFC-74](rfc-74-program.md)). |
...
| D9 | The first index covers the `specify:` namespace only. | Third-party descriptors, publisher identity, and namespace trust stay Stage 3, gated on roadmap RM-21 rather than on the migration program. |
