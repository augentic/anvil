...
- **`references`** — the adapter shelf (`resolve(id) -> bytes`, [RFC-51](rfc-51-adapter-wit.md)): a real guest export served from prose embedded in the module at build time, not a `wasi:filesystem/preopens` fallback.
