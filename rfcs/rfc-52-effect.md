...
- The `working-tree` capability over `wasi:filesystem` — a host-materialized mutable project tree (a `descriptor` for guest code, and a host-reported `local-path` for the agent `eval` backend) — and the content-addressed `change-set` records that carry a build's result across nodes to `merge`.
...
- **The agent path is a bounded escape hatch:** the host-provisioned node-local path (`local-path`) deliberately exposes a real OS path to the filesystem-capable `eval` backend, because an external agent cannot hold a Wasm `descriptor`. It is scoped to one node and one operation and bracketed by the materialized tree in and the `change-set` out — not a reintroduction of the global shared disk.
