...
2. **Domain types.** Add `MigrationLog`, `MigrationEntry`, `MigrationOverride` types in `workflow` (`crates/workflow/src/migration_log/`). Mirror the `Registry` posture: `serde(deny_unknown_fields)`, `path()` / `load()` / `append()` helpers, byte-stable sort.
