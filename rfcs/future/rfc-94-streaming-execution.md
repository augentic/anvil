...
This RFC adds a higher-level automation runner. It reuses RFC-88 planning revisions, RFC-91 refinement manifests, RFC-92 work items, RFC-90 build records, and the existing slice lifecycle. It adds no Progressive or Approved state.
...
RFC-94 lands as a forward patch. It does not revise the implemented RFC-86 text or the in-flight RFC-88 text.
...
- A **run policy** is a closed deployment-owned profile that authorizes unattended candidate work through `built`. Its digest enters every admission and result. RFC-98 defines the stricter promoted autonomy-policy kind required for `merged`.
...
Each published leaf becomes an RFC-92 Phase-A `refine` work item keyed by `(slice, refine, input-digest)`. Independent survey, decomposition, extract, and refinement calls share the bounded local pool.
...
The RFC-94 run-policy class is `candidate`. RFC-98 extends the closed class enum with `low | moderate | high | critical` and records the selected autonomy risk profile in the compiled member-policy digest.
...
4. compose only disjoint same-base member patches through RFC-92's deterministic composition kernel;
...
RFC-94 amends RFC-86 D27 and RFC-90's wave-only envelope: `built` projects from a successful `BuildRecord` plus its matching `target.wave.opened | target.candidate-batch.recorded` fact. Merge readiness still requires a current target wave and accepted dependency frontier.
...
[RFC-95](../rfc-95-native-verification.md) may provide host-attested profiles. Until required profiles exist, policy reports the actual model-assisted assurance and may stop rather than claim stronger evidence.
...
Unattended accepted-CID mutation is deferred to [RFC-98](../rfc-98-policy-gated-autonomy.md). Plan closure, completed builds, claims, elapsed time, or a passing model review never imply merge authority.

## Plan-run facts

RFC-94 replaces neither per-writer journals nor operation facts. It adds:
...
- Land RFC-92 Phase-A work items, bounded pool, cancellation, and local claims.
...
- Extend the RFC-92 scheduler with candidate batches and candidate dependency frontiers.
- Add `--through built --policy <profile>` and reject policy-free unattended build.
- Extend the `BuildRecord` authorization anchor to `wave | candidate-batch`; reuse RFC-90 reports and RFC-92 same-base composition without writing accepted domain rounds or waves.
...
- **Wait for all of RFC-92 before refining progressively.** Refinement needs the work-item scheduler and read-heavy pool, not target task graphs, composition, domain rounds, or multi-member commit.
...
- **Let passing builds imply merge authority.** Build evidence and accepted-state authority are separate. RFC-98 owns any future policy-gated merge.
