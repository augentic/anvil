...
> Status: Deferred · Depends: [RFC-28](done/rfc-28-standards-contract.md), [RFC-32](done/rfc-32-standards-enforcement.md), [RFC-33a](rfc-33a-standards-finding-suppression.md) · Enables: [roadmap RM-10](roadmap.md#rm-10-ci-native-standards-enforcement) (when triggered), [roadmap RM-14](roadmap.md#rm-14-local-structured-workflow-events)

## Abstract

[RFC-33a](rfc-33a-standards-finding-suppression.md) delivers in-source ignore directives, the initial `status` enum widening (adding `ignored`), the `disposition` field with the `directive?` sub-field, and the `lint-completed` journal event. It does not deliver the **cross-run** lifecycle: how a project acknowledges a body of legitimate findings as baseline debt, how scans diff against prior runs, and how operators stage remediation across releases.
...
4. **CLI surface** — `specrun lint baseline {write, drop, diff}` subcommands plus `--no-baseline` / `--baseline <path>` flags on `specrun lint`.
5. **Status enum widening** — widens `schemas/review/finding.schema.json` a second time, adding `new` and `baselined` to the `status` enum (extending RFC-33a's initial widening) and the optional `disposition.baseline` sub-field. Also supplies producers for the pre-existing `fixed` and `accepted` values inherited from RFC-28.
...
[RFC-33a](rfc-33a-standards-finding-suppression.md) §"Motivation" enumerates three pressure points after RFC-32 lands. The first (intentional exceptions need to live with the code) is addressed by RFC-33a's ignore directives. This RFC addresses the remaining two:
...
The principles that govern this layer are stated in full in [RFC-33a](rfc-33a-standards-finding-suppression.md) §"Principles". RFC-33b activates the two that were dormant while it was deferred:
...
- **(6) One writer per file.** `.specify/lint/baseline.json` is only written by `specrun lint baseline write`. `.specify/lint/last.json` is only written by `specrun lint`. No skill, no agent, no other CLI verb edits either file.
...
| --- | --- | --- |
| **D1 Baseline file** | A scan-profile-scoped baseline lives at `.specify/lint/baseline.json`. | New `schemas/review/baseline.schema.json`; new `Baseline` DTO in `specify-domain`; `specrun lint` loads and matches by `fingerprint`. |
...
| **D7 Last-run persistence** | `.specify/lint/last.json` holds the previous run's envelope verbatim. | One additional write at scanner exit; one new schema `schemas/review/run.schema.json` (same shape as live emission). |
...
The decision IDs (D1, D2, D7, D9, D10, D11) are preserved from the original combined RFC-33 so prior references continue to resolve. [RFC-33a](rfc-33a-standards-finding-suppression.md) owns D3, D4, D5, D6, D8, D12, D13.
...
| `specrun lint --target omnia` | `baseline.json` only | `baseline.json` |
| `specrun lint --target omnia` | `baseline.json` + `baseline.omnia.json` | `baseline.omnia.json` only — per-target supersedes project-wide |
...
```bash
specrun lint --no-baseline                    # ignore every baseline file regardless of suffix
specrun lint --baseline path/to/file.json     # explicit override; bypasses the §"Baseline file" matrix
specrun lint baseline write                   # consume .specify/review/last.json and write the baseline
...
- `specrun lint baseline write` reads `.specify/lint/last.json` rather than re-scanning (D11). It refuses with `review-baseline-write-stale` when `last.json` is missing, or when its mtime is older than the most recent file mtime under any scoped artifact path. Operators recover by re-running `specrun lint` or by passing `--rescan` to capture and persist a fresh envelope inline.
...
| File | Status | Owner |
| --- | --- | --- |
| `schemas/review/baseline.schema.json` | New | RFC-33b |
| `schemas/review/run.schema.json` | New | RFC-33b (matches the live envelope shape) |

RFC-33b widens `schemas/review/finding.schema.json` a second time: the `status` enum adds `new` and `baselined`, and the `disposition` object gains the optional `baseline?` sub-field. The widening is additive and the fingerprint algorithm is unchanged.

### Implementation plan

1. **Schemas.** Add `schemas/review/baseline.schema.json` and `schemas/review/run.schema.json`. Widen `schemas/review/finding.schema.json` additively: extend the `status` enum with `new` and `baselined`, and add the optional `disposition.baseline` sub-field.
2. **Domain types.** Add `Baseline`, `BaselineEntry`, `ReviewRun` DTOs to `specify-domain`. Reuse RFC-28's canonical-JSON helper.
...
5. **CLI surface.** Add `specrun lint baseline {write, write --append, write --rescan, drop, diff}` subcommands and the `--no-baseline` / `--baseline <path>` flags on `specrun lint`. Enforce the §"Baseline file" selection matrix, the `last.json` staleness check from D11 (`review-baseline-write-stale`), and confirm-or-`--yes` discipline on every write.
...
**For operator projects:** Additive. Without a baseline file, behaviour is identical to [RFC-33a](rfc-33a-standards-finding-suppression.md). To adopt baseline mode, run `specrun lint` (which writes `.specify/lint/last.json`) then `specrun lint baseline write`, review the file in PR, and commit.
...
Q1, Q2, and Q4 from earlier drafts have been resolved into normative decisions D10, D11, and D12 (the last lives in [RFC-33a](rfc-33a-standards-finding-suppression.md)) respectively.
...
- [RFC-33a: Standards Ignore Directives](rfc-33a-standards-finding-suppression.md) — companion RFC, active; provides the wire-shape decisions consumed here
