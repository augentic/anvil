...
- v1 parsing uses the `regex` crate (already a workspace dependency). No new dependencies were added. A JS/TS AST parser would be overkill for the canonical patterns the first three detectors need; the regex approach keeps the binary small and is consistent with the RFC's "purely mechanical" framing. Re-evaluate if a future detector hits a pattern regex cannot handle cleanly.
- The validator uses strict lexical string sort on `declared-at[]`. Entries like `src/server.ts:9` sort after `src/server.ts:42` lexically; detectors emit in file-walk order, which is deterministic but not numerically sorted within a single path. This is consistent with Change A's validator and documented in code.
- `@MessagePattern` / `@EventPattern` support is present in the `NestJsDetector` but the v1 test fixture does not exercise microservices decorators (no `@nestjs/microservices` dependency in the fixture). The detector code path exists and is documented; the fixture deferral is intentional per plan scope.
- For inline Express handlers, `touches` is limited to the declaration file. For named handlers, `touches` walks the handler file's import graph via BFS (relative imports only, `node_modules` excluded). This is a v1 simplification; function-level call-graph tracing would require an AST parser.
...
- Add the stub at `docs/explanation/legacy-migration-at-scale.md` referencing RFC-21 / RFC-22 as the eventual home for cross-change scale concerns (per RFC §"Implementation Plan" step 8). **Amendment (Change H):** `make checks` enforces a no-RFC-citation rule on user-facing docs, so the stub points to the Decision log instead of naming RFC-21 / RFC-22 directly. The RFCs remain the canonical design homes; the indirection keeps the docs predicate clean.
