# RFC-23: Replace `/change:plan` with `/change:draft` + `/change:finalize`
...
2. **Brief scaffold** — `specify change create <change-name> [--source <key>=<path-or-url> ...]` when `change.md` is absent. Writes `change.md` and `plan.yaml` together (atomic refusal if either already exists). Skipped under `extend`.
...
| --- | --- |
| 1 Brief (`specify change create`) | `/change:draft` step 2 |
...
| Invocation | `/change:draft <name> [from/against/source/extend/dry-run]` | `/change:execute [supervised|dry-run|loop]` (unchanged) | `/change:finalize <name> [dry-run]` |
...
| Owns on-disk state? | No (writes via `specify change create` / `specify plan {add, amend}` only) | No (unchanged) | No (writes via `specify workspace push` / `specify change finalize` only) |
...
The umbrella mode and the planner verb both go away. The migration paths differ because the substitutions are not byte-equivalent.

**`/change:plan <name>` (default mode → `/change:draft <name>`)**: byte-equivalent substitution. For one release window, `/change:plan <name>` prints a deprecation notice on stderr and forwards to `/change:draft <name>` with the same positional inputs (`from`, `against`, `source`, `extend`, `dry-run`). The forward exits with the new skill's exit code unmodified.

```text
deprecated: /change:plan is deprecated; use /change:draft.
            forwarding to /change:draft; this forward will be removed in <release>.
```

**`/change:plan <name> orchestrate` (umbrella → no equivalent)**: not byte-equivalent. The umbrella ran straight from validate to execute to finalize; the new design requires a human review pause and a deliberate `/change:execute` invocation. There is no automatic forward. For one release window, `/change:plan <name> orchestrate` runs `/change:draft <name>` (the equivalent of the umbrella's first three steps), then prints a hard-error notice naming the new lifecycle:

```text
removed: /change:plan orchestrate is removed in <release>.
         the new change lifecycle requires explicit operator review between
         authoring and execution. drafted plan.yaml is at <path>.

         next steps:
           1. review plan.yaml; edit with `specify plan amend` if needed
           2. /change:execute loop
           3. /change:finalize <name>
```

Operators who scripted `/change:plan ... orchestrate` in CI need to either (a) split into the three new commands with an explicit review step (or scripted no-op review), or (b) write a thin shell wrapper that runs all three. The framework does not ship the wrapper because automatic execution after planning is exactly the property this RFC removes.

After the named release, both the `/change:plan` forward and the `/change:plan orchestrate` notice are removed; the verb is gone.

The `specify change` CLI verbs are untouched. `specify change create` (called by `/change:draft`), `specify change show`, and `specify change finalize` (called by `/change:finalize`) keep their current shapes.
...
3. **Add the `/change:plan` deprecation forward.** When invoked without the `orchestrate` positional, print the deprecation notice on stderr and exec `/change:draft <name> [...]` with the same positional inputs. When invoked *with* `orchestrate`, run `/change:draft <name>` and print the removed-orchestrate notice naming the new lifecycle. Add regression fixtures asserting both notice texts and exit-code passthroughs.
4. **Delete the orchestrate mode internals.** Remove `plugins/change/skills/plan/orchestration.md`, `references/re-entry.md`, `references/shapes.md`, and any other plan-skill files specific to the umbrella. The `/change:plan` shim only carries the deprecation/removal forwards above; it never re-implements the umbrella sequence.
5. **Mark the `/change:plan` shim for removal.** Tag the shim's SKILL.md with the release in which it will be deleted. After that release, remove `plugins/change/skills/plan/` entirely.
...
8. **Update cross-RFC references.** RFC-20 (`rfcs/rfc-20-survey.md`) currently references `/change:initiate` and `/change:plan` interchangeably; rewrite those references to `/change:draft`. The `## Pipeline ordering` table's step numbers stay the same; only the owning skill name changes. Other RFCs that mention `/change:plan` or `/change:plan orchestrate` get the same treatment (audit during implementation).
9. **CHANGELOG.** Add an entry recording the verb rename (`/change:plan` → `/change:draft`), the new `/change:finalize` skill, the removal of the `orchestrate` mode, the deprecation window, and the release in which the forwards are removed.
10. **Acceptance.** Add a regression to the cross-repo Deno acceptance suite asserting that the three-skill flow produces the same merged-PR outcome the umbrella did, given the same inputs and an automated review step (no-op `specify plan status` between draft and execute). Add a separate fixture asserting the `/change:plan orchestrate` removal notice appears and exits non-zero in the named release.
...
- **Replace `/change:plan <name> orchestrate [...]`** with the three-skill flow:
  ...
  2. *(review `plan.yaml`; edit with `specify plan amend` if needed; abort by running `specify change finalize --abort` or by deleting the change brief)*
...
- For one release window, `/change:plan` keeps working for the default-mode case (with a deprecation notice) and `/change:plan orchestrate` runs draft + prints a removal notice. After that release, both are gone.
...
The breaking changes (removal of `/change:plan`, removal of the `orchestrate` umbrella) land in the release named by the deprecation notices.
...
**Make `/change:draft` a pass-through wrapper around a new CLI verb `specify change draft`.** Rejected. The composition discipline is the same as the rest of the change layer: the skill is the orchestrator over Layer 1 CLI verbs and per-capability briefs; a new top-level CLI verb would duplicate the brief pipeline outside the skill surface.
...
**Drop `/change:plan` and `orchestrate` immediately without a deprecation forward.** Rejected. Tutorials, operator scripts, and CI pipelines reference both invocations; a hard removal in the same release as the rename would break consumers. One release window with deprecation notices (and a hard-error notice for the umbrella) is the minimum acceptable courtesy; longer windows are an open question (see §Open Questions).
...
- **Removing `/change:plan` and `orchestrate` immediately.** The deprecation forward (default mode) and removal notice (`orchestrate`) stay for one release window so operator scripts and tutorials migrate without breakage.
- **Renaming `specify change create` or `specify change finalize` (the CLI verbs).** The CLI surface stays stable; only the slash skills change.
...
1. **Deprecation window length.** This RFC proposes one release window for both `/change:plan` (forward) and `/change:plan orchestrate` (run-draft-then-error). Is one release enough given how many tutorials, operator scripts, and CI configurations reference the old invocations? Current preference: one release, with the deprecation notices naming the removal release explicitly so consumers have a hard date to migrate by. Revisit only if usage telemetry shows the window is too short.
2. **`/change:plan orchestrate` removal handling.** The RFC proposes "run `/change:draft`, then print a hard-error removal notice with the new lifecycle." Alternatives: (a) hard-error immediately without running draft, forcing the operator to invoke `/change:draft` manually; (b) run all three skills with a synthetic no-op review step, preserving end-to-end behaviour for one release. Current preference: the proposed shape (run draft + print notice) — it gets the operator partway through the new flow with a clear next-step prompt, without secretly preserving the no-pause behaviour.
3. **Should `/change:finalize` observe PR state directly, or call `specify workspace status`?** Today's umbrella step 6 uses `gh pr list` directly. Wrapping that in a CLI verb (`specify workspace status --pr` or similar) would centralise PR-observation logic and keep the skill thinner. Current preference: defer — the skill calls `gh` directly for v1, matching the umbrella's behaviour. A CLI wrap is a separate refactor.
...
5. **Should the human seam between draft and execute be enforced** (e.g., draft writes `.specify/plans/<change>/.review-required` that execute checks for and refuses to start without an explicit `--reviewed` flag)? Current preference: no — trust the operator. The seam is a recommendation backed by the lack of a one-command wrapper, not a gate. Revisit if telemetry shows operators consistently re-invoking `specify plan amend` mid-execute (which would suggest the review point is being skipped and is doing real work).
6. **Backporting tutorials in the same release.** Should the tutorial rewrites land in the same release as the new skills (so day-1 documentation is consistent), or in the *next* release (so the deprecation notices in the old invocations line up with the docs that mention them)? Current preference: same release; the deprecation notices already name the new commands so docs and notices agree.
7. **RFC filename.** This file is `rfc-23-initiate.md`, named for an earlier draft proposing `/change:initiate`. The new design proposes `/change:draft` and `/change:finalize` and removes `/change:plan`; the filename is misleading. Should it be renamed to `rfc-23-draft.md` or `rfc-23-decompose-plan.md`? Current preference: leave the filename for stability of in-repo and external references; the title accurately describes the new content. Revisit if cross-repo links to RFC-23 routinely surface confusion.
...
- [`/change:plan` SKILL.md](../plugins/change/skills/plan/SKILL.md) — the skill this RFC retires.
- [`/change:plan orchestration.md`](../plugins/change/skills/plan/orchestration.md) — the seven-step body this RFC redistributes across `/change:draft` and `/change:finalize`.
...
- [`specify change` CLI reference](../docs/reference/cli/change.md) — `specify change create` (called by `/change:draft`) and `specify change finalize` (called by `/change:finalize`).
