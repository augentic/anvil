...
The **model service** is the single materialization of the `infer` host; the fleet lives *inside* it. A call carries a `brief-path` handle and a typed `request`; the service routes it to a fleet member and returns a typed report — the guest never names a model. The service registers into Omnia's model-host slot as one `ModelBackend` ([RFC-57](rfc-57-omnia-model-host.md)) — Omnia sees a single backend; the fleet fans out within it.
