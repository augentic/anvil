...
- **Compatibility is declared and checked at resolve.** An adapter states the minimum CLI it needs (`specify`) and the loader enforces it at probe time, transport-independently — the adapter-level analog of `project.yaml.specify_version`.
...
| **D3 Host-CLI compatibility floor** | `adapter.yaml` carries an optional `specify` semver **floor** (an exact minimum — no ranges, matching D2's exact-only posture). The loader compares it against the running binary's version at resolve time and refuses an adapter that needs a newer CLI, reusing the `project.yaml.specify_version` precedent. | Adds an optional `specify` semver string to `schemas/{source,target}.schema.json` and the embedded `SOURCE_JSON_SCHEMA` / `TARGET_JSON_SCHEMA`; `SourceAdapter` / `TargetAdapter` carry the floor as `requires_specify: Option<semver::Version>` (`adapter/core.rs`, deserialized from the `specify` key); a resolve post-schema gate raises `adapter-cli-too-old` on the exit-3 `EXIT_VERSION_TOO_OLD` path (`Error::CliTooOld` family). Transport-independent — the check is identical whether bytes came from git, a registry, or a local path. See [Host-CLI compatibility floor (D3)](#host-cli-compatibility-floor-d3). |
...
- Because `SliceSourceBinding`, `plan.schema.json`, and the `resolve` signature all cross the two-repo contract, the [AGENTS.md §"Note to the implementing agent"](https://github.com/augentic/specify-cli/blob/main/AGENTS.md) `rg` sweep across both repos is mandatory in the same PR.
...
- **Optional and additive.** An absent `specify` means no floor — every adapter that omits it resolves exactly as today, so the field is back-compatible at the schema boundary.
...
| `adapter-cli-too-old` | D3 | error (exit 3) | the running binary's version is below an adapter's declared `specify` floor; reuses the `EXIT_VERSION_TOO_OLD` / `Error::CliTooOld` path |
...
- **D3** — a manifest parse test for the optional `specify` semver (and a malformed-floor rejection); a resolve test asserting `adapter-cli-too-old` (exit 3) when the binary is below the floor; a floor-satisfied test asserting a clean resolve; an absent-`specify` back-compat test asserting no floor check.
...
3. **D3 — host-CLI compatibility floor.** An additive optional manifest field plus a resolve-time gate reusing the exit-3 `CliTooOld` path; lands after D1 (it needs the semver `version`) and alongside D2's resolve work.
...
- **Defer the `specify` floor to RM-21 with the rest of the compatibility surface.** Rejected for the *exact floor* — RFC-48 extraction + RFC-49's single platform version give adapters an independent cadence *now*, so a first-party adapter can already outrun an old CLI before any third-party ecosystem exists. The exact floor (D3) is the minimum guard that makes independent cadence safe; only *range* policy and a compatibility matrix stay deferred.
...
- Semver-*range* `specify`-floor policy and a cross-version compatibility matrix — RM-21. The exact-floor `specify` guard is **D3** (same exact-only posture as D2).
...
- [AGENTS.md §"Exit codes"](https://github.com/augentic/specify-cli/blob/main/AGENTS.md) / `src/runtime/output.rs` (`Error::CliTooOld`, `EXIT_VERSION_TOO_OLD`) and `project.yaml.specify_version` — the project-level CLI floor D3 mirrors at adapter granularity.
