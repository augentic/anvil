...
- Platform hardening (CI rung, profiles, budgets, one type family) moves **ahead of** the walking skeleton — roughly 4–8 weeks of work native-only would have skipped. This is accepted as funding the foundational choice.
...
The scripted-model journey across the component seam, in CI, offline (~1 week): embed the engine guest plus mock source/target components; metadata dispatch, one survey/extract round-trip, one build phase report, one `read_doc` MCP hop; assert byte-identical results across two runs. If this lands cleanly, the commitment is cheap; if it fights, the findings feed the platform-hardening plan — either way the seam gains its first automated rung (T1).
