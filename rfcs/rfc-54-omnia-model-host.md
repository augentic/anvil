...
RFC-54 lists "a host-service capability in `augentic/omnia`" as a hard dependency and names the extension point an `judge` backend plugs into without specifying it. This RFC is that specification, and it earns its own document for three reasons:
...
- **It is the seam two Specify RFCs lean on.** RFC-54 needs *a* model host to bind `judge` to; RFC-56's fleet backends need a *contract* to register into. Specifying it once, here, keeps both honest and lets the two repos version against a named boundary.
...
| Binding `judge` during the runtime move (Specify-side adoption) | **Specify** — [RFC-54](rfc-54-omnia-runtime-move.md) |
...
There are two layers. The guest imports the Specify-owned `judge` **WIT interface** (RFC-52); Omnia's model host *implements that import* by forwarding to the deployment-bound **`ModelBackend`**:
...
pub trait ModelBackend: Send + Sync {
    /// Satisfy one guest `judge` call. `brief_path` is a HANDLE: a
...
Omnia binds exactly one `Box<dyn ModelBackend>` per deployment, selected by config — the Specify **model service** for real work, or the **replay stub** for CI — the same selection mechanism that already swaps an in-memory KV for Redis. When a guest calls the imported `judge` interface, the host adapter forwards to the bound backend, passing a `ReferenceResolver` wired to Omnia's data/KV host services and the fresh-instance path for computed refs. **The fleet lives inside the Specify backend** ([RFC-56](rfc-56-judge-fleet.md)): Omnia sees one backend and honours one-backend-per-host; the router fans out *within* it.

## Decisions to record (open until reviewed)

- **Trait shape & async.** Whether `ModelBackend::judge` is sync (baseline) with an async variant gated behind streaming/concurrency (mirrors the architecture's narrowed-async bet), and whether the contract is a native Rust trait in Omnia or itself a WIT-component boundary.
...
- **Cross-repo versioning.** How the Omnia-owned trait and the Specify-owned `judge` WIT are versioned so neither repo blocks the other.

## Phased plan

1. Add the model host as a pluggable host service in Omnia bound to the RFC-52 `judge` interface; ship the **replay backend** as the first, zero-config `ModelBackend` to prove the slot end-to-end.
...
1. `judge` is a pluggable Omnia host service bound to the Specify-owned WIT interface; a guest's `judge` import resolves to a deployment-bound `ModelBackend`.
...
6. [RFC-54](rfc-54-omnia-runtime-move.md) can bind `judge` to this host, closing its cross-repo dependency; the `augentic/omnia` workspace gate stays green, and this RFC's prose passes `make lint` here.
