...
>
> Builds on [RFC-86](rfc-86-change-facts.md)'s facts, explicit execution-authorization epoch, and per-leaf input fences, [RFC-87](rfc-87-working-trees.md)'s content-addressed trees and private workspaces, and [RFC-104](rfc-104-system-archaeology.md)'s reviewed system model and migration-wave handoff. [RFC-95](rfc-95-publication-sets.md) publishes the results; [RFC-100](rfc-100-distributed-execution.md) transports them between nodes.
>
...
2. Binding resolves exactly one current reviewed handoff, verifies its definition, system-model, migration-plan, and wave digests, then pins the wave's exact delivery repositories, CIDs, sources, and adapter versions into `discovery.yaml`.
3. `plan author` obtains any focused delivery leads the wave still needs, recursively partitions only that wave into persisted conflict domains, and projects the terminal domains into `plan.yaml.slices`.
...
1. **Bind reviewed wave** — validate the RFC-104 handoff, resolve its exact delivery targets and sources, select adapters, and write the pinned result to `discovery.yaml`.
...
Before decomposition, the engine resolves one closed, versioned model-capability profile for each target. A profile contains the complexity scoring policy and operation-specific thresholds for the configured model class. This RFC consumes its slice-split threshold. RFC-96 also consumes its task threshold. The profile is an engine input, not a model answer.
...
`emery slice merge <slice>` resolves the slice's frozen wave and refuses until every member is complete; it can never manufacture a singleton prefix. In this RFC's serial executor every wave has one member. RFC-96's concurrent executor opens deterministic bounded ready antichains and the same command may therefore commit the complete multi-member wave containing the named slice. A result from an older base outside the open wave is stale and must rebuild; it cannot be silently attached.
...
Repository-host writes remain out of scope. Create a new target on the forge when needed. RFC-95 owns push, branch, pull-request, merge, and other publication writes.
...
11. Execution ends with one accepted CID per touched target and no commit or branch; RFC-95 publishes it. Copying the change home before RFC-100 does not transport source or result tree objects.
