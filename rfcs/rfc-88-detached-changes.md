...
2. **Survey sources** — survey every pinned source into `leads.md`; RFC-96 may fan these independent calls out without changing their stable merge order.
...
- **Owning survey/extract fan-out concurrency here** — Discover-topology reads may use D9's concurrency budget; Author-slices survey parallelism is RFC-96. Mixing them would gate the product path on the scale track.
