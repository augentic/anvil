...
>
> Depends on completed [RFC-86](rfc-86-change-facts.md) (the change home is an RFC-86 fact tree: artifacts, per-actor event logs, projected status, recorded approvals) and [RFC-87](rfc-87-working-trees.md) (local slot materialization, read-only source trees, leases, immutable source snapshots, and the value↔tree boundary). Reuses [RFC-71](rfc-71-deployment.md)'s landed pull-on-miss adapter resolution.
>
...
| - | -------- | ----------- |
| D1 | **The change is the unit of location, and its home is an RFC-86 fact tree in its own git repository.** `emery change open` initializes the change repository — plan, per-actor event logs, approvals, slice artifacts committed; materialized slots beneath its gitignored scratch prefix — valid for the change's duration, deletable after finalize. There is no platform repo. | "Where does this change live?" has a one-repository answer on any machine — clone it to share it. Extended planning, multi-operator refinement, and spec review are ordinary git push / pull / PR over the change repository. Ephemeral means *disposable after finalize*, not scratch: deleting an unpushed change repository mid-change forfeits its facts, exactly as deleting any unpushed repository would. |
...
- Change-local worktrees live under the change directory and use RFC-87's shared host mirror. Removing the change directory removes no mirror or forge state.
...
- **Phase A — Detached change home.** `emery change open`, change-scoped `plan.yaml` / journal / slice artifacts in the change directory; manual member binding (operator supplies repo URLs); ephemeral slots over [RFC-87](rfc-87-working-trees.md).
...
2. Inference resolves only the selected adapter. Explicit adapter bindings bypass matching, while both paths pass RFC-87's canonicalization, read-only grant, symlink, and overlap gates.
