...
11. Execution ends with one accepted CID per touched target and no forge write. Until RFC-95 lands there is no commit, branch, or publication worktree; after it lands, RFC-95 may materialize a `change/<plan>` worktree as an execute side effect and the operator authors the Git commit. Copying the change home before RFC-100 does not transport source or result tree objects.
