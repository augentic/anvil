...
>
> Related: [RFC-79](rfc-79-swarm-build.md) (the swarm build this RFC is the enabling layer for — focused convergent build requests replace the fat legs), [RFC-80](rfc-80-synthesis-redesign.md) (the deeper synthesis redesign D8 defers to), [RFC-18](../future/rfc-18-slm.md) (cheaper generation backend — orthogonal; this RFC shrinks the frontier-model cost itself), [RFC-90](../rfc-90-build-verification.md) (the current engine-owned model-assisted verification and repair loop; it supersedes RFC-79's proposed host-owned gate), [RFC-55](rfc-55-working-tree.md) (materialized working trees — per-worker isolation and the deployment that re-activates bodies-on-the-wire).
...
- Host-owned `verify` profiles ([RFC-60](../rfc-90-build-verification.md)) — complementary later; this RFC does not move cargo out of the agent loop.
...
- **The swarm build itself — owned by [RFC-79](rfc-79-swarm-build.md).** Decomposing the fat legs into focused convergent requests, the verify convergence gate (promoting [RFC-60](../rfc-90-build-verification.md)), backend support for concurrent completions and per-worker workspace policy, and agent-pool lifecycle management all live there. This RFC is its enabling layer: small requests only work when the per-request fixed overhead (D1–D3), timeout semantics (D4), and session model (D5) are already right.
