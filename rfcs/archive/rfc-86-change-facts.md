...
>
> **Historical amendment (superseded by RFC-91):** the plan-centric surface cut first replaced per-slice breakouts with `plan author → plan execute → plan archive` and execute-owned refinement. The original decision body below retains that closed trail as history; the implemented four-stage contract is stated next.
>
...
>
> **Amendment (gap deferral — [RFC-86a](rfc-86a-gap-deferral.md)):** the per-epoch waiver, explicit deferral verb, and `strict | defer` policy surfaces are deleted. At each build gate the engine unconditionally appends one digest-bound `gap.deferred` fact per open `[unknown]` or `[conflict]`, excludes those requirements from `BuildRequest` obligations, and proceeds; merge conserves them as baseline debt. Ready remains clean-gap, while debt-carrying work reaches build through Authorized. Waiver and earlier policy prose below is historical.
>
> **Programme:** Implemented foundation of the [Services Delivery Programme](../platform.md) — the fact substrate every later active or parked option consumes. Phases A–C are landed against [RFC-87](rfc-87-working-trees.md)’s workspace contract: recorded `base.yaml` pins, content-addressed `builds/<digest>.yaml` records, and one-member waves. [RFC-88](rfc-88-detached-changes.md) resolved the remaining stand-ins: merge-time `apply` is deleted, the in-place change home is `.emery/change/`, and detached changes run over accepted CIDs.
>
> **Owns:** the fact-based change substrate — projected status, per-writer event logs, claims (ownership, not authority), `plan.execute.started` as an explicit authorization epoch with typed `closed-plan` coverage, one-member target-wave merge facts, pinned judgment inputs (landed RFC-87 `SnapshotId` / wire `cid`), merge-finalized requirement identity, and the single-writer desktop as the degenerate case of the same substrate. RFC-91 owns the standalone `plan refine` stage; RFC-86a owns unconditional gap deferral and debt conservation.
>
...
1. **Shift specs left into a fenced stage.** The implemented public surface is `plan author → plan refine → plan execute → plan archive`. Refine creates no code grant; execute requires fresh refinement manifests and opens the authorization epoch. There is no separate `approve` verb or `refine-under-epoch` path. At the build gate RFC-86a auto-defers open gaps out of build scope and conserves them as debt.
...
Together: better specs before generation, and a state model that can travel without making stored status or an approval file authoritative.
...
The body below preserves RFC-86's original closed decisions and examples. Where it describes the three-stage surface, execute-owned refinement, `refine-under-epoch`, or per-epoch unknown waivers, the implemented RFC-91 and RFC-86a amendments in the header supersede it. The current operator contract is `plan author → plan refine → plan execute → plan archive`, with unconditional gate-time gap deferral and durable debt conservation.
...
| Merge has no wave shape              | Merge is a single-slice write-back. Series RFCs need an immutable one-member target wave (build authorization vs commit authorization) so RFC-88 can attach accepted-CID semantics and RFC-96 can widen membership later.         |
...
1. **Author** — `emery plan author` (via `/emery:plan`) reconciles explicitly bound sources into slices in the implemented in-place loop. Under RFC-104/88 detached mode, it instead imports one reviewed wave, binds its exact delivery inputs, and decomposes that wave. Produces the authored plan artifacts (`discovery.md` / RFC-88 `leads.md` + `decomposition.yaml` as applicable, `change.md`, `plan.yaml`). **Stops here** — no slice synthesis. The operator can review delivery topology before paying for refine. RFC-104's system-wide survey, extraction, and architecture review happen before this change lifecycle; folding slice refine into author remains rejected (see D14).
...
Every `slice.build.started` and merge-wave member binds its authorization epoch plus the exact leaf, spec, dependency frontier, and pinned base it consumed. RFC-88 extends that fence with lead-catalog, decomposition-revision, and target-`cid` digests. Any changed input makes the result stale independently of whether the epoch remains open. [RFC-99](../future/rfc-99-streaming-execution.md) owns the future authority mode that may build ready leaves while survey continues but cannot commit a target wave; accepted-`cid` mutation still requires a later reviewed closed-plan gesture. There is no `approved` rung.
...
1. **Pins** — Judgment legs run against recorded [RFC-87](rfc-87-working-trees.md) `SnapshotId`s / wire `cid`s (and baseline-spec digests), not ambient trees. Pin *authorship* closes when inputs are knowable (see D4 / D25): source ids at plan authoring (in-place) or at reviewed-wave binding / detached plan-author intake ([RFC-88](rfc-88-detached-changes.md) — no `change approve` verb); refine assembles each slice's `base.yaml` from those source pins plus the baseline digest before extract; build reads a recorded target-base pin before `prepare` (retiring today's self-freeze). If pins move later, validate reports staleness instead of silently building on drift.
...
Substrate decisions D1–D11 match the series-facing spine assumed by [RFC-88](rfc-88-detached-changes.md) (which amends D1’s concrete homes) and [RFC-95](rfc-95-publication-sets.md). Product decisions D12–D27 are this branch’s closed gap / shift-left / membership / Phase B stand-in layer.
...
| D15 | `[unknown]` **always blocks build** *(amended by [RFC-86a](rfc-86a-gap-deferral.md))*                   | Thin intent is not an exception — close the gap or waive it explicitly on execute; generation must not invent missing information. *RFC-86a as implemented: every open gap is gate-minted as a durable deferral and leaves build scope before generation*                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| D25 | **Pins use landed RFC-87 snapshot identity; wire name is** `cid`                                        | Pin *wire identity* in plan/binding artifacts is `cid` (`sha256:…`); Rust type remains RFC-87's `SnapshotId`. This RFC owns *when pins are written and what consumes them*: source ids close when the source set closes — at plan authoring (in-place) or at reviewed-wave binding / detached plan-author intake ([RFC-88](rfc-88-detached-changes.md); there is no `change approve` pin-close site); refine copies those pins into `base.yaml` and adds the baseline-spec digest before extract; build reads the recorded target-base pin before `prepare` (replacing the interim freeze-at-build stand-in). Exact on-disk `base.yaml` shape stays an implementation detail. Phase B does **not** wait on further RFC-87 work |
...
| `emery plan execute`                           | Sole privileged-start surface (D6; RFC-88 D8). **At start** appends `plan.execute.started` with typed `closed-plan` coverage (per-leaf `existing` / `refine-under-epoch`, optional `unknown-waivers`, detached digests as required by RFC-88). Drives refine → build → merge; enforces the gap policy and slice-status gates before **build**; runs build → merge under one-member target waves (D9). For `[unknown]` leftovers only: repeatable `--waive <slice>/<req>` with required `--reason` (see D17). No `--force`, no bulk/all-gaps waive, no separate `plan waive` / `approve` / `plan refine` verb. *(superseded by RFC-91 and [RFC-86a](rfc-86a-gap-deferral.md): execute requires exact refinement digests; waiver/policy argv and coverage fields are deleted; the build gate emits one durable deferral per open row)* |
...
| `emery plan status`                            | After a fresh author, **resume** is `emery plan execute` / `/emery:execute` (D26). Next-actions may project `refine <slice>` / review-gaps while unrefined or gappy; then `plan execute` (build / merge) when Ready, or `plan execute --waive…` when skipping Ready (D22). Projects Authorized when a covering epoch exists — never an `approved` rung. *(RFC-91/RFC-86a: refinement is a separate drain; execute gate-mints deferrals for remaining open gaps, and status counts the resulting debt)*                                                                                                                                                                                                                        |
...
| **C** | Gap inventory (+ shared-lead presentation rollup), execute gap gate (no `approve` / no `plan refine` verb), typed `closed-plan` coverage with optional waivers *(RFC-91/RFC-86a: coverage now carries exact refinement digests only; the waiver field is deleted and the gap gate emits durable deferrals)*, shift-left practice via execute’s refine phase, RFC-88 `refine-under-epoch` alignment; sibling-series consistency pass (D20) | Public rhythm stays author → execute → archive; preferred path reviews topology then lets execute refine and park on gaps before build; starting execute opens the authorization epoch; multi-homed leads correlate in the gaps projection without changing the per-req gate. Phase C also updated shipped “nothing is stamped” / leftover approve-vocabulary prose when the durable epoch landed |
...
17. ~~**RFC-88**~~ `change approve` ~~**naming vs this RFC’s no-**~~`approve` ~~**plan surface**~~ **Closed — superseded by [RFC-88](rfc-88-detached-changes.md).** That RFC removes `emery change approve` (and `emery change open` / standalone delivery discover): reviewed-wave binding and topology recording are internal phases of detached `plan author`; `plan execute` is the only delivery authorization surface and records this RFC's `plan.execute.started` (extended with detached and RFC-104 handoff digests). RFC-104 has separate architecture review seams but does not authorize product mutation (D25).
...
- Fold refine into `plan author` / `/emery:plan` — spends slice synthesis before the operator can re-cut the delivery list; collapses topology review and spec review; fights RFC-88, which folds reviewed-wave binding into author, not refine (see D14). RFC-104's earlier system extraction is definition work and does not synthesize slice artifacts.
...
- Defer multi-slice multi-writer claims to RFC-96/100 — leaves D7 and per-writer logs without the cardinality rule they exist for (see D10, D23).
...
- Plan authoring (in-place) or reviewed-wave binding / detached plan-author intake closes per-source `SnapshotId` / `cid` pins at plan scope when the source set is known (D4 / D25). There is no `change approve` pin-close site.
