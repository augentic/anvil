...
Every `slice.build.started` and merge-wave member binds its authorization epoch plus the exact leaf, spec, dependency frontier, and pinned base it consumed. RFC-88 extends that fence with lead-catalog, decomposition-revision, and target-`cid` digests. Any changed input makes the result stale independently of whether the epoch remains open. [RFC-99](rfc-99-streaming-execution.md) owns the future authority mode that may build ready leaves while survey continues but cannot commit a target wave; accepted-`cid` mutation still requires a later reviewed closed-plan gesture. There is no `approved` rung.
