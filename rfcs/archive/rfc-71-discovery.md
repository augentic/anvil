...
> **Status: Superseded (archived).** Source selection ownership moved to [RFC-87 Detached Changes](../rfc-87-detached-changes.md#source-adapter-selection). Do not implement this document; historical prior art only (filename keeps the old number; active [RFC-71](../rfc-71-deployment.md) is deployment).
>
...
The closed `workload-kind` vocabulary is the join key between a repository profile ([RFC-72](rfc-72-migration.md)) and a target's `produces` ([RFC-87](../rfc-87-detached-changes.md)). Every value below is grounded in existing adapter grammar rather than speculative:
...
| D7 | Discovery emits an immutable recommendation report and stops. It never installs, binds, or writes config. | Approval is the consumer's gate — plan-time Gate 1 for sources ([RFC-72](rfc-72-migration.md)), Gate M1 for targets ([RFC-87](../rfc-87-detached-changes.md)). |
