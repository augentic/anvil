...
> Status: Implemented foundation of the [Services Delivery Programme](../platform.md). [RFC-86](rfc-86-change-facts.md) Phase B retired the interim build-time ambient freeze and slice-local `build/patch.yaml`; recorded `base.yaml` pins and content-addressed `builds/<digest>.yaml` records are the authority. Merge-time `apply` is deleted: merge captures a private workspace and records an accepted CID (`target.merge.wave-committed`); merge never writes the operator checkout ([RFC-95](rfc-95-publication-sets.md) D11's publication worktree — in place only for a single-member set on a clean-at-parent repository — is the sole engine write near one, and it is not a merge act). [RFC-104](rfc-104-system-archaeology.md) uses D2's read-only values for location-backed definition sources. RFC-88 consumes those pins and separately amends D4 and acceptance criterion 4 to admit each target repository's durable state into the tree; binding and plan artifacts name the tree identity a **CID** (`SnapshotId` is that value).
>
> Deployment note (post-implementation): the workspace kernel (`project::workspace`) is wasm-clean engine library code, not a host capability. The former five-op `emery:workspaces` WIT import is deleted; the shipped deployment's guest runs the kernel in-guest — tree I/O over its filesystem preopens, snapshot objects through the standard `wasi:blobstore` import (the omnia-backends filesystem backend at the snapshots root), and executable mode through the two-function `emery:exec-mode` capability (`wasi:filesystem` carries no permission bits). The host contract shrank from workflow semantics to blob storage plus exec mode; native deployments call the same kernel in-process. Digest identity is deployment-independent by the canonical manifest encoding. The blobstore backend is filesystem-backed, not in-memory, because snapshot objects must survive across separate CLI invocations — every `emery` run is a fresh process, and a stopped `emery plan execute` resumes in a later process against the pins its refine phase recorded (`base.yaml`, `builds/<digest>.yaml`, plan CIDs); RFC-95/96/100 then consume the same snapshots for publication, distribution, and transport. Snapshot lifetime is scoped to the change: `plan archive` is the collection point that sweeps objects whose GC roots belong only to the archived plan (D2).
>
...
[RFC-96](rfc-96-concurrent-execution.md) owns deterministic composition of same-base results. [RFC-100](../future/rfc-100-distributed-execution.md) owns transport of the referenced snapshot objects.
...
[RFC-86](rfc-86-change-facts.md) supplies claims, pinned inputs, `plan.execute.started`, and result facts. [RFC-96](rfc-96-concurrent-execution.md) supplies scheduling, composition, and domain convergence. [RFC-106](../future/rfc-106-task-graphs.md) supplies target-proposed task decomposition and write ownership when a slice is too large for one build call. [RFC-100](../future/rfc-100-distributed-execution.md) supplies placement, fencing, and transport. [RFC-95](rfc-95-publication-sets.md) exports each final accepted CID into a publication worktree and supplies branches, pull-request markers, and publication verification.
