...
> **Status: Superseded (archived).** Migration intake and topology ownership moved to [RFC-87 Detached Changes](../rfc-87-detached-changes.md). Do not implement this document; historical prior art only (filename keeps the old number; active [RFC-72](rfc-72-materialization.md) is managed materialization).
>
...
| D9 | Plan-time survey stays serial in the first cut. | Repeat-until-drained is repository-at-a-time ([RFC-87](../rfc-87-detached-changes.md)), so a `--jobs` fan-out is a throughput optimisation with no correctness role yet. It stays deferred, and the current first-failure-aborts behaviour remains the contract. |
...
- Selecting or binding target adapters ([RFC-87](../rfc-87-detached-changes.md) owns that policy)
