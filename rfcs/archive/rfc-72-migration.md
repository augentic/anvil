...
> **Status: Superseded (archived).** Design ownership moved to [Migration Walking Skeleton](../rfc-70-program.md) Part B. Do not implement this document; historical prior art only (filename keeps the old number; active [RFC-72](../future/rfc-72-materialization.md) is managed materialization).
>
...
| D8 | Source snapshots are immutable, live out of tree, and are never the target slot. | A repository that is both migration source and target keeps evidence integrity while its slot is being written ([RFC-72](../future/rfc-72-materialization.md)). |
| D9 | Plan-time survey stays serial in the first cut. | Repeat-until-drained is repository-at-a-time ([RFC-70](../rfc-70-program.md)), so a `--jobs` fan-out is a throughput optimisation with no correctness role yet. It stays deferred, and the current first-failure-aborts behaviour remains the contract. |
...
- Selecting or binding target adapters ([RFC-70](../rfc-70-program.md) owns that policy)
