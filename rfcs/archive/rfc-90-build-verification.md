...
That shape hides the most important control loop from lifecycle policy and makes RFC-96 concurrency and RFC-106 task graphs unsafe: splitting writers before replacing the loop would remove the only mechanism that currently returns failed checks to the code writer.
...
A model-assisted verification pass may execute tests stored in the candidate workspace, including tests authored or edited by the same build. A green result therefore means **the candidate passes its own reported checks**, not that it passed an independent or protected oracle. The report source and operator output must preserve that assurance boundary. RFC-96 may protect independently governed verification inputs at domain granularity; RFC-106 may protect them at task-grant granularity; RFC-97 defines host-owned profile and protected-oracle evidence.
...
The phase event is also the join anchor for model-backend telemetry when the deployment exposes it: effective route and model identity, input and output tokens, and reported cost. Missing usage data remains missing rather than estimated by the engine. These observations stay outside report digests and lifecycle projection; RFC-96's harness comparison derives cost per accepted result and worker spend instead of optimizing per-call price. RFC-106, if activated, may additionally project spend induced by decomposition.
...
- **Wait for RFC-97 / native host verification before exposing the loop** — leaves retry authority hidden in prose and blocks RFC-96's observable concurrent execution on the native-verification follow-on. (RFC-97 itself does not wait on `wasi:exec`; rejecting the wait is about shipping the engine-owned phase machine first.)
