...
> Status: Superseded by [RFC-88 Concurrent Execution](../rfc-88-concurrent-execution.md), which merges this RFC's build-time swarm with RFC-80's refine-time redesign. Retained for the original evidence and staging detail.
>
...
The shared verify-repair channel is replaced by a **host-owned convergence gate** over [RFC-60](../rfc-87-verify-profiles.md) verify profiles:
...
Nothing in D1–D5 may assume process-locality beyond the stage it ships in. Values ([RFC-55](rfc-55-working-tree.md) `revision` / `changeset`) are the only connective tissue between orchestrator and Stage C workers; a worker's node is a scheduling decision. The cloud-hosted service is this architecture with a remote pool bound in — not a fork of it.
