...
- **Reaching adapters.** When the workflow needs an adapter operation (`survey` / `extract` / `build` / `merge`), it calls the host-satisfied `source` / `target` import with the plan-bound `adapter-id`, and the host routes it to a fresh adapter instance ([RFC-56](rfc-56-runtime-move.md)). Because identity is a call argument, the workflow guest **owns its own fan-out** — it loops `survey(id)` over every bound source in guest code (Law 3: control flow lives in deterministic guest code), rather than being re-instantiated per source by native orchestration. When it needs judgment, it calls `eval`.
