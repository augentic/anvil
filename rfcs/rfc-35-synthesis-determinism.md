...
4. **Add `specrun slice reconciliation write` (D6).** New handler under `crates/domain/src/slice/`. Reads spec files and evidence, writes `reconciliation.yaml` atomically, emits journal event. Golden tests against existing synthesis fixtures.
