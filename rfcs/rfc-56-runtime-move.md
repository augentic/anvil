...
**Transport is pluggable behind the seam.** `selection` is a typed contract, not a wire protocol — the host decides how each selected call reaches its callee. In-process host-mediated dynamic linking is the default (local, synchronous); when the callee lives on another node the host carries the same `build(id, …)` over [wRPC](https://github.com/bytecodealliance/wrpc) — a WIT-native, transport-agnostic RPC backend (NATS, QUIC, …) that owns the encode -> request -> await -> decode round-trip natively, including async `stream` / `future` values. Plain records cross by value, but a live resource (the `working-tree` `descriptor`) cannot, so the remote path ships the content-addressed `revision` / `changeset` and the callee re-materializes its own tree ([RFC-55](rfc-55-working-tree.md)) rather than transporting the descriptor. wRPC is a pinned, swappable backend dependency that never enters the `augentic:specify` contract, so the guest's view stays purely typed and deployment topology stays a function of the bound backends, not the guest code.
...
- **wRPC is a pre-1.0 dependency.** The remote transport rides wRPC, which is pre-1.0; it is pinned and confined behind the backend boundary so its churn never reaches the `augentic:specify` contract or the guests. The decision and its constraints (resources do not cross; ship `revision` / `changeset` and re-materialize callee-side) are recorded in the engine workspace's `DECISIONS.md`.
