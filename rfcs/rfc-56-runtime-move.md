...
- **Omnia (the repo) provides the generic floor** — the Wasmtime-based interpreter, the pluggable host-service framework (the same mechanism that swaps an in-memory KV for Redis), the general-purpose host traits (`wasi:filesystem`, `wasi:blobstore`, `kv`, …) that Specify's deterministic-effect backends plug into, and the one sanctioned *new* host slot an `eval` backend plugs into — the **model host**, specified in [RFC-54](rfc-54-model-host.md). It carries zero Specify domain knowledge.
...
- **Effect-WIT ownership & versioning.** Specify owns the effect interfaces (above). The `eval` interface is authored in [`wit/model.wit`](../wit/model.wit) under the Specify-owned `augentic:model` package (renamed from `omnia:model` to match the ownership claim — Specify owns and versions it; Omnia hosts it), held separate from the `augentic:specify` adapter contract (RFC-51). Still open: how the `augentic:model` and `augentic:specify` package versions relate to each other and to the host floor (RFC-47).
