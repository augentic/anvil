...
[RFC-13](rfc-13-extensibility.md) reframes the runtime so that **capabilities are the only extension point**, and lands one domain capability (`omnia` / `contracts` / `vectis` / …) plus three first-party platform components (registry, plan, initiative — reframed as platform components by RFC-13) (`plan@v1`, `registry@v1`, `initiative@v1`) per project. RFC-14 is the follow-up that lets a single repository carry **more than one domain capability** — for example, a service repo that maintains both Rust+WASM code (`omnia@v1`) and the API contracts that surround it (`contracts@v1`), or a platform repo that hosts code, infrastructure, and design tokens side by side.
...
**A repository is a workspace; a workspace contains zero or more scopes; each scope activates exactly one domain capability.** The workspace is the unit that owns platform components (`plan`, `initiative`, `registry`), the lock file, the change archive, and the cross-scope coordination surface. A scope is the unit that owns a domain capability, its artefacts, and its briefs. Path resolution and change creation become scope-aware; nothing else changes.
...
The workspace root itself is **not a scope** in the path-resolution sense — it is the container that holds platform components, the lock file, the change archive, and the registry. Platform components (`plan@v1`, `registry@v1`, `initiative@v1`) activate **once** at the workspace root and are visible from every scope.
...
This change is also the canonical extension of RFC-13's Phase 1 work: `ProjectConfig::baseline_path(&capability, artifact_id)` becomes `baseline_path(&scope, &capability, artifact_id)`. The `Scope` newtype is the only structural type the change loop gains.
...
Workspace-wide platform components (`plan`, `registry`, `initiative`) are **always reachable** from any scope without a `@<scope>` qualifier — they live at the workspace root, not in any scope.
...
1. **It breaks the "core never switches on a capability name" principle.** A multi-scope change has multiple pipelines to run, each from a different capability, with no single owning capability. The change loop becomes capability-aware again, undoing RFC-13's reframe.
...
Every per-scope artefact (`changes/`, scope-local rules) lives under `<scope>/.specify/`. The split is mechanical: anything coordinating across scopes lives at the workspace root; anything specific to a scope's domain capability lives in the scope.
...
- **Scope-level plan / initiative / registry.** Platform components remain workspace-wide. A scope cannot have its own `plan.yaml` or `registry.yaml` separate from the workspace's.
...
A staged landing on top of RFC-13 phases 1–4, each independently testable and shippable. Every stage preserves working `/spec:draft → /spec:build → /spec:adopt` for Mode A (single domain capability, no workspace).
...
1. **Mode A keeps working byte-for-byte.** Every phase's acceptance criterion runs `/spec:draft → /spec:build → /spec:adopt` on the canonical omnia change with the **flat** `capability: <url>` shape preserved. No path on disk changes for Mode A repos.
...
- [RFC-2: Execution](archive/rfc-2-execution.md) — owns the `plan@v1` component and `/spec:execute --loop`. RFC-14's "cross-scope changes route through the plan loop" stance leans on RFC-2's existing dependency-ordering primitives.
