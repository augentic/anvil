...
The agent binds `project` on each slice by matching the scope against each project's `target`, `description`, and `capabilities`. When `projects[]` has exactly one entry the agent may omit `project` and the kernel auto-binds it. When more than one project exists the agent must name one explicitly; the CLI never chooses among candidates.
