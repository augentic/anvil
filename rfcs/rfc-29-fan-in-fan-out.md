...
2. **Agent-led plan-time lead reconciliation** ([RFC-29b](rfc-29b-reconciliation.md)) - an agent-led cross-source matching step that partitions each source's `Lead[]` into `lead-groups[]` (including semantic matches that exact id / alias cannot catch) and emits `slice-candidates[]` that bind each `(concept-id, target)` pair to a target, running under a stable input/output envelope wrapped by a CLI-owned projection kernel: locked blocks, advisory blocks, schema validation, the global lead-partition invariant, slice-name derivation, journal events, and the existing plan writers.
...
| ID                                     | Decision                                                                                                                                                                                                                                                                                               | Home                                                                |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| **D1 Source operation**                | The CLI runs source adapter `survey` and `extract` operations.                                                                                                                                                                                                                                         | [shipped — `DECISIONS.md`](https://github.com/augentic/specify-cli/blob/main/DECISIONS.md#source-operations-d1)                                            |
| **D2 Lead reconciliation**             | Agent-led cross-source matching of `Lead[]` into `lead-groups[]`, binding each `(concept-id, target)` slice candidate to a target and (workspace mode) a registry project, under a CLI-owned projection kernel (locked blocks, advisory blocks, partition invariant, project-binding validation, slice-name derivation, plan writers). | [29b](rfc-29b-reconciliation.md)                                    |
...
specrun plan propose --dry-run --format json                 # D2 request envelope (locked + advisory blocks) # 29b
...
Cross-source lead matching and target binding are the `/spec:plan` agent step's judgment (D2); `specrun plan propose --dry-run` seeds it with locked and advisory blocks, and `specrun plan propose --from` is the kernel that validates the grouping and writes the slices through the existing plan writers.
...
All slice-model, build-request, and build-report schemas key on `(slice, target)` per D5 — none carries `outputs[]` or `output-id`. `proposal.schema.json` discriminates request vs response via closed `kind: request | response`: the request carries `leads` (summary plus optional `block: { id, basis? }`), `blocks[]` (locked or advisory, with `keys[]` and adjacency for advisory blocks), `targets`, and the registry `projects[]` topology; the response carries `lead-groups[]` (members with `match-basis`, optional `tentative` flags, optional `block-id`, and optional grouping `rationale`) plus `slice-candidates[]` (each `(concept-id, target)` binding with optional `slice-name`, `depends-on` in derived slice names, and the bound `project`). `synthesis.schema.json` discriminates request vs response via closed `kind: request | response`; the response `model` `$ref`s `**draft-model.schema.json`**, not the persisted model (D3a).
...
| `plan.reconcile.completed`            | M2a        | `specrun plan propose --from` validated the agent grouping, enforced the partition / required-group invariants, derived slice names, and wrote `plan.yaml.slices[]`. |
...
| `plan-reconcile-lead-orphan`                 | M2a       | `plan propose --from` | The response cites a lead `key` absent from the request's surveyed lead inventory.                                                                                                                        |
| `plan-reconcile-partition`                   | M2a       | `plan propose --from` | The response is not a well-formed **global** partition: a surveyed lead is unaccounted for, or a lead `key` appears in more than one group.                                                               |
| `plan-reconcile-required-group-split`         | M2a       | `plan propose --from` | The response splits a deterministic required group (exact id / alias / cross-reference).                                                                                                                  |
| `plan-reconcile-concept-orphan`               | M2a       | `plan propose --from` | A `slice-candidates[].concept-id` value does not name a declared `lead-groups[]` entry.                                                                                                                   |
| `plan-reconcile-concept-unbound`              | M2a       | `plan propose --from` | A declared `lead-groups[]` entry has no `slice-candidates[]` row, so the concept would not produce any plan slice.                                                                                         |
| `plan-reconcile-project-binding-required`    | M2a       | `plan propose --from` | The response omits `project` on a slice candidate in workspace mode (request carried a non-empty `projects[]`), or sets `project` on a slice candidate in single-repo mode.                               |
| `plan-reconcile-project-orphan`              | M2a       | `plan propose --from` | The response binds a slice candidate to a `project` absent from `registry.yaml`.                                                                                                                          |
| `plan-reconcile-project-target-mismatch`     | M2a       | `plan propose --from` | The response binds a slice candidate to a project whose `registry.yaml` target does not equal the candidate's `target`.                                                                                   |
