...
8. Update `/spec:refine` to call the CLI command. The engine handles the dispatch and the kernel projection; the agent's contribution is authoring the requirement set, its claims, its agreement verdict, and prose — not driving the lifecycle, not deriving `sources`/`status`/winners, and no longer hand-authoring `provenance.yaml`.
...
- **Shared kernel, shared direction — and the contradiction this RFC resolves.** RFC-35 (landed) deliberately keeps `provenance.yaml` agent-authored and adds drift validation only: it found that the claim→requirement mapping "is not mechanically derivable from `Sources:` lines alone and remains synthesis judgment," so it deferred any writer verb. RFC-29's earlier draft contradicted that — it had the kernel *derive* `provenance.yaml` and `status` from each requirement's source-key `sources`, which a source-key list cannot support (it loses the losing claims and cannot tell `agreed` from `divergence`/`conflict`). This revision resolves the contradiction by carrying the missing input: the synthesis **response** now carries the per-requirement `claims` (the `(source, claim-id)` pairs behind each requirement) and an `agreement` verdict, so the kernel projects `provenance.yaml`, `sources`, winners, and `status` *faithfully* (agreement is judgment; winner selection is authority mechanics — see §"Status and provenance derivation"). RFC-35's "no writer verb" stance was correct *for its inputs*; RFC-29 supplies the claim-level input that makes `specrun slice provenance` a faithful projection rather than a lossy reconstruction. RFC-35's `specrun slice provenance` is therefore the thin standalone entry point onto the same projection kernel RFC-29 D3 wraps inside `specrun slice synthesize`; whichever RFC lands the module first, the other consumes it, and both produce the byte-identical `provenance.yaml` schema the landed `slice-provenance-drift` gate already enforces.
