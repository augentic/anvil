...
specrun plan propose --dry-run --format json                 # D2 request envelope (locked + advisory clusters) # 29b
...
Cross-source lead matching and target binding are the `/spec:plan` agent step's judgment (D2); `specrun plan propose --dry-run` seeds it with locked and advisory clusters, and `specrun plan propose --from` is the kernel that validates the grouping and writes the slices through the existing plan writers.
...
All slice-model, build-request, and build-report schemas key on `(slice, target)` per D5 — none carries `outputs[]` or `output-id`. `proposal.schema.json` discriminates request vs response via closed `kind: request | response`: the request carries `leads[]` (one row per raw `(source-key, lead-id)` lead, with summary and optional `cluster-binding: { cluster-id, basis? }`), `clusters[]` (locked or advisory, with `shared-facets[]` and adjacency for advisory clusters; cluster membership lives on per-row `cluster-binding.cluster-id`), `targets`, and the registry `projects[]` topology; the response carries `concepts[]` (members as `{ source-key, lead-id, match-basis, tentative? }`, optional `from-cluster-id`, and optional grouping `rationale`) plus `slices[]` (each `(concept-id, target)` binding with optional `name`, `depends-on` in derived slice names, and the bound `project`). `synthesis.schema.json` discriminates request vs response via closed `kind: request | response`; the response `model` `$ref`s `**draft-model.schema.json`**, not the persisted model (D3a).
...
| `plan-reconcile-lead-orphan`                 | M2a       | `plan propose --from` | The response cites a `(source, lead)` pair absent from the request surfacing catalog.                                                                                                                        |
| `plan-reconcile-partition`                   | M2a       | `plan propose --from` | The response is not a well-formed **global** partition: a surveyed surfacing is unaccounted for, or a `(source, lead)` pair appears in more than one concept.                                                               |
| `plan-reconcile-required-group-split`         | M2a       | `plan propose --from` | The response splits a deterministic required group (exact id / alias).                                                                                                                  |
