...
3. **Slice synthesis engine** - an agent-led cross-modal synthesis step (which decides the requirement set, declares each requirement's contributing `(source, claim-id)` claims and an `agreement` verdict, and authors its prose) running under a stable input/output envelope, wrapped by a CLI-owned projection kernel that projects over the agent's structure: RFC-27 authority resolution, REQ-id assignment, `sources` and winner-marker derivation, status derivation, provenance projection into `provenance.yaml`, and drift validators.
...
| **D3 Slice synthesis engine** | The agent-led synthesis step owns cross-modal reconciliation of `Evidence[]` into the requirement set — which requirements exist, how claims merge or split, each requirement's contributing `(source, claim-id)` claims, an `agreement` verdict (`agreed` / `disagreed`) over those claims, its prose (`title` / `statement` / `scenarios` / `notes`), and the four Markdown artifacts. The CLI owns the projection kernel that *projects over* that structure (RFC-27 authority resolution, REQ-id assignment, `sources` and winner-marker derivation, status derivation, provenance into `provenance.yaml`, drift validation) and the synthesis envelope. The kernel never reconstructs the requirement set, never picks a winner the resolved authority did not, and never overrides the agent's agreement verdict. | Add `specrun slice synthesize <slice>`. The engine resolves authority via the RFC-27 resolver, dispatches the synthesis step (`agent` by default, or a declared `executable` synthesis tool per D10) with the Evidence map and resolved authority, then derives each requirement's `sources` from its contributing claims, applies authority to mark winners and derive `status` from the agreement verdict, assigns ids, projects `provenance.yaml`, validates, and persists. `/spec:refine` stops hand-coding the reconciliation projection (and stops hand-authoring `provenance.yaml`) and shells out to the engine. |
...
3. After the synthesis step returns its requirement set, contributing claims, agreement verdicts, and prose, the kernel projects over that structure — assigning REQ-ids in declaration order, deriving each requirement's `sources` from its contributing claims, applying authority to mark winners and derive `status` from the agreement verdict, and writing `provenance.yaml` — then validates and persists the merged artifacts:
...
The micro-resolver currently pinned in tests becomes black-box coverage for the production resolver. The kernel resolves authority **before** dispatch and passes the resolution into the envelope, so the synthesis step knows which claims win when it decides what each requirement draws on; the step never re-decides authority and never marks winners itself. After the step returns its requirement set — each requirement carrying its contributing `(source, claim-id)` claims and an `agreement` verdict — the kernel projects the resolution over those claims: it marks the winning and losing claims, derives `status` (an `agreed` verdict yields `agreed`; a `disagreed` verdict is resolved by authority into `divergence` with a unique winner, or `conflict` when the top authority class ties), derives the `sources` set, and writes `provenance.yaml`.
...
| Contributing claims | `agreement` | Kernel `status` | `provenance.yaml` `resolution` | Winner markers |
| --- | --- | --- | --- | --- |
...
The agent's agreement verdict is authoritative for `agreed`-vs-not — value agreement is semantic and the CLI does not adjudicate it. As a non-blocking guard, the kernel runs a cheap normalised-string inequality check over the contributing claims of any requirement the agent marked `agreed`; a mismatch emits a `review`-kind finding (`slice-synthesize-agreement-suspect`, advisory, never a transition blocker) so an operator can eyeball a possible mislabel without the CLI re-litigating semantics.
...
- `requirements[].contributing-claims`, `requirements[].agreement`, `requirements[].sources`, or any `sources` field elsewhere in the slice model;
...
project: identity-service
requirements:
    # agent authors: title, statement, scenarios, unit, contributing-claims, agreement.
...
| `slice-model-provenance-drift`      | `model.yaml.requirements[].contributing-claims` disagrees with `provenance.yaml`, compared at `(source, claim-id)` granularity, for any matching `REQ-*`.                      |
| `slice-model-target-drift`      | `model.yaml.target` (or `model.yaml.project`) disagrees with `plan.yaml.slices[<slice>].target` / `.project`.      |
| `slice-model-source-orphan`     | A `contributing-claims[]` entry references a `(source, claim-id)` whose source key is absent from `model.yaml.sources[].key` or whose claim id is absent from that source's Evidence.                        |
...
- **D8 invariant (requirement-set target-neutrality).** The two slices share the `docs:identity-api` lead and the same `authority-overrides`; `identity-service` additionally reconciles `legacy:identity-api`. On the shared lead's requirements, the two slices' `requirements[]` entries are validated for **semantic equivalence** — same intent, same contributing `(source, claim-id)` claims and agreement verdict, equivalent scenarios — by a fixture-local check (golden text on the simplest cases; LLM-judge with a fixed grader prompt on the more elaborate cases), even though the slices bind different targets. Byte-equality across targets is not asserted; target-neutrality of intent is. This proves shape briefs and target binding do not leak into the requirements section.
...
```json
{
  ...
  "$defs": {
    ...
    "modelRequirement": {
      "type": "object",
      "additionalProperties": false,
      "required": ["id", "title", "status", "sources", "statement", "contributing-claims"],
      "properties": {
        "id":        { "$ref": "#/$defs/reqId" },
        ...
        "agreement": {
          ...
          "description": "Agent-authored value-agreement verdict over `contributing-claims`. Required when more than one claim contributes; omitted for 0 or 1 claim. The kernel derives `status` and winner markers from this verdict plus RFC-27 authority (RFC-29 §Status and provenance derivation)."
        },
        "contributing-claims": {
          "type": "array",
          "items": { "$ref": "#/$defs/modelContributingClaim" },
        ...
        "sources":   {
          ...
          "description": "Kernel-derived: the unique source keys of `contributing-claims`, ordered by resolved authority then alpha. Not authored by the synthesis step."
        },
...
| `slice-model-provenance-drift`                           | 2    | `model.yaml.requirements[].contributing-claims` disagrees with `provenance.yaml` at `(source, claim-id)` granularity.                                          |
| `slice-model-target-drift`                           | 2    | `model.yaml.target` (or `model.yaml.project`) disagrees with `plan.yaml.slices[<slice>].target` / `.project`. |
| `slice-model-source-orphan`                          | 2    | A `contributing-claims[]` entry references a `(source, claim-id)` whose source key is absent from `model.yaml.sources[].key` or whose claim id is absent from that source's Evidence.                   |
...
| `slice-synthesize-kernel-field-usurped`           | 2    | A synthesis-step response set a kernel-owned field it does not author — a `requirements[].id` (`REQ-NNN`), `requirements[].status`, `requirements[].sources`, or a `contributing-claims[].winner` value. The engine derives ids, sources, winners, and status; it rejects the response rather than persisting it. (Orphan `(source, claim-id)` claims are caught separately by `slice-model-source-orphan`.) |
| `slice-synthesize-forbidden-input-leak`           | 2    | A synthesis-step response's requirements section (entries, `contributing-claims`, `agreement`, `statement`, `title`, `scenarios`, `notes`) demonstrably referenced `target` or `shape-brief` content (detected by fixture-local target-neutrality probes). |
...
2. Add the production authority resolver and the projection kernel to `specify-workflow`. The kernel resolves RFC-27 authority over `Evidence[]` **before** dispatch, and — **after** the synthesis step returns its requirement set, each requirement's contributing `(source, claim-id)` claims, and its `agreement` verdict — projects over that structure: assigns `REQ-NNN` ids in declaration order, derives each requirement's `sources` from its contributing claims, applies authority to mark winners and derive `status` from the agreement verdict (per §"Status and provenance derivation"), and writes `provenance.yaml`. The kernel never invents, drops, or re-groups requirements, never selects a winner the resolved authority did not, never overrides the agreement verdict, and never reads the bound `target` or its `shape` brief. It also emits the advisory `slice-synthesize-agreement-suspect` `review`-kind finding when an `agreed` requirement's claim values differ under cheap normalised-string comparison.
3. Commit `schemas/slice/synthesis-envelope.schema.json` (request + response) and embed it as `SYNTHESIS_ENVELOPE_JSON_SCHEMA`. The request carries the Evidence map, the resolved-authority path, shape brief, and the closed `forbidden-inputs-for-requirements-reconciliation` constraint list (no reconciliation skeleton). The response carries the populated `model.yaml` — the agent-authored requirement set, each requirement's contributing `(source, claim-id)` claims, its `agreement` verdict, `unit`, and prose — plus the four Markdown artifacts.
...
8. Update `/spec:refine` to call the CLI command. The engine handles the dispatch and the kernel projection; the agent's contribution is authoring the requirement set, its contributing claims, its agreement verdict, and prose — not driving the lifecycle, not deriving `sources`/`status`/winners, and no longer hand-authoring `provenance.yaml`.
9. Extend `specrun slice validate` with the six slice-model drift checks and their `Error` variants (`slice-model-{schema,requirement-drift,provenance-drift,target-drift,source-orphan,cross-ref-orphan,id-grammar}`). Drift validation runs identically regardless of execution mode.
...
- **Shared kernel, shared direction — and the contradiction this RFC resolves.** RFC-35 (landed) deliberately keeps `provenance.yaml` agent-authored and adds drift validation only: it found that the claim→requirement mapping "is not mechanically derivable from `Sources:` lines alone and remains synthesis judgment," so it deferred any writer verb. RFC-29's earlier draft contradicted that — it had the kernel *derive* `provenance.yaml` and `status` from each requirement's source-key `sources`, which a source-key list cannot support (it loses the losing claims and cannot tell `agreed` from `divergence`/`conflict`). This revision resolves the contradiction by carrying the missing input: the synthesis **response** now carries claim-level `contributing-claims` and an `agreement` verdict, so the kernel projects `provenance.yaml`, `sources`, winners, and `status` *faithfully* (agreement is judgment; winner selection is authority mechanics — see §"Status and provenance derivation"). RFC-35's "no writer verb" stance was correct *for its inputs*; RFC-29 supplies the claim-level input that makes `specrun slice provenance` a faithful projection rather than a lossy reconstruction. RFC-35's `specrun slice provenance` is therefore the thin standalone entry point onto the same projection kernel RFC-29 D3 wraps inside `specrun slice synthesize`; whichever RFC lands the module first, the other consumes it, and both produce the byte-identical `provenance.yaml` schema the landed `slice-provenance-drift` gate already enforces.
