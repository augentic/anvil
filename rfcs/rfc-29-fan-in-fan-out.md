...
2. **Agent-led plan-time lead reconciliation** ([RFC-29b](rfc-29b-reconciliation.md)) - an agent-led cross-source matching step that partitions each source's `Lead[]` into `scopes[]` and emits `slices[]` that bind each `(scope-id, project)` pair to a project from the request (the kernel derives `target` from the bound project), running under a stable input/output envelope wrapped by a CLI-owned projection kernel: schema validation, the global lead-partition invariant, slice-name derivation, journal events, and the existing plan writers. Cross-source matching is agent judgment; the operator curates at Gate 1 after propose.
...
| **D2 Lead reconciliation**             | Agent-led cross-source matching of `Lead[]` into `scopes[]`, binding each `(scope-id, project)` response slice to a project from the request (auto-bound when only one project exists; the kernel derives `target` from the bound project), under a CLI-owned projection kernel (partition invariant, project-binding validation, slice-name derivation, plan writers). Operator curates at Gate 1 after propose. | [29b](rfc-29b-reconciliation.md)                                    |
...
| `plan.reconcile.agent`                | M2a        | The lead-matching step (D2) ran in `agent` mode (the default and designed centre). One event per `specrun plan propose --from` invocation; its payload echoes each multi-member scope's `rationale` for audit/replay.                             |
| `plan.reconcile.completed`            | M2a        | `specrun plan propose --from` validated the agent grouping, enforced the partition invariant, derived slice names, and wrote `plan.yaml.slices[]`. |
...
| `plan-reconcile-scope-duplicate`              | M2a       | `plan propose --from` | Two `scopes[]` entries share the same `scope-id`, making slice bindings ambiguous.                                                                                                               |
...
| `plan-reconcile-slice-name-collision`         | M2a       | `plan propose --from` | Two agent-supplied explicit `name` values clash in one response. Kernel-derived names key on the unique `(scope-id, project)` pair and never collide.                                        |
