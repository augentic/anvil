...
2. **Agent-led plan-time lead reconciliation** ([RFC-29b](rfc-29b-reconciliation.md)) - an agent-led cross-source matching step that partitions each source's `Lead[]` into `concepts[]` (including semantic matches that exact id / alias cannot catch) and emits `slices[]` that bind each `(concept-id, target)` pair to a target, running under a stable input/output envelope wrapped by a CLI-owned projection kernel: locked groups, schema validation, the global lead-partition invariant, slice-name derivation, journal events, and the existing plan writers.
...
| **D2 Lead reconciliation**             | Agent-led cross-source matching of `Lead[]` into `concepts[]`, binding each `(concept-id, target)` response slice to a target and (workspace mode) a registry project, under a CLI-owned projection kernel (locked groups, partition invariant, project-binding validation, slice-name derivation, plan writers). | [29b](rfc-29b-reconciliation.md)                                    |
...
| `plan-reconcile-lead-orphan`                 | M2a       | `plan propose --from` | The response cites a `(source-key, lead-id)` pair absent from the request lead catalog.                                                                                                                        |
| `plan-reconcile-partition`                   | M2a       | `plan propose --from` | The response is not a well-formed **global** partition: a surveyed lead is unaccounted for, or a `(source-key, lead-id)` pair appears in more than one concept.                                                               |
| `plan-reconcile-locked-group-split`           | M2a       | `plan propose --from` | The response splits a kernel-proven locked group (exact id / alias) across concepts.                                                                                                                  |
| `plan-reconcile-concept-orphan`               | M2a       | `plan propose --from` | A `slices[].concept-id` value does not name a declared `concepts[]` entry.                                                                                                                   |
| `plan-reconcile-concept-unbound`              | M2a       | `plan propose --from` | A declared `concepts[]` entry has no `slices[]` row, so the concept would not produce any plan slice.                                                                                         |
| `plan-reconcile-project-binding-required`    | M2a       | `plan propose --from` | The response omits `project` on a slice in workspace mode (request carried a non-empty `projects[]`), or sets `project` on a slice in single-repo mode.                               |
| `plan-reconcile-project-orphan`              | M2a       | `plan propose --from` | The response binds a slice to a `project` absent from `registry.yaml`.                                                                                                                          |
| `plan-reconcile-project-target-mismatch`     | M2a       | `plan propose --from` | The response binds a slice to a project whose `registry.yaml` target does not equal the slice's `target`.                                                                                   |
