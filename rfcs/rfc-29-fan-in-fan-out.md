...
| `plan.reconcile.agent`                | M2a        | The lead-matching step (D2) ran in `agent` mode (the default and designed centre). One event per `specrun plan propose --from` invocation; its payload echoes each scope's `rationale` for audit/replay.                             |
