...
| [RFC-29c — Slice Synthesis Engine and Typed Model](rfc-29c-synthesis.md) | **M2b**   | D3, D4, D5, D8, D10, D13 |
...
2. **Agent-led plan-time lead reconciliation** (M2a, **shipped** — durable spec in [`specify-cli` `DECISIONS.md` §"Lead reconciliation (D2)"](https://github.com/augentic/specify-cli/blob/main/DECISIONS.md#lead-reconciliation-d2)) - an agent-led cross-source matching step that emits `slices[]`, each carrying a `scope` id, its matched `sources[]` (at most one lead per source), and a bound `project` from the request (the kernel derives `target` from the bound project), running under a stable input/output envelope wrapped by a CLI-owned projection kernel: schema validation, the global lead-partition invariant over scopes, slice-name derivation, journal events, and the existing plan writers. Cross-source matching is agent judgment; the operator curates at Gate 1 after propose.
...
| **D2 Lead reconciliation**             | Agent-led cross-source matching of `Lead[]` into `slices[]`, each carrying a `scope` id, its matched `sources[]` (at most one lead per source), and a `project` from the request (auto-bound when only one project exists; the kernel derives `target` from the bound project), under a CLI-owned projection kernel (partition invariant over scopes, same-source-fusion rejection, project-binding validation, slice-name derivation, plan writers). Operator curates at Gate 1 after propose. | [shipped — `DECISIONS.md`](https://github.com/augentic/specify-cli/blob/main/DECISIONS.md#lead-reconciliation-d2)                                    |
| **D3 Slice synthesis**                 | Agent-led cross-modal reconciliation of `Evidence[]` into the requirement set; CLI owns the synthesis envelope and the projection kernel (authority resolution, REQ-ids, status, rendered source lists, winner derivation, inline provenance, rendering). The agent-authored and persisted shapes are one schema (`model.schema.json`); the kernel re-derives its owned fields and ignores any the agent supplied (normalize, never reject).                                                         | [29c](rfc-29c-synthesis.md)                                         |
...
specrun archive prune --keep 20 --older-than 90 --format json                                  # M3 (prune the archive cache; ledger + git keep history)
...
| `plan.reconcile.agent`                | M2a        | `specrun plan propose --from` validated the agent response and is about to write slices. One event per successful invocation; payload carries `plan-name`, deduped `scopes[]` (`scope` id + optional `rationale`), and `slice-count`. Emitted atomically with `plan.reconcile.completed` — the skill does not call `specrun journal emit` for D2.                             |
| `plan.reconcile.completed`            | M2a        | `specrun plan propose --from` wrote `plan.yaml.slices[]`. Payload carries `plan-name`, `slice-count`, and derived `slice-names[]`. |
...
| `slice.synthesize.agent`              | M2b        | The synthesis envelope was dispatched to the operator's agent. One event per invocation.                                                        |
...
| `slice.archive.created`               | M3         | A merged or dropped slice directory was archived. Payload carries `slice`, `touched-specs`, the merge `outcome` summary, and the merge commit SHA — this is the durable append-only outcome-ledger entry that outlives the (prunable) archived folder. |
...
| `plan-reconcile-partition`                   | M2a       | `plan propose --from` | The response is not a well-formed **global** partition: collapsing `slices[]` by `scope`, a surveyed lead is unaccounted for, or a `(source, lead)` pair appears in more than one scope.                                                               |
| `plan-reconcile-slice-source-collision`       | M2a       | `plan propose --from` | A scope names two leads from the same source (a slice's `sources[]` carries the same `source` twice). Cross-source matching never fuses one source's candidate slices.                                                               |
| `plan-reconcile-fanout-source-mismatch`       | M2a       | `plan propose --from` | Two response slices share a `scope` id but carry differing `sources[]` sets, so the scope's reconciled membership is ambiguous.                                                                                                   |
| `plan-reconcile-slice-duplicate`              | M2a       | `plan propose --from` | After project auto-bind, two response slices share the same `(scope, project)` pair.                                                                                                   |
| `plan-reconcile-slice-name-collision`         | M2a       | `plan propose --from` | Two agent-supplied explicit `name` values clash in one response. Kernel-derived names key on the unique `(scope, project)` pair and never collide.                                        |
...
| `archive-prune-bad-entry`                    | M3        | `archive prune`       | An entry under `.specify/archive/` does not begin with a `YYYY-MM-DD-` date prefix, so its archive date cannot be read.                                                                                    |
...
| **M2b** ([RFC-29c](rfc-29c-synthesis.md))      | **Ready.** Single-artifact slice model (provenance inline, `provenance.yaml` as an on-demand projection), normalize-not-reject kernel projection, rendering pipeline, and D13 read-path guarantee are pinned.               |
...
The corpus is buildable from artifacts the framework already keeps — baseline `spec.md`, archived per-slice provenance, and the append-only outcome ledger (the `slice.archive.created` journal entries) — so the ledger is the natural first brick, with no graph database or writeback protocol required. A cheap intermediate that needs no index at all: when a slice touches a unit already present in the baseline, feed that existing `spec.md` into the synthesis envelope as read-only context (one optional, non-provenance-bearing field). That step is the litmus test for whether the fuller index earns its keep.
