...
2. **Agent-led plan-time lead reconciliation** ([RFC-29b](rfc-29b-reconciliation.md)) - an agent-led cross-source matching step that partitions each source's `Lead[]` into `scopes[]` (including semantic matches that exact id / alias cannot catch) and emits `slices[]` that bind each `(scope-id, project)` pair to a project from the request (the kernel derives `target` from the bound project), running under a stable input/output envelope wrapped by a CLI-owned projection kernel: locked groups, schema validation, the global lead-partition invariant, slice-name derivation, journal events, and the existing plan writers.
...
| **D2 Lead reconciliation**             | Agent-led cross-source matching of `Lead[]` into `scopes[]`, binding each `(scope-id, project)` response slice to a project from the request (auto-bound when only one project exists; the kernel derives `target` from the bound project), under a CLI-owned projection kernel (locked groups, partition invariant, project-binding validation, slice-name derivation, plan writers). | [29b](rfc-29b-reconciliation.md)                                    |
...
specrun plan propose --dry-run --format json                 # D2 request envelope (flat lead catalog + locked groups) # 29b
...
Cross-source lead matching and target binding are the `/spec:plan` agent step's judgment (D2); `specrun plan propose --dry-run` seeds it with a flat lead catalog and kernel-proven locked groups, and `specrun plan propose --from` is the kernel that validates the grouping and writes the slices through the existing plan writers.
...
All slice-model, build-request, and build-report schemas key on `(slice, target)` per D5 — none carries `outputs[]` or `output-id`. `proposal.schema.json` discriminates request vs response via closed `kind: request | response`: the request carries `leads[]` (one row per raw `(source-key, lead-id)` lead, with summary, optional `aliases[]`, and an optional `group: { group-id, match-basis }` block on rows the kernel proved are the same work by exact id / alias; a locked group is defined implicitly by the rows that share `group-id`, with no separate group index) and the `projects[]` topology (always at least one project; a single regular project is synthesized from `project.yaml`; each project entry carries its `target` adapter); the response carries `scopes[]` (members as `{ source-key, lead-id, match-basis? }` — `match-basis: semantic` is agent-authored on open leads, while locked members omit it and the kernel carries their proven basis forward — and optional grouping `rationale`) plus `slices[]` (each `(scope-id, project)` binding with optional `name`, `depends-on` in derived slice names, and the bound `project` — optional when only one project exists, since the kernel auto-binds it; the kernel derives each slice's `target` from the bound project's `projects[].target`). `synthesis.schema.json` discriminates request vs response via closed `kind: request | response`; the response `model` `$ref`s `**draft-model.schema.json`**, not the persisted model (D3a).
...
| `plan.reconcile.completed`            | M2a        | `specrun plan propose --from` validated the agent grouping, enforced the partition / locked-group invariants, derived slice names, and wrote `plan.yaml.slices[]`. |
...
| `plan-reconcile-partition`                   | M2a       | `plan propose --from` | The response is not a well-formed **global** partition: a surveyed lead is unaccounted for, or a `(source-key, lead-id)` pair appears in more than one scope.                                                               |
| `plan-reconcile-locked-group-split`           | M2a       | `plan propose --from` | The response splits a kernel-proven locked group (exact id / alias) across scopes.                                                                                                                  |
| `plan-reconcile-scope-orphan`               | M2a       | `plan propose --from` | A `slices[].scope-id` value does not name a declared `scopes[]` entry.                                                                                                                   |
| `plan-reconcile-scope-unbound`              | M2a       | `plan propose --from` | A declared `scopes[]` entry has no `slices[]` row, so the scope would not produce any plan slice.                                                                                         |
| `plan-reconcile-slice-duplicate`              | M2a       | `plan propose --from` | After project auto-bind, two response slices share the same `(scope-id, project)` pair.                                                                                                   |
| `plan-reconcile-slice-name-collision`         | M2a       | `plan propose --from` | Slice-name derivation produced two identical names in one response.                                                                                                                         |
