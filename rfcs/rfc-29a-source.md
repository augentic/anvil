...
| **D9 Adapter execution** (source side) | Source adapters declare a closed `execution: tool \| agent` field; `tool` dispatches synchronously; `agent` uses the two-phase `--phase prepare \| finalize` handoff. Target dispatch lands in [RFC-29d](rfc-29d-target.md). |
...
Source and target adapters declare a closed `execution` field on their respective `adapter.yaml`. This milestone lands the source side and the shared schema additions; the target side (`build` / `merge` dispatch) lands in [RFC-29d](rfc-29d-target.md).
...
1. **Prepare** (`--phase prepare`, the default). The CLI resolves the adapter, builds the four-root sandbox layout, scaffolds the output target (`evidence/` for `extract`, the lead-set target for `survey`), emits the `source.execution.agent` journal event, and prints a handoff envelope on stdout. `survey` prints `{ adapter, version, briefs-dir, source-dir?, scratch-dir, leads[], execution: "agent" }` (no `evidence-dir`); `extract` adds `evidence-dir` and a single-element `leads` array. Control returns to the agent.
2. **Finalize** (`--phase finalize`). The agent runs the brief against the prepared directory and writes outputs to the declared paths; a follow-up CLI call runs validate-before-visible, the `discovery.md` merge (`survey`) / Evidence persist (`extract`), and the cache write. `tool`-execution adapters ignore the phase flag — a single call runs the whole operation.
...
Making `execution` required was a coordinated two-repo migration that landed in one PR: (1) add the `execution` enum to both schemas without `required`; (2) stamp `execution: agent` on all eight first-party `adapter.yaml` files (five sources plus the three targets `omnia` / `vectis` / `contracts`, which take `agent` as a placeholder until M3 wires real target dispatch); (3) flip `required` on; (4) add the `adapter-execution-mode-required` loader rejection and the `adapter-execution-agent-cache-conflict` parse check; (5) run the cross-repo `rg` sweep the `crates/workflow/src/adapter/` touch rule mandates.
