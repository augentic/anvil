...
```text
<change>/                        # git-backed: `.emery/change/` in-place, its own repository detached (RFC-88)
  change.md
...
| - | -------- | ----------- |
| D1 | **The change is a self-contained, git-backed fact tree.** In-place mode it is `.emery/change/` in the product repository (already committable — only `scratch/` and `workspace/` are ignored), sitting beside but never containing durable project state (`project.yaml`, `specs/`, `decisions/`); detached mode ([RFC-88](rfc-88-detached-changes.md)) it is its own repository. Sharing, review, and durability are ordinary git push / pull / PR. | Extended planning survives by construction. A second operator clones the change; a reviewer reads a diff of specs; nothing new is invented for transport, backup, or history. |
...
- **Phase C — Execution authorization and multi-actor.** Approval facts emitted by `plan execute`, claim conflicts as typed violations, multi-actor change-tree merge coverage, shift-left flow proven end to end: author → refine all slices → review the committed spec set → execute (build + merge only).
