...
> **Status:** Implemented — step 1 of the platform-migration series ([platform.md](platform.md)). Phases A–C and D20 landed; product acceptance closeout (S23) complete. Series stand-ins deferred to [RFC-88](rfc-88-detached-changes.md): merge-time `apply` and the flat `.emery/` change home.
>
...
>
> **Series:** Step 1 of the [platform-migration series](platform.md) by product ownership — the fact substrate every later step consumes. Recorded `base.yaml` pins, content-addressed `builds/<digest>.yaml` records, and one-member waves ride [RFC-87](rfc-87-working-trees.md)’s workspace contract. The remaining series stand-ins live in later RFCs — merge-time `apply` and the flat `.emery/` change home ([RFC-88](rfc-88-detached-changes.md) deletes `apply` and relocates the change home). Later series RFCs assume this substrate’s authorization epoch, per-leaf coverage, and one-member target waves.
>
...
| `emery plan status`                            | After a fresh author, **resume** is `emery plan execute` / `/emery:execute` (D26). Next-actions may project `refine <slice>` / review-gaps while unrefined or gappy; then `plan execute` (build / merge) when Ready, or `plan execute --waive…` when skipping Ready (D22). Projects Authorized when a covering epoch exists — never an `approved` rung                                                                                                                                                                                                                        |
