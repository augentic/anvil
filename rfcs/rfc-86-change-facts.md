...
> **Status:** Draft — gap/shift-left, multi-slice claim, in-scope membership, pin/value vocabulary, no-`approve`-verb, no-`plan refine`-verb, post-author resume, and change-home layout decisions closed (D11–D28); [open review questions](#open-questions) below must close before an implementation plan is cut. Sibling-series consistency remains an implementation task (D19), not a product blocker. Change-home layout, the absence of RFC-88 `change approve`, the public `author → execute → archive` surface, and `refine-under-epoch` follow [RFC-88](rfc-88-detached-changes.md).
>
...
>
> **Owns:** the fact-based change substrate — projected status, per-actor event logs, claims, execute-implied approval facts, pinned judgment inputs (using the landed RFC-87 snapshot identity), merge-finalized requirement identity, shift-left refine practice / gap gate before build (no separate `approve` or `plan refine` verb), and the single-actor desktop as the degenerate case of the same substrate.
>
...
Preferred path   author → review topology → slice refine (in-scope) → review gaps → execute (build → merge)
Also allowed     execute with refine-under-epoch (RFC-88) for leaves still missing specs; gap policy still blocks build
```

- Operators review **topology** after author (before paying for extract/synthesis when they choose the preferred path), then read **real specs** after `slice refine` and check the **gap inventory** before generation. Prose quality is human-owned — the engine does not record how spec reading happens (see D17).
...
1. **Author** — `emery plan author` (via `/emery:plan`) surveys bound sources and reconciles leads into slices (detached mode also runs RFC-88’s internal discovery / decomposition phases). Produces the authored plan artifacts (`discovery.md` / RFC-88 `leads.md` + `decomposition.yaml` as applicable, `change.md`, `plan.yaml`). **Stops here** — no extract or synthesis. The operator can review topology before paying for refine. Folding refine into author is rejected (see D13).

### Preferred path — specs before generation

2. **Review topology** — Human pause: re-cut slices / decomposition if needed. No topology-approve verb (see D21). Author epilogues and fresh-plan `plan status` still resume at `emery plan execute` / `/emery:execute` (see D28) — that is the next *plan* verb; refine is a breakout.
3. **Refine** — `emery slice refine` per slice (or agent/status-driven fan-out over unrefined in-scope slices — on the plan and not dropped, see D24): extract Evidence, synthesize `proposal.md`, `spec.md`, `design.md`, `tasks.md`, and `model.yaml`, and record input pins. Claims are exclusive per slice (D23).
4. **Review gaps** — Read `spec.md` and related slice artifacts, and the **gap inventory** (see below). Deal with typed issues before generation: close conflicts, clear or plan to waive unknowns. Prose quality and acceptance-criteria depth are operator-owned (see D17).
5. **Iterate** — Fix inputs (richer intent/docs, authority overrides, corrected sources), then re-refine only the affected slices with `emery slice refine <slice>`.

### Execute — authorization, then code

6. **Execute** — `emery plan execute` (via `/emery:execute`) **is** the plan-approval gesture and the sole privileged-start surface (RFC-88 D8). It records a covering execute-implied approval / `plan.execute.started` fact. For each in-scope leaf it accepts either an **existing spec digest** (preferred shift-left path) or RFC-88 **`refine-under-epoch`** coverage when specs are not yet present. Under that epoch, `slice refine` may still run before build. Once every leaf that will build is refined, conflicts are gone, and remaining `[unknown]`s are either cleared (Ready — D22) or explicitly waived per requirement on this command (D16), execute runs **build → merge**. Gap and slice-status gates refuse **build** when issues have not been dealt with. There is no prior `approve` verb (see D20 / D26).
7. **Finalize** — Operator publishes; archive as today (`plan archive` / `/emery:finalize`).

`/emery:plan` remains an ultrathin wrapper over `plan author` only. Hand-driven breakouts (`emery slice refine` / `build` / `merge`) still work. This RFC adds **no** `emery plan refine` batch verb and **no** `approve` verb. The public workflow stays `plan author → plan execute → plan archive`.

One-slice changes stay the same shape: author → (optional topology pause) → `slice refine` → review → `plan execute` (build → merge), or author → `plan execute` with refine-under-epoch then build. `plan status` resume after a fresh author is `emery plan execute` / `/emery:execute` (D28); next-actions may still name `emery slice refine <slice>` while in-scope slices are unrefined; close-gaps or `plan execute --waive…` when refined with open unknowns (not Ready — D22); and `emery plan execute` when Ready (see D20 / D26).

### Who owns what

| Work | Preferred before execute | Under execute |
| ---- | ------------------------ | ------------- |
| Survey and propose slices (`plan author`) | yes | no |
| Extract and synthesize specs (`slice refine`) | yes (shift-left practice) | allowed when authorization covers `refine-under-epoch` (RFC-88) |
| Review specs (prose) and close typed gaps | yes | yes — before build |
| Imply plan approval by starting execute | human review precedes it when possible | yes — `plan execute` is the gesture; gap/status gates enforce readiness before build (D20 / D26) |
...
**Gate authority stays per requirement** — each row is still `(slice, req, status)`. When the same lead is multi-homed across slices (coverage is at-least-once; cross-cutting leads in `change.md`), one thin or contradictory lead can surface as several inventory rows. `plan gaps` therefore also offers a **presentation rollup**: annotate or group open findings that share a contributing `(source, lead)`, and suggest the slice-selector set for a follow-up `slice refine` after the shared input is fixed (see D18). The rollup is navigation only — it does not merge findings, change the execute gap gate, or introduce lead-wide waivers.
...
| ------- | ----------- | --------- |
| `[unknown]` | Operator supplies the missing information: enrich a source (intent, docs, captures), or re-scope the lead so the requirement is not invented. Build stays blocked until the tag clears or that requirement is explicitly waived on `plan execute` (see D16) | `emery slice refine` that slice (and any siblings the rollup lists for the same lead) |
...
**`plan execute`** (before entering build for a leaf — whether specs were pre-refined or produced under `refine-under-epoch`) checks a gap policy alongside existing slice-status / lifecycle gates:
...
| `[divergence]` | **Allow, but list** — informational only; authority already chose (`intent` > `documentation` > `behaviour`, plus any per-slice override). No acknowledgment or decision required to start execute or enter build. Override and re-refine only when the wrong source won (see D15) |

If the policy fails, execute refuses (or stops before build) and prints the inventory. The operator may:
...
| Approved | A covering execute-implied approval fact exists for the current plan and specs (may carry unknown-waivers and/or refine-under-epoch coverage). Distinct from Ready even when the waive list is empty — Ready means “clean path to build”; Approved means “execute has begun under a covering binding” (D20 / D26) |
...
| D10 | Hard cut (pre-1.0) | No compatibility shims for old status fields, single journal, or unrecorded approval |
| D11 | **Shift-left refine is preferred practice; execute owns authorization + build/merge** | Specs are preferred before generation; public surface stays author → execute → archive. Execute may authorize RFC-88 `refine-under-epoch`; gap policy still blocks build |
| D12 | **Gaps gate build, not refine success** | Incomplete Evidence can still refine; it cannot silently enter build |
| D13 | **No `plan refine` verb; author stays topology-only; refine is `slice refine`** | Aligns with RFC-88’s three-verb public surface. Author / `/emery:plan` do not extract or synthesize. Rejected: fold refine into author; invent `emery plan refine` as a fourth plan verb |
| D14 | **`[unknown]` always blocks build** | Thin intent is not an exception — close the gap or waive it explicitly on execute; generation must not invent missing information |
...
| D19 | **Sibling-series consistency is an implementation task** | When implementing this RFC (especially Phase C), review `platform.md` and remaining unimplemented series RFCs (RFC-88…RFC-92; RFC-87 is already Implemented) for prose that still assumes a mandatory `plan approve` verb, interactive execute auto-*waive*, or a `plan refine` verb this RFC rejects. Do not litigate or cascade-rewrite those docs as part of freezing this RFC; later RFCs may further review this work and roll back or refactor anything that becomes obsolete. Known drift is listed under [open questions](#open-questions) |
| D20 | **No `approve` CLI verb; starting execute implies plan approval** | `emery plan execute` / `/emery:execute` is the approval gesture. Gap and slice-status gates refuse undealt issues before build. When gates pass, execute records a covering approval-implied fact (digests + gap outcome + any unknown-waivers). Rejected: a mandatory `emery plan approve` (or any other `approve`) verb before execute |
...
| D26 | **Remove / never ship an Emery CLI `approve` verb for plan build** | Operators review and deal with issues; simply starting execute implies approval. Internal slice-status and gap gates prevent undealt work from entering build. Any prior draft of this RFC that introduced `emery plan approve` is superseded |
...
| Command | Change |
| ------- | ------ |
| `emery plan approve` (any `approve` plan verb) | **Removed / never shipped.** Not part of the CLI surface. Starting `plan execute` implies plan approval; gap and slice-status gates enforce readiness before build (see D20 / D26). No `--slices` topology-approve substitute (D21) |
| `emery plan refine` | **Not shipped.** This RFC adds no plan-phase refine batch verb (D13). N-many refine uses `emery slice refine` plus status next-actions / agent fan-out. Aligns with RFC-88’s three-verb public surface |
...
| `emery plan execute` | **Is** the plan-approval gesture and sole privileged-start surface (D20 / D26; RFC-88 D8). Enforces the gap policy and slice-status gates before **build**; on success records a covering approval-implied fact (digests + gap outcome + any unknown-waivers); accepts existing per-leaf spec digests **or** RFC-88 `refine-under-epoch` coverage; runs build → merge for ready leaves. For `[unknown]` leftovers only: repeatable `--waive <slice>/<req>` with required `--reason` (see D16). No `--force`, no bulk/all-gaps waive, no separate `plan waive` / `approve` / `plan refine` verb |
| `emery slice refine` | The refine implementation and breakout (shift-left practice, gap closure, single-slice re-refine, and under-epoch refine); records input pins |
| `/emery:plan` | Unchanged contract: elicit → `emery plan author` → relay; stops after topology. Does **not** run refine |
| `emery plan status` | After a fresh author, **resume** is `emery plan execute` / `/emery:execute` (D28). Next-actions may include `slice refine <slice>` / review-gaps while unrefined or gappy; then `plan execute` (build / merge) when Ready, or `plan execute --waive…` when skipping Ready (D22) |
...
| **B** | Recorded pins (D4 / D25) + merge-time requirement ids; replace interim freeze-at-build with a recorded pin read (depth of `build/patch.yaml` re-home — [open question 1](#open-questions)) | Safer parallel refine; drift diagnostics; build prepares from a recorded base pin instead of freezing the ambient tree |
| **C** | Gap inventory (+ shared-lead presentation rollup), execute gap gate (no `approve` / no `plan refine` verb), shift-left practice proven via `slice refine`, RFC-88 `refine-under-epoch` alignment; sibling-series consistency pass (D19) | Public rhythm stays author → execute → archive; preferred path refines with `slice refine` and reviews gaps before build; starting execute implies approval; multi-homed leads correlate in `plan gaps` without changing the per-req gate. Implementation also reviews `platform.md` / RFC-88…RFC-92 for stale mandatory-`plan-approve` or invented-`plan-refine` assumptions — without blocking on a full cascade rewrite here |
...
4. **Pins (D25):** refine records `base.yaml` against RFC-87 `SnapshotId`s (and baseline-spec digest); build prepares from a recorded base pin rather than freezing the ambient product tree; validate detects pin drift. (Depth of `build/patch.yaml` re-home — open question 1.)
5. **Shift-left practice (D11 / D13):** after authoring, `/emery:plan` / `plan author` do not run refine. Specs are minted by `emery slice refine`. The Phase C happy path refines in-scope slices and deals with gaps before build; execute with existing spec digests then performs build and merge only. RFC-88 `refine-under-epoch` remains an allowed authorization coverage for leaves that lack specs at execute start — gap policy still blocks build. The CLI ships no `emery plan refine`.
...
3. ~~**How does plan-phase / shift-left refine start?**~~ **Closed — D13 (revisited against RFC-88).** `/emery:plan` / `emery plan author` stop after topology — they do **not** extract or synthesize. Specs are minted by the existing `emery slice refine` breakout (N-many via status next-actions / agent fan-out over unrefined in-scope slices). This RFC **adds no** `emery plan refine` verb — the public plan surface stays RFC-88’s `author → execute → archive`. Shift-left remains the preferred Phase C practice (refine and review gaps before build); RFC-88 `refine-under-epoch` remains allowed authorization coverage when specs are absent at execute start. Rejected: folding refine into `plan author` (pays synthesis before topology review; blurs the two review seams; fights RFC-88 D3 / R7, which fold *discovery* into author, not refine); inventing `emery plan refine` as a fourth public plan verb (fights RFC-88’s three-verb surface and the synced substrate’s “adds no workflow subcommand” posture).
4. ~~**Should `[unknown]` block by default?**~~ **Closed — D14.** Always block **build**. `[unknown]` means insufficient information was available to the agent; the operator must provide it (or explicitly waive on execute) before build. Rejected: warn-only for intent-only / N=1 (desk-testing — unpredictable generation that compounds through later phases); context-sensitive defaults keyed on source count or change shape (two policies to teach; under-protects thin multi-slice intent).
...
9. ~~**Sibling docs**~~ **Closed — D19.** Consistency with `platform.md` and later series RFCs is an **implementation task** for this RFC (especially Phase C): review them for stale mandatory `plan approve` / interactive auto-*waive* / invented `plan refine` assumptions. RFC-87 is already Implemented — the remaining unimplemented consumers are RFC-88…RFC-92 plus the spine docs. This RFC does **not** cascade-rewrite those documents at decision-freeze time; known contradictions are tracked as open question 2 (spine/platform; RFC-88 itself is aligned — see closed items 15–17). Later RFCs remain free to review the landed shape and roll back or refactor anything that becomes obsolete. Rejected: freezing Phase C product design on a full sibling rewrite now; treating all series-doc drift as a blocker to freezing this RFC’s product decisions.
10. ~~**Is a separate `plan approve` required in the happy path?**~~ **Closed — D20 / D26.** **No.** Starting `plan execute` / `/emery:execute` implies plan approval after the operator has reviewed and dealt with issues. Gap and slice-status gates refuse undealt work before build; when they pass, execute records a covering approval-implied fact (digests + gap outcome + any unknown-waivers) for audit and stale-on-re-refine. Rejected: a mandatory `emery plan approve` (or any other plan `approve`) verb before execute — extra ceremony with no job once human review and status/gap gates exist; agents and operators would rubber-stamp it. Auto-waive on execute remains rejected (D16). Prior draft text that required `plan approve` and forbade execute minting approval is superseded.
11. ~~**Topology-only approval: command shape and consumers**~~ **Closed — D21.** **No topology-approve CLI surface.** Human pause after author is the primary topology review seam — catch slicing issues before expensive refine — and needs no engine gate and no recorded `--slices` / topology-approve verb. Handoff is social/git. Rejected: hard-gating refine on topology approval (second mandatory ceremony; fights solo-laptop / N=1); soft warn-only refine gate (noise agents ignore); optional `plan approve --slices` as a plan-phase recording surface (extra verb next to a CLI that deliberately has no `approve`); a separate topology verb for the same reason.
...
17. ~~**Post-author resume**~~ **Closed — D28.** After successful author, the operator-visible **resume** remains `emery plan execute` / `/emery:execute`. Author epilogues and `plan status` for a freshly authored plan must project that resume (Acceptance #14). Topology/decomposition review stays human-owned prose in the hint (“review the plan, then run …”), not a verb (D21). Status may still name `emery slice refine <slice>` as a *next-action* while in-scope slices are unrefined. Aligns with today’s wire and with RFC-88’s public `author → execute → archive` surface. Rejected: invent `emery plan refine` as the post-author resume (fourth plan verb; fights D13 / RFC-88); fold refine into author so the resume can stay execute while specs appear mid-author (option F — rejected by D13); require a topology-approve pause command before any next step.
...
- An `emery plan refine` (or other plan-phase refine batch) verb — refine stays `slice refine`; public surface stays author → execute → archive (see D13 / D28).
- Folding extract / synthesize into `plan author` / `/emery:plan` — author stays topology-only; RFC-88 folds discovery into author, not refine (see D13).
...
- A mandatory or optional `emery plan approve` (or other plan `approve`) verb before execute — extra ceremony; operator review plus status/gap gates already cover the job; starting execute is the approval gesture (see D20 / D26).
...
- Fold refine into `plan author` / `/emery:plan` — spends extract/synthesis before the operator can re-cut the slice list; collapses topology review and spec review; fights RFC-88, which folds discovery into author, not refine (see D13).
- Invent `emery plan refine` as a fourth public plan verb — fights RFC-88’s `author → execute → archive` surface and the synced substrate’s “adds no workflow subcommand” posture; N-many refine uses `slice refine` + status (see D13 / D28).
- Point post-author resume at a new `plan refine` verb — same fourth-verb problem; keep resume at `plan execute` and let status suggest `slice refine` as a next-action (see D28).
- Forbid RFC-88 `refine-under-epoch` so execute can never authorize refine — over-constrains the sibling authorization shape; adopt under-epoch coverage and keep the gap policy on **build** (see D11).
...
- Hard- or soft-gating refine on topology / slice-list approval — second ceremony (or ignorable warning) without improving the execute gate; human pause after author is enough (see D21).
...
- Exclude already-merged from the membership noun — smuggles execute into Ready; covering fact binds digests at execute-start time (see D24).
...
- Build reads the recorded target-base pin and passes it to `prepare` — delete the interim `seam.freeze()` at build start once pins exist (open question 1 for how far the `build/patch.yaml` re-home must go in the same phase). Do not scope interim `apply` deletion into this RFC — [RFC-88](rfc-88-detached-changes.md) owns that cut.
...
- No guest `plan refine` orchestration and no `plan refine` CLI verb (D13). `slice refine` remains the refine implementation; claims each slice exclusively (D23; in-scope = on the plan and not dropped — D24). Claiming slice B must not require slice A to be unclaimed or refined.
- Guest `plan execute` is the sole privileged-start surface: records a covering execute-implied approval / `plan.execute.started` fact (D20 / D26); accepts per-leaf existing spec digests **or** RFC-88 `refine-under-epoch` coverage; may dispatch `slice refine` under that epoch; enforces gap + slice-status gates before **build**; then runs build → merge. There is no `plan approve` or `plan refine` operation to call or wire. A single execute process may walk entries sequentially, but must not reimpose a plan-wide single-active gate that prevents another actor from claiming a different slice (D23). Build/merge continue to use the landed Workspaces capability.
- Author epilogue / fresh-plan `plan status` resume stays `emery plan execute` / `/emery:execute` (D28). Unrefined next-actions may name `emery slice refine <slice>`.
...
- New events: per-slice claim + existing refine events, `plan.execute.started` / approval-implied fact (unknown-waivers nested here; refine-under-epoch coverage as needed), claim/release, `fact.retracted`, identity-mapped merge. Do not invent a `plan.approved` event that implies a separate approve verb. Do not invent a `plan.refined` batch event that implies a `plan refine` verb.
...
- Shift-left fixture: author → `slice refine` (per in-scope slice) → gaps → fix conflicts / waive unknowns on `plan execute` → build/merge; also cover execute with `refine-under-epoch` then refine-before-build; refuse conflict-waive and bulk-waive shapes; refuse build while gaps remain without matching `--waive` (D20 / D26); refine succeeds with no topology-approve ceremony (D21). Ready/Approved fixture: refined + open unknowns projects not-Ready; `plan execute --waive…` reaches Approved without Ready; clearing unknowns then projects Ready before a clean execute (D22). In-scope fixture: drop one of two refined slices; `plan gaps` / Ready / execute gap gate ignore the dropped entry while the sibling remains on the plan (D24). CLI surface fixtures: `emery plan approve` and `emery plan refine` are absent (unknown subcommand / not wired). Post-author resume fixture: author hint and fresh-plan status resume name `emery plan execute` / `/emery:execute` (D28).
...
- During Phase C implementation, review `platform.md`, roadmap, and series RFCs for prose that still assumes a mandatory `plan approve` verb, interactive auto-waive, or an invented `plan refine` verb (open question 2). Refresh “depends on completed 86” / completion-order language where RFC-87’s stand-in landing made it stale. Record or fix drift as ordinary delivery work; do not block this RFC’s product freeze on a cascade rewrite. Follow [RFC-88](rfc-88-detached-changes.md) when reconciling siblings — it has no `change approve`, uses execute as the sole authorization surface, keeps the three-verb public workflow, and defines `refine-under-epoch` (closed items 15–17).

**Hard cut**

- Pre-1.0: re-init over migration; no shims for old status fields, single journal, plan-wide single-active-entry, global synthesize-time ids, a plan `approve` verb, a plan `refine` verb, or build-time ambient freeze once recorded pins exist.
