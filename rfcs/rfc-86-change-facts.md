...
This RFC replaces those four mechanics with one substrate: **a change is a self-contained tree of immutable facts; all status is projected, never stored; every judgment output is a persisted, pinned, digest-identified artifact; and `plan execute` records a digest-bound `plan.execute.started` fact before privileged work.** There is no plan-approval verb, status, or artifact — execute is the only operator surface, and the fact is an orchestration event like `slice.build.started`. Everything else — the WIT seam, the adapter contract, the delta-spec grammar, the evidence/model schemas, the validation registries, and the slice loop — stays load-bearing.
...
- **Facts** — append-only events in per-actor logs: claims, validations, execute starts, build outcomes, merges, retractions. The existing closed `EventKind` taxonomy, re-homed and extended.
...
Privileged work (build, merge) may proceed only while a `plan.execute.started` fact covers the current plan digest and whichever spec digests existed when that fact was appended — a gate over facts, not a projected plan status. There is no `approved` rung.
...
| D6 | **`plan execute` records `plan.execute.started`.** Before privileged work, `emery plan execute` appends `plan.execute.started` with the current plan digest and per-slice spec digests then in scope; the event envelope identifies the actor. Execution after shift-left refinement covers specs, while execution before refinement covers topology only. There is no plan-approval verb, status, file, or projected `approved` rung — the fact is an orchestration event in the same family as `slice.build.started`. | "Who started execution against which digests" is answerable from the fact log; a changed plan or spec requires a fresh execute start before privileged work continues. |
...
- `emery plan execute` — appends `plan.execute.started` over the current plan and spec digests before privileged work; otherwise unchanged as the drained loop.
...
- New `EventKind` variants: `plan.execute.started`, `slice.claimed`, `slice.claim-released`, `fact.retracted`, `slice.merge.identity-mapped` — extending the closed taxonomy.
...
- `plan.execute.started` has a closed payload: `plan-digest`, a sorted `spec-digests` map (empty for topology-only start), and optional `discovery-digest` (absent in-place and required for detached changes by RFC-88). The common event envelope supplies actor and per-actor sequence. There is no `approvals/` directory and no plan-approval vocabulary in status or CLI.
...
```json
{"timestamp":"2026-08-05T04:30:00Z","actor":"operator-a","sequence":7,"event":"plan.execute.started","payload":{"plan-digest":"sha256:…","spec-digests":{"orders-api":"sha256:…"},"discovery-digest":"sha256:…"}}
```
...
4. A plan refined fully before execution runs as a build-and-merge-only loop; a build failure leaves every artifact and fact intact and re-entry resumes from the projection; `plan execute` records topology-scope or spec-scope `plan.execute.started` according to the artifacts then present, and a changed plan or spec requires a fresh execute-start fact before privileged work continues.
...
7. `cargo make ci` is green; projection determinism, fact-union merge, claim conflicts, identity finalization, staleness, execute-start digest coverage, and retraction walk-back are covered as crate-level integration tests over local fixtures.
