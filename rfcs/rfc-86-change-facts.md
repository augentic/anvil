...
> **Status:** Draft — substrate decisions D1–D11 aligned with [RFC-88](rfc-88-detached-changes.md) / [RFC-89](rfc-89-publication-sets.md); product decisions D12–D27 closed (gap/shift-left, multi-slice claim, in-scope membership, pin/`cid` vocabulary, Phase B stand-in retirement bar, no-`approve`-verb, no-`plan refine`-verb, post-author resume). All [open review questions](#open-questions) are closed — ready for an implementation plan. Remaining sibling-doc drift (shipped “nothing is stamped” prose, leftover `plan approve` / `plan refine` assumptions outside the spine) is ordinary Phase C delivery under D20, not a product blocker.
>
...
- Spine/`platform.md` authority and chronology are already refreshed (closed item 1): D6 / D17 win; 87 landed with stand-ins; ownership order ≠ landing chronology. During Phase C implementation, review remaining series RFCs and shipped operator docs (`AGENTS.md`, `workflow.md`, CLI help) for leftover `plan approve` / projected `approved` / invented `plan refine` / “nothing is stamped” wording — update those when `plan.execute.started` lands. Record or fix drift as ordinary delivery work. Follow [RFC-88](rfc-88-detached-changes.md) when reconciling siblings — it has no `change approve`, uses execute as the sole authorization surface, keeps the three-verb public workflow, defines `refine-under-epoch`, and routes accepted-`cid` through this RFC’s D9 waves (closed items 16–19).
