...
> Status: **Draft v0 — provisional until the decision gate ([decisions/](decisions/) ADR-0001…0006) is accepted.** Sections marked `[ADR-NNNN]` take their final shape from that decision. This document is written forward from the operator journey in [product.md](product.md), not backward from the current implementation. It becomes v1 in remediation Phase 2 and is thereafter the document every agent task brief cites.
>
...
One process, one change home, one loop. Native-only deployment `[ADR-0002]`: adapters are compiled-in crates behind the `adapter::Source` / `adapter::Target` operations traits. The WIT contract is retained as the documented seam shape for a future component deployment; it is not linked into the shipped binary.

### Kept kernels (ported, not rewritten)

| Kernel | Why it survives both reviews |
| --- | --- |
| Content-addressed snapshot store + CID identity | Store-neutral, deployment-independent digests; load-bearing for detached delivery |
...
adapters (sibling)    — compiled-in first-party sources and targets over TargetContext (A10/A13/A15)
```
...
- **One generated type family** for Claim / Lead / PhaseReport used by SDK parse, converters, and artifacts (A1, A8, A16). Claims carry per-kind extras; extract fails when required extras are absent. No "unmodeled keys are ignored" paths.
...
| --- | --- |
| Component/marketplace adapter distribution | third-party adapter engagement (ADR-0002) |
...
The executable definition of this document. Scripted model, offline, temp change home; runs in CI on every push; green is the definition of done for every remediation increment:
