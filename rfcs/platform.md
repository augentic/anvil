...
> Status: Planning spine for the active RFC-86…RFC-98 / RFC-103 programme. RFC-99 through RFC-102 are parked and deliberately excluded from the active dependency map. Each RFC owns its decisions; this document owns delivery sequence, programme state, and fit.
>
...
**Current product gap:** RFC-88 must retire merge-time `apply`, establish accepted-CID execution, and deliver the detached multi-repository change loop. RFC-95 then seals results for publication.
...
- **Active** — accepted direction with an observed services need or an immediate evidence gap.
- **Evidence-gated** — accepted architecture whose implementation starts only when its stated measurement or engagement trigger fires.
- **Parked** — preserved design option, excluded from the active dependency map and staffing plan.

An RFC number is a stable design identity, not a promise that every lower number must be implemented first. Delivery order is the sequence below. RFC-93 and RFC-103 were renumbered while both were unimplemented: operator boundary moved forward to RFC-93; outcome learning moved to RFC-103 after its required signal and evaluation producers. RFC-99 through RFC-102 retain their identities while parked and are never reused.

## Active delivery order

### 1. Instrument and govern the current loop
...
[RFC-88 Detached Changes](rfc-88-detached-changes.md) remains the active product contract. Implement it in internal cuts rather than creating more RFCs:
...
### 4. Prove trustworthy outcomes

[RFC-97 Native Verification](rfc-97-native-verification.md) remains one RFC with two phases:

- **Phase A** depends only on implemented RFC-87/90 and supplies host-attested `slice-attempt` verification.
- **Phase B remains unchanged** and follows RFC-96 with `frontier-domain | complete-domain` contexts, protected-input closure, and distributed placement.
...
[RM-29 Governed model evaluation asset](roadmap.md#rm-29-governed-model-evaluation-asset) follows stable RFC-92 usage and RFC-97 Phase A verification; RFC-98 adds the modernization conservation rung. Public evidence is a separate deliverable from RFC-103's private blind promotion harness.

### 5. Learn from real outcomes
...
RFC-94, RFC-96, RFC-97, and RFC-98 enrich the same outcome record as they land. They are not hard prerequisites for creating the record.

### Retained scale lane

[RFC-96 Concurrent Execution](rfc-96-concurrent-execution.md) remains active and unchanged. It follows stable RFC-88/91 contracts and owns work-item scheduling, task graphs, deterministic composition, domain convergence, and multi-member waves.

It is not on the default services critical path. Start it when RFC-92/RFC-97 timing shows model work or serial refinement is a material programme bottleneck, or when a contracted engagement requires the task-graph and domain-convergence semantics directly. A concurrency cap of one remains the reference mode.

RFC-97 Phase B remains attached to RFC-96 exactly as specified in RFC-97. Parking the later platform programme does not remove that phase.
...
5. **Publish evidence before claiming a moat.** RM-29 tests the smaller-model-under-governance thesis rather than repeating it.
...
7. **Scale only when scale is the bottleneck.** RFC-96 remains designed and available; later platform modes remain parked.
...
Lifecycle integrity is not caller authorization. [RFC-93](rfc-93-operator-boundary.md) records actor identity and enforces a separate deployment-owned operator grant before guest dispatch. Actor class is not a permission bit; a grant cannot waive an engine gate.

As the operator becomes an agent, read-only topology, proposal, gap, and stop projections become the human audit surface over that driver. [RM-24](roadmap.md#rm-24-operator-control-surface) is therefore assurance work, not merely ergonomics.

## Lessons absorbed from comparable systems
