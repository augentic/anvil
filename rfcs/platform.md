...
| Step | RFC                                      | Title                | Delivers                                                                                                                                                                                                                                                                                                               | Depends on                   |
| ---- | ---------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| 5    | [RFC-90](rfc-90-build-verification.md)   | Build Verification   | Engine-owned build loop over separate `build` / `repair` / `verify` / `review` WIT ops; bounded repair policy; durable intermediate reports; final report assembly; deterministic native verification handed to [RFC-93](rfc-93-native-verification.md)                                                                      | completed 87                 |
...
- **RFC-86 is product-ownership step 1 and Phases A–C are landed** — the fact substrate every later step consumes (projected status, per-writer logs, `plan.execute.started`, recorded pins, one-member waves, `builds/<digest>.yaml`). Product acceptance checklist closeout remains the final gate. It deleted the mechanics later steps would otherwise have synchronized (stored status, the single journal file, synthesis-time identity, unrecorded execute starts) and delivered the shift-left refine / gap-gate flow.
...
```text
RFC-86  (fact substrate — Phases A–C landed; acceptance closeout)
...
- **[RFC-93 Native Verification Profiles](rfc-93-native-verification.md)** (future) — deterministic host-tool verification after the platform series; joins [RFC-94](rfc-94-platform-readiness.md) Phase C as a worker capability.
- **[RFC-94 Platform Readiness](rfc-94-platform-readiness.md)** (draft) — hosted/fleet readiness spine after RFC-92: Omnia conformance gates, adapter values and fleet locks, capability-scoped workers, model/secret/ingress host policy, and multi-tenant deployment roots. Does not redefine claims, lifecycle, or convergence.
