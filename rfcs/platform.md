...
A whole-codebase review of that implemented substrate is [architecture-review.md](architecture-review.md) (second-pass findings folded in): verdict, findings, and a subtraction-first corrective sequence. It is a review, not an RFC, and it argues for closing those cuts before staffing RFC-92 / RFC-94 / RFC-97. **A remediation programme is now the plan of record** — [remediation-plan.md](remediation-plan.md), gated by the product definition ([product.md](product.md)) and the decision log ([decisions/](decisions/)); live scope is the specification generator ([ADR-0008](decisions/0008-spec-generator-programme.md)). Feature-RFC staffing is frozen until the spec walking skeleton is green.
