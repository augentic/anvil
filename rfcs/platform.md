...
2. **Pin authorship timing** — source snapshots close at plan authoring or detached discovery. The landed cut currently freezes the product tree into refine-time `base.yaml`; RFC-91 retires that record — the refinement manifest pins source/planning/baseline inputs and wave open records the then-current target base before `prepare`. No build consumes an unrecorded ambient tree.
