...
RFC-86, RFC-87, and RFC-90 are implemented: per-writer facts, `plan.execute.started`, recorded pins, one-member waves, `BuildRecord`s, private `prepare` / `capture` / `discard` workspaces, and the engine-owned `build → verify ⇄ repair → review ⇄ repair` phase machine with durable reports. What remains for the series is product detach (RFC-88/89) and staged, concurrent scale: one change still runs in one repository (or a hand-tended workspace of them), serially — one model call at a time, with publication tracked in the operator's head. The baseline walls (`cargo make wasm-omnia-r9k`) were a ~30-minute serialized build with an unobservable nested review team, an 11–54 minute synthesis call, and no way to run two of anything at once.
...
6. **Refinement becomes a fenced stage.** [RFC-91](rfc-91-staged-refinement.md) adds serial plan-wide refinement, covers complete reviewed refinement bundles, and moves target-base selection to wave open. [RFC-92](rfc-92-concurrent-execution.md) turns the serial loops into `(slice, phase, input-digest)` work items and concurrent frontiers; RFC-94 adds streaming build authority and deferred commit across partially published branches.
...
A wave manifest names its build authorization. Its committed fact separately names closed-plan commit authorization. RFC-91 keeps `plan.execute.started` as the closed-plan build-and-commit grant over reviewed refinement manifests. RFC-94 introduces the second privileged mode, generalizes the grant vocabulary, and limits streaming authority to build until a later closed-plan gesture revalidates commit. A claim or projected `in-progress` status never grants execution authority.
...
| 9    | [RFC-94](future/rfc-94-streaming-execution.md) | Streaming Execution | Partial publication of closed branches; generalized plan-run grants; streaming build authority; digest-bound invalidation; and deferred accepted-CID commit behind a later reviewed closed-plan gesture | completed 88, 91, 92 (orthogonal to 93 — single-node streaming does not wait on distribution) |
...
- **After the stem, the series is a braid, not two independent pipelines:** RFC-90's engine-owned build verification and repair orchestration is implemented; RFC-88 still settles recursive authoring, while RFC-91 follows the 86/87/90 substrate with a serial specs-first stage, complete refinement coverage, and wave-time target bases. **RFC-92 joins 88, 90, and 91** to replace serial cursors with concurrent phase work and task pools. RFC-93 then distributes the completed local model. RFC-89 (publication) and RFC-93 (multi-node execution) remain orthogonal — 93 does not wait on 89.
- **RFC-91 is the staged-refinement prerequisite.** It owns the closed-plan review boundary and can land against the current flat topology and serial executor. It deliberately does not predesign concurrent claims or streaming grants.
- **RFC-92 owns the local phase scheduler.** It turns RFC-91's serial refinement and execute loops into fenced work items, concurrent frontiers, and operation-scoped local claims before widening waves and task execution.
- **RFC-94 owns streaming authority.** It combines RFC-88's immutable revisions, RFC-91's reviewed manifests, and RFC-92's scheduler, then defines partial publication, generalized plan-run grants, build-only authority, invalidation, and deferred commit. It does not wait on RFC-93.
...
Product-ownership edges stay as the tables and diagram above; landing chronology already split the stem (87 ahead of 86), and RFC-90's verification branch has now landed ahead of RFC-88's product branch. RFC-91 follows the implemented 86/87/90 substrate; RFC-92 starts when RFC-88's decomposition artifacts and RFC-91's refinement-manifest and wave-base contracts are stable, because RFC-92 itself owns the local phase scheduler.
...
emery plan execute    →  cover the reviewed refinement manifests;
...
RFC-91 keeps authority simple: refinement creates no code-work grant, while starting execute covers exact reviewed refinement manifests and authorizes the closed build-and-commit run. There is no separate `approve` verb and no interactive auto-waive of gaps. RFC-92 replaces the serial cursors with local phase work items and operation claims; RFC-93 distributes them; RFC-94 later adds the distinct streaming build authority that justifies a generalized plan-run grant.
...
- **Earlier partial execution** — promoted to [RFC-94 Streaming Execution](future/rfc-94-streaming-execution.md), step 9 on the scale track. RFC-88 retains immutable lead and decomposition revisions, RFC-91 supplies reviewed refinement manifests, and RFC-92 supplies phase work items. RFC-94 owns the streaming grant and deferred-commit policy that pipeline them.