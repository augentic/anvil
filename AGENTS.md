...
specify-runtime                  # dev-only: the specify-runtime-replay binary (the same runtime! macro over ModelDefault, for CI) + the composed-deployment integration tests
...
| Engineering standards layer (`specify-standards` / `specify-schema`, rules parse/resolve, `specify rules export`)                                                                     | [`DECISIONS.md` §"Standards layer split into `specify-standards` and `specify-schema`](./DECISIONS.md#standards-layer-split-into-specify-standards-and-specify-schema) |
