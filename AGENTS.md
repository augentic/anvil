...
- **The model never authors a document.** Synthesis is three schema-gated judgments — a grouping partition (two or more sources), a `SpecAnswer`, a `DesignAnswer` — each validated against engine facts (the id floor, the rows, the section plan) with bounded findings repair, then rendered by `specify/render.rs`. A change to the rendered layout re-ids every revision and must update the byte-for-byte fixtures under `tests/specify/`; the reader in `artifact/` only reads what the renderer wrote, so extend both together (the round-trip unit test beside the renderer holds them). Prompt changes live in `crates/engine/prose/synthesis/` and change no fixture.
