...
emery (root) # Omnia deployment unit under src/: wasm32 engine guest cdylib (src/lib.rs — bare model provider, wasi:cli/run) + shipped runtime (src/main.rs, one omnia::runtime! embedding $OUT_DIR/emery.cwasm; static, CWD-rooted deployment policy inline — the invocation directory mounts read-only as `.`, the wasi:keyvalue/wasi:blobstore hosts bind the generation state to the durable omnia-filesystem store (compiled-in root `.omnia/storage`), and the plugins block declares the source seam with a composed acquirer: MountAcquire first, so local `.wasm` adapters load fresh each run, then the omnia.host registry acquirer over the project CAS at `.omnia/cache/wasm-pkg` for exact package references; bare names still dispatch statically declared guests)
```
