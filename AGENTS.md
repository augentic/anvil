...
- **source adapter** — input role with two operations: `enumerate` (plan time) and `extract` (slice time). Examples: `intent`, `documentation`, `code-typescript`, `screenshots`, `runtime-fixtures` (RFC-27 §D1; consumes captured fixture trees and emits `kind: example` Evidence claims with `fixture-digest: sha256:…` anchors and default `authority: behaviour`). Lives at `adapters/sources/<name>/adapter.yaml`.
