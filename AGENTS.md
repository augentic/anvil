...
engine       # the transport-neutral spec generator — the `Specify { bindings }` and `Show { document }` operations (serde DTOs implementing omnia's Handler<P>, returning `Serialize`-only bodies), the binding DTO + its rules (emery_engine::sources — SourceBinding / BindingContent and validate: non-empty, unique keys, selector parse, digest/registry gating, preopen-relative roots), extract leg (resolve + the contract's claim gate re-run over the wire as `bad_request`) over the provider's Source capability, reconcile/synthesise (embedded synthesis prose), the fail-closed spec AST, the revision store (the private `store` module; its `Diff` DTO is exported through `emery_engine::specify`, and the `CURRENT` keyvalue key at the crate root); plus the ported kernels: emery_engine::AdapterSelector + resolve (selector parsing, loader-backed local-component loading over the Plugins capability, the adapter floor gate) and emery_engine::preopen (preopen-relative path normalization); plus the `Provider` alias bound. No clap, no toml, no terminal text, no exit codes — omnia's HTTP overlay routes over it unchanged
