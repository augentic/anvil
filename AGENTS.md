...
workflow-lib             # workflow layer — depends on {error,schema,artifacts,diagnostics} (also owns workflow::agents — init-time AGENTS.md context-fence generation — workflow::judgment — the guest judgment legs plus judgment::model's JudgmentModel capability trait (WASI-backed on wasm32 via a target-gated omnia-wasi-model dep, MockModel off it; mirrored from specify-adapters crates/adapter) — and config::tools, the parse-clean project-scope tools[] DTOs); NOT on standards (no wasmtime in its graph)
...
workflow                 # wasm32 wasi:cli/run shim over dispatch + workflow-lib (the core guest — published as specify:core@<binary version> on release; no committed artifact)
...
- `crates/standards/src/rules/` — rules parser and resolver pipeline (`parse.rs`, `resolve.rs`, `resolve/{filter,sort}.rs`). Kept out of `workflow-lib` by the standards-layer split.
