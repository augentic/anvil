...
The matching CLI validation surface is the declared `contract` WASI tool, run via `specify extension run contract -- "$PROJECT_ROOT/contracts" --format json`.
...
| Concern | Where |
| ------- | ----- |
| Materialize CLI + export conventions | `specify extension run vectis -- materialize assets` — codified in [`specify-adapters` `adapters/targets/vectis/extension/DECISIONS.md` §K](https://github.com/augentic/specify-adapters/blob/main/adapters/targets/vectis/extension/DECISIONS.md#k--materialization-and-render-by-kind) |
...
| Bootstrap `app-icon` gate | `plan-bootstrap-app-icon-missing` on `specify plan validate`; bootstrap-only — codified in [`adapters/targets/vectis/extension/DECISIONS.md` §L](https://github.com/augentic/specify-adapters/blob/main/adapters/targets/vectis/extension/DECISIONS.md#l--bootstrap-app-icon-gate) |
...
- Crossing a major is a hard cut: no silent compatibility aliases for old manifests, verbs, brief paths, or slash-namespaces, and no migration framework. Pre-1.0, a major bump means re-init — `specify init --upgrade` bumps the pin over an existing project; anything deeper is a fresh `specify init`. See the cli repo's [`DECISIONS.md`](https://github.com/augentic/specify-cli/blob/main/DECISIONS.md) "Bootstrap and upgrade lifecycle" decision.
