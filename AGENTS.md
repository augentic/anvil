...
specify (root crate) # Omnia deployment unit under src/: guest cdylib (wasm32, one guest::export!() over the guest crate, published as specify:engine@<binary version>) + shipped runtime + the examples/wasm adapter components (adapter::source!/target! over mock::Adapter)
```
