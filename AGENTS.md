...
- **target adapter** — output role with three operations: `guidance` (read by core synthesis), `build`, and `merge`. Ships as a single WebAssembly component exporting the WIT `target` interface; the guest crate lives at `targets/<name>/` in `augentic/emery-adapters`. Examples: `omnia`, `vectis`, `contracts`. See [`docs/reference/adapter-contract.md`](docs/reference/adapter-contract.md) for the full source / target contract and [`docs/explanation/adapter-anatomy.md`](docs/explanation/adapter-anatomy.md) for the conceptual picture, including the [adapter-vs-Cursor-plugin manifest boundary](docs/explanation/adapter-anatomy.md#adapter-manifests-vs-cursor-plugin-manifests).
...
Engineering standards reach consumer projects through the target adapters' embedded prose, applied by their build review prompts — there is no engine-side lint or rules-export surface. Build-time `REVIEW.md` and operator plan review are separate surfaces.
...
The default rhythm is `/emery:plan` → operator reviews the plan and runs `emery plan execute` (running it is the approval) → `/emery:finalize`; `/emery:execute` wraps the middle step. The operator surface, in the order it appears in a project's life:

- `/emery:init` — scaffold `.emery/`, run once per project.
- `/emery:plan` — wrap the guest-routed `emery plan author`: survey each bound source, reconcile leads into `slices[]`, author the review prose, validate. Exits after authoring and prints the literal `emery plan execute` command. An existing `plan.yaml` requires operator confirmation and `--force`, which recreates the plan unconditionally.
- `emery plan execute` — the guest-routed drained loop: invoking it on an authored plan is the approval (nothing is stamped or recorded) — it runs refine → build → merge per entry until every per-entry `status` is `done` or a stop condition halts it. `/emery:plan` never runs it; `/emery:execute` wraps it.
...
`/emery:plan` authors the plan and exits for operator review; the operator approves by running `emery plan execute` (directly or through `/emery:execute`) — running it is the approval, and it drives the loop; `/emery:finalize` closes it. Plan *entries* are only ever written via `emery plan add` / `emery plan amend`; per-entry `in-progress` is only ever written by `emery plan advance`; per-entry `done` is only ever written by `emery slice merge`. Per-entry status walks backwards only via `emery plan undo <entry>`, which refuses to skip rungs (`done → in-progress`, then `in-progress → pending`; `--to <status>` walks multiple rungs in one call) and fires one `plan.transition.undone` journal event per rung. The phase skills themselves stay unaware of the plan — they operate slice-by-slice. Hand-driven fallback: `emery plan advance` → `/emery:refine` → `/emery:build` → `/emery:merge`, repeat until drained.
...
launcher                 # native-only deployment-policy crate (publish = false) — the macro-facing mount and resolver expressions over one per-process anchoring (launcher::Policy: project-root walk, --project-dir override, Locations::from_env captured once, writable mount dirs created pre-run, the read-only self-named `adapter add` seed preopen, the per-invocation pre-bound HTTP listener (`launcher::http_listener` — split bind policy, any bind failure is a startup failure, its address injected as the guest-visible `HTTP_ADDR`) feeding the `/mcp/<axis>/<name>` `http_paths` hook (`launcher::mcp_route`) onto routed adapter ids; the global store is host-owned with no guest mount) plus the fail-closed adapters-only omnia::GuestResolver over the one captured ExecutionPaths: pinned routed ids resolve the store with pull-on-miss install (launcher::install — anonymous OCI pull from the compiled first-party GHCR constant ghcr.io/augentic/emery-adapters, layer/digest/wasm-magic validation, atomic store write, sidecar recording tree digest + OCI repository + manifest digest), bare routed ids resolve local-first (cache seed, else newest store version, else pull-latest provisioning via launcher::install::resolve_latest — the registry's newest exact-SemVer tag; the argv-derived refresh set from `adapter upgrade` / `init` forces the registry check) with every settled identity logged to stderr (adapter-sidecar-missing / adapter-digest-mismatch / adapter-not-found / adapter-install-failed / adapter-install-invalid / adapter-latest-failed / adapter-latest-none) — the engine is embedded in the binary; the launcher is the only downloader in the deployment; depends on {project,transport}
model                    # native-only model-backend crate (publish = false) — the ModelBackend enum linked into the shipped runtime's WasiModel slot: EMERY_MODEL_BACKEND selects cursor-agent (omnia_cursor, the default) or Claude Code (model::claude — a spawned `claude --print --output-format stream-json --verbose` agent with prompt spill, the dual inactivity/cap watchdog, `--mcp-config` + `--strict-mcp-config` MCP wiring, tool_use/tool_result transcript pairing, reported token usage, and the two-attempt Format::repair loop over `--resume`), and only the selected variant connects, so each mode needs only its own CLI on PATH; model::retry wraps every completion in bounded exponential backoff over transport-failure signatures (unreachable provider, killed spawn, stalled stream) and passes everything else — notably schema-repair exhaustion — straight through. No cross-backend fallback. Depends on no workspace crate
wasi-workspaces          # host capability crate — the `emery:adapter/workspaces` WIT import implemented host-side (WasiWorkspaces + the WasiWorkspacesCtx backend trait); linked into the shipped runtime by omnia::runtime! and backed by the launcher's workspaces backend over the project::workspace kernel
...
probe                    # lab-only library (publish = false) — the typed eval case runner (probe::case: Workflow / Build cases over real emery verbs, stable retained sandboxes, gates), grading, telemetry, sandbox helpers (no runtime/catalog/model backend); feature = "client": the shared live-model composition (probe::client — DevModel over the same model::ModelBackend the shipped binary links, so eval honors EMERY_MODEL_BACKEND; the console + optional EVAL_LOG file tracing init, argv dispatch) consumed by each repo's eval composition example (examples/eval/ here and in augentic/emery-adapters); target of `cargo make lab` and `cargo make eval` via the root example
...
- `crates/project/src/snapshot.rs` + `crates/project/src/workspace/` — the RFC-87 vocabulary (`SnapshotId`, `CodePatch`, the ignore policy) and the native-only private-workspace kernel: the content-addressed snapshot store (sha256 blobs + tree manifests under `$EMERY_HOME/snapshots/`, write-once with verify-on-read), `prepare` / `capture` / `discard` over `$EMERY_HOME/workspaces/`, age-based orphan GC, and the interim `apply` that writes a patch's touched paths onto the product tree (deleted by RFC-89). The seam capability is `project::seam::Workspaces`; the shipped deployment backs it with `crates/wasi-workspaces` (the host-side WIT import) over `launcher::Workspaces`.
...
```text
src/main.rs              shipped binary — one omnia::runtime! invocation: embedded engine bytes, launcher mount/resolver expressions, the model backend
...
crates/project/          foundation — init, adapter resolution, config, journal, registry, plan + slice data models, snapshot vocabulary + private-workspace kernel, seam traits, judgment kernel
...
crates/wasi-workspaces/  host capability crate — the workspaces WIT import implemented host-side, backed by the launcher's workspaces backend
crates/model/            native-only model backend — cursor-agent or Claude Code selected by EMERY_MODEL_BACKEND, with bounded retry on transport failures
crates/probe/            lab-only library — the typed eval case runner plus the shared live-model client (feature = "client")
...
- **Do NOT widen public APIs to test a private kernel.** Widening trades durable surface stability for coverage you already have; prefer collapse-and-keep. The target is *near-zero* `src` unit tests — no redundant or integration-reachable ones — not literal zero. Use `CRATE=<crate> cargo make cov` to prove coverage holds when removing unit tests.
...
cargo make eval <case>    # the live-model prompt-evaluation rung (one eval case; bare lists them); operator-invoked, needs credentials for whichever backend EMERY_MODEL_BACKEND selects
...
1. Read the matching [`docs/standards/`](./docs/standards/) doc before changing error layering, exit codes, atomic writes, the YAML library, the JSON envelope shape, the workflow type names (`Target*` / `Plugin` / `SliceSourceBinding` / `Divergence`), the per-entry plan status ladder (`pending | in-progress | done`), the journal event taxonomy, the per-axis cache layout, or adding a new workspace crate.
