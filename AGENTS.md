...
argv                     # wasm-clean transport assembly — explicit typed command/HTTP routers over Invoker, exhaustive Args-to-Input TryFrom conversions, projectors, and exit contract
...
specify (root crate) # Omnia deployment unit under src/: guest lib (wasm32, exporting wasi:cli/run + wasi:http/incoming-handler over the shared typed argv routers, published as specify:core@<binary version>) + shipped runtime
```
...
Part of the CLI wire contract. `Exit::from(&Error)` in [`crates/argv/src/output.rs`](./crates/argv/src/output.rs) is the single source of truth.
...
crates/argv/              shared clap grammar, conversions, envelopes, exit contract
