...
The test surface is three rungs: native integration tests over `crates/testkit`'s fixture adapter and scripted model doubles (`cargo make test`, per push; the assembled judgment prompts are pinned by request goldens re-recorded with `REGENERATE_GOLDENS=1`), one WASM boundary smoke over the real component seam (`cd examples && cargo make test-wasm`, weekly/path-filtered/manual; required for release), and one native prompt-evaluation harness over a live model (`cd examples && cargo make prompt-eval`, explicit). `omnia-testkit` owns generic model/scripted/runtime test mechanics; Specify's `testkit` crate owns the fixture adapter, provider, answer corpus, and request goldens; the suites own workflow scenario semantics and assertions. See [`docs/standards/testing.md`](docs/standards/testing.md) and [`docs/contributing/quality-gates.md`](docs/contributing/quality-gates.md).
...
testkit                  # dev-only test-support crate (publish = false) — the fixture adapter core (both WIT axes), the unified capability Provider (Scripted), scripted answers, request goldens, MockCmd, fs/git helpers, env guards, plan builders; dev-dep'd (legally cyclically) by project/slice/change/transport suites and the examples package
...
crates/testkit/ dev-only test-support crate — fixture adapter core, unified Provider, scripted answers, request goldens
