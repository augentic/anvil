...
- `/change:execute` (drive a change's `plan.yaml` through define → build → merge; RFC-2 Layer 2, fully landed — `dry-run` preview, supervised single-slice run, self-heal on startup, `loop` mode with terminal summary + SIGINT/SIGTERM handling, and `sources` execution wiring). Historical `/spec:execute` has been removed; use `/change:execute`.
