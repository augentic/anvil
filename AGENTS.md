...
- **Widening is a last resort:** do NOT alter public APIs simply to support integration tests — prefer collapse-and-keep. The target is *near-zero* unit tests (no redundant or integration-reachable ones), not literal zero. `cargo llvm-cov nextest` remains the brake that ensures coverage holds during migrations; adapter posture is enforced in `augentic/specify-adapters` by the WIT contract plus each adapter crate's `tests/` suite and that repo's operator-invoked change example.

The test surface is two rungs: native integration tests over `crates/mock`'s adapter catalog, the offline `crates/linked` provider, and scripted model doubles (`cargo make test`, per push), and one native prompt-evaluation rung over a live model (`cargo make eval`, explicit; the `crates/eval` library driven by the `crates/lab` binary). There is no automated WASM boundary rung: the real component seam is exercised by the operator-invoked change example (`cargo make change-run`, live model; see [`examples/change/README.md`](examples/change/README.md)) and by the matching change example in `augentic/specify-adapters`. `omnia-testkit` owns generic model/scripted/runtime test mechanics; Specify's `mock` crate owns the mock adapter, catalog registry, session helpers, and answer corpus; the suites own workflow scenario semantics and assertions. See [`docs/standards/testing.md`](docs/standards/testing.md) and [`docs/contributing/quality-gates.md`](docs/contributing/quality-gates.md).
