...
specify (root crate) # Omnia deployment unit under src/: guest cdylib (wasm32, one guest::export!() over the guest crate, published as specify:core@<binary version>) + shipped runtime + the examples/wasm adapter components (adapter::source!/target! over mock::Adapter)
```
...
examples/                wasm (Omnia-hosted component seam: mock adapter components + omnia.toml) and static (in-process native host over the same mock adapters)
```
