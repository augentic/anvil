...
scenario                 # non-shipped quality layer — canonical scenario DTOs, typed assertion registry, deterministic grading with the registered-probe Evaluators registry (grade::hard_with), the semantic Judge trait + rubric grading (evaluate::semantic), the run-bundle layout + report-completeness validation (scenario::bundle), and the embedded canonical scenario + rubric catalogs (scenario::catalog, evaluate::semantic::CATALOG_YAML — consumed by revision-pinned downstream harnesses); spawns no processes
