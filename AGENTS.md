...
Leaf → root. Each publishing package is `emery-<crate>` on crates.io; Rust `use` paths follow the package name (`emery_adapter::`, `emery_engine::`, …). The root package and `test-utils` stay `publish = false`; the mock adapter is a root-package example, not a crate. Scripted capability doubles come from omnia's `omnia-test` crate (dev-dependency, native only), not a local crate.
...
test-utils   # unpublished — the component rung's harness over omnia-test: build.rs drives `omnia_test::build::Components` to compile `--example adapter` to wasm32-wasip2 and generate the `MOCK_ADAPTER` path; the lib's `run` overlays the shipped deployment (`omnia_test::host::Deployment::from(emery::manifest())` — the scenario's project mount, path root, argv, and declared adapter guests) and drives it through `emery::Hooks` over `omnia_test::host::Backends<ScriptedModel>`; not a production crate
emery (root) # Omnia deployment unit under src/: wasm32 engine guest cdylib (src/lib.rs wasm32 arm — bare model provider, wasi:cli/run) + shipped runtime (src/lib.rs native arm, one omnia::runtime! embedding $OUT_DIR/emery.cwasm, exporting `main` / `manifest` / `Hooks`; src/main.rs calls `emery::main()`; static, CWD-rooted deployment policy inline — the invocation directory mounts read-only as `.`, the wasi:keyvalue/wasi:blobstore hosts bind the generation state to the durable omnia-filesystem store (compiled-in root `.omnia/storage`), and the plugins block declares the source seam with the declarative locations list: the `.` path root, so local `.wasm` adapters load fresh each run, then the omnia.host registry endpoint for exact package references, read fresh (no project cache); bare names still dispatch statically declared guests)
```
...
- The root `tests/` scenario suites are the default home for every CLI-reachable behavior: `tests/specify.rs` (the `specify` → `show` product arc), `tests/command.rs` (the CLI wire contract), and `tests/plugin.rs` (plugin-rule mentions vs the shipped grammar) drive the in-process command router over scripted capabilities (`tests/support/mod.rs`, assembling omnia-test's `guest::{Scripted, ScriptedLoader, Memory, Namespaced}` with the local `Source` double through `omnia_test::delegate!`) and read like usage documentation. `tests/component.rs` is the component rung: the shipped deployment (`emery::manifest()`, overlaid by `test-utils` through `omnia_test::host::Deployment`) plus the built mock adapter under the real omnia runtime, owning only the wasm boundary itself — `wasi:cli/run`, the seam lowering, the real path loader and digest pin, the reference-tool streams — observed through exit status and storage handles.
