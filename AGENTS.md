...
- Target review prompts symlink `agent-teams.md` from each adapter's `references/` directory to the shared `adapters/codex/references/runtime/review-team-protocol.md` overlay, which resolves to the canonical `docs/reference/review-team-protocol.md`. If a symlink target is removed, the prompt's documentation may reference content that no longer resolves.
