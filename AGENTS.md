...
- **Adapter names are unique across axes** — a name appears under `sources/<name>/` xor `targets/<name>/`, never both. The store carries no axis segment, so a colliding name would make a binding's axis ambiguous; the `<axis>:<name>` adapter-id routing at the describe/dispatch seam is the enforcement point.
