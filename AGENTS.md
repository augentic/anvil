...
launcher                 # native-only deployment-policy crate (publish = false; RFC-70 Stage 3) — the macro-facing mount and resolver expressions over one per-process anchoring (launcher::Policy: project-root walk, --project-dir override, Locations::from_env captured once, writable mount dirs created pre-run, the read-only self-named `adapter add` seed preopen) plus the fail-closed adapters-only omnia::GuestResolver over the one captured ExecutionPaths: routed ids are verify-and-load only (store for pins, project cache for bare names; adapter-sidecar-missing / adapter-digest-mismatch / adapter-not-found) — the engine is embedded in the binary and the engine guest's ensure legs remain the sole adapter hydrator; no download path exists in the crate; depends on {project,transport}
