...
**Authoring standards** (`docs/standards/`) govern docs house style and the thin skill-wrapper shape; Developer Guide links are enforced in CI by `mdbook-linkcheck2` (`cargo make links` / `mdbook build docs`, config in `docs/book.toml`). The rest — including skill-wrapper shape — is applied in review. **Engineering standards** (rules in `augentic/specify-adapters` — `codex/rules/universal/` and per-adapter `prose/rules/` overlays, embedded in each target adapter's component and served by its references server) govern generated and hand-written code in consumer projects. Do not conflate them.
...
- `/spec:plan` — wrap the guest-routed `specify plan author`: survey each bound source, reconcile leads into `slices[]`, author the Gate 1 prose, validate. Exits at `plan.lifecycle: pending` and prints the literal `specify plan approve` command.
- `specify plan approve` — **Gate 1.** Nameless (there is one active plan), operator-only stamp; `/spec:plan` never writes `approved` itself, and `/spec:execute` runs it only behind an explicit operator confirmation (`--actor` stays `operator`).
...
The test surface is two rungs: native integration tests over `crates/mock`'s adapter catalog, the offline `crates/native` provider, and scripted model doubles (`cargo make test`, per push), and one native prompt-evaluation rung over a live model (`cargo make eval`, explicit; the `crates/probe` library for trial/scenario runners plus the shared cursor client, composed by the root `eval` example). There is no automated WASM boundary rung: the real component seam is exercised by the operator-invoked wasm example (`cargo make wasm-run`, live model; see [`examples/wasm/README.md`](examples/wasm/README.md)) and by the matching wasm example in `augentic/specify-adapters`. `omnia-testkit` owns generic model/scripted/runtime test mechanics; Specify's `mock` crate owns the mock adapter, catalog registry, session helpers, and answer corpus; the suites own workflow scenario semantics and assertions. See [`docs/standards/testing.md`](docs/standards/testing.md) and [`docs/contributing/quality-gates.md`](docs/contributing/quality-gates.md).
...
probe                    # lab-only library (publish = false) — multi-step live trial, prompt scenarios, grading, telemetry, sandbox (no runtime/catalog/Cursor); feature = "client": the shared cursor composition (probe::client — DevModel + argv dispatch) consumed by each repo's eval composition example (examples/eval/ here and in augentic/specify-adapters); target of `cargo make specify` and `cargo make eval` via the root example
...
- `crates/project/src/agents.rs` — crate-private init-time `AGENTS.md` scaffold (short fenced template + `.specify/context.lock`). `specify init` generates them when `AGENTS.md` is absent; skips inside materialised workspace slots.
...
cargo make eval           # the live-model prompt-evaluation rung (the root eval example's eval mode); operator-invoked, needs cursor-agent credentials
cargo make specify -- ARGS # any specify verb through the native mock shim (the root eval example's command mode)
