...
harness/runtime          # PARKED test rig: the runtime-replay binary (the same runtime! macro over ModelDefault) + the composed-deployment integration tests — workspace member, excluded from `cargo make test`; run on demand (harness/README.md)
harness/fixtures         # echo adapter fixtures, omnia's examples/ pattern (skeleton source/target guests as cdylib examples) the parked composed tests load
...
Part of the CLI wire contract. `Exit::from(&Error)` in [`crates/dispatch/src/output.rs`](./crates/dispatch/src/output.rs) is the single source of truth.
...
harness/                 PARKED composed-deployment test rig (runtime + echo fixtures; harness/README.md)
...
| Engineering standards layer (adapter-embedded rules)                                                                                                                                  | [`DECISIONS.md` §"Standards chain moved to the adapters"](./DECISIONS.md#standards-chain-moved-to-the-adapters-diagnostics-merged-into-schema)                                                                                                                                                         |
