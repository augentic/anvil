...
- **adapter train** — the set of first-party adapters released together under one SemVer (the `emery-adapters` `[workspace.package]` version), independent of the host binary's version. The host embeds one train recommendation (`FIRST_PARTY_ADAPTER_TRAIN` in `project::adapter`, surfaced by `emery --version` as `(adapters <train>)`): a bare first-party name at ensure time (init / `plan author`) with no seeded project-cache entry auto-pins to `emery:<name>@<train>`, and the pin is persisted (`project.yaml` / `plan.yaml`) before first use. The noun spans both repos; bumping the constant is a release-checklist step ([`docs/release.md`](docs/release.md)).

Do not confuse that noun with **Cursor plugins** under `plugins/` (e.g. `plugins/emery/`). Those are the IDE distribution surface for `/emery:*` skill wrappers and marketplace manifests; they are invisible to the `emery` CLI.
...
| Layer                     | Role                                          | Examples                                                                                            |
| ------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **Workflow**              | Phase orchestration and lifecycle transitions | `/emery:plan`, `emery plan execute`, `emery plan transition`                                     |
...
The default rhythm is `/emery:plan` → operator stamps `approved` → `emery plan execute` → `/emery:finalize`; `/emery:execute` carries the two middle steps (an explicit Gate 1 confirmation, then the drained loop). The operator surface, in the order it appears in a project's life:

- `/emery:init` — scaffold `.emery/`, run once per project.
- `/emery:plan` — wrap the guest-routed `emery plan author`: survey each bound source, reconcile leads into `slices[]`, author the Gate 1 prose, validate. Exits at `plan.lifecycle: pending` and prints the literal `emery plan approve` command.
- `emery plan approve` — **Gate 1.** Nameless (there is one active plan), operator-only stamp; `/emery:plan` never writes `approved` itself, and `/emery:execute` runs it only behind an explicit operator confirmation (`--actor` stays `operator`).
- `emery plan execute` — the guest-routed drained loop; refuses unless the plan is `approved`, then runs refine → build → merge per entry until every per-entry `status` is `done` or a stop condition halts it. `/emery:execute` wraps the Gate 1 confirmation plus this verb.
...
- `/emery:merge` — breakout: wrap `emery slice merge run` for one slice; the only writer of per-entry `done`.
...
N=1 is degenerate, not special: `intent.survey` produces one lead, the operator stamps `approved`, and `emery plan execute` drives the same single-slice rhythm as a 12-slice change.
...
The CLI surface skills depend on is documented in [`emery` `--help`](cli). The headline groups: `emery init` (with the re-entry flag `--upgrade`, which bumps the `emery` pin and re-scaffolds preservation-safe files only, and `--platforms <csv>`, which declares the project's target platform set — required when the target adapter declares `platforms.required`), `emery adapter {add}` (pre-init, axis-neutral seeding of a local `.wasm` component into the project component cache — the only route into bare-name resolution besides a local component at init), `emery source {resolve, survey, extract}` and `emery target {resolve}` (the adapter debug/breakout surface), `emery slice {list, refine, model show, build, validate, provenance, merge, drop}`, `emery plan {author, approve, execute, add, amend, remove, transition, next, status, archive}` (`plan status` is the read-only next-action projection — `refine|build|merge <slice>` / `stop <reason>` / `drained` — over plan entries, slice metadata, and the journal tail), `emery archive {prune}` (retention-policy GC over the prunable slice/plan archive), and `emery journal {emit, show}` (`emit` — the guarded front door onto the closed journal taxonomy for agent-orchestrated phases; `show` — the read-only `--filter`/`--limit` projection over the journal). `emery source survey`/`extract` resolve `<source>` against `plan.yaml.sources.<key>` and run the bound source adapter's compiled-in prompt through one guest orchestration each (source extraction is agent-only). `emery slice build <slice>` is the guest-routed target-build verb: the orchestration assembles + schema-validates the build request, emits `target.execution.agent`, drives the target build prompts, validates the report, and owns the `built` transition, journaling `slice.build.started` / `.succeeded` / `.failed`. `emery slice merge` dispatches the bound target's phased merge gates (WIT `merge-phase`: `preflight` before the deterministic commit, `postflight` after it), schema-gates and persists each gate's report, and fires `slice.merge.started`, then `slice.merge.succeeded` / `slice.merge.failed` — or `slice.merge.postflight-failed` when the postflight gate fails after the commit (non-rollback, the merge stands) — alongside the durable `slice.archive.created`.
...
`/emery:plan` authors the plan and exits at Gate 1; the operator stamps `approved` (directly or through `/emery:execute`'s confirmation gate); `emery plan execute` drives the loop; `/emery:finalize` closes it. Plan *entries* are only ever written via `emery plan add` / `emery plan amend`; plan *lifecycle* is only ever written via `emery plan approve`; per-entry `in-progress` is only ever written by `emery plan next`; per-entry `done` is only ever written by `emery slice merge`. Per-entry status walks backwards only via `emery plan transition <entry> --undo`, which refuses to skip rungs (`done → in-progress`, then a second call for `in-progress → pending`) and fires one `plan.transition.undone` journal event per rung. The phase skills themselves stay unaware of the plan — they operate slice-by-slice. Hand-driven fallback: `emery plan next` → `/emery:refine` → `/emery:build` → `/emery:merge`, repeat until drained.
...
- **First-party adapters resolve from GHCR or the seeded project cache** — `emery init <adapter>` accepts a package reference (`emery:omnia@1.0.0`) or the first-party shorthand (`omnia`, `omnia@1.0.0`). A semver pin installs automatically on first use: the native launcher pulls the published Wasm OCI artifact from the compiled first-party mapping (`ghcr.io/augentic/emery-adapters/<name>:<version>`) into the global single-file store (`<store-root>/<name>@<version>.wasm`) — pull-on-miss applies to every command that resolves a pin, not just init, and no project configuration can redirect the mapping. A bare name is the unpinned shorthand: at ensure time (init / `plan author`) it resolves a seeded project-cache entry when present, else auto-pins to the embedded **adapter train** (`emery:<name>@FIRST_PARTY_ADAPTER_TRAIN`) and installs through the same pull-on-miss path, persisting the pin before first use; at resolve/dispatch time a bare routed id remains cache-only (`<project-cache>/components/<name>.wasm`, populated by `emery adapter add <path.wasm>` — pre-init, axis-neutral — or a local `.wasm` component at init) and never pulls. Cache hits always win, so the co-dev seed is never shadowed by a published component. There is no sibling-checkout or build-tree probe — an adapter built elsewhere reaches the project through `adapter add`, a local component at init, or a pinned install. GitHub URLs are refused (`adapter-github-uri-unsupported`).
...
crates/launcher/         native-only deployment policy (RFC-71 Stage 3) — anchoring, mount + resolver macro expressions, fail-closed adapters-only GuestResolver (verify-and-load)
