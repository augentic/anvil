...
emery (root) # Omnia deployment unit under src/: wasm32 engine guest cdylib (src/lib.rs — bare model provider, wasi:cli/run) + shipped runtime (src/main.rs, one omnia::runtime! embedding $OUT_DIR/emery.cwasm; static, CWD-rooted deployment policy inline — the invocation directory mounts read-only as `.`, the wasi:keyvalue/wasi:blobstore hosts bind the generation state to the durable omnia-filesystem store (default `.omnia/storage`), and the plugins block declares the source seam with acquire: MountAcquire so local `.wasm` adapters load fresh each run; bare/package adapters still dispatch statically declared guests — registry acquisition is deferred)
```
...
- **Path adapters load dynamically; the rest stay static.** `emery specify <adapter>` accepts a package reference (`emery:intent@1.0.0`), the first-party shorthand (`intent@1.0.0`), a bare name, or a project-relative local `.wasm` path. A local `.wasm` loads through the deployment's `omnia:plugins/loader` capability on every run that names it — read fresh (nothing mirrors, a deleted file refuses typed), optionally pinned by the binding's `digest`, the resolved digest reported in the envelope (the journey host in `examples/runtime.rs` loads the built mock component by path). Bare and package references still dispatch only guests declared in the runtime invocation (`src/main.rs`) until registry acquisition lands; dispatch beyond the declared set fails at dispatch. There is no download path, and GitHub URLs are refused.
