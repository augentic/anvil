...
`specify lint project` and `specify rules export` retired from the operational surface (YAGNI); engineering standards reach consumer projects through the target adapters' embedded prose, applied by their build review prompts. Build-time `REVIEW.md` and plan Gate 1 `approved` are separate surfaces.
...
| Engineering standards layer (adapter-embedded rules; the retired `standards` crate and `specify rules export`)                                                                        | [`DECISIONS.md` §"Standards layer split into `standards` and `schema`](./DECISIONS.md#standards-layer-split-into-standards-and-schema)                                                                                                                                                                 |
