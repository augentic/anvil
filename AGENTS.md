...
- `crates/project/src/journal.rs` — newline-delimited JSON per-actor event logs at `<project_dir>/.emery/events/<actor>.jsonl` (union read by `(timestamp, actor, sequence)`); closed `Event` / `EventKind` taxonomy with kebab-case wire ids and `snake_case` Rust variants joined by `#[serde(rename = "…")]`, including the single `PlanReconcileCompleted` variant covering a successful `plan author` write and `plan.entry.advanced`.
