...
3. Drive the documented operator script using `cursor-agent` (or your editor's slash-command runner). The skill bodies under `plugins/spec/skills/` orchestrate `/spec:plan` -> Gate 1 -> `/spec:execute` -> `/spec:finalize`; the deterministic-boundary harness in [`tooling/tests/`](../../../../tooling/tests/) does **not** simulate this layer.
...
6. After every run, file follow-up issues (in the augentic/specify repo) for any gap the deterministic harness in `tooling/tests/` does not yet cover. Link the issue back to the run-summary.
