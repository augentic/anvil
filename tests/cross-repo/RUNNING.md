...
2. **Operator scenario sweep — manual.** The cross-repo scenarios in [`runs/2.0.0/`](runs/2.0.0/README.md) plus the per-target generated-output-correctness gate. A schema-valid `build/report.yaml` with `status: success` proves the build envelope held, not that the generated code compiles or replays — so each exercised target must also pass `cargo check` / `cargo test` / its replay suite (and the equivalent verification for non-Rust targets). A slice whose generated output fails these checks is not done, regardless of envelope validity.
...
1. **Wave 0 — release blocker.** Scenario #1 (pure intent, N=1). **Hard halt:** if #1 fails, file the failure into [`runs/2.0.0/01-pure-intent.md`](runs/2.0.0/01-pure-intent.md), do not run any other scenario, triage, then resume from #1 once green. No scenario in Wave 1 or 2 is meaningful while #1 is red.
...
When the whole queue is `passed` (or `deferred` with sign-off), record the gate as green in the [queue README](runs/2.0.0/README.md) and flip RM-05 from *Partial* to *Done* in [`rfcs/roadmap.md`](../../rfcs/roadmap.md).
...
- [`runs/2.0.0/README.md`](runs/2.0.0/README.md) — the scenario queue and status legend.
