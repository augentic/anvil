...
mod drop {
    ...
    fn drop_transitions_and_archives() {
        ...
        // The envelope renders the path project-relative; resolve it
        // against the project root (join is absolute-safe either way).
        let archived = project.root().join(archive_path);
        let archived_meta = fs::read_to_string(archived.join("metadata.yaml")).unwrap();
...
mod plan_dir {
    //! Global `--plan-dir` on forwarded verbs.
    //!
    //! The workflow guest anchors plan artifacts at the `"."` preopen,
    //! so a plan root that is not the working directory can never be
    //! honoured in-guest. The forwarding front refuses a foreign
    //! `--plan-dir` loudly (`argument`, exit 2) instead of letting the
    //! guest silently ignore it; a value resolving to the working
    //! directory itself is a no-op and the verb proceeds normally.
    ...
    #[test]
    fn foreign_plan_dir_refused() {
        let project = stage_synthesizable_slice_without_plan();
        let elsewhere = tempdir().expect("foreign plan root");
        fs::write(
            elsewhere.path().join("plan.yaml"),
            "name: elsewhere\nlifecycle: pending\nsources: {}\nslices: []\n",
        )
        .expect("write foreign plan.yaml");
        ...
        assert!(
            message.contains("--plan-dir"),
            "refusal must name the flag and the plan-root constraint: {message}"
        );
    }

    #[test]
    fn cwd_plan_dir_is_noop() {
        // Pointing `--plan-dir` at the working directory itself resolves
        // to the same plan root the guest anchors on — the flag is a
        // no-op and the verb proceeds into ordinary validation instead
        // of the argument refusal.
        let project = stage_synthesizable_slice_without_plan();
        ...
        if output.status.code() == Some(2) {
            let envelope = parse_json(&output.stderr);
            assert_ne!(
                envelope["error"], "argument",
                "a cwd-equal --plan-dir must not trip the foreign-root refusal: {envelope:#}"
            );
        }
    }
}
