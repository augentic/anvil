...
mod checks {
    ...
    /// Directories the workspace scan never descends into: build output
    /// (`target/`) and dot-directories (`.git/`, the gitignored
    /// `evals/.sandbox/` scratch tree, editor caches).
...
const GATED_RULES: [(&str, &str); 4] = [
    ...
    (
        ...
        "runtime dispatch must not hardcode first-party adapter names (see rfcs/rfc-61-omnia-migration.md — the contract stays adapter-agnostic)",
    ),
];
