...
Cross-package test support lives in the dev-only `testkit` workspace crate (`crates/testkit`): the scripted `Model` mock (`testkit::MockModel`) consumed by the `workflow` suites.
