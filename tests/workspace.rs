...
use common::{
    Project, expected_cache_dir, init_workspace, omnia_component, parse_stdout, run_git,
    specify_cmd,
};
...
fn sync_mirror_skips_self_slot() {
    ...
    // itself; mirroring there would copy the workspace component cache
    // onto itself. The self-slot is skipped: the init-seeded component
    // cache survives a sync byte-identical.
    let tmp = tempdir().expect("tempdir");
    let root = tmp.path().join("root");
    fs::create_dir_all(&root).expect("root");
    specify_cmd()
        ...
        .assert()
        .success();
    let seeded = expected_cache_dir(&root).join("components/omnia.wasm");
    let seeded_bytes = fs::read(&seeded).expect("init must seed the component cache");
    fs::write(
    ...
    assert_eq!(
        fs::read(&seeded).expect("seeded component after sync"),
        seeded_bytes,
        "the init-seeded component cache entry must survive a sync over a `url: .` self-slot"
    );
}

#[test]
fn sync_mirror_keeps_foreign_cache_entries() {
    // Per-file copy-over. Slot cache components the workspace does not
    // own (e.g. an init-time seed in the slot) are never pruned;
    // workspace-owned components are refreshed on re-sync.
    let tmp = tempdir().expect("tempdir");
    let peer = tmp.path().join("peer-proj");
    // The peer is itself a Specify project; the slot gate keys on its
    // `.specify/` directory before mirroring components into the slot cache.
    fs::create_dir_all(peer.join(".specify")).expect("peer .specify");
    ...
    let foreign = expected_cache_dir(&peer).join("components");
    fs::create_dir_all(&foreign).expect("foreign cache dir");
    fs::write(foreign.join("slot-local.wasm"), "slot-only component\n").expect("foreign component");
    let root = tmp.path().join("root");
    fs::create_dir_all(&root).expect("root");
    specify_cmd()
        .current_dir(&root)
    ...
    assert!(
        slot_cache.join("omnia.wasm").is_file(),
        "sync must mirror the workspace component into the slot cache"
    );
    assert_eq!(
        fs::read_to_string(slot_cache.join("slot-local.wasm")).expect("foreign entry"),
        "slot-only component\n",
    ...
    // foreign entry alone.
    let ws_component = expected_cache_dir(&root).join("components/omnia.wasm");
    fs::write(&ws_component, "workspace copy v2\n").expect("edit workspace component");
    specify_cmd().current_dir(&root).args(["workspace", "sync"]).assert().success();

    assert_eq!(
        fs::read_to_string(slot_cache.join("omnia.wasm")).expect("mirrored entry"),
        "workspace copy v2\n",
        "re-sync must refresh the mirrored copy"
    );
    assert!(slot_cache.join("slot-local.wasm").is_file(), "the foreign entry must survive re-sync");
}
