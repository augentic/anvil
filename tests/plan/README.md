...
These are shared plan-generation scenario documents. They live under `tests/plan/` because `/spec:plan` is orchestration: it authors a change-level plan that coordinates slices rather than owning one adapter's slice loop.
