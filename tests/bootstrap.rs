...
mod plugins {
  ...
  "metadata": {{ "description": "d", "version": "0.27.2", "pluginRoot": "plugins" }},
  "plugins": [ {} ]
}}"#,
            entries.join(", ")
        );
    ...
    fn refresh_deletes_scoped_cache() {
        ...
        let journal = fs::read_to_string(project.path().join(".specify/journal.jsonl"))
...
mod upgrade {
    ...
    fn dry_run_reports_channel_commands() {
        ...
        let args: Vec<&str> = commands[0]["args"]
            .as_array()
            .expect("args")
            .iter()
