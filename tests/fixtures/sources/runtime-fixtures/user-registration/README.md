# `runtime-fixtures` source adapter fixture — `user-registration`

Worked example for the [`runtime-fixtures`](../../../../../adapters/sources/runtime-fixtures/) source adapter (RFC-27 §D1). Exercises both operations of the source contract: `enumerate` emits one candidate per `tests/data/replay/<handler>/` directory under `## Candidate inventory`; `extract` emits one `kind: example` claim per scenario file, each carrying a `sha256:` fixture digest computed over the on-disk bytes.

## Layout

```text
inputs/
  tests/data/replay/user-registration/
