...
Worked example for the `documentation` source adapter at [`adapters/sources/documentation/`](../../../../adapters/sources/documentation/). Exercises both operations of the W2.2 contract: `survey` emits one lead per top-level concept under `## Lead inventory` in `discovery.md`; `extract` returns one Evidence YAML per lead with `documentation` authority and the `requirement` / `criterion` / `decision` / `section` claim kinds.
...
The Evidence YAMLs under `expected/evidence/` validate against [`schemas/evidence.schema.json`](https://github.com/augentic/specify-cli/blob/main/schemas/evidence.schema.json) in the CLI repo. The lead blocks in `expected/discovery.md` follow the grammar in [`schemas/discovery/lead.schema.json`](https://github.com/augentic/specify-cli/blob/main/schemas/discovery/lead.schema.json).
