...
1. **Operator runs `/spec:build` from the workspace.**
   ...
   - Workspace routing rule kicks in identically to the loop: save CWD = workspace; resolve `project-a` through `registry.yaml`; slot already materialised; `chdir` into `.specify/workspace/project-a/`. Emit `Routing: auth-rotate → project-a (.specify/workspace/project-a/)`.
