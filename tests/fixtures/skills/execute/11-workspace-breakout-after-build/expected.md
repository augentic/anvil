...
Pins the cross-cutting workspace + breakout contract: `/spec:execute` parks on a build failure in `project-a`; the operator stays at the workspace root and runs `/spec:build` standalone; the breakout body resolves the active slice's project and `chdir`s into the slot without operator intervention.
...
- `.specify/plan.lock` is the same file in both runs: workspace root, never per-slot. Two `/spec:build` shells cannot race on the same plan even when they target different slots.
