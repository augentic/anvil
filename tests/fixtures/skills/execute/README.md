...
Static fixtures pinning the four `/spec:execute` acceptance scenarios (§W3.3 of [`rfcs/rfc-25-plan.md`](../../../../rfcs/rfc-25-plan.md); the normative workflow contract lives in [`rfc-25-workflow.md`](../../../../rfcs/rfc-25-workflow.md)):
...
| #11 — workspace breakout after build failure | [`11-workspace-breakout-after-build/`](11-workspace-breakout-after-build/) | `/spec:execute` parks on `auth-rotate` in `project-a`; operator runs `/spec:build` from the workspace root; the breakout resolves the active slice's project and `chdir`s into the slot without operator intervention. |
