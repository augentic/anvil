...
4. `specrun target resolve omnia --format json` returns the resolved manifest path; the body reads `adapters/targets/omnia/briefs/merge.md` from there.
...
7. `specrun slice merge password-hash-rotate --format json` is called exactly once. The CLI atomically:
