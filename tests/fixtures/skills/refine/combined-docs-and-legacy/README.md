...
Multi-source slice where a `documentation` Evidence and a `code-typescript` Evidence agree on a `claim-id` after reconciliation. Matches workflow §Worked multi-source `plan.yaml` (slice `identity-user-registration`).

Playbook rules exercised:

- [`claim-reconciliation.md`](../../../../../plugins/spec/references/synthesis/claim-reconciliation.md) — reconciliation on `claim-id` (`users.register.email-validation`); behaviour `excerpt` corroborates the `documentation` `requirement`.
