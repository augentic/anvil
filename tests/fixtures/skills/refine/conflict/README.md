...
Two `documentation` Evidence contribute claims with the same `claim-id` but disagreeing bodies. Authority is tied at `documentation` — synthesis cannot pick a winner, so the requirement carries `[conflict]` and preserves both values for operator reconciliation. Matches RFC-25 §Acceptance scenario #5c.
