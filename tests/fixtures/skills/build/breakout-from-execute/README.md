...
3. Inherit `SPECIFY_PLAN_LOCK_HELD=1` to any sub-invocations it spawns (the same brief-loaded child shells, the verify-repair loop sub-shells, `specrun` invocations).
...
The same contract applies symmetrically to `/spec:merge` invoked under `SPECIFY_PLAN_LOCK_HELD=1` from the loop. See [`tests/fixtures/skills/merge/`](../../merge/) for the merge-side success/failure fixtures.
