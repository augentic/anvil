...
3. `specrun plan next --format json` is called as normal (the parent loop already wrote the entry's `in-progress` status).
...
6. On success, `specrun slice transition session-cookie-harden built --format json` runs and the body returns. On failure, the structured stop hint is emitted (see [`../failure-replay/expected-stop-hint.md`](../failure-replay/expected-stop-hint.md)).
