...
The fixtures are documentation pins for the skill body. They are not yet executable end-to-end — the runner that consumes them lands when `/spec:execute` (W3.3) and the per-target build briefs (W2.5–W2.7) are wired into a CLI-side acceptance harness.
