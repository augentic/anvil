...
1. `specrun plan next --format json` returns the entry `name: password-hash-rotate, target: omnia, status: in-progress`.
...
4. `specrun target resolve omnia --format json` returns the resolved manifest path; the body reads `adapters/targets/omnia/briefs/build.md` from there.
...
7. `specrun slice transition password-hash-rotate built --format json` is called exactly once. The CLI stamps `.metadata.yaml`.
