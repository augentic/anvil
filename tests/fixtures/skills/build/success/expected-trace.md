...
4. `specify target resolve omnia --format json` returns the resolved manifest path; the body reads `targets/omnia/briefs/build.md` from there.
5. The brief's verify-repair loop converges: `cargo fmt --check`, `cargo check`, `cargo clippy -- -D warnings`, `cargo test` all pass; `targets/omnia/briefs/build.md` § Code reviewer emits `REVIEW.md` with no critical / high findings outstanding.
