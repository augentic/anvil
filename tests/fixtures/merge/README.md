...
Ground-truth fixtures for the spec parser and merge engine in `artifacts::spec` / `workflow::merge`. They are a **frozen regression baseline** against the current Rust implementation.
...
- `expected-merged.md` — the canonical merged output. The Rust merge engine in `workflow::merge` must reproduce this byte-for-byte.
