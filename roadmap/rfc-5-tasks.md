...
| 10 | `vectis add-shell` (incl. `app.rs` parser) | [ ] | Same `Stub` -> `Success` transition; `&AddShellArgs` is already plumbed through, including the platform string -> `AssemblyKind` mapping that already lives in `add_shell::run` for prereq scoping (lift it if the parser also wants it). `args.version_file` is plumbed but unread (chunk 4); drop its `#[allow(dead_code)]` when you call `Versions::resolve(&dir, args.version_file.as_deref())`. **Both `init::ios::scaffold` and `init::android::scaffold` are already public and reusable** -- chunk 10 should call them directly rather than re-implementing per-shell template walking. The `init::mod::build_params` helper builds the `Params` placeholder map from `app_name`/`android_package`/`versions` and is called by both `init::ios` and `init::android`; chunk 10 should re-export or relocate it to a shared module (e.g. `init::params::build`) so `add_shell::run` can call it without re-deriving the map. The `init::android::scaffold` signature is `(project_dir, android_package, caps, params, versions, run_build) -> AndroidScaffold`; `add-shell` will need to derive `android_package` either from the parsed `app.rs` (if a `// @vectis-package = "..."` marker lands later) or from `--android-package` + the parsed app name. |
| 11 | `vectis update-versions` | [ ] | Same `Stub` -> `Success` transition; `&UpdateVersionsArgs` is already plumbed through. **`args.version_file` here is the *write target*, not a resolution override** (different from the same-named flag on `init`/`verify`/`add-shell` where it overrides resolution). For "current" pins use `Versions::resolve(&dir, None)` (or `Versions::embedded()` when no user/project file exists -- `embedded()` is already public for this purpose, drop its `#[allow(dead_code)]` then). The `--verify` flag already widens the prereq scope to all three assemblies (chunk 2); when you implement `--verify`'s scaffold-and-build loop you can rely on those checks already having run. Construct `VectisError::Internal` for transient registry/network failures; chunk 4 already started constructing `Internal` (for malformed embedded defaults), and chunk 7 started constructing `VectisError::Verify` (so by the time chunk 11 lands, every variant of `VectisError` is actively constructed -- no more `#[allow(dead_code)]` on the enum). **Android placeholder names introduced by chunk 3c**: `__AGP_VERSION__`, `__KOTLIN_VERSION__`, `__COMPOSE_BOM_VERSION__`, `__KTOR_VERSION__`, `__KOIN_VERSION__`. The `Versions::android.ndk` field exists but is `Option<String>` and absent from embedded defaults; if `update-versions` learns to detect the latest NDK release, write into the user file as `Some(...)`. **Required pin bump (carried over from chunk 7's verification):** `cargo_swift = "0.9"` produces a Swift package that does not compile under Xcode 16+ / Swift 6 because cargo-swift 0.9's bindgen names the binary target `RustFramework` while the modulemap declares the module `sharedFFI`, and Xcode/Swift PM cannot resolve `import sharedFFI` against that mismatch. cargo-swift 0.11 fixes this by renaming the binary target to match. Bump the embedded defaults to `cargo_swift = "0.11"` and `uniffi = "=0.31.0"` (uniffi_bindgen 0.31.0 is what 0.11 ships) and re-run the chunk-7 + chunk-9 verification matrices end-to-end before committing. The runtime macros used in `templates/vectis/core/ffi.rs` (`uniffi::Object`, `uniffi::export`, `uniffi::constructor`, `uniffi::Error`, `uniffi(flat_error)`) are stable across 0.29 → 0.31 -- no template changes expected, but verify with `cargo check --features uniffi` after the bump. |
...
- **Chunk 8 verification deviations** (recorded in the status row): (1) required Gradle 8.x on PATH for the wrapper bootstrap (system Gradle was 9.4.1 from Homebrew); downloaded Gradle 8.13 to `/tmp/gradle-8.13`. (2) Used `$HOME/scratch/vectis-8-check` instead of `/tmp/vectis-8-check` because macOS rotated `/tmp` mid-session (same hazard chunks 6/7 hit). (3) Pre-existing `cargo fmt --check` drift in `templates/mod.rs` and `versions.rs` is from chunks 5/4 -- not in chunk-8 scope.
