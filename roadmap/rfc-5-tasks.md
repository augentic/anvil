...
| 6 | `vectis init` core capability variants | [ ] | Reuse the comma-splitting pattern from chunk 2's `init::run` (`split(',').map(str::trim).filter(...)`); chunk 5's `init::run::parse_shells` already returns a typed `Vec` from a comma list -- lift it into a generic helper if `--caps` follows the same pattern. The chunk-5 evaluator already supports both modes (drop-region vs. drop-markers); chunk 6 only has to start populating `&[Capability]` from the CLI parser. **Replace the chunk-5 `--caps` guard** in `init::run` (currently returns `InvalidProject` for any non-empty value pointing at chunk 6) with the real parser. The `Capability` enum and `marker_tag()` already exist in `templates::mod.rs` -- drop the `#[allow(dead_code) // populated by chunk 6]` annotations as you wire them in. The five-cap matrix from the RFC maps to chunk 3a's templates as follows: `http`/`kv`/`time`/`platform` each have CAP blocks in `workspace-cargo.toml`, `shared-cargo.toml`, and `app.rs`; `sse` only has a CAP block in `shared-cargo.toml` today (no Effect variant) -- decide here whether to add `Sse(...)` to the Effect enum and corresponding update arms. Capability-version placeholders (`__CRUX_HTTP_VERSION__` etc.) are already wired through `Params` in chunk 5; the values come from chunk 4's `Versions` struct (no extra plumbing needed). When adding new placeholders, slot them into `templates::mod.rs::substitute_placeholders` in superstring-first order (regression-tested by `substitutes_app_name_lower_before_app_name`). The chunk-5 verification gate `cargo clippy --all-targets -- -D warnings` will rerun for every cap combo (`""`, `"http"`, `"kv"`, `"http,kv"`, `"http,kv,time,platform,sse"`) -- chunk 5 already patched the workspace `[workspace.lints.clippy]` block (lint groups carry `priority = -1`; `cargo_common_metadata` and `multiple_crate_versions` are `allow`), so cap variants should not need additional lint hygiene as long as new dependencies do not introduce per-lint groups of their own. |
