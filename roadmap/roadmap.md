...
## [Horizon 2: Iterative Legacy Migration](migration.md)
...
The migration design is ready — the loop, manifest format, and slice strategy are fully specified. Implementation depends on the CLI from Horizon 1: the deterministic operations (`specify migrate init`, `specify migrate next`, `specify migrate status`) are natural extensions of the CLI's subcommand surface. The skill-level orchestration (extract → define → build → merge) already works today; what's missing is the manifest-driven automation layer.

## [Horizon 3: Multi-Repo Coordination](multi-repo.md)
...
## [Horizon 4: Type-Safe Skill Expression](dsl.md)
